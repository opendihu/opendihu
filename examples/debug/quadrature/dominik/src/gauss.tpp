...
constexpr int Gauss<NumberGaussPoints>::numberEvaluationsD() {
  return NumberGaussPoints;
}

template <unsigned int NumberGaussPoints>
template <typename ValueType>
// ValueType Gauss<NumberGaussPoints>::computeIntegral(const typename
// std::array<ValueType,numberEvaluations()> &evaluations)
ValueType Gauss<NumberGaussPoints>::computeIntegral(
    const typename std::array<ValueType, numberEvaluations> &evaluations) {
  return Gauss<NumberGaussPoints>::template computeIntegral<ValueType>(
      evaluations.begin());
}
...
ValueType Gauss<5>::
    ...
    computeIntegral(
        const typename std::array<ValueType, numberEvaluations>::const_iterator
            evaluationsIter) {
  ValueType result{};
  std::array<double, 5> weights{(322. - 13. * sqrt(70.)) / 900. / 2.,
