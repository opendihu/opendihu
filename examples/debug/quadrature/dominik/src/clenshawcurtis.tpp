// weights are calculated corresponding to the implementation in CCweights()
// which is located in quadrature.cpp
...
template <unsigned int NumberCCPoints>
constexpr int CC<NumberCCPoints>::numberEvaluationsD() {
  return NumberCCPoints;
}

template <unsigned int NumberCCPoints>
template <typename ValueType>
// ValueType CC<NumberCCPoints>::computeIntegral(const typename
// std::array<ValueType,numberEvaluations()> &evaluations)
ValueType CC<NumberCCPoints>::computeIntegral(
    const typename std::array<ValueType, numberEvaluations> &evaluations) {
  return CC<NumberCCPoints>::template computeIntegral<ValueType>(
      evaluations.begin());
}

// quadrature with 1 CC point
template <>
template <typename ValueType>
ValueType CC<1>::
    // computeIntegral(const typename std::array<ValueType,
    // numberEvaluations()>::const_iterator evaluationsIter)
    computeIntegral(
        const typename std::array<ValueType, numberEvaluations>::const_iterator
            evaluationsIter) {
  ValueType result{};
  result = 2. * (*evaluationsIter) / 2.;
  return result;
}

// quadrature with 2 CC points
template <>
template <typename ValueType>
ValueType CC<2>::
    // computeIntegral(const typename std::array<ValueType,
  ...
  for (int i = 0; i < 2; i++) {
    result += weights[i] * (*(evaluationsIter + i)) / 2.;
  }
  return result;
}

// quadrature with 3 CC points
template <>
template <typename ValueType>
ValueType CC<3>::
    // computeIntegral(const typename std::array<ValueType,
...
ValueType CC<4>::
    ...
    computeIntegral(
        const typename std::array<ValueType, numberEvaluations>::const_iterator
            evaluationsIter) {
  ValueType result{};
  std::array<double, 4> weights{1. / 9., 8. / 9., 8. / 9., 1. / 9.};
  for (int i = 0; i < 4; i++) {
    result += weights[i] * (*(evaluationsIter + i)) / 2.;
  }
  return result;
...
ValueType CC<5>::
    ...
    computeIntegral(
        const typename std::array<ValueType, numberEvaluations>::const_iterator
            evaluationsIter) {
  ValueType result{};
  std::array<double, 5> weights{0.0666666666666666, 0.5333333333333333,
...
ValueType CC<6>::
    ...
    computeIntegral(
        const typename std::array<ValueType, numberEvaluations>::const_iterator
            evaluationsIter) {
  ValueType result{};
  std::array<double, 6> weights{0.040000000000000008, 0.36074304120001122,
...
ValueType CC<7>::
    ...
    computeIntegral(
        const typename std::array<ValueType, numberEvaluations>::const_iterator
            evaluationsIter) {
  ValueType result{};
  std::array<double, 7> weights{0.028571428571428577, 0.2539682539682539,
