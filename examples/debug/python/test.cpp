...
int main(int argc, char *argv[]) {
  ...
  // std::string("/store/software/opendihu/dependencies/python/install"); const
  // wchar_t *pythonSearchPathWChar = Py_DecodeLocale(pythonSearchPath.c_str(),
  // NULL); Py_SetPythonHome((wchar_t *)pythonSearchPathWChar);

  // std::string
  // pythonPath("/store/software/opendihu/dependencies/python/install/lib/python3.6");
  ...
  // Data::TimeStepping<BasisOnMesh::BasisOnMesh<Mesh::StructuredRegularFixedOfDimension<1>,BasisFunction::LagrangeOfOrder<1>>,
  // 1> data(settings.getPythonConfig());
  ...
  PyObject *file =
      PyObject_CallMethod(ioModule, "open", "ss", filename.c_str(), "w");
  Py_DECREF(ioModule);
  ...
  std::string text = "print(\"hello\");config={\"format\": \"PythonFile\", "
                     "\"filename\": \"out_diffusion2d\", \"binary\": False}";
  // int ret = 0;
  try {
