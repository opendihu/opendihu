...
#pragma omp parallel for
  for (int i = 0; i < nInstances_; i++) {
    if (omp_get_thread_num() == 0) {
      ...
      msg << omp_get_thread_num() << ": running " << nInstances_
    ...
    if (!instances_[i].initialized_) {
      ...
      LOG(TRACE) << omp_get_thread_num()
                 << ": TimeSteppingSchemeOde::initialize -*";
      ...
      std::shared_ptr<Mesh::Mesh> mesh =
          instances_[i].discretizableInTime_.mesh();
      instances_[i].data_->setMesh(
          std::static_pointer_cast<BasisOnMesh::BasisOnMesh<
              Mesh::StructuredRegularFixedOfDimension<1>,
              BasisFunction::LagrangeOfOrder<1>>>(mesh));
      instances_[i].data_->initialize();

      instances_[i].timeStepOutputInterval_ = PythonUtility::getOptionInt(
          instances_[i].specificSettings_, "timeStepOutputInterval", 100,
...
#pragma omp parallel for
  for (int i = 0; i < 2; i++) {
    ...
    for (int j = 0; j < 10000; j++) {
      std::cout << int(omp_get_thread_num()) << "/"
                << int(omp_get_num_threads()) << " start i=" << i << ", j=" << j
                << std::endl;
      ...
      LOG(INFO) << omp_get_thread_num()
                << ": discretizableInTime::initializeLinearSolver";
      ...
      PyObject *file = PyObject_CallMethod(ioModule, "open", "ss",
                                           filename.str().c_str(), "w");
      Py_DECREF(ioModule);
