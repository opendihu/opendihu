// g++ -std=c++14 std-simd.cpp -I
// $OPENDIHU_HOME/dependencies/vc/install/include/
// -L$OPENDIHU_HOME/dependencies/vc/install/lib -lVc -I/store/software/std-simd
// && ./a.out g++ -std=c++17 std-simd.cpp -I
// $OPENDIHU_HOME/dependencies/vc/install/include/
// -L$OPENDIHU_HOME/dependencies/vc/install/lib -lVc -I/store/software/std-simd
// && ./a.out
...
template <typename T>
constexpr T iif(const typename T::mask_type &mask, const T &trueValue,
                const T &falseValue) {
  T result(falseValue);
...
template <typename T>
constexpr T isnegative(bool condition, const T &trueValue,
                       const T &falseValue) {
  return condition ? trueValue : falseValue;
}

template <typename T> typename T::mask_type isnegative(T x) { return x < 0; }

template <typename Mask> int count(const Mask &mask) {
  int result(0);
...
namespace Vc_1 {
template <typename Mask> int count(const Mask &mask) { return mask.count(); }

template <typename T> typename T::value_type min(const T &x) { return x.min(); }

template <typename T> typename T::value_type max(const T &x) { return x.max(); }
} // namespace Vc_1
...
int main(int argc, char *argv[]) {
...
#endif
  std::cout << "double size: " << Vc::double_v::size()
            << ", int size: " << Vc::int_v::size() << std::endl;
  std::cout << "a=" << a << ", b=" << b << ", c=" << c << ", d=" << d
            << std::endl;
  std::cout << "a+b=" << a << ", log(a)=" << log(a) << ",abs(a)=" << abs(a)
            << ",exp(a)=" << exp(a) << std::endl;
  std::cout << "a+b=" << a << ", log(a)=" << Vc::log(a)
            << ",abs(a)=" << Vc::abs(a) << ",exp(a)=" << Vc::exp(a)
            << std::endl;
  ...
  k[1] = 1;
  std::cout << "k=" << k
            << ", isnegative.count(k): " << Vc::count(Vc::isnegative(k))
            << std::endl;
  // std::cout << "pow(b,2)=" << pow(b,2) << std::endl;
  std::cout << "b=" << b << ", min(b)=" << Vc::min(b)
            << ", max(b)=" << Vc::max(b) << std::endl;
