...
int main(int argc, char *argv[]) {
  ...
  CHKERRQ(ierr);
  ierr = MatSetType(globalMatrix, MATMAIJ);
  CHKERRQ(ierr);
  ...
  // nComponents*nNodesGlobal); CHKERRQ(ierr);
  ierr = MatSetSizes(globalMatrix, PETSC_DECIDE, PETSC_DECIDE,
                     nComponents * nNodesGlobal, nComponents * nNodesGlobal);
  CHKERRQ(ierr);

  int nNonZerosDiagonal = nNodesGlobal;
  int nNonZerosOffdiagonal = nNodesGlobal;
  ierr = MatMPIAIJSetPreallocation(globalMatrix, nNonZerosDiagonal, NULL,
                                   nNonZerosOffdiagonal, NULL);
  CHKERRQ(ierr);
  ierr = MatSeqAIJSetPreallocation(globalMatrix, nNonZerosDiagonal, NULL);
  CHKERRQ(ierr);
  ...
  IS indexSet;
  ierr = ISCreateGeneral(PETSC_COMM_SELF, localDofNos.size(),
  localDofNos.data(), PETSC_COPY_VALUES, &indexSet); CHKERRQ(ierr);
  */
  // ierr = MatGetLocalSubMatrix(globalMatrix, indexSet, indexSet,
  ...
  LOG(INFO) << "rowIndices: " << rowIndices
            << ", columnIndices: " << columnIndices << ", set values "
            << values;

  // ierr = MatSetValuesLocal(localMatrix, rowIndices.size(), rowIndices.data(),
  // columnIndices.size(), columnIndices.data(), values.data(), ADD_VALUES);
  // CHKERRQ(ierr); ierr = MatSetValuesLocal(globalMatrix, rowIndices.size(),
  // rowIndices.data(), columnIndices.size(), columnIndices.data(),
  // values.data(), ADD_VALUES); CHKERRQ(ierr);
  ierr = MatSetValues(globalMatrix, rowIndices.size(), rowIndices.data(),
                      columnIndices.size(), columnIndices.data(), values.data(),
                      ADD_VALUES);
  CHKERRQ(ierr);
  ...
  // PetscUtility::getStringMatrix(localMatrix);
