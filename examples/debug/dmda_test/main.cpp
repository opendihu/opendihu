...
DMDA_STENCIL_BOX, globalSize_[0], globalSize_[1], PETSC_DECIDE, PETSC_DECIDE,
                      nDofsPerElement, ghostLayerWidth, NULL, NULL, &da);
CHKERRV(ierr);
  ...
  ierr = DMDAGetInfo(da, NULL, NULL, NULL, NULL, &nRanks_[0], &nRanks_[1], NULL,
NULL, NULL, NULL, NULL, NULL, NULL); CHKERRV(ierr);
...
int main(int argc, char *argv[]) {
  ...
  if (ownRankNo == 0) {
    ...
    ierr = VecCreateGhost(MPI_COMM_WORLD, nNodesLocal, nNodesGlobal, nGhosts,
                          ghostDofGlobalNos.data(), &globalVector);
    CHKERRQ(ierr);
    VecZeroEntries(globalVector);

    ierr = VecGetLocalToGlobalMapping(globalVector, &localToGlobalMapping);
    CHKERRQ(ierr);
    ...
    // here VecSetValues is used, not VecSetValuesLocal, because it is a plain
    // vector
  ...
  ierr = MatCreate(MPI_COMM_WORLD, &globalMatrix);
  CHKERRQ(ierr);
  // ierr = MatSetType(globalMatrix, MATMAIJ); CHKERRQ(ierr);
  ierr = MatSetFromOptions(globalMatrix);
  CHKERRQ(ierr);
  ierr = MatSetSizes(globalMatrix, nComponents * nNodesLocal,
                     nComponents * nNodesLocal, nComponents * nNodesGlobal,
                     nComponents * nNodesGlobal);
  CHKERRQ(ierr);
  ierr = MatSetUp(
      globalMatrix); // either MatSetUp or MatSetPreallocation has to be used
  ierr = MatSetLocalToGlobalMapping(globalMatrix, localToGlobalMapping,
                                    localToGlobalMapping);
  CHKERRQ(ierr);

  int nRows, nColumns, nRowsLocal, nColumnsLocal;
  ierr = MatGetSize(globalMatrix, &nRows, &nColumns);
  CHKERRQ(ierr);
  ierr = MatGetLocalSize(globalMatrix, &nRowsLocal, &nColumnsLocal);
  CHKERRQ(ierr);
  LOG(DEBUG) << "matrix global: " << nRows << "x" << nColumns
  ...
  IS indexSet;
  ierr = ISCreateGeneral(PETSC_COMM_WORLD, localDofNos.size(),
                         localDofNos.data(), PETSC_COPY_VALUES, &indexSet);
  CHKERRQ(ierr);
  ierr = MatGetLocalSubMatrix(globalMatrix, indexSet, indexSet, &localMatrix);
  CHKERRQ(ierr);
  ...
  LOG(DEBUG) << "INSERT_VALUES: rowIndices: " << rowIndices
             << ", columnIndices: " << columnIndices << ", values " << values;

  ierr = MatSetValuesLocal(localMatrix, rowIndices.size(), rowIndices.data(),
                           columnIndices.size(), columnIndices.data(),
                           values.data(), INSERT_VALUES);
  CHKERRQ(ierr);
  // ierr = MatSetValuesLocal(globalMatrix, rowIndices.size(),
  // rowIndices.data(), columnIndices.size(), columnIndices.data(),
  // values.data(), ADD_VALUES); CHKERRQ(ierr);

  ierr =
      MatRestoreLocalSubMatrix(globalMatrix, indexSet, indexSet, &localMatrix);
  CHKERRQ(ierr);

  ierr = MatAssemblyBegin(globalMatrix, MAT_FLUSH_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(globalMatrix, MAT_FLUSH_ASSEMBLY);
  CHKERRQ(ierr);

  ierr = MatGetLocalSubMatrix(globalMatrix, indexSet, indexSet, &localMatrix);
  CHKERRQ(ierr);

  LOG(DEBUG) << "ADD_VALUES: rowIndices: " << rowIndices
             << ", columnIndices: " << columnIndices << ", values " << values;
  ...
  values[0] = 10.0;
  // ierr = MatSetValuesLocal(localMatrix, rowIndices.size(), rowIndices.data(),
  // columnIndices.size(), columnIndices.data(), values.data(), INSERT_VALUES);
  // CHKERRQ(ierr);
  ...
  ierr = MatAssemblyBegin(globalMatrix, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  ierr = MatAssemblyEnd(globalMatrix, MAT_FINAL_ASSEMBLY);
  CHKERRQ(ierr);
  ...
  // show entries
  ierr = MatView(globalMatrix, PETSC_VIEWER_STDOUT_WORLD);
  CHKERRQ(ierr);
  // exit(0);
  //  again get submatrix
  ierr = MatGetLocalSubMatrix(globalMatrix, indexSet, indexSet, &localMatrix);
  ...
  rowIndices[0] = 1;
  const int diag = 77;
  ierr = MatZeroRowsColumnsLocal(globalMatrix, rowIndices.size(),
                                 rowIndices.data(), diag, NULL, NULL);
  CHKERRQ(ierr);

  // ierr = MatRestoreLocalSubMatrix(globalMatrix, indexSet, indexSet,
  // &localMatrix); CHKERRQ(ierr);
  ...
  // LOG(DEBUG) << " globalMatrix: " <<
  // PetscUtility::getStringMatrix(globalMatrix);
