...
int main(int argc, char *argv[]) {
  ...
  ierr = DMDACreate1d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, M, 1, 1, NULL, &da);
  CHKERRQ(ierr);

  // get global coordinates of local partition
  PetscInt x, m;
  ierr = DMDAGetCorners(da, &x, NULL, NULL, &m, NULL, NULL);
  CHKERRQ(ierr);
  std::cout << ownRankNo << "/" << size << ": own range is from x=" << x
            << ", with size m=" << m << std::endl;

  // get local sizes on the ranks
  const PetscInt *lxData;
  ierr = DMDAGetOwnershipRanges(da, &lxData, NULL, NULL);
  CHKERRQ(ierr);
