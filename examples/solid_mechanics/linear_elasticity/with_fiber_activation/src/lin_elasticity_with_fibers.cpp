...
int main(int argc, char *argv[]) {
  ...
  Control::Coupling<
      ...
      Control::MultipleInstances<
          ...
          // class that prescribes values on a 1D fiber mesh
          PrescribedValues<FunctionSpace::FunctionSpace<
              Mesh::StructuredDeformableOfDimension<1>,
              BasisFunction::LagrangeOfOrder<1>>>>,

      // timestepping scheme that wraps the linear elasticity and computes the
      // quasi-static problem, this also add the active stress term to the right
      // hand side
