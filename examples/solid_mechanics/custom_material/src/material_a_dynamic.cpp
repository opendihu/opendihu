...
struct Material : Equation::SolidMechanics::HyperelasticityBase {
  ...
  static constexpr bool usesFiberDirection =
      false; //< if the decoupled form uses the 4th or 5th invariants, Ibar4,
             // Ibar5, this means it is an anisotropic material
  static constexpr bool usesActiveStress =
      false; //< if the value of an active stress term will be added to the
             // stress, this is only possible with MuscleContractionSolver
  ...
  static constexpr int nMaterialParameters =
      3; //< number of material parameters, this has to match the number of
  ...
  //! Normally, this is used for incompressible materials, then set
  //! isIncompressible to true. Set usesFiberDirection to true if you use
  //! lambda, Ibar4 or Ibar5. It should only use the variables Ibar1, Ibar2,
  //! Ibar4, Ibar5, lambda (lambda is an alias for sqrt(Ibar4))
  ...
  //! only used for compressible formulation (isIncompressible == false), J =
  //! det(F) It should only use the variable J.
  ...
  //! the two decoupled functions, I1 = tr(C), I2 = 1/2 (tr(C)^2 - tr(C^2)), I3
  //! = det(C) = J^2 It should use the variables I1, I2, I3.
  static constexpr auto d = INT(2) * (c1 + INT(2) * c2);
  static const auto constexpr strainEnergyDensityFunctionCoupled =
      c * pow(sqrt(I3) - INT(1), INT(2)) - d * ln(sqrt(I3)) +
      c1 * (I1 - INT(3)) + c2 * (I2 - INT(3));

  //! Another coupled form of the strain energy function, Ψ(C), dependend on
  //! right Cauchy Green tensor C=(C11,...,C33)  and fiber direction
  //! a=(a1,a2,a3). Set usesFiberDirection to true if you use this. It should
  //! only use the variables C11, C12, C13, C22, C23, C33, a1, a2, a3, I4 (I4 is
  //! an alias for a0•C a0) (I5 can be computed manually from C and a).
  static const auto constexpr strainEnergyDensityFunctionCoupledDependentOnC =
      INT(0);
};

int main(int argc, char *argv[]) {
  // Solves nonlinear hyperelasticity, dynamic problem

  // initialize everything, handle arguments and parse settings from input file
  DihuContext settings(argc, argv);

  // define problem
  TimeSteppingScheme::DynamicHyperelasticitySolver<Material> problem(settings);
