...
variables.fiber_file = "../meshes/muscle_box.bin"
# -------------- end user parameters ----------------
...
print("n elements: {} x {} x {}".format(nx,ny,nz))
...
# Neumann BC at bottom nodes
k = nz-1
variables.elasticity_neumann_bc = [{"element": k*nx*ny + j*nx + i, "constantVector": [5.0,0.0,10.0], "face": "2+"} for j in range(ny) for i in range(nx)]
#variables.elasticity_neumann_bc = []

variables.constant_body_force = [0,0,0]
variables.dt_3D = 1

#initial_values_displacements = [[1.0,0.0,0.0] for j in range(my) for i in range(mx)] + [[0.0,0.0,0.0] for _ in range(mx*my*(mz-1))]
initial_values_displacements = [[0.0,0.0,0.0] for _ in range(mx*my*mz)]
...
config = {
  ...
  "PreciceAdapter": {        # precice adapter for muscle
    ...
    "couplingEnabled":          True,                       # if the precice coupling is enable, if not it is simply the normal solver (for debugging)
      ...
      "Term2": {        # solid mechanics
        "MuscleContractionSolver": {
          ...
          "DynamicHyperelasticitySolver": {
            ...
            "initialValuesDisplacements":  initial_values_displacements,     # the initial values for the displacements, vector of values for every node [[node1-x,y,z], [node2-x,y,z], ...]
