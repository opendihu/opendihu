...
variables.scenario_name = "only_tendon_debug"
...
#traction_vector = [0.4*variables.force, 0.3*variables.force, 0.5*variables.force]     # the traction force in specified in the reference configuration
face = "2+"
...
config = {
  ...
  "PreciceAdapter": {        # precice adapter for bottom tendon
    ...
    "HyperelasticitySolver": {
      ...
      "snesRelativeTolerance":      1e-3,                         # relative tolerance of the nonlinear solver
      "snesLineSearchType":         "l2",                         # type of linesearch, possible values: "bt" "nleqerr" "basic" "l2" "cp" "ncglinear"
      "snesAbsoluteTolerance":      1e-3,                         # absolute tolerance of the nonlinear solver
      ...
      #"loadFactors":                list(np.logspace(-4,0,4)),   # load factors for every timestep
      #"loadFactors":                [0.5, 1.0],                   # load factors for every timestep
      "loadFactors":                [],                           # no load factors, solve problem directly
      "loadFactorGiveUpThreshold":  0.8e-4,                         # a threshold for the load factor, when to abort the solve of the current time step. The load factors are adjusted automatically if the nonlinear solver diverged. If the load factors get too small, it aborts the solve.
      ...
      "initialValuesDisplacements":  list(zip(x_positions,y_positions,z_positions)),     # the initial values for the displacements, vector of values for every node [[node1-x,y,z], [node2-x,y,z], ...]
...
print(config["PreciceAdapter"]["HyperelasticitySolver"]["initialValuesDisplacements"])
