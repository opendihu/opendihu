 ...
 * ALGEBRAIC[3] is alpha_n in component potassium_channel_n_gate
 * (per_millisecond). ALGEBRAIC[7] is beta_n in component
 * potassium_channel_n_gate (per_millisecond). CONSTANTS[5] is g_L in component
 * leakage_current (milliS_per_cm2). CONSTANTS[8] is E_L in component
 * leakage_current (millivolt). RATES[0] is d/dt V in component membrane
 * (millivolt). RATES[1] is d/dt m in component sodium_channel_m_gate
 * (dimensionless). RATES[2] is d/dt h in component sodium_channel_h_gate
 * (dimensionless). RATES[3] is d/dt n in component potassium_channel_n_gate
 * (dimensionless).
 */
...
void computeRates(double VOI, double *CONSTANTS, double *RATES, double *STATES,
                  double *ALGEBRAIC) {
  ALGEBRAIC[1] = (-0.100000 * (STATES[0] + 50.0000)) /
                 (exp(-(STATES[0] + 50.0000) / 10.0000) - 1.00000);
  ...
  ALGEBRAIC[3] = (-0.0100000 * (STATES[0] + 65.0000)) /
                 (exp(-(STATES[0] + 65.0000) / 10.0000) - 1.00000);
  ALGEBRAIC[7] = 0.125000 * exp((STATES[0] + 75.0000) / 80.0000);
  RATES[3] = ALGEBRAIC[3] * (1.00000 - STATES[3]) - ALGEBRAIC[7] * STATES[3];
  ALGEBRAIC[0] = CONSTANTS[3] * pow(STATES[1], 3.00000) * STATES[2] *
                 (STATES[0] - CONSTANTS[6]);
  ALGEBRAIC[4] =
      CONSTANTS[4] * pow(STATES[3], 4.00000) * (STATES[0] - CONSTANTS[7]);
  ALGEBRAIC[8] = CONSTANTS[5] * (STATES[0] - CONSTANTS[8]);
  RATES[0] = -(-CONSTANTS[2] + ALGEBRAIC[0] + ALGEBRAIC[4] + ALGEBRAIC[8]) /
             CONSTANTS[1];
}
