...
TEST(NumberingsTest, DofNodeNumberingDeformableLinear1D) {
  ...
  // global_no_t meshPartition->getElementNoGlobalNatural(element_no_t
  // elementNoLocal) std::array<int,D>
  // meshPartition_->getCoordinatesGlobal(node_no_t nodeNoLocal) global_no_t
  // functionSpace->getNodeNoGlobalNatural(global_no_t elementNoGlobalNatural,
  // int nodeIndex) const node_no_t
  // functionSpace->getNeighbourNodeNoLocal(node_no_t nodeNoLocal, Mesh::face_t
  // direction) node_no_t functionSpace->getNodeNo(element_no_t elementNo, int
  // nodeIndex) meshPartition->getNodeNoGlobalNatural(coordinatesGlobal)
  // node_no_t functionSpace->meshPartition()->getNodeNoLocal(std::array<int,D>
  // coordinate)
...
config = {
  ...
  typedef SpatialDiscretization::FiniteElementMethod<
      ...
      BasisFunction::LagrangeOfOrder<1>, Quadrature::Gauss<1>, Equation::None>
      ProblemType;
  ...
  std::shared_ptr<Partition::MeshPartition<FunctionSpace>> meshPartition =
  ...
  for (element_no_t elementNoLocal = 0;
    ...
    global_no_t elementNoGlobalNatural =
        meshPartition->getElementNoGlobalNatural(elementNoLocal);

    VLOG(1) << "";
    VLOG(1) << "elementNoLocal = " << elementNoLocal
            << ", elementNoGlobalNatural = " << elementNoGlobalNatural;

    // loop over local nodes, with ghosts
    for (int nodeIndex = 0; nodeIndex < functionSpace->nNodesPerElement();
         nodeIndex++) {
      node_no_t nodeNoLocal =
          functionSpace->getNodeNo(elementNoLocal, nodeIndex);
      ...
      std::array<global_no_t, D> coordinatesGlobal =
          meshPartition->getCoordinatesGlobal(nodeNoLocal);
      std::array<int, D> coordinatesLocal;
      for (int i = 0; i < D; i++) {
        coordinatesLocal[i] =
            coordinatesGlobal[i] - meshPartition->beginNodeGlobalNatural(i);
      }

      node_no_t nodeNoLocalFromCoordinates =
          functionSpace->meshPartition()->getNodeNoLocal(coordinatesLocal);

      VLOG(1) << "coordinatesGlobal: " << coordinatesGlobal;

      global_no_t nodeNoGlobalNaturalFromCoordinates =
          meshPartition->getNodeNoGlobalNatural(coordinatesGlobal);
      global_no_t nodeNoGlobalNatural = functionSpace->getNodeNoGlobalNatural(
          elementNoGlobalNatural, nodeIndex);

      VLOG(1) << "nodeNoGlobalNaturalFromCoordinates: "
              << nodeNoGlobalNaturalFromCoordinates
              << ", nodeNoGlobalNatural: " << nodeNoGlobalNatural;
      VLOG(1) << "nodeNoLocalFromCoordinates: " << nodeNoLocalFromCoordinates
              << ", nodeNoLocal: " << nodeNoLocal;
  ...
  std::vector<
      std::pair<std::pair<Mesh::face_t, Mesh::face_t>, std::array<int, D>>>
      complementFaces = {
  ...
  for (int i = 0; i < complementFaces.size(); i++) {
    ...
    for (node_no_t nodeNoLocal = 0;
         nodeNoLocal < functionSpace->nNodesLocalWithoutGhosts();
         nodeNoLocal++) {
      node_no_t neighbourNodeNo =
          functionSpace->getNeighbourNodeNoLocal(nodeNoLocal, face0);
      VLOG(1) << "nodeNoLocal = " << nodeNoLocal << ", face " << face0
              << ", neighbour: " << neighbourNodeNo;

      if (neighbourNodeNo != -1) {
        // compare difference between global coordinates of nodes
        std::array<global_no_t, D> coordinatesGlobalNeighbour =
            meshPartition->getCoordinatesGlobal(neighbourNodeNo);
        std::array<global_no_t, D> coordinatesGlobalOrigin =
            meshPartition->getCoordinatesGlobal(nodeNoLocal);

        std::array<int, D> realDifference;
        for (int j = 0; j < D; j++) {
          realDifference[j] =
              coordinatesGlobalNeighbour[j] - coordinatesGlobalOrigin[j];
        }

        VLOG(1) << "coordinates origin: " << coordinatesGlobalOrigin
                << ", neighbour: " << coordinatesGlobalNeighbour
      ...
      if (neighbourNodeNo != -1 &&
          neighbourNodeNo < functionSpace->nNodesLocalWithoutGhosts()) {
        node_no_t originNode =
            functionSpace->getNeighbourNodeNoLocal(neighbourNodeNo, face1);
...
config = {
  ...
  typedef SpatialDiscretization::FiniteElementMethod<
      Mesh::StructuredDeformableOfDimension<1>,
      BasisFunction::LagrangeOfOrder<2>, Quadrature::Gauss<1>, Equation::None>
      ProblemType;
  ProblemType problem(settings);
