...
config = {
  ...
  pythonConfig.replace(pos, strToReplace.length(), "out/fast_vc/fibers");
...
for i in range(n_values):
  ...
  print("vc file no. {}, error: {}".format(i, error))

total_error /= n_values
print("vc avg error: {}".format(total_error))
  ...
  LOG(DEBUG) << "error between not_fast and fast_vc: " << error;

  ASSERT_LE(error, 0.1);

  // check also with gpu code (not on gpu here)

  strToReplace = "out/fast_vc/fibers";
  pos = pythonConfig.find(strToReplace);
  pythonConfig.replace(pos, strToReplace.length(), "out/fast_gpu/fibers");

  strToReplace = "\"vc\"";
  pos = pythonConfig.find(strToReplace);
  pythonConfig.replace(pos, strToReplace.length(), "\"gpu\"");
  ...
  // compare results of problem1 and problem3
  command = R"(
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
for i in range(n_values):
  ...
  print("gpu file no. {}, error: {}".format(i, error))

total_error /= n_values
print("gpu avg error: {}".format(total_error))

)";
  returnValue = PyRun_SimpleString(command.c_str());
  PythonUtility::checkForError();
  LOG(DEBUG) << returnValue;
  ASSERT_EQ(returnValue, 0);
  ...
  totalError = PyObject_GetAttrString(mainModule, "total_error");

  error = PythonUtility::convertFromPython<double>::get(totalError);
  LOG(DEBUG) << "error between not_fast and fast_gpu: " << error;

  ASSERT_LE(error, 1.35);
}
