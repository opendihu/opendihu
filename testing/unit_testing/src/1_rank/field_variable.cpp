...
config = {
  ...
  SpatialDiscretization::FiniteElementMethod<
      ...
      BasisFunction::LagrangeOfOrder<>, Quadrature::Gauss<2>, Equation::None>
      finiteElementMethod(settings);
  finiteElementMethod.run();

  // std::shared_ptr<FieldVariable::FieldVariable<FunctionSpace::Generic>>
  // createGenericFieldVariable(std::shared_ptr<Partition::Manager>
  // partitionManager, int nEntries, std::string name);
  ...
  std::shared_ptr<FieldVariable::FieldVariable<FunctionSpace::Generic, 1>>
      fieldVariable = meshManager->createGenericFieldVariable(5, "test");
  ...
  PetscErrorCode ierr;
  ierr = VecShift(fieldVariable->valuesGlobal(), 1);
  CHKERRV(ierr);
...
config = {
  ...
  std::shared_ptr<FunctionSpace1> functionSpace1 =
      meshManager->createFunctionSpace<FunctionSpace1>(
          "functionSpace1", nElements1, physicalExtent, nRanks);
  std::shared_ptr<FunctionSpace2> functionSpace2 =
      meshManager->createFunctionSpace<FunctionSpace2>(
          "functionSpace2", nElements2, physicalExtent, nRanks);

  // create dense matrix, 9x49
  std::shared_ptr<PartitionedPetscMat<FunctionSpace1, FunctionSpace2>> matrix =
      std::make_shared<PartitionedPetscMat<FunctionSpace1, FunctionSpace2>>(
          functionSpace1->meshPartition(), functionSpace2->meshPartition(), 1,
          "T");
  ...
  matrix->setValues(rowIndices.size(), rowIndices.data(), columnIndices.size(),
                    columnIndices.data(), values.data(), INSERT_VALUES);
  ...
  std::vector<double> resultValues(resultRowIndices.size() *
                                   resultColumnIndices.size());

  matrix->getValues(resultRowIndices.size(), resultRowIndices.data(),
                    resultColumnIndices.size(), resultColumnIndices.data(),
                    resultValues.data());
...
config = {
  ...
  typedef std::shared_ptr<
      FieldVariable::FieldVariableBaseFunctionSpace<FunctionSpaceType>>
      FieldVariableBaseFunctionSpaceType;
  typedef std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 1>>
      FieldVariable1Type;
  typedef std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 2>>
      FieldVariable2Type;
  typedef std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 3>>
      FieldVariable3Type;
  ...
  FieldVariableBaseFunctionSpaceType aBase =
      functionSpace->createFieldVariable("a", {"x", "y"});
  FieldVariableBaseFunctionSpaceType bBase =
      functionSpace->createFieldVariable("b", 2);
  FieldVariableBaseFunctionSpaceType cBase =
      functionSpace->createFieldVariable("c");
  FieldVariable3Type d = functionSpace->template createFieldVariable<3>("d");

  FieldVariable2Type a = std::static_pointer_cast<
      FieldVariable::FieldVariable<FunctionSpaceType, 2>>(aBase);
  FieldVariable2Type b = std::static_pointer_cast<
      FieldVariable::FieldVariable<FunctionSpaceType, 2>>(bBase);
  FieldVariable1Type c = std::static_pointer_cast<
      FieldVariable::FieldVariable<FunctionSpaceType, 1>>(cBase);
  ...
  std::vector<Vec2> v1 = {Vec2({1.0, 2.0}), Vec2({3.0, 4.0}), Vec2({5.0, 6.0}),
  ...
  std::vector<double> reference0 = {0.0, 0.0, 5.0, 0.0, 1.0, 1.0, 0.0, 7.0, 3.0,
  ...
  std::vector<double> reference1 = {
      0.0, 0.0, 6.0, 0.0, 2.0, 2.0, 0.0, 8.0, 4.0, 0.0, 10.0, 0.0, 0.0,
  ...
  std::array<Vec2, 3> reference4 = {Vec2({0.0, 0.0}), Vec2({7.0, 8.0}),
                                    Vec2({5.0, 6.0})};
  ...
  std::array<double, FunctionSpaceType::nDofsPerElement()> reference5 = {
      5.0, 0.0, 1.0, 7.0, 3.0, 0.0, 0.0, 0.0, 0.0};
  ...
  std::array<double, FunctionSpaceType::nDofsPerElement()> reference6 = {
      6.0, 0.0, 2.0, 8.0, 4.0, 0.0, 0.0, 0.0, 0.0};
  ...
  std::array<Vec2, FunctionSpaceType::nDofsPerElement()> reference7 = {
      Vec2({5.0, 6.0}), Vec2({0.0, 0.0}), Vec2({1.0, 2.0}),
      Vec2({7.0, 8.0}), Vec2({3.0, 4.0}), Vec2({0.0, 0.0}),
  ...
  std::array<Vec2, 3> reference9 = {Vec2({9.0, 9.0}), Vec2({9.0, 9.0}),
                                    Vec2({9.0, 9.0})};
  ...
  void
  setValues(FieldVariable<FunctionSpace::FunctionSpace<Mesh::StructuredDeformableOfDimension<D>,BasisFunctionType>,nComponents>
  &rhs);
  */
  ...
  std::shared_ptr<FieldVariable::FieldVariableBaseFunctionSpace<FunctionSpace<MeshType,BasisFunctionType>>>
  createFieldVariable(std::string name, std::vector<std::string>
  componentNames);

  //! create a non-geometry field field variable with no values being set, with
  given number of components, the component names will be the numbers
  std::shared_ptr<FieldVariable::FieldVariableBaseFunctionSpace<FunctionSpace<MeshType,BasisFunctionType>>>
  createFieldVariable(std::string name, int nComponents=1);

  //! create a non-geometry field field variable with no values being set, with
  given number of components, the component names will be the numbers template
  <int nComponents>
  ...
  */
  ...
  finishGhostManipulation has to be called to apply the cached changes void
  setValues(std::vector<dof_no_t> &dofGlobalNos,
  std::vector<std::array<double,nComponents>> &values, InsertMode
  petscInsertMode=INSERT_VALUES)
  ...
  non-derivative values are retrieved void getValuesWithoutGhosts(int
  componentNo, std::vector<double> &values, bool onlyNodalValues=false);

  //! for a specific component, get values from their global dof no.s, as array,
  therefore templated by the number of elements, N, to retrieve template<int N>
  void getValuesWithoutGhosts(int componentNo, std::array<dof_no_t,N>
  dofGlobalNo, std::array<double,N> &values);

  //! for a specific component, get values from their global dof no.s, as vector
  void getValuesWithoutGhosts(int componentNo, std::vector<dof_no_t>
  dofGlobalNo, std::vector<double> &values);

  //! get values from their global dof no.s for all components
  template<int N>
  void getValuesWithoutGhosts(std::array<dof_no_t,N> dofGlobalNo,
  std::array<std::array<double,nComponents>,N> &values)

  //! for a specific component, get the values corresponding to all
  element-local dofs template<int N> void getElementValues(int componentNo,
  element_no_t elementNo,
  std::array<double,FunctionSpaceType::nDofsPerElement()> &values);

  //! get the values corresponding to all element-local dofs for all components
  void getElementValues(element_no_t elementNo,
  ...
  //! set a single dof (all components) , after all calls to setValue(s),
  finishGhostManipulation has to be called to apply the cached changes void
  setValue(dof_no_t dofGlobalNo, double value, InsertMode
  petscInsertMode=INSERT_VALUES)

  //! set values for all components for dofs, after all calls to setValue(s),
  finishGhostManipulation has to be called to apply the cached changes void
  setValues(std::vector<dof_no_t> &dofGlobalNos, std::vector<double> &values,
  InsertMode petscInsertMode=INSERT_VALUES)
*/
}
...
config = {
  ...
  std::shared_ptr<FunctionSpaceType> functionSpace =
      std::static_pointer_cast<FunctionSpaceType>(
          finiteElementMethod.functionSpace());
...
config = {
  ...
  a->getElementValues(0, 1, values5);
  std::array<double, FunctionSpaceType::nDofsPerElement()> reference5 = {
      5.0, 0.0, 9.0, 1.0, 0.0, 0.0, 3.0, 0.0, 0.0};
  ASSERT_EQ(values5, reference5);
  ...
  a->getElementValues(1, 1, values6);
  std::array<double, FunctionSpaceType::nDofsPerElement()> reference6 = {
      6.0, 0.0, 10.0, 2.0, 0.0, 0.0, 4.0, 0.0, 0.0};
  ASSERT_EQ(values6, reference6);
  ...
  a->getElementValues(1, values7);
  std::array<Vec2, FunctionSpaceType::nDofsPerElement()> reference7 = {
      Vec2({5.0, 6.0}), Vec2({0.0, 0.0}), Vec2({9.0, 10.0}),
      Vec2({1.0, 2.0}), Vec2({0.0, 0.0}), Vec2({0.0, 0.0}),
