 ...
 *   A DiscretizableInTime class has no own slotConnectorData, since is has no
 * own data. The relevant data (solution vector) is stored in the timestepping
 * scheme. However, the algebraics and parameters are stored here (in the data
 * object). Therefore, the CellmlAdapter has a copy of the slotConnectorData of
...
public:
  typedef FieldVariable::FieldVariable<FunctionSpaceType, nAlgebraics>
      FieldVariableAlgebraics;
  typedef FieldVariable::FieldVariable<FunctionSpaceType, nStates>
      FieldVariableStates;
  // typedef SlotConnectorData<FunctionSpaceType,nStates,nAlgebraics>
  ...
  //! get a raw r/w memory pointer to the values of the parameters field
  //! variable, to be used in the rhs function the array pointed to by this
  //! pointer contains parameter values for all instances, nAlgebraics
  //! parameters for each instance, in struct-of-array memory layout
  double *parameterValues();

  //! initialize data structures
  void initialize();

  //! assign the field variable that stores the states, this variable is owned
  ...
  void
  setAlgebraicAndParameterNames(const std::vector<std::string> &algebraicNames,
                                const std::vector<std::string> &parameterNames,
                                const std::vector<std::string> &slotNames);

  //! get the parameteValues_ pointer from the parameters field variable, then
  //! the field variable can no longer be used until restoreParameterValues()
  //! gets called
  ...
  //! initialize the slotConnectorData object of the timestepping scheme, this
  //! method is called once in the initialize() of the timestepping scheme It
  //! adds all slots that were defined in the CellmlAdapter to the
  //! slotConnectorData of the timestepping scheme
  void setSlotConnectorDataTimestepping(
...
private:
  ...
  //! initialize the slotConnectorData_ object, this can only be done after the
  //! states variable has been set
  ...
  std::vector<std::string>
      parameterNames_; //< component names of the parameter field variable
  std::vector<std::string>
      slotNames_; //< names of the data slots that are used for slot connectors
