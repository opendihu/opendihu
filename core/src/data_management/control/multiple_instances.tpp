...
MultipleInstances<FunctionSpaceType, BaseDataType>::MultipleInstances(
    DihuContext context)
...
void MultipleInstances<FunctionSpaceType, BaseTimesteppingType>::
  ...
  for (typename std::vector<BaseTimesteppingType>::iterator iter =
           instances.begin();
  ...
  if (!instances.empty()) {
    VLOG(1)
        << " set Function space from instance data to MultipleInstances data";
...
template <typename FunctionSpaceType, typename BaseDataType>
void MultipleInstances<FunctionSpaceType, BaseDataType>::print() {
  if (!VLOG_IS_ON(4))
    return;
}
...
MultipleInstances<FunctionSpaceType,
  ...
  std::vector<typename BaseDataType::FieldVariablesForOutputWriter>
      instancesFieldVariablesForOutputWriter;
  ...
  for (typename std::vector<std::shared_ptr<BaseDataType>>::const_iterator
    ...
    instancesFieldVariablesForOutputWriter.push_back(
        (*iter)->getFieldVariablesForOutputWriter());
  }
