...
public:
  ...
  using FiniteElementsBase<FunctionSpaceType,
                           nComponents>::FieldVariablesForOutputWriter;

  //! initialize stifness parameters, then call the initialize method of the
  //! base class
  ...
  void setActiveStress(std::shared_ptr<FieldVariable::FieldVariable<
  ...
  //! set the pointer to rightHandSideActive, the actual variable is stored in
  //! the quasi_static_linear_elasticity class
  void setRightHandSideActive(
      std::shared_ptr<
          FieldVariable::FieldVariable<FunctionSpaceType, nComponents>>
          rightHandSideActive);

  //! get a debugging field variable that contains the rhs contribution from the
  //! active stress
