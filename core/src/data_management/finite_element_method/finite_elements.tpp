...
FiniteElements<FunctionSpaceType, nComponents, Term,
               Equation::hasGeneralizedLaplaceOperator<Term>>::
...
FiniteElements<FunctionSpaceType, nComponents,
...
template <typename FunctionSpaceType, int nComponents, typename Term>
void FiniteElements<
    FunctionSpaceType, nComponents, Term,
    Equation::hasGeneralizedLaplaceOperator<Term>>::initialize() {
...
template <typename FunctionSpaceType, int nComponents>
void FiniteElements<FunctionSpaceType, nComponents,
                    Equation::Dynamic::DirectionalDiffusion>::
    initialize(
  ...
  DiffusionTensorDirectional<FunctionSpaceType>::initialize(
      this->functionSpace_, direction, spatiallyVaryingPrefactor,
      useAdditionalDiffusionTensor);
}

//! initialize, store the reference geometry as copy of the current geometry
template <typename FunctionSpaceType, int nComponents, typename Term>
void FiniteElements<FunctionSpaceType, nComponents, Term,
                    Equation::isLinearElasticity<Term>>::initialize() {
  ...
  referenceGeometry_->setUniqueName(
      StringUtility::getFirstNE(this->uniquePrefix_, "finite_elements_") +
      "referenceGeometry");
...
void FiniteElements<
    FunctionSpaceType, nComponents, Term,
    Equation::isLinearElasticity<Term>>::updateGeometry(double scalingFactor) {
  PetscErrorCode ierr;
  ...
  ierr = VecWAXPY(this->functionSpace_->geometryField().valuesGlobal(),
                  scalingFactor, this->solution()->valuesGlobal(),
                  this->referenceGeometry_->valuesGlobal());
  CHKERRV(ierr);

  this->functionSpace_->geometryField().startGhostManipulation();

  if (VLOG_IS_ON(1)) {
    VLOG(1) << "scalingFactor: " << scalingFactor
            << ", solution: " << *this->solution()
            << ", referenceGeometry: " << *this->referenceGeometry_
...
void FiniteElements<FunctionSpaceType, nComponents, Term,
    ...
    computeStrain(std::shared_ptr<FieldVariable::FieldVariable<
                      FunctionSpaceType, nComponents * nComponents>>
                      strain) {
  // compute strain as epsilon_ab = 1/2(u_Lb dphi_L(x)/dx_a + u_Ma
  // dphi_M(x)/dx_b) compute strain as epsilon_ab = 1/2*u_Lb dphi_L(x)/dx_a +
  // 1/2*u_La dphi_L(x)/dx_b)
  ...
  std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, nComponents>>
      displacements = this->solution();

  // We set values at nodes, without any integration. The gradient dphi_L/dx_a
  // is potentially discontinous, therefore we loop over elements. The value at
  // each node is set multiple times, once from each adjacent element, the last
  // value wins. It is important to not communicate ghost values here, because
  // we do not add values.
  ...
  for (element_no_t elementNoLocal = 0;
    ...
    for (int dofIndexL = 0; dofIndexL < nDofsPerElement; dofIndexL++) {
      ...
      std::array<VecD<D>, nDofsPerElement> gradPhi =
          this->functionSpace_->getGradPhi(xi);

      // compute inverse jacobian
      Tensor2<D> inverseJacobian = this->functionSpace_->getInverseJacobian(
          geometryValues, elementNoLocal, xi);
      ...
      for (int a = 0; a < D; a++) {
        for (int b = 0; b < D; b++) {
          epsilon[a * D + b] =
              0.5 * (displacementValues[dofIndexL][b] * dPhiL_dX[a] +
                     displacementValues[dofIndexL][a] * dPhiL_dX[b]);
        }
      }
      ...
      if (VLOG_IS_ON(1)) {
        int k = dofNoLocal /
                (this->functionSpace_->meshPartition()->nNodesGlobal(0) *
                 this->functionSpace_->meshPartition()->nNodesGlobal(1));
        VLOG(1) << "k: " << k << ", compute strain dof " << dofIndexL
                << ", displacements z: " << displacementValues[dofIndexL][2]
                << ", dphi/dx: " << dPhiL_dX[2] << ", strain: " << epsilon;
      }
