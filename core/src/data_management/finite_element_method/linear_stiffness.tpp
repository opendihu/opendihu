...
void LinearStiffness<FunctionSpaceType, nComponents>::initialize() {
  FiniteElementsBase<FunctionSpaceType, nComponents>::initialize();

  // load K and μ from settings
  bulkModulus_ = this->context_.getPythonConfig().getOptionDouble(
      "bulkModulus", 1.0, PythonUtility::ValidityCriterion::Positive);
  shearModulus_ = this->context_.getPythonConfig().getOptionDouble(
      "shearModulus", 1.0, PythonUtility::ValidityCriterion::Positive);
}

//! get the value of the 2nd order stiffness tensor, C_abcd
template <typename FunctionSpaceType, int nComponents>
double LinearStiffness<FunctionSpaceType, nComponents>::linearStiffness(
  ...
  return bulkModulus_ * delta_ab * delta_cd +
         shearModulus_ * (delta_ac * delta_bd + delta_ad * delta_bc -
                          2. / 3 * delta_ab * delta_cd);
}

//! set values for active stress
template <typename FunctionSpaceType, int nComponents>
void LinearStiffness<FunctionSpaceType, nComponents>::setActiveStress(
...
//! set the pointer to rightHandSideActive, the actual variable is stored in the
//! quasi_static_linear_elasticity class
template <typename FunctionSpaceType, int nComponents>
void LinearStiffness<FunctionSpaceType, nComponents>::setRightHandSideActive(
    std::shared_ptr<
...
LinearStiffness<FunctionSpaceType, nComponents>::activeStress() {
...
LinearStiffness<FunctionSpaceType, nComponents>::rightHandSideActive() {
