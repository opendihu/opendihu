...
/** A Diffusion tensor that points in a given fiber direction and can have a
 *spatially varying prefactor, i.e. c(x)*A(f(x)),  f(x)...fiber direction at x,
 *c(x)...scalar prefactor
 *
 **/
...
public:
  ...
  //! spatiallyVaryingPrefactor can be nullptr, then the factor is constant 1
  void
