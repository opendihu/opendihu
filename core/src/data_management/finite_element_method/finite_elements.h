 ...
 */
template <typename FunctionSpaceType, int nComponents, typename Term>
class FiniteElements<FunctionSpaceType, nComponents, Term,
...
/** for directional diffusion use the diffusion tensor that depends upon a
 * direction field
 */
template <typename FunctionSpaceType, int nComponents>
class FiniteElements<FunctionSpaceType, nComponents,
...
/** for linear elasticity use the class that holds linear elastcity parameters,
 * K and μ
 */
template <typename FunctionSpaceType, int nComponents, typename Term>
class FiniteElements<FunctionSpaceType, nComponents, Term,
...
public:
  ...
  using LinearStiffness<FunctionSpaceType,
  ...
  //! scaled by scalingFactor
  void updateGeometry(double scalingFactor = 1.0);

  //! compute the strain from the current displacement (which is the solution
  //! field variable)
  void computeStrain(std::shared_ptr<FieldVariable::FieldVariable<
                         FunctionSpaceType, nComponents * nComponents>>
                         strain);

protected:
  std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 3>>
      referenceGeometry_; //< reference geometry
};
