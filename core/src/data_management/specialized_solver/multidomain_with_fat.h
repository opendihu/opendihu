...
public:
  typedef FieldVariable::FieldVariable<FunctionSpaceFatType, 1>
      FieldVariableFatType;
  typedef FieldVariable::FieldVariable<FunctionSpaceFatType, 3>
      GeometryFieldVariableFatType;
  typedef typename Multidomain<FunctionSpaceType>::FieldVariableType
      FieldVariableType;
  typedef typename Multidomain<FunctionSpaceType>::GradientFieldVariableType
      GradientFieldVariableType;

  //! constructor
  MultidomainWithFat(DihuContext context);

  //! assign the data object of MultidomainSolver which contains all data
  //! without the fat layer
  void setDataMultidomain(
      std::shared_ptr<Multidomain<FunctionSpaceType>> dataMultidomain);
...
private:
  ...
  std::shared_ptr<Multidomain<FunctionSpaceType>>
      dataMultidomain_; //< data object of multidomain
  std::shared_ptr<FieldVariableFatType>
      extraCellularPotentialFat_; //< phi_e value on fat layer
};
