...
DiffusionAdvectionSolver<FunctionSpaceType>::DiffusionAdvectionSolver(
    DihuContext context)
    : Data<FunctionSpaceType>::Data(context) {}

template <typename FunctionSpaceType>
void DiffusionAdvectionSolver<FunctionSpaceType>::initialize() {
...
template <typename FunctionSpaceType>
void DiffusionAdvectionSolver<FunctionSpaceType>::getPetscMemoryParameters(
...
void DiffusionAdvectionSolver<FunctionSpaceType>::createPetscObjects() {
  ...
  this->vMatrix_ = std::make_shared<PartitionedPetscMat<FunctionSpaceType>>(
      meshPartition, 1, nNonZerosDiagonal, nNonZerosOffdiagonal, "vMatrix");
}
...
DiffusionAdvectionSolver<FunctionSpaceType>::solution() {
  return this->solution_;
}

template <typename FunctionSpaceType>
std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 1>>
DiffusionAdvectionSolver<FunctionSpaceType>::increment() {
  return this->increment_;
}

template <typename FunctionSpaceType>
std::shared_ptr<PartitionedPetscMat<FunctionSpaceType>>
DiffusionAdvectionSolver<FunctionSpaceType>::vMatrix() {
  return this->vMatrix_;
}

template <typename FunctionSpaceType>
std::shared_ptr<
    typename DiffusionAdvectionSolver<FunctionSpaceType>::SlotConnectorDataType>
DiffusionAdvectionSolver<FunctionSpaceType>::getSlotConnectorData() {
...
DiffusionAdvectionSolver<
  ...
  return std::make_tuple(
      ...
      this->increment_ // add all field variables that should appear in the
                       // output file. Of course, this list has to match the
