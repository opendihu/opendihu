 ...
 */
template <typename FunctionSpaceType, int nComponents1 = 1,
          int nComponents2 = 1>
...
public:
  ...
  typedef SlotConnectorData<FunctionSpaceType, nComponents1, nComponents2>
      SlotConnectorDataType;

  //! constructor
  PrescribedValues(DihuContext context);

  //! return a reference to the fieldVariable with given index with nComponents1
  //! components
  std::shared_ptr<FieldVariable1Type> fieldVariable1(int index);

  //! return a reference to the fieldVariable with given index with nComponents2
  //! components
  std::shared_ptr<FieldVariable2Type> fieldVariable2(int index);

  //! initialize and create all field variables
  void initialize(std::vector<std::string> fieldVariable1Names,
