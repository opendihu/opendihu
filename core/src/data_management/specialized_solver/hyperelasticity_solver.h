 ...
 *   Typically, PressureFunctionSpace is linear and DisplacementsFunctionSpace
 * is quadratic.
 */
template <typename PressureFunctionSpace, typename DisplacementsFunctionSpace,
          typename Term, bool withLargeOutput>
class QuasiStaticHyperelasticityBase : public Data<DisplacementsFunctionSpace> {
public:
  typedef FieldVariable::FieldVariable<DisplacementsFunctionSpace, 3>
      DisplacementsFieldVariableType;
  typedef FieldVariable::FieldVariable<PressureFunctionSpace, 3>
      DisplacementsLinearFieldVariableType;
  ...
  typedef FieldVariable::FieldVariable<DisplacementsFunctionSpace, 6>
      StressFieldVariableType; // Voigt notation
  typedef FieldVariable::FieldVariable<DisplacementsFunctionSpace, 9>
      DeformationGradientFieldVariableType; // row-major

  //! define the type of output connection variables, i.e. the values that will
  //! be transferred if the solver is part of a splitting or coupling scheme
  typedef SlotConnectorData<DisplacementsFunctionSpace, 3>
  ...
  //! variable
  void setDisplacementsFunctionSpace(
  ...
  //! add the displacements to the reference geometry to obtain the current
  //! geometry, for both function spaces (linear and quadratic) \param
  //! scalingFactor factor with which to scale the displacements \param
  //! updateLinearVariables if the variables of the linear discretized mesh
  //! should be update from the quadratic mesh, this is only needed if the
  //! pressure output writer is used
  void updateGeometry(double scalingFactor = 1.0,
                      bool updateLinearVariables = true);
...
protected:
  ...
  std::shared_ptr<DisplacementsFieldVariableType>
      traction_; //< t, the traction in current configuration
  std::shared_ptr<DisplacementsFieldVariableType>
 ...
 *  Depending on the Term if it uses fiberDirection information, also output a
 * fiber direction field. The normal isotropic Mooney-Rivlin thus has no fiber
 * direction output.
 *
 *  withLargeOutput = false, Term::usesFiberDirection = false
 */
template <typename PressureFunctionSpace, typename DisplacementsFunctionSpace,
          typename Term, bool withLargeOutput = false,
          typename DummyForTraits = Term>
class QuasiStaticHyperelasticity
    : public QuasiStaticHyperelasticityBase<PressureFunctionSpace,
                                            DisplacementsFunctionSpace, Term,
                                            withLargeOutput> {
public:
  using QuasiStaticHyperelasticityBase<
      PressureFunctionSpace, DisplacementsFunctionSpace, Term,
      withLargeOutput>::QuasiStaticHyperelasticityBase;
...
class QuasiStaticHyperelasticity<
    ...
    : public QuasiStaticHyperelasticityBase<
          PressureFunctionSpace, DisplacementsFunctionSpace, Term, true> {
public:
  using QuasiStaticHyperelasticityBase<PressureFunctionSpace,
                                       DisplacementsFunctionSpace, Term,
                                       true>::QuasiStaticHyperelasticityBase;
...
public:
  using QuasiStaticHyperelasticityBase<PressureFunctionSpace,
                                       DisplacementsFunctionSpace, Term,
                                       false>::QuasiStaticHyperelasticityBase;
 ...
 *  With the normal class, QuasiStaticHyperelasticity only variables on the
 * DisplacementsFunctionSpace can be written. Using an object of this class, the
 * pressure values can be written. Use the following, where data_ is an object
 * of QuasiStaticHyperelasticityPressureOutput.
 *
...
public:
  ...
  // code to output pressure field variable (it is not possible to output both
  // displacements and pressure, because the function spaces are different)
  typedef std::tuple<
