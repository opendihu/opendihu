...
/**  The datastructures for the chaste adapter where chaste computes a static
 * hyperelastic problem. The field variables here will receive the resulting
 * values from the chaste data structures.
 */
template <typename FunctionSpace>
class QuasiStaticNonlinearElasticityChaste : public Data<FunctionSpace> {
public:
  ...
  typedef FieldVariable::FieldVariable<FunctionSpace, 9>
...
private:
  ...
  std::shared_ptr<StressFieldVariableType>
      activeStress_; //< field variable of the active stress in the muscle
  std::shared_ptr<VectorFieldVariableType>
      displacement_; //< field variable of the displacement
};
