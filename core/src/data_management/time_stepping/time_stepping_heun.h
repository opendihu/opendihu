...
/**  The datastructures used for Heun timestepping schemes, store all data as of
 * timestepping schemes and an additional algebraic increment vector.
 */
...
public:
  ...
  //! return a reference to the algebraic increment vector, the PETSc Vec can be
...
private:
  ...
  std::shared_ptr<FieldVariableType>
      algebraicIncrement_; //< the additional vector for delta u*
  // std::shared_ptr<FieldVariableType> algebraicSolution_; // / < the
  // additional vector for u* // don't need this anymore
};
