...
/**  The datastructures used for timestepping schemes, essentially stores a
 * solution and an increment vector. nComponents is a constant that determines
 * the number of components of each field variable. The time stepping works
 * independent of that constant, it is only important for the output files, how
 * many values there are associated with a single node. E.g. for a cellml model
 * the nComponents should be set to the number of components of this model.
 */
...
public:
  ...
  //! return a reference to the increment vector, the PETSc Vec can be obtained
  //! via fieldVariable->valuesGlobal()
...
protected:
  ...
  std::shared_ptr<FieldVariableType>
      solution_; //< the vector of the variable of interest
  std::shared_ptr<FieldVariableType>
      increment_; //< the vector for delta u, (note, this might be reduced in
                  //future to only a sub-part of the whole data vector if memory
  ...
  std::vector<std::shared_ptr<ScalarFieldVariableType>>
