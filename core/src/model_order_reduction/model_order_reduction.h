 ...
 */
template <typename FunctionSpaceRows> class MORBase {
public:
  typedef Data::ModelOrderReduction<FunctionSpaceRows>
      DataMOR; // type of Data object
...
protected:
  //! Map to the reduced order space. Modification to MatMult in case that size
  //! of vector x does not match to the columns of the matrix.
  virtual void MatMultReduced(Mat mat, Vec x, Vec y);

  //! Map to the full order space. Modification to MatMult in case that size of
  //! vector y does not match to the rows of the matrix.
  virtual void MatMultFull(Mat mat, Vec x, Vec y);

  std::shared_ptr<DataMOR>
      dataMOR_; //< contains matrices basis and reduced matrices
