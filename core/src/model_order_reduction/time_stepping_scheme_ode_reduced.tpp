...
TimeSteppingSchemeOdeReduced<TimeSteppingType>::TimeSteppingSchemeOdeReduced(
    DihuContext context, std::string name)
    : MORBase<typename TimeSteppingType::FunctionSpace>(
          context["ModelOrderReduction"]),
      ::TimeSteppingScheme::TimeSteppingSchemeOdeBase<::FunctionSpace::Generic,
                                                      1>(
          context["ModelOrderReduction"], name),
      fullTimestepping_(context["ModelOrderReduction"]), initialized_(false) {
  LOG(DEBUG) << "Constructor TimeSteppingSchemeOdeReduced, given context: "
             << context.getPythonConfig();
  ...
  if (this->specificSettingsMOR_.hasKey("nReducedBases")) {
    this->nReducedBases_ = this->specificSettingsMOR_.getOptionInt(
        "nReducedBases", 10, PythonUtility::Positive);
    LOG(DEBUG) << "nReducedBases: " << this->nReducedBases_;
  }

  if (this->specificSettingsMOR_.hasKey("nRowsSnapshots")) {
    this->nRowsSnapshots_ = this->specificSettingsMOR_.getOptionInt(
        "nRowsSnapshots", 10, PythonUtility::Positive);
  ...
  if (this->context_.meshManager()->hasFunctionSpace("functionSpaceReduced")) {
    ...
    this->functionSpaceRed = this->context_.meshManager()
                                 ->template functionSpace<GenericFunctionSpace>(
                                     "functionSpaceReduced");
  } else {
    // create the functionspace for the reduced order
    LOG(DEBUG) << "nReducedBases: " << this->nReducedBases_;
    this->functionSpaceRed =
        this->context_.meshManager()->createGenericFunctionSpace(
            this->nReducedBases_, "functionSpaceReduced");
    LOG(DEBUG) << "functionSpaceRed";
  }

  if (this->context_.meshManager()->hasFunctionSpace(
          "functionSpaceRowsSnapshots")) {
    // take the existing function space
    this->functionSpaceRowsSnapshots =
    ...
    this->functionSpaceRowsSnapshots =
        this->context_.meshManager()->createGenericFunctionSpace(
            this->nRowsSnapshots_, "functionSpaceRowsSnapshots");
    LOG(DEBUG) << "functionSpaceRowsSnapshots";
  }

  this->data_ =
      std::make_shared<::Data::TimeStepping<::FunctionSpace::Generic, 1>>(
          context); // create data object
}
...
void TimeSteppingSchemeOdeReduced<TimeSteppingType>::setInitialValues() {
  ...
  Vec &solution =
      this->fullTimestepping_.data().solution()->getValuesContiguous();
  ...
  VLOG(2) << "setInitialValues() mat_sz_1: " << mat_sz_1
          << " mat_sz_2: " << mat_sz_2;
...
template <typename TimeSteppingType>
void TimeSteppingSchemeOdeReduced<TimeSteppingType>::initialize() {
  if (initialized_)
    return;
  ...
  VLOG(1) << "initialized full-order solution: "
          << *this->fullTimestepping_.data().solution();

  LOG(DEBUG) << "fullTimestepping_ has function space: "
             << this->fullTimestepping_.data().functionSpace()->meshName();
...
template <typename TimeSteppingType>
void TimeSteppingSchemeOdeReduced<TimeSteppingType>::run() {
  // initialize
  this->initialize();
...
template <typename TimeSteppingType>
TimeSteppingType
TimeSteppingSchemeOdeReduced<TimeSteppingType>::fullTimestepping() {
  return fullTimestepping_;
}

template <typename TimeSteppingType>
std::shared_ptr<typename TimeSteppingSchemeOdeReduced<
    TimeSteppingType>::SlotConnectorDataType>
TimeSteppingSchemeOdeReduced<TimeSteppingType>::getSlotConnectorData() {
