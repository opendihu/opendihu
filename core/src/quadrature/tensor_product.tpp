...
constexpr int TensorProductBase<D, Quadrature>::numberEvaluations() {
...
std::array<std::array<double, 1>,
           TensorProductBase<1, Quadrature>::numberEvaluations()>
TensorProduct<1, Quadrature>::samplingPoints() {
  std::array<std::array<double, 1>,
             TensorProductBase<1, Quadrature>::numberEvaluations()>
      samplingPoints;
  std::array<double, Quadrature::numberEvaluations()> samplingPoints1D =
      Quadrature::samplingPoints();
...
// 2D sampling points
template <typename Quadrature>
std::array<Vec2, TensorProductBase<2, Quadrature>::numberEvaluations()>
TensorProduct<2, Quadrature>::samplingPoints() {
  std::array<Vec2, TensorProductBase<2, Quadrature>::numberEvaluations()>
      samplingPoints;
  std::array<double, Quadrature::numberEvaluations()> samplingPoints1D =
      Quadrature::samplingPoints();
  ...
  for (int j = 0; j < Quadrature::numberEvaluations(); j++) {
    for (int i = 0; i < Quadrature::numberEvaluations();
         i++, samplingPointNo++) {
      samplingPoints[samplingPointNo] = {samplingPoints1D[i],
                                         samplingPoints1D[j]}; // x y
    }
  }
  return samplingPoints;
}
...
// 3D sampling points
template <typename Quadrature>
std::array<Vec3, TensorProductBase<3, Quadrature>::numberEvaluations()>
TensorProduct<3, Quadrature>::samplingPoints() {
  std::array<Vec3, TensorProductBase<3, Quadrature>::numberEvaluations()>
      samplingPoints;
  std::array<double, Quadrature::numberEvaluations()> samplingPoints1D =
      Quadrature::samplingPoints();
...
ValueType TensorProduct<1, Quadrature>::computeIntegral(
...
template <typename Quadrature>
template <typename ValueType>
ValueType TensorProduct<2, Quadrature>::computeIntegral(
    const std::array<ValueType,
                     TensorProductBase<2, Quadrature>::numberEvaluations()>
        &evaluations) {
  const std::array<double, Quadrature::numberEvaluations()> weights =
      Quadrature::quadratureWeights();
  ...
  for (int y = 0; y < Quadrature::numberEvaluations(); y++)
  {
    ...
    evaluationsY[y] = Quadrature::template
  computeIntegral<ValueType>(evaluations.begin()+offset);
  }
...
template <typename Quadrature>
template <typename ValueType>
ValueType TensorProduct<3, Quadrature>::computeIntegral(
    const std::array<ValueType,
  ...
  for (int k = 0; k < Quadrature::numberEvaluations(); k++) {
    for (int j = 0; j < Quadrature::numberEvaluations(); j++) {
      for (int i = 0; i < Quadrature::numberEvaluations(); i++) {
        const int index = k * Quadrature::numberEvaluations() *
                              Quadrature::numberEvaluations() +
                          j * Quadrature::numberEvaluations() + i;
  ...
  /*
    ...
    for (int z=0; z<Quadrature::numberEvaluations(); z++)
    {
      ...
      for (int y=0; y<Quadrature::numberEvaluations(); y++)
      {
        // index of first evaluation that belongs to the list for the current y
        size_t offset = y*Quadrature::numberEvaluations() +
    z*Quadrature::numberEvaluations()*Quadrature::numberEvaluations();
