// weights are calculated corresponding to the implementation in
// ClenshawCurtisweights() which is located in
// examples/quadrature/dominik/src/quadrature.cpp
...
ValueType ClenshawCurtis<NumberIntegrationPoints>::computeIntegral(
    const typename std::array<
        ValueType, ClenshawCurtis<NumberIntegrationPoints>::numberEvaluations()>
        &evaluations) {
  return ClenshawCurtis<NumberIntegrationPoints>::template computeIntegral<
...
template <>
template <typename ValueType>
ValueType ClenshawCurtis<1>::computeIntegral(
