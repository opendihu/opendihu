...
Strang<TimeStepping1, TimeStepping2>::Strang(DihuContext context)
    : OperatorSplitting<TimeStepping1, TimeStepping2>(context,
                                                      "StrangSplitting") {}
...
void Strang<TimeStepping1, TimeStepping2>::advanceTimeSpan(
  ...
  LOG(DEBUG) << "  Strang::advanceTimeSpan: timeSpan=[" << this->startTime_
             << "," << this->endTime_ << "]"
  ...
  for (; timeStepNo < this->numberTimeSteps_;) {
    ...
    {
      LOG(INFO) << "Strang, timestep " << timeStepNo << "/"
                << this->numberTimeSteps_ << ", t=" << currentTime;
    }

    LOG(DEBUG) << "  Strang: time step " << timeStepNo
               << ", t: " << currentTime;
    LOG(DEBUG) << "  Strang: timeStepping1 (first half) setTimeSpan ["
               << currentTime << ", " << midTime << "]";
    ...
    if (this->durationLogKey_ != "")
      Control::PerformanceMeasurement::start(
          this->logKeyTimeStepping1AdvanceTimeSpan_);
    ...
    SlotConnectorDataTransfer<typename TimeStepping1::SlotConnectorDataType,
                              typename TimeStepping2::SlotConnectorDataType>::
        transfer(this->timeStepping1_.getSlotConnectorData(),
                 this->timeStepping2_.getSlotConnectorData(),
                 *this->slotsConnection_);

    if (this->durationLogKey_ != "") {
      Control::PerformanceMeasurement::stop(this->logKeyTransfer12_);
      Control::PerformanceMeasurement::start(
          this->logKeyTimeStepping2AdvanceTimeSpan_);
    }
    ...
    this->timeStepping2_.setTimeSpan(currentTime,
    ...
    SlotConnectorDataTransfer<typename TimeStepping2::SlotConnectorDataType,
                              typename TimeStepping1::SlotConnectorDataType>::
        transfer(this->timeStepping2_.getSlotConnectorData(),
                 this->timeStepping1_.getSlotConnectorData(),
                 *this->slotsConnection_);
    ...
    LOG(DEBUG) << "  Strang: timeStepping1 (second half) advanceTimeSpan ["
               << midTime << ", " << currentTime + this->timeStepWidth_ << "]";
    // set timespan for timestepping1
    this->timeStepping1_.setTimeSpan(midTime,
                                     currentTime + this->timeStepWidth_);
     ...
     * no need to transfer data again, since next operator splitting step will
     * start with timeStepping1 (which has the actual data).
     *
     * Then we could skip output of this->timeStepping2_.data(). However, if
     * they are needed in between two operator splitting steps (for example in
     * the 3-scale muscle model) then this->timeStepping2_.data() should have
     * the actualized data.
     */

    /* option 2. transfer data. (not implemented)
     * the second option to not transfer the data again is needed, because two
     * subsequent transfers in the same direction yield an error with shared
     * field variables. When this will be needed in the future, there have to be
     * actually two transfers timeStepping1->timeStepping2 and
     * timeStepping->timeStepping1 again, to have a matching VecResetArray to
     * the previous VecPlaceArray.
     */
#if 0
    LOG(DEBUG) << "  Strang: transfer timeStepping1 -> timeStepping2";
    ...
    Control::PerformanceMeasurement::setParameter("currentSimulationTime",
