...
public:
  ...
  //! matrix
  virtual void setValue(PetscInt row, PetscInt col, PetscScalar value,
                        InsertMode mode) = 0;

  //! wrapper of MatSetValues for a single value, sets a local value in the
  //! matrix
  virtual void setValues(PetscInt m, const PetscInt idxm[], PetscInt n,
  ...
  virtual void zeroRowsColumns(PetscInt numRows, const PetscInt rows[],
                               PetscScalar diag) = 0;
  ...
  //! indexing
  virtual void getValues(PetscInt m, const PetscInt idxm[], PetscInt n,
                         const PetscInt idxn[], PetscScalar v[]) const = 0;

  //! get entries from the matrix that are locally stored, uses the global
  //! indexing
  virtual void getValuesGlobalPetscIndexing(PetscInt m, const PetscInt idxm[],
                                            PetscInt n, const PetscInt idxn[],
                                            PetscScalar v[]) const = 0;
...
protected:
  std::shared_ptr<Partition::MeshPartition<RowsFunctionSpaceType>>
      meshPartitionRows_; //< the mesh partition object which stores how the
                          // mesh is decomposed and what is the local portion,
                          // for the rows of the matrix
  std::shared_ptr<Partition::MeshPartition<ColumnsFunctionSpaceType>>
      meshPartitionColumns_; //< the mesh partition object which stores how the
                             // mesh is decomposed and what is the local
                             // portion, for the columns of the matrix
