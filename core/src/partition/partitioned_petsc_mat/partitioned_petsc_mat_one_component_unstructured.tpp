...
PartitionedPetscMatOneComponent<
    ...
    : PartitionedPetscMatOneComponentBase<
          ...
          FunctionSpace::FunctionSpace<
              Mesh::UnstructuredDeformableOfDimension<D>, BasisFunctionType>>(
          meshPartitionRows, meshPartitionColumns, name) {
  MatType matrixType = MATAIJ; // sparse matrix type
  createMatrix(matrixType, nNonZerosDiagonal, nNonZerosOffdiagonal);
}
...
PartitionedPetscMatOneComponent<
    ...
    : PartitionedPetscMatOneComponentBase<
          ...
          FunctionSpace::FunctionSpace<
              Mesh::UnstructuredDeformableOfDimension<D>, BasisFunctionType>>(
          meshPartition, meshPartition, name) {
  VLOG(1) << "create PartitionedPetscMatOneComponent<unstructured> from "
             "existing matrix, from meshPartition "
          << meshPartition;
...
void PartitionedPetscMatOneComponent<
    ...
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>>::
    createMatrix(MatType matrixType, int nNonZerosDiagonal,
  ...
  ierr = MatCreate(this->meshPartitionRows_->mpiCommunicator(), &this->matrix_);
  CHKERRV(ierr);
  ierr =
      MatSetSizes(this->matrix_, nRowDofs, nColumnDofs, nRowDofs, nColumnDofs);
  CHKERRV(ierr);

  ierr = MatSetType(this->matrix_, matrixType);
  CHKERRV(ierr);

  // ierr = MatSetFromOptions(this->matrix_); CHKERRV(ierr);
  ierr = PetscObjectSetName((PetscObject)this->matrix_, this->name_.c_str());
  CHKERRV(ierr);
  ...
  if (std::string(matrixType) == MATMPIDENSE ||
    ...
    ierr = MatMPIAIJSetPreallocation(this->matrix_, nNonZerosDiagonal, NULL,
                                     nNonZerosOffdiagonal, NULL);
    CHKERRV(ierr);
    ierr = MatSeqAIJSetPreallocation(this->matrix_, nNonZerosDiagonal, NULL);
    CHKERRV(ierr);
  }
}
...
void PartitionedPetscMatOneComponent<
  ...
  PetscErrorCode ierr;
  ierr = MatSetValues(this->matrix_, 1, &row, 1, &col, &value, mode);
  CHKERRV(ierr);
}
...
void PartitionedPetscMatOneComponent<
    ...
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>>::
    setValue(Vc::int_v rows, Vc::int_v columns, PetscScalar value,
             InsertMode mode) {
  // this wraps the standard PETSc MatSetValue on the local matrix
  PetscErrorCode ierr;
  for (int vcComponentNo = 0; vcComponentNo < Vc::double_v::size();
    ...
    if (rowNo != -1) {
      ...
      if (columnNo != -1) {
        ierr =
            MatSetValues(this->matrix_, 1, &rowNo, 1, &columnNo, &value, mode);
        CHKERRV(ierr);
      }
    }
  }
...
void PartitionedPetscMatOneComponent<
    ...
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>>::
    setValue(Vc::int_v rows, Vc::int_v columns, Vc::double_v values,
             InsertMode mode) {
  ...
  for (int vcComponentNo = 0; vcComponentNo < Vc::double_v::size();
    ...
    if (rowNo != -1) {
      PetscInt columnNo = columns[vcComponentNo];
      if (columnNo != -1) {
        ierr = MatSetValues(this->matrix_, 1, &rowNo, 1, &columnNo,
...
void PartitionedPetscMatOneComponent<
  ...
  PetscErrorCode ierr;
  ierr = MatSetValues(this->matrix_, m, idxm, n, idxn, v, addv);
  CHKERRV(ierr);
}
...
void PartitionedPetscMatOneComponent<
  ...
  PetscErrorCode ierr;
  ierr = MatZeroRowsColumns(this->matrix_, numRows, rows, diag, NULL, NULL);
  CHKERRV(ierr);

  // assemble the global matrix
  ierr = MatAssemblyBegin(this->matrix_, MAT_FLUSH_ASSEMBLY);
  CHKERRV(ierr);
  ierr = MatAssemblyEnd(this->matrix_, MAT_FLUSH_ASSEMBLY);
  CHKERRV(ierr);
}
...
void PartitionedPetscMatOneComponent<
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>,
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>>::zeroRows(PetscInt numRows,
                                                               const PetscInt
                                                                   rows[]) {
  if (VLOG_IS_ON(2)) {
    std::stringstream stream;
  ...
  if (numRows != 0) {
    ...
    ierr = MatZeroRowsLocal(this->matrix_, numRows, rows, 0.0, NULL, NULL);
    CHKERRV(ierr);
  } else {
    std::vector<PetscInt> rows = {0};
    ierr = MatZeroRowsLocal(this->matrix_, 0, rows.data(), 0.0, NULL, NULL);
    CHKERRV(ierr);
  }
}
template <int D, typename BasisFunctionType>
void PartitionedPetscMatOneComponent<
  ...
  ierr = MatZeroEntries(this->matrix_);
  CHKERRV(ierr);
}
...
void PartitionedPetscMatOneComponent<
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>,
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>>::assembly(MatAssemblyType
                                                                   type) {
  ...
  // assemble the global matrix
  ierr = MatAssemblyBegin(this->matrix_, type);
  CHKERRV(ierr);
  ierr = MatAssemblyEnd(this->matrix_, type);
  CHKERRV(ierr);
}
...
void PartitionedPetscMatOneComponent<
  ...
  // assemble the global matrix
  ierr = MatAssemblyBegin(this->matrix_, MAT_FINAL_ASSEMBLY);
  CHKERRV(ierr);
  ierr = MatAssemblyEnd(this->matrix_, MAT_FINAL_ASSEMBLY);
  CHKERRV(ierr);

  // access the global matrix
  ierr = MatGetValues(this->matrix_, m, idxm, n, idxn, v);
  CHKERRV(ierr);
}
...
void PartitionedPetscMatOneComponent<
    ...
    setValuesGlobalPetscIndexing(PetscInt m, const PetscInt idxm[], PetscInt n,
                                 ...
                                 InsertMode addv) {
  // this wraps the standard PETSc MatSetValues, for the global indexing, only
  // retrieves locally stored indices
  PetscErrorCode ierr;
...
Mat &PartitionedPetscMatOneComponent<
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>,
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>>::valuesGlobal() {
  return this->matrix_;
}
...
void PartitionedPetscMatOneComponent<
  ...
  // mpiCommunicator
  PetscUtility::dumpMatrix(filename, format, this->matrix_,
