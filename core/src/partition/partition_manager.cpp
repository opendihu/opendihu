...
Manager::Manager(PythonConfig specificSettings)
    : specificSettings_(specificSettings), nextRankSubset_(nullptr) {}

void Manager::setRankSubsetForNextCreatedPartitioning(
...
void Manager::setRankSubsetForCollectiveOperations(
  ...
  if (!rankSubsetForCollectiveOperations_) {
    ...
    // if the rank subset for collective operations was set earlier, only
    // overwrite if with a rank subset that includes more ranks this happens
    // when more MultipleInstances are nested
    rankSubsetForCollectiveOperations_ = rankSubset;
  }
}
...
std::shared_ptr<RankSubset> Manager::rankSubsetForCollectiveOperations() {
  if (rankSubsetForCollectiveOperations_ == nullptr) {
    // if rank subset was not set, constructs a rank subset with all ranks
    // (MPICommWorld)
    rankSubsetForCollectiveOperations_ =
        std::make_shared<Partition::RankSubset>();
  }

  VLOG(1) << "get rankSubsetForCollectiveOperations "
          << *rankSubsetForCollectiveOperations_;
