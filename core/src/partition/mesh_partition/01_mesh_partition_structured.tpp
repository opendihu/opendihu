...
MeshPartition<FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
    ...
    : MeshPartitionBase(rankSubset), nElementsGlobal_(nElementsGlobal),
      isDegenerate_(false), hasFullNumberOfNodes_({false}),
      nDofsLocalWithoutGhosts_(-1) {
  VLOG(1) << "create MeshPartition where only the global size is known, "
          << "nElementsGlobal: " << nElementsGlobal_
          << ", rankSubset: " << *rankSubset
          << ", mesh dimension: " << MeshType::dim();

  if (MeshType::dim() == 1 && nElementsGlobal_[0] == 0) {
    initialize1NodeMesh();
  } else if (!rankSubset->ownRankIsContained()) {
    // initialize degenerate mesh where own rank does not hold any nodes (only
    // other ranks may)
    isDegenerate_ = true;
  ...
  LOG(DEBUG) << "nElementsLocal_: " << nElementsLocal_
             << ", nElementsGlobal_: " << nElementsGlobal_
...
MeshPartition<FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
              Mesh::isStructured<MeshType>>::
    MeshPartition(std::array<node_no_t, MeshType::dim()> nElementsLocal,
                  ...
                  std::shared_ptr<RankSubset> rankSubset)
    : MeshPartitionBase(rankSubset), beginElementGlobal_(beginElementGlobal),
      nElementsLocal_(nElementsLocal), nElementsGlobal_(nElementsGlobal),
      nRanks_(nRanks), isDegenerate_(false), hasFullNumberOfNodes_({false}),
      nDofsLocalWithoutGhosts_(-1) {
  // partitioning is already prescribed as every rank knows its own local size

  LOG(DEBUG) << "create MeshPartition where every rank already knows its own "
                "local size. "
             << "nElementsLocal: " << nElementsLocal
             << ", nElementsGlobal: " << nElementsGlobal
             << ", beginElementGlobal: " << beginElementGlobal
             << ", nRanks: " << nRanks << ", rankSubset: " << *rankSubset;
  ...
  if (MeshType::dim() == 1 && nElementsGlobal_[0] == 0) {
    ...
    std::array<std::vector<element_no_t>, MeshType::dim()>
        localSizesOnRanks; // [dimensionIndex][rankNo]
    ...
    for (int dimensionIndex = 0; dimensionIndex < MeshType::dim();
      ...
      for (int rankNo = 0; partitionIndex < nRanks_[dimensionIndex];
           rankNo += rankStride, partitionIndex++) {
        VLOG(1) << "   rankNo: " << rankNo;
        VLOG(1) << "   localSizesOnRanks: "
                << localSizesOnRanks[dimensionIndex][rankNo];
        localSizesOnPartitions_[dimensionIndex][partitionIndex] =
            localSizesOnRanks[dimensionIndex][rankNo];
    ...
    LOG(DEBUG) << "determined localSizesOnPartitions_: "
               << localSizesOnPartitions_;
...
void MeshPartition<FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
                   Mesh::isStructured<MeshType>>::
    extractLocalNodesWithoutGhosts(std::vector<T> &vector,
                                   int nComponents) const {
  LOG(DEBUG) << "extractLocalNodesWithoutGhosts, nComponents=" << nComponents
             << ", input: " << vector;
  ...
  if (MeshType::dim() == 1) {
    ...
    assert(vector.size() >=
           ((beginNodeGlobalNatural(1) + nNodesLocalWithoutGhosts(1) - 1) *
                nNodesGlobal(0) +
            beginNodeGlobalNatural(0) + nNodesLocalWithoutGhosts(0)) *
               nComponents);
    for (global_no_t j = beginNodeGlobalNatural(1);
      ...
      for (global_no_t i = beginNodeGlobalNatural(0);
        ...
        for (int componentNo = 0; componentNo < nComponents; componentNo++) {
          result[resultIndex++] =
              vector[(j * nNodesGlobal(0) + i) * nComponents + componentNo];
        }
      }
    }
  } else if (MeshType::dim() == 3) {
    assert(vector.size() >=
           ((beginNodeGlobalNatural(2) + nNodesLocalWithoutGhosts(2) - 1) *
                nNodesGlobal(1) * nNodesGlobal(0) +
            (beginNodeGlobalNatural(1) + nNodesLocalWithoutGhosts(1) - 1) *
                nNodesGlobal(0) +
            beginNodeGlobalNatural(0) + nNodesLocalWithoutGhosts(0)) *
               nComponents);
    for (global_no_t k = beginNodeGlobalNatural(2);
      ...
      for (global_no_t j = beginNodeGlobalNatural(1);
        ...
        for (global_no_t i = beginNodeGlobalNatural(0);
          ...
          for (int componentNo = 0; componentNo < nComponents; componentNo++) {
            result[resultIndex++] =
                vector[(k * nNodesGlobal(1) * nNodesGlobal(0) +
                        j * nNodesGlobal(0) + i) *
                           nComponents +
                       componentNo];
          }
    ...
    for (global_no_t j = beginNodeGlobalNatural(1);
      ...
      for (global_no_t i = beginNodeGlobalNatural(0);
        ...
        for (int dofOnNodeIndex = 0; dofOnNodeIndex < nDofsPerNode;
             dofOnNodeIndex++) {
          result[resultIndex++] =
              vector[(j * nNodesGlobal(0) + i) * nDofsPerNode + dofOnNodeIndex];
        }
      }
    }
  } else if (MeshType::dim() == 3) {
    if (vector.size() <
        ((beginNodeGlobalNatural(2) + nNodesLocalWithoutGhosts(2) - 1) *
         ...
         beginNodeGlobalNatural(0) + nNodesLocalWithoutGhosts(0)) *
            nDofsPerNode) {
      LOG(FATAL)
          << "In extractLocalDofsWithoutGhosts, vector.size(): "
          ...
          << ", nNodesGlobal: " << nNodesGlobal(0) << "," << nNodesGlobal(1)
          << "," << nNodesGlobal(2)
...
void MeshPartition<FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
  ...
  LOG(DEBUG) << "refine mesh partition by refinementFactors "
             << refinementFactor;
  for (int i = 0; i < MeshType::dim(); i++) {
