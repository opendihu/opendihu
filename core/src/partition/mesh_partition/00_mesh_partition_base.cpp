...
int MeshPartitionBase::nRanks() const { return this->rankSubset_->size(); }

int MeshPartitionBase::ownRankNo() { return this->rankSubset_->ownRankNo(); }
...
void MeshPartitionBase::createLocalDofOrderings() {
  ...
  PetscErrorCode ierr;
  ierr =
      ISCreateGeneral(PETSC_COMM_SELF, nDofsLocalWithGhosts,
                      dofNosLocal_.data(), PETSC_COPY_VALUES, &dofNosLocalIS_);
  CHKERRV(ierr);

  // create IS dofNosLocalNonGhostIS_;
  ierr = ISCreateGeneral(PETSC_COMM_SELF, nDofsLocalWithoutGhosts,
                         dofNosLocal_.data(), PETSC_COPY_VALUES,
                         &dofNosLocalNonGhostIS_);
  CHKERRV(ierr);
}
...
MeshPartitionBase::dofNosLocalNaturalOrdering() const {
  return dofNosLocal_;
}
...
const IS &MeshPartitionBase::dofNosLocalIS() const { return dofNosLocalIS_; }
