...
std::ostream &operator<<(std::ostream &stream,
  ...
  PetscInt *numprocs;
  PetscInt **indices;
  ierr = ISLocalToGlobalMappingGetInfo(localToGlobalMapping, &nproc, &procs,
                                       &numprocs, &indices);
  ...
  stream << "), indices of nodes (in local numbering) shared with neighbors "
            "(sorted by global numbering) : (";
  ...
  ierr = ISLocalToGlobalMappingGetSize(localToGlobalMapping, &nLocalIndices);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
  stream << nLocalIndices << " local to Petsc: [";

  PetscInt const *localIndices;
  ierr = ISLocalToGlobalMappingGetIndices(localToGlobalMapping, &localIndices);
  CHKERRABORT(MPI_COMM_WORLD, ierr);
