...
class MeshPartition<
    FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                 BasisFunctionType>,
    Mesh::UnstructuredDeformableOfDimension<D>> : public MeshPartitionBase {
public:
  //! constructor
  ...
  //! return the number of local nodes for coordinateDirection == 0 and 1
  //! otherwise (this is needed for combineFiles with paraview output writer and
  //! 3D meshes)
  ...
  //! get the global element no for a local element no, this only has an effect
  //! for structured meshes, not for unstructured meshes
  ...
  //! from a vector of values of global node numbers remove all that are
  //! non-local, nComponents consecutive components are assumed for each dof
  template <typename T>
  ...
  node_no_t getNodeNoLocal(std::array<global_no_t, D> coordinatesGlobal,
                           bool &isOnLocalDomain) const;

  //! get the local dof no for the global coordinates of the node
  dof_no_t getDofNoLocal(std::array<global_no_t, D> coordinatesGlobal,
