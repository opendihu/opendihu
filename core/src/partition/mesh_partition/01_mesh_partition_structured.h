 ...
 *  The items under consideration are elements, i.e. localSize, etc. refer to
 * number of elements.
 *
 *  You can iterate over non-ghost and ghost nodes together using
 *  for (node_no_t localNodeNo = 0; localNodeNo <
 * meshPartition_->nNodesLocalWithGhosts(); localNodeNo++)
 *  {
 ...
 *  for (node_no_t localNodeNo = 0; localNodeNo <
 * meshPartition_->nNodesLocalWithoutGhosts(); localNodeNo++)
 *  {
 *    for (int dofIndex = 0; dofIndex < nDofsPerNode; dofIndex++)
 *    {
 *      dof_no_t dofLocalNo = localNodeNo*nDofsPerNode + dofIndex;
...
class MeshPartition<FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
                    Mesh::isStructured<MeshType>> : public MeshPartitionBase {
public:
  using MeshPartitionBase::nRanks;

  //! constructor, determine the decomposition by PETSc
  MeshPartition(std::array<global_no_t, MeshType::dim()> nElementsGlobal,
                std::shared_ptr<RankSubset> rankSubset);

  //! constructor from prescribed partition
  MeshPartition(std::array<element_no_t, MeshType::dim()> nElementsLocal,
                std::array<global_no_t, MeshType::dim()> nElementsGlobal,
                std::array<global_no_t, MeshType::dim()> beginElementGlobal,
                std::array<int, MeshType::dim()> nRanks,
                std::shared_ptr<RankSubset> rankSubset);
  ...
  //! number of nodes in the local partition specified by partitionIndex or the
  //! current partition if partitionIndex == -1
  node_no_t nNodesLocalWithGhosts(int coordinateDirection,
                                  int partitionIndex = -1) const;

  //! number of nodes in the partition specified by partitionIndex or the
  //! current partition if partitionIndex == -1
  ...
  //! global no of first local node in the partition specified by partitionIndex
  //! or the current partition if partitionIndex == -1
  global_no_t beginNodeGlobalNatural(int coordinateDirection,
                                     int partitionIndex = -1) const;
  ...
  //! Consider the partition specified by partitionIndex or the current
  //! partition if partitionIndex == -1.
  bool hasFullNumberOfNodes(int coordinateDirection,
                            int partitionIndex = -1) const;

  //! get a vector with the local sizes on every rank
  const std::vector<element_no_t> &
  ...
  //! get the global natural node no for the global coordinates of this node,
  //! this can be combined with getCoordinatesGlobal
  global_no_t getNodeNoGlobalNatural(
  ...
  //! get the global node coordinates (x,y,z) of the node given by its local
  //! node no. This also works for ghost nodes.
  ...
  //! from global natural coordinates compute the local coordinates, set
  //! isOnLocalDomain to true if the node with global coordinates is in the
  //! local domain
  std::array<int, MeshType::dim()> getCoordinatesLocal(
  ...
  std::array<global_no_t, MeshType::dim()>
  getCoordinatesGlobalOfNodeNoGlobalNatural(
      global_no_t nodeNoGlobalNatural) const;
  ...
  //! nodes (return -1 for ghost nodes)
  ...
  //! get the local node no for its local coordinates, also works for ghost
  //! nodes
  node_no_t
  ...
  //! check if the given node is owned by the own rank, then return true, if
  //! not, neighbourRankNo is set to the rank by which the node is owned
  ...
  //! specified face. A layer with given width of element inside the domain
  //! touching the specified face is determined.
  ...
  void getBoundaryElements(
      Mesh::face_or_edge_t face, int boundaryLayerWidth, int &neighbourRankNo,
  ...
  //! get the partitioning index in the coordinate direction, i.e. the no. of
  //! this rank in this direction, the total number of ranks in each direction
  //! can be retrieved by nRanks
  int ownRankPartitioningIndex(int coordinateDirection);

  //! refine the partitioning by multiplying the number of elements by
  //! refinementFactor
...
protected:
  ...
  //! initialize mesh partition for a mesh with only 0 elements, 1 node and 1
  //! dof
  void initialize1NodeMesh();

  //! initialize for a mesh with 0 elements and 0 nodes and dofs, this occurs
  //! when the mpi communicator is MPI_COMM_NULL
  void initializeDegenerateMesh();

  //! Initialize the vector dofNosLocalNaturalOrdering_.
  //! If the vector is already initialized by a previous call to this method, it
  //! has no effect.
  ...
  //! create the DM object for the node partitioning, such that is follows the
  ...
  //! get the index in terms of partitions of the partition that contains the
  //! given node no
  std::array<int, MeshType::dim()> getPartitioningIndex(
      std::array<global_no_t, MeshType::dim()> nodeNoGlobalNatural) const;

  //! get the number of nodes in the global Petsc ordering that are in
  //! partitions prior to the one given by partitionIndex
  global_no_t nNodesGlobalPetscInPreviousPartitions(
      std::array<int, MeshType::dim()> partitionIndex) const;

  //! get the node no in global petsc ordering from global coordinates
  global_no_t getNodeNoGlobalPetsc(
      std::array<global_no_t, MeshType::dim()> coordinatesGlobal) const;
...
std::ostream &
operator<<(std::ostream &stream,
           std::shared_ptr<ISLocalToGlobalMapping> localToGlobalMapping);
