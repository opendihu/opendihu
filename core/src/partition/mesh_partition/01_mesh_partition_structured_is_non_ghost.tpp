...
bool MeshPartition<
    ...
    Mesh::isStructured<MeshType>>::isNonGhost(node_no_t nodeNoLocal,
  ...
  std::array<int, MeshType::dim()> coordinatesLocal =
      getCoordinatesLocal(nodeNoLocal);

  VLOG(2) << "isNonGhost(" << nodeNoLocal
  ...
  if (MeshType::dim() == 1) {
    if (coordinatesLocal[0] < nNodesLocalWithoutGhosts(0) ||
        hasFullNumberOfNodes(0)) // node is not at x+
    {
      return true;
    } else // node is at x+
    {
    ...
    VLOG(2) << " isNonGhost(nodeNoLocal=" << nodeNoLocal
            ...
            << hasFullNumberOfNodes(1);

    if (coordinatesLocal[0] < nNodesLocalWithoutGhosts(0) ||
        hasFullNumberOfNodes(0)) // node is not at x+
    {
      if (coordinatesLocal[1] < nNodesLocalWithoutGhosts(1) ||
          hasFullNumberOfNodes(1)) // node is not at y+
      {
        return true;
      } else // node is at y+
      {
        // node is at top (y+)
        neighbourRankNo = (ownRankPartitioningIndex_[1] + 1) * nRanks_[0] +
    ...
    } else // node is at x+
    {
      ...
      {
        // node is at right (x+)
        neighbourRankNo = ownRankPartitioningIndex_[1] * nRanks_[0] +
        ...
        hasFullNumberOfNodes(0)) // node is not at x+
    {
      if (coordinatesLocal[1] < nNodesLocalWithoutGhosts(1) ||
          hasFullNumberOfNodes(1)) // node is not at y+
      {
        if (coordinatesLocal[2] < nNodesLocalWithoutGhosts(2) ||
            hasFullNumberOfNodes(2)) // node is not at z+
        {
          return true;
        } else // node is at z+
        {
          // node is at top (z+)
          neighbourRankNo =
              (ownRankPartitioningIndex_[2] + 1) * nRanks_[0] * nRanks_[1] +
      ...
      } else // node is at y+
      {
            ...
            hasFullNumberOfNodes(2)) // node is not at z+
        {
          // node is at y+
          neighbourRankNo =
              ownRankPartitioningIndex_[2] * nRanks_[0] * nRanks_[1] +
...
int MeshPartition<
    FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
    Mesh::isStructured<MeshType>>::neighbourRank(Mesh::face_or_edge_t face) {
  /*
  ...
  * */
  if (face == Mesh::face_or_edge_t::faceEdge0Minus) {
    ...
    if (ownRankPartitioningIndex_[0] == 0 ||
        ownRankPartitioningIndex_[1] == 0) {
      return -1;
    } else {
      int neighbourRankNo = (ownRankPartitioningIndex_[1] - 1) * nRanks_[0] +
                            ownRankPartitioningIndex_[0] - 1; // 2D case
      if (MeshType::dim() == 3) {
        neighbourRankNo +=
            ownRankPartitioningIndex_[2] * nRanks_[0] * nRanks_[1];
      }
    ...
    if (ownRankPartitioningIndex_[0] == 0 ||
        ownRankPartitioningIndex_[1] == nRanks_[1] - 1) {
      return -1;
    } else {
      int neighbourRankNo = (ownRankPartitioningIndex_[1] + 1) * nRanks_[0] +
                            ownRankPartitioningIndex_[0] - 1; // 2D case
...
void MeshPartition<FunctionSpace::FunctionSpace<MeshType, BasisFunctionType>,
  ...
  if (face == Mesh::face_or_edge_t::faceEdge0Minus ||
      face == Mesh::face_or_edge_t::faceEdge0Plus) {
    if (face == Mesh::face_or_edge_t::faceEdge0Minus) {
    ...
    if (MeshType::dim() == 3) {
      ...
      nBoundaryElements[2] = nElementsLocal_[2];
             ...
             face == Mesh::face_or_edge_t::faceEdge1Plus) {
    assert(MeshType::dim() >= 2);
    ...
    if (MeshType::dim() == 3) {
      ...
      nBoundaryElements[2] = nElementsLocal_[2];
             ...
             face == Mesh::face_or_edge_t::faceEdge2Plus) {
    assert(MeshType::dim() == 3);
  ...
  const int averageNDofsPerElement1D = FunctionSpace::FunctionSpaceBaseDim<
      1, BasisFunctionType>::averageNDofsPerElement();
  const int nDofsPerNode =
      FunctionSpace::FunctionSpaceBaseDim<1, BasisFunctionType>::nDofsPerNode();

  int nBoundaryDofsTotal = 1;
  switch (MeshType::dim()) {
  case 1:
    nBoundaryDofsTotal =
        (nBoundaryElements[0] * averageNDofsPerElement1D + nDofsPerNode);
    break;
  case 2:
    nBoundaryDofsTotal =
        (nBoundaryElements[0] * averageNDofsPerElement1D + nDofsPerNode) *
  ...
  LOG(DEBUG) << "getBoundaryElements: nBoundaryElements: "
             << nBoundaryElements[0] << "x" << nBoundaryElements[1] << "x"
             << nBoundaryElements[2]
             << ", nBoundaryDofsTotal: " << nBoundaryDofsTotal
             << " nNodesLocalWithoutGhosts: (" << nNodesLocalWithoutGhosts(0)
             << "," << nNodesLocalWithoutGhosts(1) << ","
             << nNodesLocalWithoutGhosts(2) << ")"
             << ", nNodesLocalWithGhosts: (" << nNodesLocalWithGhosts(0) << ","
             << nNodesLocalWithGhosts(1) << "," << nNodesLocalWithGhosts(2)
             << ")";
  ...
  for (int dimensionIndex = 0; dimensionIndex < MeshType::dim();
       dimensionIndex++) {
    nDofs[dimensionIndex] =
        nBoundaryElements[dimensionIndex] * averageNDofsPerElement1D +
        nDofsPerNode;
    boundaryDofIndexStart[dimensionIndex] =
        boundaryElementIndexStart[dimensionIndex] * averageNDofsPerElement1D;
  }

  LOG(DEBUG) << "nDofs: " << nDofs
             << ", boundaryElementIndexStart: " << boundaryElementIndexStart
             << ", boundaryDofIndexStart: " << boundaryDofIndexStart;

  if (MeshType::dim() == 1) {
    int dofIndex = 0;
    for (int i = boundaryDofIndexStart[0];
         i < boundaryDofIndexStart[0] + nDofs[0]; i++, dofIndex++) {
      dofNos[dofIndex] = dofNosLocalNaturalOrdering_[i];
    }
  } else if (MeshType::dim() == 2) {
    int nDofsRow = nElementsLocal_[0] * averageNDofsPerElement1D + nDofsPerNode;
    int dofIndex = 0;
    for (int j = boundaryDofIndexStart[1];
         j < boundaryDofIndexStart[1] + nDofs[1]; j++) {
      for (int i = boundaryDofIndexStart[0];
           i < boundaryDofIndexStart[0] + nDofs[0]; i++, dofIndex++) {
    ...
    int nDofsRow = nElementsLocal_[0] * averageNDofsPerElement1D + nDofsPerNode;
    int nDofsPlane =
        (nElementsLocal_[1] * averageNDofsPerElement1D + nDofsPerNode) *
        nDofsRow;
    // LOG(DEBUG) << "nDofsRow: " << nDofsRow << ", nDofsPlane: " << nDofsPlane;
    int dofIndex = 0;
    for (int k = boundaryDofIndexStart[2];
         k < boundaryDofIndexStart[2] + nDofs[2]; k++) {
      for (int j = boundaryDofIndexStart[1];
           j < boundaryDofIndexStart[1] + nDofs[1]; j++) {
        for (int i = boundaryDofIndexStart[0];
             i < boundaryDofIndexStart[0] + nDofs[0]; i++, dofIndex++) {
          dofNos[dofIndex] =
              dofNosLocalNaturalOrdering_[k * nDofsPlane + j * nDofsRow + i];
          // LOG(DEBUG) << "   (i,j,k) = (" << i << "," << j << "," << k << "),
          // dofIndex=" << dofIndex << "<" << nBoundaryDofsTotal << ", index "
          // << k*nDofsPlane + j*nDofsRow + i << "<" <<
          // dofNosLocalNaturalOrdering_.size()
