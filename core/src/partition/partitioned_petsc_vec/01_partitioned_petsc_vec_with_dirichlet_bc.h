...
/** This vector accounts for prescribed Dirichlet BC values. Only non-BC values
 * are stored in the vector. The getValues and setValues methods handle normal
 * indices, without special treatment of BC dofs. Internally, setting values for
 * Dirichlet BC dofs has no effect and getting values for Dirichlet BC dofs
 * gives the actual prescribed values. The internal data representation,
 * (representationCombinedLocal and representationCombinedGlobal) uses a new
 * ordering that simply skips Dirichlet BC dofs. Furthermore, the values for
 * different components are stored after each other: Example: 5 dofs, 2
 * components, dof2c0 and dof4c1 have prescribed Dirichlet values, then the
 * vector will contain [dof0c0, dof1c0, dof3c0, dof4c0, dof0c1, dof1c1, dof2c1,
 * dof3c1] in the serial case, plus ghost values in the parallel case. The
 * nComponentsDirichletBc template parameter has no special meaning, it is only
 * used for the derived class PartitionedPetscVecForHyperelasticity
 */
template <typename FunctionSpaceType, int nComponents,
          int nComponentsDirichletBc = nComponents>
...
public:
  ...
  //! constructor only for derived class, does not initialize, dummy is needed
  //! to differentiate to normal constructor
  PartitionedPetscVecWithDirichletBc(
      std::shared_ptr<Partition::MeshPartition<FunctionSpaceType>>
          meshPartition,
      std::shared_ptr<SpatialDiscretization::DirichletBoundaryConditions<
  ...
  //! wrapper to the PETSc VecSetValues, acting on the local data or global
  //! data, the indices ix are the local dof nos
  void setValues(int componentNo, PetscInt ni, const PetscInt ix[],
                 const PetscScalar y[], InsertMode iora = INSERT_VALUES);
  ...
  //! wrapper to the PETSc VecSetValue, acting on the local data or global data,
  //! the row is local dof no
  void setValue(int componentNo, Vc::int_v row, Vc::double_v value,
                InsertMode mode = INSERT_VALUES);

  //! wrapper to the PETSc VecGetValues, acting on the local data or global
  //! data, the indices ix are the local dof nos
  void getValues(int componentNo, PetscInt ni, const PetscInt ix[],
                 PetscScalar y[]) const;

  //! get values from a different vector but using the indexing for the global
  //! vector
  void getValuesGlobal(Vec vector, int componentNo, PetscInt ni,
  ...
  //! get the index in the internal vector in global numbering, from componentNo
  //! and local dof no
  ...
  //! get the Petsc Vec that contains all components but no values for dirichlet
  //! BC dofs
  Vec &valuesGlobal();

  //! get a reference to the global Vec, by which it is possible to change the
  //! Vec
  Vec &valuesGlobalReference();

  //! get the index in the internal vector in local numbering, from componentNo
  //! and local dof no
  ...
  //! get a reference to the internal dofNoLocalToDofNoNonBcGlobal_ data
  //! structure
  const std::array<std::vector<dof_no_t>, nComponents> &
  dofNoLocalToDofNoNonBcGlobal() const;

  //! set the internal representation to be combined global, i.e. using the
  //! global vector (vectorCombinedWithoutDirichletDofsGlobal_), if it was
  //! local, ghost buffer entries are discarded (use finishGhostManipulation to
  //! consider ghost dofs)
  void setRepresentationGlobal();

  //! update the dirichlet BC values, the numberings stay the same
  //! @param newValues the new BC values as given by the python dict, vector of
  //! pairs of (dofNo,Vec of values)
...
protected:
  //! prepare internal variables such that the vector can be created by
  //! createVector afterwards
  //! @param offsetInGlobalNumberingPerRank Unused numbers in the non-bc global
  //! numbering at the end of the local numbers of each rank, only needed for
  ...
  //! set the internal representation to be combined local, i.e. using the local
  //! vector (vectorCombinedWithoutDirichletDofsLocal_), ghost buffer is not
  //! filled (use startGhostManipulation to consider ghost dofs)
  void setRepresentationLocal();

  //! communicate the local values of boundaryConditionValues_ to neighbouring
  //! processes and receive those values for ghost dofs
  void communicateBoundaryConditionGhostValues();
...
#include "partition/partitioned_petsc_vec/01_partitioned_petsc_vec_with_dirichlet_bc_initialize.tpp"
