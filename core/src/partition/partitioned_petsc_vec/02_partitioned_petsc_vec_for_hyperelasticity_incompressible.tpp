...
PartitionedPetscVecForHyperelasticity<
  ...
  componentNoPressure_ = nComponents; // the pressure component is the last one,
                                      // equal to nComponents (=3 or 6)

  // initialize variables for 3 or 6 displacement/velocity components and 1
  // pressure component
...
void PartitionedPetscVecForHyperelasticity<
    DisplacementsFunctionSpaceType, PressureFunctionSpaceType, Term,
    nComponents,
    std::enable_if_t<Term::isIncompressible, Term>>::initializeForPressure() {
  VLOG(1) << "initializeForPressure, nEntriesGlobal_: " << this->nEntriesGlobal_
          << ", nEntriesLocal_: " << this->nEntriesLocal_
          << ", nNonBcDofsGhosts_: " << this->nNonBcDofsGhosts_;

  const int nDofsLocalWithGhosts =
      meshPartitionPressure_->nDofsLocalWithGhosts();
  const int nDofsLocalWithoutGhosts =
      meshPartitionPressure_->nDofsLocalWithoutGhosts();

  VLOG(1) << "initializeForPressure nDofsLocalWithGhosts: "
          << nDofsLocalWithGhosts
          << ", nDofsLocalWithoutGhosts: " << nDofsLocalWithoutGhosts;

  // the local without ghosts number of entries in the vector, without the
  // Dirichlet BC dofs
  this->nNonBcDofsWithoutGhosts_[componentNoPressure_] =
      nDofsLocalWithoutGhosts;

  // number of ghost values
  int nGhostsPressure = nDofsLocalWithGhosts - nDofsLocalWithoutGhosts;

  this->nonBcGhostDofNosGlobal_.reserve(this->nonBcGhostDofNosGlobal_.size() +
                                        nGhostsPressure);

  // set mapping from component no and local dof no to the global numbering, for
  // non-ghost dofs
  this->dofNoLocalToDofNoNonBcGlobal_[componentNoPressure_].resize(
      nDofsLocalWithoutGhosts);
  std::iota(this->dofNoLocalToDofNoNonBcGlobal_[componentNoPressure_].begin(),
            this->dofNoLocalToDofNoNonBcGlobal_[componentNoPressure_].end(),
            this->nonBcDofNoGlobalBegin_ + this->nDofsLocal_);
  this->dofNoLocalToDofNoNonBcGlobal_[componentNoPressure_].reserve(
      nDofsLocalWithGhosts);

  // add ghost numbers for pressure
  const std::vector<PetscInt> &ghostDofNosGlobalPetsc =
      meshPartitionPressure_->ghostDofNosGlobalPetsc();
  ...
  MPIUtility::handleReturnValue(
      MPI_Scan(&this->nDofsLocal_, &nPreviousDisplacementDofs, 1, MPI_INT,
               MPI_SUM, this->meshPartitionPressure_->mpiCommunicator()),
      "MPI_Scan");
  MPIUtility::handleReturnValue(
      MPI_Allgather(&nPreviousDisplacementDofs, 1, MPI_INT,
                    nDisplacementDofs.data(), 1, MPI_INT,
                    this->meshPartitionPressure_->mpiCommunicator()),
      "MPI_Allgather");

  VLOG(1) << "nPreviousDisplacementDofs: " << nPreviousDisplacementDofs
          << ", nDisplacementDofs: " << nDisplacementDofs;

  int i = 0;
  for (int dofNoLocal = nDofsLocalWithoutGhosts;
       dofNoLocal < nDofsLocalWithGhosts; dofNoLocal++, i++) {
    node_no_t nodeNoLocal =
        dofNoLocal / PressureFunctionSpaceType::nDofsPerNode();

    int neighbourRankNo;
    meshPartitionPressure_->isNonGhost(nodeNoLocal, neighbourRankNo);

    global_no_t nonBcGhostDofNoGlobal =
        ghostDofNosGlobalPetsc[i] + nDisplacementDofs[neighbourRankNo];
    this->nonBcGhostDofNosGlobal_.push_back(nonBcGhostDofNoGlobal);

    this->dofNoLocalToDofNoNonBcGlobal_[componentNoPressure_].push_back(
        nonBcGhostDofNoGlobal);

    VLOG(1) << "dofNoLocal " << dofNoLocal << " is on rank " << neighbourRankNo
            << ", this rank has previous nDisplacementDofs: "
            << nDisplacementDofs[neighbourRankNo]
            << ", ghost dof globalPetsc: " << ghostDofNosGlobalPetsc[i]
            << ", nonBcGhostDofNoGlobal: " << nonBcGhostDofNoGlobal;
  }

  // set mapping from component no and local dof no to the local number of the
  // non-bc dof numbering
  this->dofNoLocalToDofNoNonBcLocal_[componentNoPressure_].resize(
      nDofsLocalWithGhosts);

  // non-ghost dofs
  std::iota(this->dofNoLocalToDofNoNonBcLocal_[componentNoPressure_].begin(),
            this->dofNoLocalToDofNoNonBcLocal_[componentNoPressure_].begin() +
                nDofsLocalWithoutGhosts,
  ...
  std::iota(this->dofNoLocalToDofNoNonBcLocal_[componentNoPressure_].begin() +
                nDofsLocalWithoutGhosts,
            this->dofNoLocalToDofNoNonBcLocal_[componentNoPressure_].end(),
  ...
  this->boundaryConditionValues_[componentNoPressure_].resize(
      nDofsLocalWithGhosts, -1.0);
  this->isPrescribed_[componentNoPressure_].resize(nDofsLocalWithGhosts, false);

  VLOG(1) << "after initializeForPressure:";
  VLOG(1) << "dofNoLocalToDofNoNonBcLocal_: "
          << this->dofNoLocalToDofNoNonBcLocal_;
  VLOG(1) << "dofNoLocalToDofNoNonBcGlobal_: "
          << this->dofNoLocalToDofNoNonBcGlobal_;
...
IS PartitionedPetscVecForHyperelasticity<
  ...
  int startIndex = this->nonBcDofNoGlobalBegin_ +
...
std::ostream &operator<<(
    std::ostream &stream,
    const PartitionedPetscVecForHyperelasticity<DisplacementsFunctionSpaceType,
