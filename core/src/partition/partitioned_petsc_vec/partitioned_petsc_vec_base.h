...
/** Base class for partitioned petsc vectors, this just holds a pointer to the
 * meshPartion object and stores the data representation. There are 3
 * representations: representationLocal: This is the normal case, getValues and
 * setValues work on the local vectors. Ghost dofs are included in the local
 * vectors. representationGlobal: The actual information is in the global
 * vectors. This is needed, when the values should be accessed using
 * valuesGlobal(), for direct use with Petsc functions.
 * representationContiguous: The current local data is stored in a contiguous
 * way, i.e. all components after each other. This is needed for access by
 * CellML.
 *
 */
template <typename FunctionSpaceType> class PartitionedPetscVecBase {
public:
  //! constructor
