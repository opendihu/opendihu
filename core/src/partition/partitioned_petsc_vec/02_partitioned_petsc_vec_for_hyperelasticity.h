...
/** This vector is an extension of PartitionedPetscVecWithDirichletBc (read the
 * description there). In its standard version (nComponents == 3), it is a
 * combined vector of 3 displacements components and 1 pressure component. If
 * nComponents == 6, it also has velocity components, so 6 components in total
 * (3 displacements, 3 velocities) + 1 pressure.
 *
 ...
 *                                  ( p)
 *
 *  for the compressible formulation, where the pressure is given by a
 * constitutive equation: for nComponents = 3:     for nComponents = 6:
 *
 *      (ux)                        (ux)
 *      (uy)                        (uy)
 *      (uz)                        (uz)
 ...
 *                                  (vz)
 *
 *  The template parameter nComponents is the number of components without
 * pressure, i.e. 3 for displacements only or 6 for displacements and
 * velocities.
 */
template <typename DisplacementsFunctionSpaceType,
          typename PressureFunctionSpaceType, typename Term,
          int nComponents = 3, typename DummyForTraits = Term>
class PartitionedPetscVecForHyperelasticity {};

/** Partition specialization for incompressible formulation, which has the (p)
...
class PartitionedPetscVecForHyperelasticity<
    ...
    : public PartitionedPetscVecWithDirichletBc<
          DisplacementsFunctionSpaceType, nComponents + 1,
          nComponents> // <DisplacementsFunctionSpaceType,4,3>
{
public:
  //! constructor
  ...
  //! write the vector values to a file in natural ordering, such that the
  //! output for different numbers of ranks can be compared
  void dumpGlobalNatural(std::string filename);

  //! the meshPartition of the pressure function space, the other meshPartition,
  //! the one of the displacements function space can be obtained by
  //! meshPartition()
  std::shared_ptr<Partition::MeshPartition<PressureFunctionSpaceType>>
  meshPartitionPressure();

  //! get a string representation of the whole vector in global natural ordering
  //! @param horizontal if the string is for console output (less newlines) or
  //! for file output
  std::string getString(bool horizontal = true,
                        std::string vectorName = "") const;

  //! get the number of displacement dofs ux,uy,uz without Dirichlet Bc
  //! constraint dofs
  dof_no_t nDisplacementDofsWithoutBcLocal();

  //! get the number of velocity dofs vx,vy,vz without Dirichlet Bc constraint
  //! dofs (if nComponents == 6), or 0 if there are no velocities (if
  //! nComponents == 3)
  ...
  //! get a Petsc index set (IS) of all global non-bc indices comprising
  //! ux,uy,uz. This is a sequence of consecutive numbers (e.g. 5,6,7,...) and
  //! can be used to access submatrices of the global system matrix.
  IS displacementDofsGlobal();

  //! get a Petsc index set (IS) of all global non-bc indices comprising
  //! vx,vy,vz, only if nComponents == 6
...
protected:
  //! do the additional initalization after the initialization of the parent
  //! class, that accounts also for the pressure component
...
class PartitionedPetscVecForHyperelasticity<
    ...
    : public PartitionedPetscVecWithDirichletBc<
          DisplacementsFunctionSpaceType, nComponents,
          nComponents> // <DisplacementsFunctionSpaceType,3,3>
{
public:
  //! constructor
  PartitionedPetscVecForHyperelasticity(
