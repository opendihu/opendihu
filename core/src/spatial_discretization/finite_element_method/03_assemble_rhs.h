...
/** specialisation for RegularFixed not linear Lagrange, and other meshes of any
 * dimension D (do proper integration of rhs)
 *
 * Class that sets the right hand side vector by integrating the integrand over
 * the elements. What to integrate is given by the class template Term.
 */
...
class AssembleRightHandSide
    : public BoundaryConditions<FunctionSpaceType, QuadratureType, nComponents,
                                Term> {
public:
  // use constructor of base class
  using BoundaryConditions<FunctionSpaceType, QuadratureType, nComponents,
                           Term>::BoundaryConditions;

protected:
  //! Transform values in rhs vector into FEM discretized values by multiplying
  //! them with the integrate basis functions
...
class AssembleRightHandSide<
    ...
    QuadratureType, 1, Term, Equation::hasLaplaceOperator<Term>>
...
public:
  ...
  using BoundaryConditions<
      FunctionSpace::FunctionSpace<Mesh::StructuredRegularFixedOfDimension<1ul>,
                                   BasisFunction::LagrangeOfOrder<1>>,
      QuadratureType, 1, Term>::BoundaryConditions;
...
Term> class
AssembleRightHandSide<FunctionSpace::Mixed<LowOrderFunctionSpaceType,HighOrderFunctionSpaceType>,
...
QuadratureType, nComponents, Term>
{
public:
  // use constructor of base class
  using
BoundaryConditions<FunctionSpace::Mixed<LowOrderFunctionSpaceType,HighOrderFunctionSpaceType>,
QuadratureType, nComponents, Term>::BoundaryConditions;

protected:
  //! Transform values in rhs vector into FEM discretized values by multiplying
them with the integrate basis functions void
