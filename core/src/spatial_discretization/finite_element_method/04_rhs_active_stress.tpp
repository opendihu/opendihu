...
void FiniteElementMethodRhs<
    FunctionSpaceType, QuadratureType, nComponents,
    Equation::Static::LinearElasticityActiveStress>::setRightHandSide() {
  ...
  for (element_no_t elementNoLocal = 0;
    ...
    std::array<VecD<nComponents * nComponents>, nDofsPerElement>
        activeStressValues;
    ...
    for (unsigned int samplingPointIndex = 0;
      ...
      // evaluate function to integrate at samplingPoints[i], write value to
      // evaluations[i]
      std::array<double, D> xi = samplingPoints[samplingPointIndex];

      // compute integration factor
      const std::array<Vec3, D> jacobian =
      ...
      Tensor2<D> inverseJacobian =
          functionSpace->getInverseJacobian(geometryValues, elementNoLocal, xi);

      std::array<VecD<D>, nDofsPerElement> gradPhiParameterSpace =
          functionSpace->getGradPhi(xi);
      ...
      for (int dofIndexL = 0; dofIndexL < nDofsPerElement; dofIndexL++) {
        ...
        VecD<D> gradPhiWorldSpace =
            inverseJacobian * gradPhiParameterSpace[dofIndexL];
        ...
        for (int a = 0; a < D; a++) {
          ...
          for (int b = 0; b < D; b++) {
            ...
            // If grad phi is positive, this means extension in the muscle
            // tissue. If ε>0, there is no value of the active stress. The
            // virtual work -δW_ext = δW_int = σ_active : ε is therefore zero.
            // If ε<0, we have contraction. There exists a virtual work -δW_ext
            // = δW_int = σ_active : ε.
            if (gradPhi < 0) {
              entryLa += gradPhi * sigma_ab;
            }
            VLOG(2) << "     " << samplingPointIndex << "," << dofIndexL
                    << ", sigma_" << a << b << ": " << sigma_ab
                    << ", grad phi: " << gradPhiWorldSpace[b];
          }
          evaluationsArray[samplingPointIndex][dofIndexL * nComponents + a] =
    ...
    for (int dofIndexL = 0; dofIndexL < nDofsPerElement; dofIndexL++) {
      int k =
          int(dofNosLocal[dofIndexL] /
              (this->data_.functionSpace()->meshPartition()->nNodesGlobal(0) *
               this->data_.functionSpace()->meshPartition()->nNodesGlobal(1)));
      VLOG(1)
          << "node k: " << k
          << ", f: " << integratedValues[dofIndexL * nComponents + 0] << " "
          << integratedValues[dofIndexL * nComponents + 1] << " "
          << integratedValues[dofIndexL * nComponents + 2] << ", sigma: "
          << activeStressValues[dofIndexL][MathUtility::sqr(nComponents) - 1];
      ...
      for (int componentNo = 0; componentNo < nComponents; componentNo++) {
        ...
        VLOG(1) << "active rhs, dof " << dofNosLocal[dofIndexL]
                << ", component " << componentNo << ", add value " << value
                << " active stress: " << activeStressValues;
        rightHandSide->setValue(componentNo, dofNosLocal[dofIndexL], value,
                                ADD_VALUES);
        if (rightHandSideActive)
          rightHandSideActive->setValue(componentNo, dofNosLocal[dofIndexL],
                                        value, ADD_VALUES);
      }
    } // index L
  } // elementNoLocal
