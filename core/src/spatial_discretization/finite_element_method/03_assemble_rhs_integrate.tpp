...
void AssembleRightHandSide<FunctionSpaceType, QuadratureType, nComponents, Term,
                           Dummy>::multiplyRightHandSideWithMassMatrix() {
      ...
      for (int j = 0; j < nDofsPerElement; j++) {
        ...
        for (int rowComponentNo = 0; rowComponentNo < nComponents;
          ...
          for (int columnComponentNo = 0; columnComponentNo < nComponents;
            ...
            // getValuesAtIndices replaces operator[] (i.e.
            // "rhsValues[dofNosLocal[j]]") and is necessary for it to work also
            // with Vc::double_v
            double_v_t value =
                integratedValue *
                getValuesAtIndices(rhsValues,
                                   dofNosLocal[j])[columnComponentNo];
            VLOG(2) << "  dof pair (" << i << "," << j
                    << "), integrated value: " << integratedValue
                    << ", rhsValue[" << dofNosLocal[j]
                    << "]: " << getValuesAtIndices(rhsValues, dofNosLocal[j])
                    << " = " << value;

            rightHandSide->setValue(rowComponentNo, dofNosLocal[i], value,
                                    ADD_VALUES);
          }
        }
      } // j
