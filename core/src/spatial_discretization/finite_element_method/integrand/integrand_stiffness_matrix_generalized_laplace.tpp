...
EvaluationsType
IntegrandStiffnessMatrix<1, EvaluationsType, FunctionSpaceType, 1, double_v_t,
                         element_no_v_t, Term,
  ...
  // initialize gradient vectors of ansatz function phi_i, for node i of current
  // element
  std::array<std::array<double, 1>, FunctionSpaceType::nDofsPerElement()>
      gradPhi = data.functionSpace()->getGradPhi(xi);
  ...
  for (int i = 0; i < FunctionSpaceType::nDofsPerElement(); i++) {
    for (int j = 0; j < FunctionSpaceType::nDofsPerElement(); j++) {
      double integrand =
          diffusionTensor * gradPhi[i][0] * gradPhi[j][0] * integralFactor;
      evaluations(i, j) = integrand;
      VLOG(2) << "   dofs(" << i << "," << j << ") integrand=" << gradPhi[i][0]
              << "*" << gradPhi[j][0] << "*" << integralFactor << "="
              << integrand;
    }
  }
...
template <typename EvaluationsType, typename FunctionSpaceType,
          typename double_v_t, typename element_no_v_t, typename Term>
EvaluationsType
IntegrandStiffnessMatrix<2, EvaluationsType, FunctionSpaceType, 1, double_v_t,
                         element_no_v_t, Term,
                         Equation::hasGeneralizedLaplaceOperator<Term>>::
    evaluateIntegrand(
        const Data::FiniteElements<FunctionSpaceType, 1, Term> &data,
  ...
  MathUtility::Matrix<2, 2, double_v_t> diffusionTensor =
      data.template diffusionTensor<double_v_t, element_no_v_t>(elementNoLocal,
                                                                xi);
  VLOG(1) << "diffusionTensor: " << diffusionTensor;

  double_v_t l1 = MathUtility::length(zeta1);
  double_v_t lh = MathUtility::length(zetah);
  double_v_t beta = MathUtility::acos(
      (zeta1[0] * zetah[0] + zeta1[1] * zetah[1] + zeta1[2] * zetah[2]) /
      (l1 * lh));
  ...
  std::array<double_v_t, 4> transformationMatrix = {
      ...
      sin(alpha) * cos(alpha) / l2squared, sin(alpha) * cos(alpha) / l2squared,
  ...
  // initialize gradient vectors of ansatz function phi_i, for node i of current
  // element
  std::array<Vec2, FunctionSpaceType::nDofsPerElement()> gradPhi =
  ...
  for (int i = 0; i < FunctionSpaceType::nDofsPerElement(); i++) {
    ...
    // LOG(DEBUG) << "diffusionTensor: " << diffusionTensor << ", phi " <<
    // gradPhi[i] << " -> " << diffusionTensorGradPhiI;
    for (int j = 0; j < FunctionSpaceType::nDofsPerElement(); j++) {
      double_v_t integrand =
          MathUtility::applyTransformation(
              transformationMatrix, diffusionTensorGradPhiI, gradPhi[j]) *
          integrationFactor;
      evaluations(i, j) = integrand;
    }
  }
...
template <typename EvaluationsType, typename FunctionSpaceType,
          typename double_v_t, typename element_no_v_t, typename Term>
EvaluationsType
IntegrandStiffnessMatrix<3, EvaluationsType, FunctionSpaceType, 1, double_v_t,
                         element_no_v_t, Term,
                         Equation::hasGeneralizedLaplaceOperator<Term>>::
    evaluateIntegrand(
        const Data::FiniteElements<FunctionSpaceType, 1, Term> &data,
  ...
  MathUtility::Matrix<3, 3, double_v_t> diffusionTensor =
      data.template diffusionTensor<double_v_t, element_no_v_t>(elementNoLocal,
                                                                xi);
  // MathUtility::Matrix<3,3,double_v_t> diffusionTensor =
  // data.diffusionTensor(elementNoLocal, xi);

  // compute the combined 3x3 transformation diffusion matrix T = J^{-1} A
  // J^{-T} and the absolute of the determinant of the jacobian
  double_v_t determinant;
  std::array<double_v_t, 9> transformationDiffusionMatrix =
      MathUtility::computeTransformationDiffusionMatrixAndDeterminant(
          jacobian, diffusionTensor, determinant);
...
//#ifdef DEBUG
  ...
  std::array<Vec3, FunctionSpaceType::nDofsPerElement()> gradPhi =
      data.functionSpace()->getGradPhi(xi);
  ...
  for (int i = 0; i < FunctionSpaceType::nDofsPerElement(); i++) {
    for (int j = 0; j < FunctionSpaceType::nDofsPerElement(); j++) {
      ...
      //! computes gradPhi[i]^T * T * gradPhi[j] where T is the symmetric
      //! transformation matrix combined with the diffusion tensor
      double_v_t integrand =
          MathUtility::applyTransformation(transformationDiffusionMatrix,
                                           gradPhi[i], gradPhi[j]) *
          MathUtility::abs(determinant);

#ifndef NDEBUG
      if (!MathUtility::isFinite(integrand)) {
        LOG(ERROR) << "Value entry (" << i << "," << j
                   << ") in stiffness matrix is nan or inf (" << integrand
                   << "). ";
        ...
        LOG(INFO) << "elementNoLocal: " << elementNoLocal << ", xi: " << xi
                  << ", gradPhi[" << i << "] = " << gradPhi[i] << ", gradPhi["
                  << j << "] = " << gradPhi[j] << ", diffusionTensor:";
