...
EvaluationsType IntegrandStiffnessMatrix<1, EvaluationsType, FunctionSpaceType,
                                         1, double_v_t, element_no_v_t, Term,
...
template <typename EvaluationsType, typename FunctionSpaceType,
          typename double_v_t, typename element_no_v_t, typename Term>
EvaluationsType IntegrandStiffnessMatrix<2, EvaluationsType, FunctionSpaceType,
                                         1, double_v_t, element_no_v_t, Term,
                                         Equation::hasLaplaceOperator<Term>>::
    evaluateIntegrand(
        const Data::FiniteElements<FunctionSpaceType, 1, Term> &data,
  ...
  for (int i = 0; i < FunctionSpaceType::nDofsPerElement(); i++) {
    for (int j = 0; j < FunctionSpaceType::nDofsPerElement(); j++) {
      double_v_t integrand = MathUtility::applyTransformation(
                                 transformationMatrix, gradPhi[i], gradPhi[j]) *
                             integrationFactor;
      evaluations(i, j) = integrand;
    }
  }
...
template <typename EvaluationsType, typename FunctionSpaceType,
          typename double_v_t, typename element_no_v_t, typename Term>
EvaluationsType IntegrandStiffnessMatrix<3, EvaluationsType, FunctionSpaceType,
                                         1, double_v_t, element_no_v_t, Term,
                                         Equation::hasLaplaceOperator<Term>>::
    evaluateIntegrand(
        const Data::FiniteElements<FunctionSpaceType, 1, Term> &data,
  ...
  // compute the 3x3 transformation matrix T = J^{-1}J^{-T} and the absolute of
  // the determinant of the jacobian
  double_v_t determinant;
  std::array<double_v_t, 9> transformationMatrix =
      MathUtility::computeTransformationMatrixAndDeterminant(jacobian,
                                                             determinant);
  ...
  const std::array<Vec3, FunctionSpaceType::nDofsPerElement()> gradPhi =
      data.functionSpace()->getGradPhi(xi);
