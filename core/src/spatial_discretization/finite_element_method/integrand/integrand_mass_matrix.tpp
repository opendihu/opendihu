...
template <int D, typename EvaluationsType, typename FunctionSpaceType,
          int nComponents, typename double_v_t, typename Term, typename Dummy>
EvaluationsType IntegrandMassMatrix<D, EvaluationsType, FunctionSpaceType,
                                    nComponents, double_v_t, Term, Dummy>::
    evaluateIntegrand(const std::array<VecD<3, double_v_t>, D> &jacobian,
                      const std::array<double, D> xi) {
  EvaluationsType evaluations;

  // get the factor in the integral that arises from the change in integration
  // domain from world to coordinate space
  ...
  for (int i = 0; i < FunctionSpaceType::nDofsPerElement(); i++) {
    for (int j = 0; j < FunctionSpaceType::nDofsPerElement(); j++) {
      ...
      for (int componentNo = 0; componentNo < nComponents; componentNo++) {
        double_v_t integrand = FunctionSpaceType::phi(i, xi) *
                               FunctionSpaceType::phi(j, xi) *
                               integrationFactor;

        VLOG(1) << "    integrationFactor " << integrationFactor
                << ", jacobian: " << jacobian
                << ", integrationFactor: " << integrationFactor << " -> "
                << integrand;
        evaluations(i * nComponents + componentNo,
                    j * nComponents + componentNo) = integrand;
      }
    }
  }
