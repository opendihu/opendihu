 ...
 *  In the python settings, the Dirichlet BC's need to be specified also for
 * ghost dofs, when inputMeshIsGlobal=False.
 */
...
public:
  ...
  using DirichletBoundaryConditionsBase<
      FunctionSpaceType, nComponents>::DirichletBoundaryConditionsBase;
...
class DirichletBoundaryConditions
    : public DirichletDirichletBoundaryConditionsBase<FunctionSpaceType,
                                                      nComponents> {
public:
  //! use constructor of base class
  using DirichletDirichletBoundaryConditionsBase<
      FunctionSpaceType, nComponents>::DirichletDirichletBoundaryConditionsBase;

  typedef std::array<double, nComponents> ValueType;

  //! Set the boundary conditions in system matrix systemMatrixWrite, i.e. zero
  //! rows and columns of Dirichlet BC dofs and set diagonal to 1. Store the
  //! cleared matrix values (from systemMatrixRead) in
  //! boundaryConditionsRightHandSideSummand such that they can be used for
  //! adjusting the rhs vector afterwards.
  //! @param systemMatrixRead This matrix is used to read matrix values. It can
  //! be the same as systemMatrixWrite.
  //! @param systemMatrixWrite This matrix is changed, i.e. zeroed rows and
  //! columns.
  //! @param systemMatrixAlreadySet: if this is true, then the systemMatrixWrite
  //! is not changed. This is useful in a timestepping scheme where the
  //! dirichlet BC dofs do not change over time.
  void applyInSystemMatrix(
  ...
  //! set the boundary conditions to the right hand side using the information
  //! in boundaryConditionsRightHandSideSummand that was obtained by
  //! applyInSystemMatrix if the argument boundaryConditionsRightHandSideSummand
  //! is also set as rightHandSide, only write prescribed values into
  //! rightHandSide, do not add anything
  void applyInRightHandSide(
      std::shared_ptr<
          FieldVariable::FieldVariable<FunctionSpaceType, nComponents>>
          rightHandSide,
      std::shared_ptr<
  ...
  //! This methods changes the prescribed boundary condition values to the
  //! values of a field variable. The dofs where BC values are prescribed remain
  //! the same. this is needed for the option
  //! "updatePrescribedValuesFromSolution"
  void updatePrescribedValuesFromSolution(
      std::shared_ptr<
          FieldVariable::FieldVariable<FunctionSpaceType, nComponents>>
          solution);
...
protected:
  ...
  //! update boundary condition values in ownGhostElements_ from
