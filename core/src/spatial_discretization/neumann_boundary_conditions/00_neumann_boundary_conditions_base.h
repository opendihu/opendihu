...
public:
  ...
  //! parse config and extract boundary conditions specified under the given
  //! key, store in boundaryConditionElements_
  void initialize(
  ...
  initialize(std::shared_ptr<FunctionSpaceType> functionSpace,
             const std::vector<ElementWithFaces> &boundaryConditionElements);

  //! return the negative rhs with contributions from Neumann boundary
  //! conditions, i.e. int_∂Ω T*δu_aL*phi_L ds
  ...
  /** An element with faces and corresponding prescribed Neumann boundary
   * condition values
   *
   */
  struct ElementWithFaces {
...
std::ostream &operator<<(
    ...
    const typename SpatialDiscretization::NeumannBoundaryConditionsBase<
        FunctionSpaceType, QuadratureType, nComponents>::ElementWithFaces &rhs);
