...
class FunctionSpaceDofsNodes<Mesh::StructuredRegularFixedOfDimension<D>,
                             BasisFunctionType>
    : public FunctionSpaceDofsNodesStructured<
          Mesh::StructuredRegularFixedOfDimension<D>, BasisFunctionType>,
      public std::enable_shared_from_this<FunctionSpaceDofsNodes<
          Mesh::StructuredRegularFixedOfDimension<D>, BasisFunctionType>> {
public:
  ...
  //! @param inputMeshIsGlobal if the number of elements in nElements is
  //! considered to be the global number of elements
  FunctionSpaceDofsNodes(std::shared_ptr<Partition::Manager> partitionManager,
  ...
  //! construct from element numbers and physical extent, null argument is
  //! ignored
  FunctionSpaceDofsNodes(std::shared_ptr<Partition::Manager> partitionManager,
                         std::vector<double> &null,
                         std::array<element_no_t, D> nElements,
  ...
  typedef FieldVariable::FieldVariable<
      FunctionSpace<Mesh::StructuredRegularFixedOfDimension<D>,
                    BasisFunctionType>,
      3>
 ...
 */
template <int D, typename BasisFunctionType>
class FunctionSpaceDofsNodes<Mesh::StructuredDeformableOfDimension<D>,
                             BasisFunctionType>
    : public FunctionSpaceDofsNodesStructured<
      ...
      public std::enable_shared_from_this<FunctionSpaceDofsNodes<
          Mesh::StructuredDeformableOfDimension<D>, BasisFunctionType>> {
public:
  //! constructor from python settings, it is possible to create a basisOnMesh
  //! object without geometry field, e.g. for the lower order mesh of a mixed
  //! formulation
  FunctionSpaceDofsNodes(std::shared_ptr<Partition::Manager> partitionManager,
                         PythonConfig specificSettings,
  ...
  //! constructor from node positions, nElementsPerCoordinateDirection are the
  //! local elements
  ...
  //! constructor from node positions (dummy, needed for creation from node
  //! positions from file)
  FunctionSpaceDofsNodes(
      std::shared_ptr<Partition::Manager> partitionManager,
 ...
 */
template <int D, typename BasisFunctionType>
class FunctionSpaceDofsNodes<Mesh::UnstructuredDeformableOfDimension<D>,
                             BasisFunctionType>
    : public FunctionSpaceGeometry<Mesh::UnstructuredDeformableOfDimension<D>,
                                   BasisFunctionType> {
public:
  //! inherit constructor
  using FunctionSpaceGeometry<Mesh::UnstructuredDeformableOfDimension<D>,
                              BasisFunctionType>::FunctionSpaceGeometry;

  //! fill a vector with the node position entries, nodes will contain
  //! consecutively the (x,y,z) values of just all nodes, i.e. for Hermite not
  //! the derivatives
  void getNodePositions(std::vector<double> &nodes) const;

  //! return number of nodes including ghost nodes, i.e. these nodes are known
  //! locally but some of them are owned by other ranks
...
class FunctionSpaceDofsNodes<Mesh::CompositeOfDimension<D>, BasisFunctionType>
    : public FunctionSpaceGeometry<Mesh::CompositeOfDimension<D>,
                                   BasisFunctionType>,
      public std::enable_shared_from_this<FunctionSpaceDofsNodes<
          Mesh::CompositeOfDimension<D>, BasisFunctionType>> {
public:
  using FunctionSpaceGeometry<Mesh::CompositeOfDimension<D>,
                              BasisFunctionType>::FunctionSpaceGeometry;

  //! constructor from sub function spaces, it is possible to create a
  //! basisOnMesh object without geometry field, e.g. for the lower order mesh
  ...
  //! constructor with additionally given node positions, this is needed such
  //! that mesh manager compiles but never called
  FunctionSpaceDofsNodes(std::shared_ptr<Partition::Manager> partitionManager,
                         std::vector<double> &nodePositions,
                         PythonConfig settings, bool noGeometryField = false);
