...
std::shared_ptr<FieldVariable::FieldVariableBaseFunctionSpace<
    FunctionSpace<MeshType, BasisFunctionType>>>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::createFieldVariable(
    std::string name, std::vector<std::string> componentNames) {
  // create the field variable with template parameter nComponents by a factory
  // class that perform the dynamic->static conversion
  std::shared_ptr<FieldVariable::FieldVariableBaseFunctionSpace<
      FunctionSpace<MeshType, BasisFunctionType>>>
      fieldVariable =
          FieldVariable::Factory<FunctionSpace<MeshType, BasisFunctionType>>::
              createFromFieldVariable(this->geometryField(), name,
                                      componentNames);
...
std::shared_ptr<FieldVariable::FieldVariableBaseFunctionSpace<
    FunctionSpace<MeshType, BasisFunctionType>>>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::createFieldVariable(
    std::string name, int nComponents) {
  ...
  std::shared_ptr<FieldVariable::FieldVariableBaseFunctionSpace<
      FunctionSpace<MeshType, BasisFunctionType>>>
      fieldVariable = this->createFieldVariable(name, componentNames);
...
std::shared_ptr<FieldVariable::FieldVariable<
    FunctionSpace<MeshType, BasisFunctionType>, nComponents>>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::createFieldVariable(
    std::string name) {
  // create standard component names, the strings "0","1","2",...
  std::vector<std::string> componentNames(nComponents);
  ...
  std::shared_ptr<FieldVariable::FieldVariable<
      FunctionSpace<MeshType, BasisFunctionType>, nComponents>>
      fieldVariable = std::make_shared<FieldVariable::FieldVariable<
          FunctionSpace<MeshType, BasisFunctionType>, nComponents>>(
          this->geometryField(), name, componentNames);
...
//! create a non-geometry field field variable with no values being set, with
//! given number of components and component names
template <typename MeshType, typename BasisFunctionType>
template <int nComponents>
std::shared_ptr<FieldVariable::FieldVariable<
    FunctionSpace<MeshType, BasisFunctionType>, nComponents>>
...
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::getGradPhi(
    std::array<double, MeshType::dim()> xi) const {
  // column-major storage, gradPhi[column][row] = gradPhi[dofIndex][i] =
  // dphi_dofIndex/dxi_i, columnIdx = dofIndex, rowIdx = which direction
  std::array<
      std::array<double, MeshType::dim()>,
      FunctionSpaceFunction<MeshType, BasisFunctionType>::nDofsPerElement()>
      gradPhi;
...
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::
    interpolateValueInElement(
        std::array<std::array<double_v_t, nComponents>,
                   FunctionSpaceFunction<MeshType,
...
std::array<double, MeshType::dim()>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::
    interpolateGradientInElement(
        std::array<double, FunctionSpaceFunction<MeshType, BasisFunctionType>::
                               nDofsPerElement()> &elementalDofValues,
  ...
  for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
    ...
    // VLOG(2) << "  dofIndex2=" << dofIndex << ", xi=" << xi << ",
    // gradPhiParameterSpace: " << gradPhiParameterSpace;
    ...
    for (int direction = 0; direction < D; direction++) {
      ...
      for (int k = 0; k < D; k++) {
        ...
        const double dphiDofIndex2_dxik =
            gradPhiParameterSpace[k]; // dphi_dofIndex/dxi_k
        const double dxik_dXdirection =
            inverseJacobianParameterSpace[direction][k]; // dxi_k/dX_direction

        // VLOG(2) << "     += " << dphiDofIndex2_dxik << " * " <<
        // dxik_dXdirection;

        gradPhiWorldSpaceDofIndex2[direction] +=
            dphiDofIndex2_dxik * dxik_dXdirection;
      }
    }

    // VLOG(2) << "  gradPhiWorldSpaceDofIndex2: " << gradPhiWorldSpaceDofIndex2
    //   << " multiply with solution value at dof " << dofIndex << ", " <<
    //   elementalDofValues[dofIndex];

    // VLOG(2) << " sum contributions from the other ansatz functions at this
    // dof: " << gradPhiWorldSpace;

    gradPhiWorldSpace +=
        gradPhiWorldSpaceDofIndex2 * elementalDofValues[dofIndex];
...
template <typename double_v_t>
VecD<3, double_v_t>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::getNormal(
    Mesh::face_t face,
    std::array<
        VecD<3, double_v_t>,
        FunctionSpaceFunction<MeshType, BasisFunctionType>::nDofsPerElement()>
  ...
  // Nansons formula: ds = J F^-T dS (ds, dS are normal vectors, here ds is in
  // world space, dS is in index space)
  using Vec3v = VecD<3, double_v_t>;
  const int D = MeshType::dim();

  // compute the 3xD jacobian of the parameter space to world space mapping
  std::array<Vec3v, D> jacobian = this->computeJacobian(geometryValues, xi);
  std::array<Vec3v, 3> jacobian3x3 =
      MathUtility::transformToDxD<3, D>(jacobian);

  // compute J F^-T, J = det F, F = jacobian
  std::array<Vec3v, 3> cofactor =
      MathUtility::computeCofactorMatrix<double_v_t>(jacobian3x3);

  // transform the index space normal using Nanson's formula
  Vec3 normalIndexSpace =
      MathUtility::transformToD<3, D>(Mesh::getNormal<D>(face));
  ...
  LOG(DEBUG) << "normal index space: " << Mesh::getNormal<D>(face) << ", "
             << normalIndexSpace;
...
template <typename MeshType, typename BasisFunctionType>
Vec3 FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::getNormal(
    Mesh::face_t face, element_no_t elementNoLocal,
    std::array<double, MeshType::dim()> xi) {
  ...
  std::array<Vec3, FunctionSpaceBaseDim<MeshType::dim(),
                                        BasisFunctionType>::nDofsPerElement()>
      geometryValues;
  this->getElementGeometry(elementNoLocal, geometryValues);

  // LOG(DEBUG) << "elementNoLocal: " << elementNoLocal << ", geometryValues: "
  // << geometryValues;
...
VecD<3, Vc::double_v>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::getNormal(
    Mesh::face_t face, Vc::int_v elementNoLocal,
    std::array<double, MeshType::dim()> xi) {
  // compute normal analoguous to nansons formula
  // Nansons formula: ds = J F^-T dS (ds, dS are normal vectors, here ds is in
  // world space, dS is in index space)
...
Tensor2<MeshType::dim(), double_v_t>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::getInverseJacobian(
    std::array<
        VecD<3, double_v_t>,
        FunctionSpaceFunction<MeshType, BasisFunctionType>::nDofsPerElement()>
        &geometryValues,
  ...
  // compute the 3xD jacobian of the parameter space to world space mapping
  Tensor2<D, double_v_t> jacobianParameterSpace =
      MathUtility::transformToDxD<D, D>(
          this->computeJacobian(geometryValues, xi));
  double_v_t jacobianDeterminant;
  double_v_t approximateMeshWidth = MathUtility::computeApproximateMeshWidth<
      double_v_t,
      FunctionSpaceFunction<MeshType, BasisFunctionType>::nDofsPerElement()>(
      geometryValues);
  Tensor2<D, double_v_t> inverseJacobianParameterSpace =
      MathUtility::computeInverse(jacobianParameterSpace, approximateMeshWidth,
                                  jacobianDeterminant);
...
template <typename MeshType, typename BasisFunctionType>
Tensor2<MeshType::dim()>
FunctionSpaceFieldVariable<MeshType, BasisFunctionType>::getInverseJacobian(
    element_no_t elementNo, std::array<double, MeshType::dim()> xi) {
  const int nDofsPerElement =
      FunctionSpaceFunction<MeshType, BasisFunctionType>::nDofsPerElement();
