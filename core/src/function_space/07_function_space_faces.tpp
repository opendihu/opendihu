...
void FunctionSpaceFaces<MeshType, BasisFunctionType, Mesh::isDim<1, MeshType>>::
  ...
  const int nDofsPerElement =
      FunctionSpaceBaseDim<1, BasisFunctionType>::nDofsPerElement();
...
void FunctionSpaceFaces<MeshType, BasisFunctionType, Mesh::isDim<2, MeshType>>::
  ...
  const int nDofsPer1DLine =
      FunctionSpaceBaseDim<1, BasisFunctionType>::nNodesPerElement() *
      nDofsPerNode;
  // const int nDofsPerNode1D =
  // FunctionSpaceBaseDim<1,BasisFunctionType>::nDofsPerNode();
  switch (face) {
  case Mesh::face0Minus:
    ...
    for (int i = 0; i < nNodesPerElement1D; i++) {
      for (int nodalDofIndex = 0; nodalDofIndex < nDofsPerNode;
           nodalDofIndex++) {
        dofIndices[i * nDofsPerNode + nodalDofIndex] =
            currentDofIndex + nodalDofIndex;
      }
...
void FunctionSpaceFaces<MeshType, BasisFunctionType, Mesh::isDim<3, MeshType>>::
  ...
  const int nNodesPerElement2D =
      FunctionSpaceBaseDim<2, BasisFunctionType>::nNodesPerElement();
  const int nDofsPerNode =
      FunctionSpaceBaseDim<MeshType::dim(), BasisFunctionType>::nDofsPerNode();
  const int nNodesPer1DLine =
      FunctionSpaceBaseDim<1, BasisFunctionType>::nNodesPerElement();
  const int nDofsPer1DLine = nNodesPer1DLine * nDofsPerNode;
  const int nDofsPer2DPlane = nNodesPerElement2D * nDofsPerNode;

  VLOG(1) << "getFaceDofs, MeshType: "
          << StringUtility::demangle(typeid(MeshType).name())
          << ", BasisFunctionType: "
          << StringUtility::demangle(typeid(BasisFunctionType).name())
...
int FunctionSpaceFaces<MeshType, BasisFunctionType, Mesh::isDim<3, MeshType>>::
  ...
  case Mesh::face0Minus: // left
    ...
    return nodeIndexZ * nNodesPerElement2D + nodeIndexY * nNodesPerElement1D +
           (nodeIndexX - 1);
  ...
  case Mesh::face1Minus: // front
    ...
    return nodeIndexZ * nNodesPerElement2D +
           (nodeIndexY - 1) * nNodesPerElement1D + nodeIndexX;
  ...
  case Mesh::face2Minus: // bottom
    ...
    return (nodeIndexZ - 1) * nNodesPerElement2D +
           nodeIndexY * nNodesPerElement1D + nodeIndexX;
