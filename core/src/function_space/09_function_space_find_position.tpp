...
bool FunctionSpaceFindPosition<Mesh::UnstructuredDeformableOfDimension<D>,
                               BasisFunctionType,
  ...
  VLOG(3) << "elementNoStart: " << elementNoStart
          << ", elementNoEnd: " << elementNoEnd << ", nElements: " << nElements;
  searchedAllElements = true;

  // search among all elements
  for (element_no_t currentElementNo = elementNoStart;
       currentElementNo != elementNoEnd; currentElementNo++) {
...
std::shared_ptr<FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                              BasisFunctionType>>
FunctionSpaceFindPosition<
    Mesh::UnstructuredDeformableOfDimension<D>, BasisFunctionType,
    Mesh::UnstructuredDeformableOfDimension<D>>::ghostMesh(Mesh::face_or_edge_t
                                                               faceOrEdge) {
  return nullptr;
}
