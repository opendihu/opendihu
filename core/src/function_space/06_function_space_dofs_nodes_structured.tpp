...
node_no_t FunctionSpaceDofsNodesStructured<MeshType, BasisFunctionType>::
  ...
  assert(this->meshPartition_->nElementsLocal(coordinateDirection) ==
         this->nElementsPerCoordinateDirectionLocal(coordinateDirection));
...
template <typename MeshType, typename BasisFunctionType>
node_no_t FunctionSpaceDofsNodesStructured<
    MeshType, BasisFunctionType>::nNodesLocalWithGhosts() const {
  assert(this->meshPartition_);
...
template <typename MeshType, typename BasisFunctionType>
node_no_t FunctionSpaceDofsNodesStructured<
    MeshType, BasisFunctionType>::nNodesLocalWithoutGhosts() const {
  assert(this->meshPartition_);
  return this->meshPartition_->nNodesLocalWithoutGhosts();
}

template <typename MeshType, typename BasisFunctionType>
dof_no_t FunctionSpaceDofsNodesStructured<
    MeshType, BasisFunctionType>::nDofsLocalWithGhosts() const {
  return nNodesLocalWithGhosts() * this->nDofsPerNode();
}

template <typename MeshType, typename BasisFunctionType>
dof_no_t FunctionSpaceDofsNodesStructured<
    MeshType, BasisFunctionType>::nDofsLocalWithoutGhosts() const {
  return nNodesLocalWithoutGhosts() * this->nDofsPerNode();
}

template <typename MeshType, typename BasisFunctionType>
global_no_t
FunctionSpaceDofsNodesStructured<MeshType, BasisFunctionType>::nNodesGlobal()
    const {
  assert(this->meshPartition_);
...
template <typename MeshType, typename BasisFunctionType>
global_no_t
FunctionSpaceDofsNodesStructured<MeshType, BasisFunctionType>::nDofsGlobal()
    const {
  return nNodesGlobal() * this->nDofsPerNode();
}

//! fill a vector with the node position entries, nodes will contain
//! consecutively the (x,y,z) values of just all nodes, i.e. for Hermite not the
//! derivatives
template <typename MeshType, typename BasisFunctionType>
void FunctionSpaceDofsNodesStructured<MeshType, BasisFunctionType>::
  ...
  for (node_no_t nodeGlobalNo = 0; nodeGlobalNo < this->nDofsLocalWithGhosts();
       nodeGlobalNo++) {
...
template <typename MeshType, typename BasisFunctionType>
void FunctionSpaceDofsNodesStructured<
    MeshType, BasisFunctionType>::setHermiteDerivatives() {
  ...
  if (std::is_same<BasisFunctionType, BasisFunction::Hermite>::value) {
    ...
    for (node_no_t nodeNoLocal = 0;
         nodeNoLocal < this->nNodesLocalWithoutGhosts(); nodeNoLocal++) {
      assert(localDofNo % this->nDofsPerNode() == 0);
      Vec3 nodePositionCurrent = this->geometryField_->getValue(localDofNo);

      // determine distance to neighbouring node in all directions
      // (x-,x+,y-,y+,z-,z+) distance to neighbours in x-direction
      node_no_t neighbour0MinusNodeNoLocal =
          this->getNeighbourNodeNoLocal(nodeNoLocal, Mesh::face_t::face0Minus);
      node_no_t neighbour0PlusNodeNoLocal =
          this->getNeighbourNodeNoLocal(nodeNoLocal, Mesh::face_t::face0Plus);
      ...
      if (neighbour0MinusNodeNoLocal != -1) {
        ...
        dof_no_t neighbour0MinusDofNoLocal =
            neighbour0MinusNodeNoLocal * this->nDofsPerNode() + 0;
        Vec3 nodePositionNeighbour0Minus =
            this->geometryField_->getValue(neighbour0MinusDofNoLocal);
      ...
      if (neighbour0PlusNodeNoLocal != -1) {
        // get dof No for node No, first dof of node (=0) for position value
        dof_no_t neighbour0PlusDofNoLocal =
            neighbour0PlusNodeNoLocal * this->nDofsPerNode() + 0;
        Vec3 nodePositionNeighbour0Plus =
            this->geometryField_->getValue(neighbour0PlusDofNoLocal);

        distance0Plus = nodePositionNeighbour0Plus - nodePositionCurrent;
      }

      VLOG(1) << "node local no. " << nodeNoLocal << ", \"0-\" "
              << neighbour0MinusNodeNoLocal << " (d: " << distance0Minus
              << "), \"0+\" " << neighbour0PlusNodeNoLocal
              << "(d: " << distance0Plus << ")";

      // average distance to both neighbours or take single value if there was
      // only one neighbour found (because of end of domain)
      ...
      if (D >= 2) {
        ...
        node_no_t neighbour1MinusNodeNoLocal = this->getNeighbourNodeNoLocal(
            nodeNoLocal, Mesh::face_t::face1Minus);
        node_no_t neighbour1PlusNodeNoLocal =
            this->getNeighbourNodeNoLocal(nodeNoLocal, Mesh::face_t::face1Plus);
        ...
        if (neighbour1MinusNodeNoLocal != -1) {
          // get dof No for node No, first dof of node (=0) for position value
          dof_no_t neighbour1MinusDofNoLocal =
              neighbour1MinusNodeNoLocal * this->nDofsPerNode() + 0;
          Vec3 nodePositionNeighbour1Minus =
              this->geometryField_->getValue(neighbour1MinusDofNoLocal);
        ...
        if (neighbour1PlusNodeNoLocal != -1) {
          // get dof No for node No, first dof of node (=0) for position value
          dof_no_t neighbour1PlusDofNoLocal =
              neighbour1PlusNodeNoLocal * this->nDofsPerNode() + 0;
          Vec3 nodePositionNeighbour1Plus =
              this->geometryField_->getValue(neighbour1PlusDofNoLocal);

          distance1Plus = nodePositionNeighbour1Plus - nodePositionCurrent;
        }

        VLOG(1) << "node local no. " << nodeNoLocal << ", \"1-\" "
                << neighbour1MinusNodeNoLocal << " (d: " << distance1Minus
                << "), \"1+\" " << neighbour1PlusNodeNoLocal
                << "(d: " << distance1Plus << ")";

        // average distance to both neighbours or take single value if there was
        // only one neighbour found (because of end of domain)
        if (neighbour1MinusNodeNoLocal != -1 &&
            neighbour1PlusNodeNoLocal != -1) {
      ...
      if (D == 3) {
        ...
        node_no_t neighbour2MinusNodeNoLocal = this->getNeighbourNodeNoLocal(
            nodeNoLocal, Mesh::face_t::face2Minus);
        node_no_t neighbour2PlusNodeNoLocal =
            this->getNeighbourNodeNoLocal(nodeNoLocal, Mesh::face_t::face2Plus);
        ...
        if (neighbour2MinusNodeNoLocal != -1) {
          // get dof No for node No, first dof of node (=0) for position value
          dof_no_t neighbour2MinusDofNoLocal =
              neighbour2MinusNodeNoLocal * this->nDofsPerNode() + 0;
          Vec3 nodePositionNeighbour2Minus =
              this->geometryField_->getValue(neighbour2MinusDofNoLocal);
        ...
        if (neighbour2PlusNodeNoLocal != -1) {
          // get dof No for node No, first dof of node (=0) for position value
          dof_no_t neighbour2PlusDofNoLocal =
              neighbour2PlusNodeNoLocal * this->nDofsPerNode() + 0;
          Vec3 nodePositionNeighbour2Plus =
              this->geometryField_->getValue(neighbour2PlusDofNoLocal);

          distance2Plus = nodePositionNeighbour2Plus - nodePositionCurrent;
        }

        VLOG(1) << "node local no. " << nodeNoLocal << ", \"2-\" "
                << neighbour2MinusNodeNoLocal << " (d: " << distance2Minus
                << "), \"2+\" " << neighbour2PlusNodeNoLocal
                << "(d: " << distance2Plus << ")";

        // average distance to both neighbours or take single value if there was
        // only one neighbour found (because of end of domain)
        if (neighbour2MinusNodeNoLocal != -1 &&
            neighbour2PlusNodeNoLocal != -1) {
      ...
      if (D == 1) {
        ...
        this->geometryField_->setValue(localDofNo,
                                       distance2 * distance1 * distance0);
        localDofNo++;
      }
    }
  }
