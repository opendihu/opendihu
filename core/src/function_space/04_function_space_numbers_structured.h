 ...
 * This is only possible for structured meshes because unstructured meshes store
 * the information about numbering explicitly. The numberings refer to the
 * stored information on this rank and is, thus, a local numbering. It does not
 * distinguish between ghost and interior nodes, also the ghost nodes are
 * treated the same as every other stored node.
 */
template <typename MeshType, typename BasisFunctionType,
          typename DummyForTraits = MeshType>
class FunctionSpaceNumbers {};

/** class to compute local dof and node no.s, partial specialization for
 * structured mesh, D=1
 */
template <typename MeshType, typename BasisFunctionType>
class FunctionSpaceNumbers<MeshType, BasisFunctionType,
                           Mesh::isStructuredWithDim<1, MeshType>>
    : public FunctionSpacePartition<MeshType, BasisFunctionType> {
public:
  //! inherit constructor
  using FunctionSpacePartition<MeshType,
                               BasisFunctionType>::FunctionSpacePartition;

  //! return the local dof number of element-local dof dofIndex of element
  //! elementNoLocal, the returned dof can be a ghost dof
  dof_no_t getDofNo(element_no_t elementNoLocal, int dofIndex) const;

  //! return the local node number of element-local node nodeIndex of element
  //! with local no elementNoLocal, the returned node no. can be a ghost node
  //! no.
  ...
  //! element with global no elementNoLocalGlobal
  global_no_t getNodeNoGlobalNatural(global_no_t elementNoLocalGlobalNatural,
                                     int nodeIndex) const;
  ...
  //! get neighbouring node to nodeNoLocal or -1 if there is no such node,
  //! nodeNoLocal has to be a non-ghost local node
  node_no_t getNeighbourNodeNoLocal(node_no_t nodeNoLocal,
...
public:
  ...
  //! return the local node number of element-local node nodeIndex of element
  //! elementNoLocal, the returned node no. can be a ghost node no.
  node_no_t getNodeNo(element_no_t elementNoLocal, int nodeIndex) const;
...
/** class that provides a numbering method that is the same for all
 * dimensionalities
 */
template <typename MeshType, typename BasisFunctionType,
          typename DummyForTraits = MeshType>
...
/** partial specialization for StructuredDeformableOfDimension<D> (not
 * CompositeOfDimension<D>)
 */
template <typename MeshType, typename BasisFunctionType>
class FunctionSpaceNumbersCommon<MeshType, BasisFunctionType,
