...
void FunctionSpaceDataUnstructured<D, BasisFunctionType>::parseExelemFile(
    std::string exelemFilename) {
  std::ifstream file_exelem(exelemFilename.c_str(),
                            std::ios::in | std::ios::binary);
  if (!file_exelem.is_open()) {
    LOG(WARNING) << "Could not open exelem file \"" << exelemFilename
                 << "\" for reading.";
  }
  ...
  while (!file_exelem.eof()) {
    ...
    if (line.find("Shape.") != std::string::npos &&
        line.find("Dimension=") != std::string::npos) {
      ...
      if (dimension == D) {
        ...
        LOG(WARNING) << "Exfile contains " << dimension
                     << "D data, UnstructuredDeformable mesh is " << D << "D.";
      }
      continue;
    }
    ...
    if (lineType == elementsFollow &&
        line.find("#Fields=") != std::string::npos) {
    ...
    if (lineType == fieldsFollow) {
      // if the next field description block begins or a block with elements
      // begins (i.e. the current field description block is finished)
      if (line.find(nextFieldNo.str()) != std::string::npos ||
        ...
        if (fieldNo != 0) {
          VLOG(1) << "finish previous fieldVariable component of name ["
                  << fieldName << "]";

          // extract number of components from fieldContent
          if (fieldContent.find("#Components=") == std::string::npos) {
            LOG(ERROR)
                << "Could not parse number of components for field variable \""
                << fieldName << "\".";
          }
          const int nComponents =
              StringUtility::getNumberAfterString(fieldContent, "#Components=");

          // if the field variable with this name does not exist already, create
          // new field variable object
          if (this->fieldVariable_.find(fieldName) ==
              this->fieldVariable_.end()) {
            std::pair<std::string,
                      std::shared_ptr<FieldVariableBaseFunctionSpaceType>>
                newEntry(fieldName,
                         FieldVariable::Factory<FunctionSpace<
                             Mesh::UnstructuredDeformableOfDimension<D>,
                             BasisFunctionType>>::makeShared(nComponents));
          ...
          // retrieve "this" pointer and convert to downwards pointer of most
          // derived class "FunctionSpace"
          std::shared_ptr<FunctionSpace<
              Mesh::UnstructuredDeformableOfDimension<D>, BasisFunctionType>>
              thisFunctionSpace = std::static_pointer_cast<
                  FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                BasisFunctionType>>(this->shared_from_this());
          ...
          this->fieldVariable_[fieldName]->parseHeaderFromExelemFile(
              fieldContent);
          fieldContent = "";
        }
        ...
        // increase number of next field variable and prepare string to search
        // for for next field variable description block
        fieldNo++;
        nextFieldNo.str("");
        nextFieldNo << fieldNo + 1 << ")";
      }
    ...
    if (lineType == elementsFollow) {
      ...
      if (line.find("Element:") != std::string::npos) {
        ...
        if (!elementContent.empty()) {
          // parse whole element block into elementToNode mapping
          this->elementToNodeMapping_->parseElementFromExelemFile(
              elementContent);
          for (auto &fieldVariable : this->fieldVariable_) {
            fieldVariable.second->parseElementFromExelemFile(elementContent);
          }
  ...
  for (auto &fieldVariable : this->fieldVariable_) {
    fieldVariable.second->unifyMappings(this->elementToNodeMapping_,
                                        this->nDofsPerNode());
  }

  VLOG(1) << "unifyMappings - 2";
  // remove duplicate exfile representation and element to dof mapping entries
  // that are shared among different field variables
  for (auto &fieldVariable : this->fieldVariable_) {
    for (auto &fieldVariable2 : this->fieldVariable_) {
      if (fieldVariable.first != fieldVariable2.first) {
        VLOG(1) << " unifyMappings(fieldVariable) " << fieldVariable.first
                << "," << fieldVariable2.first;

        // the template parameter is the number of components of the
        // fieldVariable2
  ...
  if (!this->fieldVariable_.empty()) {
    std::shared_ptr<FieldVariable::ElementToDofMapping> elementToDofMapping =
        this->fieldVariable_.begin()->second->elementToDofMapping();
...
template <int D, typename BasisFunctionType>
void FunctionSpaceDataUnstructured<D, BasisFunctionType>::parseExnodeFile(
    std::string exnodeFilename) {
  std::ifstream file_exnode(exnodeFilename.c_str(),
                            std::ios::in | std::ios::binary);
  if (!file_exnode.is_open()) {
    LOG(WARNING) << "Could not open exnode file \"" << exnodeFilename
                 << "\" for reading.";
  }
...
template <int D, typename BasisFunctionType>
void FunctionSpaceDataUnstructured<D, BasisFunctionType>::remapFieldVariables(
  ...
  if (settings.hasKey("remap")) {
    ...
    for (; !settings.getOptionDictEnd(keyString);
         settings.getOptionDictNext<std::string, std::string>(keyString,
                                                              dictItem)) {
      ...
      if (this->fieldVariable_.find(key) != this->fieldVariable_.end()) {
        std::shared_ptr<FieldVariableBaseFunctionSpaceType> fieldVariable =
            this->fieldVariable_[key];
        this->fieldVariable_.erase(key);
        this->fieldVariable_[value] = fieldVariable;
      } else {
        LOG(WARNING) << "Remap of field variable from \"" << key << "\" to \""
                     << value << "\" failed: no such entry.";
      }
    }
  }
  ...
  if (!this->geometryField_) {
    if (this->fieldVariable_.find("geometry") != this->fieldVariable_.end()) {
      this->geometryField_ = std::static_pointer_cast<
  ...
  if (!this->geometryField_) {
    ...
    for (auto &fieldVariableEntry : this->fieldVariable_) {
      if (fieldVariableEntry.second->isGeometryField()) {
        LOG(WARNING) << "Remap geometry field variable from \""
                     << fieldVariableEntry.first << "\" to \"geometry\".";

        std::shared_ptr<FieldVariableBaseFunctionSpaceType> fieldVariable =
            fieldVariableEntry.second;
        this->fieldVariable_.erase(fieldVariableEntry.first);
        this->geometryField_ = std::static_pointer_cast<
            FieldVariable::FieldVariable<FunctionSpaceType, 3>>(fieldVariable);
    ...
    if (!geometryFieldFound) {
      LOG(FATAL)
          << "The specified Exfiles contain no geometry field. The field must "
             "be named \"geometry\" or have the type \"coordinates\".";
    }
  }
}

template <int D, typename BasisFunctionType>
void FunctionSpaceDataUnstructured<D,
                                   BasisFunctionType>::eliminateScaleFactors() {
...
template <int D, typename BasisFunctionType>
void FunctionSpaceDataUnstructured<
    D, BasisFunctionType>::initializeValuesVector() {
