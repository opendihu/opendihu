...
/** Mesh with two different node sets as overlay, used for mixed formulations.
 * It inherits from Mesh to be able to store it as std::shared_ptr<Mesh::Mesh>
 * in mesh manager.
 */
template <typename LowOrderFunctionSpaceType,
          typename HighOrderFunctionSpaceType>
class Mixed : public Mesh::Mesh {
public:
  typedef typename HighOrderFunctionSpaceType::Mesh
      Mesh; // the mesh must be the same for lowOrder and highOrder, only the
            // FunctionSpace is different
  typedef BasisFunction::Mixed<
      typename LowOrderFunctionSpaceType::BasisFunction,
      typename HighOrderFunctionSpaceType::BasisFunction>
      BasisFunction;
  typedef LowOrderFunctionSpaceType LowOrderFunctionSpace;
  typedef HighOrderFunctionSpaceType HighOrderFunctionSpace;

  // dimension, which is the same for the LowOrderFunctionSpaceType and the
  ...
  //! this assigns the geometry_ field variable's mesh pointer to this object,
  //! it is not possible from the constructor, therefore this extra method
  void initialize();
