...
/**
 * A mesh object contains the topological information and size in terms of
 * number of nodes. Also the geometrical information for each element is stored.
 * The number of DOFs per node is not stored here, instead at data_management
 * classes. Only structured quadrilateral meshes are considered.
 */
class Mesh {
public:
  ...
  //! number of nodes in the mesh stored in the local partition, this also
  ...
  //! get if initialize was called previously and, thus, initialized_ is set to
  //! true
  bool initialized();
...
protected:
  std::string meshName_; //< the name of this mesh, which can be given in the
                         // python config and is the key by which the mesh is
                         // stored in Mesh::Manager
  PythonConfig
      specificSettings_; //< the python object of the settings for this mesh
 ...
 */
template <int D> class MeshOfDimension : public Mesh {
public:
  //! construct mesh from python settings
  ...
  //! return the dimension/template argument D as method. This is also
