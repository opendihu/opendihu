 ...
 * Each mesh can be defined in the python config under "Meshes" with a name and
 * other properties. Various components of the program can later request their
 * mesh by a call to mesh(name). If a mesh was not defined earlier, it is
 * created on the fly when it is requested.
 */
class Manager {
public:
  //! constructor
  ...
  void
  setPartitionManager(std::shared_ptr<Partition::Manager> partitionManager);

  //! return previously created mesh or create on the fly, already call
  ...
  //! check if a function space with the specified name is stored, the type is
  //! not checked
  bool hasFunctionSpace(std::string meshName);

  //! create a mesh not from python config but directly by calling an
  //! appropriate construtor. With this e.g. meshes from node positions can be
  //! created. Initialize the functionSpace from settings.
  template <typename FunctionSpaceType, typename... Args>
  std::shared_ptr<FunctionSpaceType> createFunctionSpace(std::string name,
                                                         Args &&...args);

  //! create a generic function space without mesh representation with dimension
  //! nEntries, in serial
  ...
  template <typename FunctionSpaceType>
  std::shared_ptr<FunctionSpace::Generic> createGenericFunctionSpace(
      int nEntries,
  ...
  //! create a mesh not from python config but directly by calling an
  //! appropriate construtor. With this e.g. meshes from node positions can be
  //! created. Use the given meshPartition, i.e. not the number of elements from
  //! settings.
  template <typename FunctionSpaceType, typename... Args>
  ...
  //! Create a field variable without logical mesh representation, e.g. for MOR
  //! reduced vectors. The vector contains nEntries entries, the partitioning is
  //! done by the partition manager. \param nRanks on how many ranks the field
  //! variable will be distributed, the nEntries is the local size \param name
  //! is the name of the Petsc Vec, used for debugging output.
...
private:
  //! Helper method to create a composite function space from the given
  //! settings, this uses the ManagerCompositeMesh helper class, in turn
  template <typename FunctionSpaceType>
  ...
  std::shared_ptr<Partition::Manager>
      partitionManager_;          //< the partition manager object
 ...
 */
template <typename FunctionSpaceType> class ManagerCompositeMesh {
public:
  //! helper function to create a composite mesh
  static std::shared_ptr<FunctionSpaceType> createCompositeMesh(
...
class ManagerCompositeMesh<FunctionSpace::FunctionSpace<
