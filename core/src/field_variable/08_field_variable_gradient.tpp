...
const int CONDITION_TOLERANCE =
    1000; // condition number value, if the condition number is higher, the dof
          // will get the approximated gradient

// structured mesh
//! compute the gradient field
template <typename FunctionSpaceType>
void FieldVariableGradient<
    FunctionSpaceType, 1,
    ::Mesh::isStructured<typename FunctionSpaceType::Mesh>>::
    computeGradientField(
        std::shared_ptr<
            FieldVariable<FunctionSpaceType, FunctionSpaceType::dim()>>
            gradientField,
        std::shared_ptr<FieldVariable<FunctionSpaceType, 1>>
            jacobianConditionNumberField) {
  LOG(DEBUG) << "computeGradientField (structured), functionSpaceType: "
  ...
  std::shared_ptr<FieldVariable<FunctionSpaceType, FunctionSpaceType::dim()>>
      approximatedGradientField = std::make_shared<
          FieldVariable<FunctionSpaceType, FunctionSpaceType::dim()>>(
          *gradientField, "approximatedGradient");
  ...
  std::vector<int> nAdjacentElements(
      nDofsWithGhosts,
      0); //< the number of elements that are adjacent to the node
  ...
  for (element_no_t elementNoLocal = 0;
    ...
    // compute gradient at every dof, as continuous to current element
    // (gradients have discontinuities between elements at dofs)
    ...
    double approximateMeshWidth =
        MathUtility::computeApproximateMeshWidth<double, nDofsPerElement>(
            geometryValues);

    const int nDofsPerNode = this->functionSpace_->nDofsPerNode();
    std::array<double, D> xi;

    // LOG(DEBUG) << "mesh " << this->functionSpace_->meshName() << " element "
    // << elementNoLocal << ", values: " << solutionValues;
    ...
    for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
      ...
      // VLOG(2) << "element " << elementNoLocal << " dofIndex " << dofIndex <<
      // ", xi " << xi << " g:" << geometryValues;

      // compute the 3xD jacobian of the parameter space to world space mapping
      Tensor2<D> jacobianParameterSpace = MathUtility::transformToDxD<D, D>(
          FunctionSpaceType::computeJacobian(geometryValues, xi));
      double jacobianDeterminant;
      Tensor2<D> inverseJacobianParameterSpace =
          MathUtility::template computeInverse<double>(jacobianParameterSpace,
                                                       approximateMeshWidth,
                                                       jacobianDeterminant);

      // estimate condition value of jacobian
      if (jacobianConditionNumberField != nullptr) {
        double conditionNumber = MathUtility::estimateConditionNumber(
            jacobianParameterSpace, inverseJacobianParameterSpace);

        conditionNumber /= nAdjacentElements[dofNo];
        jacobianConditionNumberField->setValue(dofNo, conditionNumber,
                                               ADD_VALUES);
      }
      ...
      std::array<double, D> gradPhiWorldSpace =
          this->functionSpace_->interpolateGradientInElement(
              solutionValues, inverseJacobianParameterSpace, xi);

      // LOG(DEBUG) << "   dof " << dofIndex << ", dofNo " << dofNo << ",
      // nAdjacentElements: " << nAdjacentElements[dofNo] << ",
      // gradPhiWorldSpace: " << gradPhiWorldSpace << ",
      // inverseJacobianParameterSpace: " << inverseJacobianParameterSpace;
      ...
      if (VLOG_IS_ON(2)) {
        ...
        if ((rankNo == 0 && dofNo == 150) || (rankNo == 1 && dofNo == 0)) {
          LOG(DEBUG) << "dofNo " << dofNo
                     << " gradPhiWorldSpace: " << gradPhiWorldSpace
                     << ", nAdjacentElements[dofNo]: "
                     << nAdjacentElements[dofNo]
                     << ",geometryValues: " << geometryValues
                     << ", for this dof: " << geometryValues[dofIndex];
          LOG(DEBUG) << "solutionValues: " << solutionValues[dofIndex]
                     << ", all: " << solutionValues;
        }
      }
      ...
      // for every adjacent face to the node, compute the difference quotient
      // times the normalized vector of that direction
      ...
      if (dofIndex % nDofsPerNode == 0) {
        ...
        // loop over all directions, all faces that are adjacent to this node in
        // this element (3 for 3D)
        for (int faceNo = 0; faceNo <= (int)Mesh::face_t::face2Plus; faceNo++) {
          int neighbourNodeIndex = this->functionSpace_->getNeighbourNodeIndex(
              nodeIndex, (Mesh::face_t)faceNo);

          if (neighbourNodeIndex !=
              -1) // if the neighbouring direction is valid inside the element
          {
            ...
            double faceLengthSquared =
                MathUtility::normSquared<3>(neighbourDirection);

            gradPhiWorldSpace2 +=
                neighbourDirection *
                ((neighbourSolutionValue - solutionValue) / faceLengthSquared);
          }
        }

        gradPhiWorldSpace2 /=
            nAdjacentElements[dofNo] * D; // D directions per element
        ...
        approximatedGradientField->setValue(dofNo, gradPhiWorldSpaceD,
                                            ADD_VALUES);
      }
    } // dofIndex
  } // elementNoLocal
  ...
  if (jacobianConditionNumberField) {
    ...
    for (node_no_t dofNoLocal = 0;
         dofNoLocal < this->functionSpace_->nDofsLocalWithoutGhosts();
         dofNoLocal++) {
      node_no_t nodeNoLocal = dofNoLocal / this->functionSpace_->nDofsPerNode();

      // get condition number
      double conditionNumber =
          jacobianConditionNumberField->getValue(dofNoLocal);

      if (conditionNumber > CONDITION_TOLERANCE) {
        LOG(DEBUG) << "dof " << dofNoLocal << " has condition number "
                   << conditionNumber << ", use approximated gradient value";

        Vec3 gradientSum;
        int nSummands = 0;

        for (int face = (int)Mesh::face_t::face0Minus;
             face <= (int)Mesh::face_t::face2Plus; face++) {
          int neighbourNodeNo = this->functionSpace_->getNeighbourNodeNoLocal(
              nodeNoLocal, (Mesh::face_t)face);
          if (neighbourNodeNo != -1) {
            int neighbourDofNo =
                neighbourNodeNo * this->functionSpace_->nDofsPerNode();
            double neighbourJacobianConditionNumber =
                jacobianConditionNumberField->getValue(neighbourDofNo);

            if (fabs(neighbourJacobianConditionNumber) <= CONDITION_TOLERANCE) {
              gradientSum += gradientField->getValue(neighbourDofNo);
              nSummands++;
              LOG(DEBUG) << "  neighbour "
                         << Mesh::getString((Mesh::face_t)face) << ", node "
                         << neighbourNodeNo << ", dof " << neighbourDofNo
                         << "  has condition number "
                         << neighbourJacobianConditionNumber << ", value "
                         << gradientField->getValue(neighbourDofNo);
            } else {
              LOG(DEBUG) << "  neighbour "
                         << Mesh::getString((Mesh::face_t)face) << ", node "
                         << neighbourNodeNo << ", dof " << neighbourDofNo
                         << "  has condition number "
                         << neighbourJacobianConditionNumber
                         << ", do not use gradient";
            }
          }
        }
        ...
        LOG(DEBUG) << "node " << nodeNoLocal << " has " << nSummands
                   << " summands, result: " << gradientSum;
      }
    }
  }
}
...
void FieldVariableGradient<
    FunctionSpaceType, 1,
    ::Mesh::UnstructuredDeformableOfDimension<FunctionSpaceType::dim()>>::
    computeGradientField(
        std::shared_ptr<
            FieldVariable<FunctionSpaceType, FunctionSpaceType::dim()>>
            gradientField,
        std::shared_ptr<FieldVariable<FunctionSpaceType, 1>>
            jacobianConditionNumberField) {
  LOG(DEBUG) << "computeGradientField (unstructured), functionSpaceType: "
             << StringUtility::demangle(typeid(FunctionSpaceType).name());
