...
/** General field variable with != 1 components. A field variable is defined on
 * a FunctionSpace, i.e. knows mesh type and basis function type.
 */
template <typename FunctionSpaceType, int nComponents,
          typename Mesh = typename FunctionSpaceType::Mesh>
class FieldVariableGradient
    : public FieldVariableSetGet<FunctionSpaceType, nComponents> {
public:
  //! inherited constructor
  using FieldVariableSetGet<FunctionSpaceType,
                            nComponents>::FieldVariableSetGet;
};

/** Scalar field variable with structured mesh.
 * A field variable is defined on a FunctionSpace, i.e. knows mesh type and
 * basis function type. Scalar field variables can compute a gradient field.
 */
