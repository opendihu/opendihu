...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  // get values
  this->values_->getValues(componentNo, nValues,
                           this->functionSpace_->meshPartition()
                               ->dofNosLocal(onlyNodalValues)
                               .data(),
                           values.data());
}
  ...
  for (int componentNo = 0; componentNo < nComponents; componentNo++) {
    ...
    this->values_->getValues(componentNo, nValues,
                             this->functionSpace_->meshPartition()
                                 ->dofNosLocal(onlyNodalValues)
                                 .data(),
                             values[componentNo].data());
  }
}

//! for a specific component, get values from their local dof no.s
template <typename FunctionSpaceType, int nComponents>
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::getValues(
  ...
  this->values_->getValues(componentNo, nValues, (PetscInt *)dofLocalNo.data(),
                           values.data() + previousSize);
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::getValues(
  ...
  this->values_->getValues(componentNo, N, (PetscInt *)dofLocalNo.data(),
                           values.data());
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::getValues(
  ...
  // prepare lookup indices for PETSc vector values_
  for (int componentIndex = 0; componentIndex < nComponents; componentIndex++) {
    this->values_->getValues(componentIndex, nValues, dofLocalNo.data(),
                             result.data() + componentIndex * nValues);
  }
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::getValues(
  ...
  for (int componentIndex = 0; componentIndex < nComponents; componentIndex++) {
    this->values_->getValues(componentIndex, nValues, dofLocalNo.data(),
                             values.data() + componentIndex * nValues);
  }
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
    indices[dofIndex] =
        this->functionSpace_->getDofNo(elementNoLocal, dofIndex);
    // LOG(DEBUG) << "getElementValues el. " << elementNoLocal << ", dof " <<
    // indices[dofIndex];
  }

  // get the values
  this->values_->getValues(componentNo, nDofsPerElement, indices.data(),
                           values.data());
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  if (elementNoLocal >= this->functionSpace_->nElementsLocal()) {
    LOG(ERROR) << "getElementValues, elementNoLocal = " << elementNoLocal
               << " >= nElementsLocal = "
               << this->functionSpace_->nElementsLocal();
  }
#endif
  assert(elementNoLocal >= 0 &&
         elementNoLocal < this->functionSpace_->nElementsLocal());
  ...
  VLOG(2) << "getElementValues element " << elementNoLocal
  ...
  for (int componentIndex = 0; componentIndex < nComponents; componentIndex++) {
    ...
    this->values_->getValues(componentIndex, nDofsPerElement, indices.data(),
                             result.data() + componentIndex * nDofsPerElement);
  }
  ...
  for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
    for (int componentIndex = 0; componentIndex < nComponents;
      ...
      // VLOG(2) << "getElementValues element " << elementNoLocal << ", dofIndex
      // " << dofIndex << " componentIndex " << componentIndex << " value: " <<
      // values[dofIndex][componentIndex];
    }
  }
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  for (int componentIndex = 0; componentIndex < nComponents; componentIndex++) {
        ...
        if (elementNoLocal[vcComponent] == -1)
          ...
          indices[dofIndex * nVcComponents + vcComponent] =
              this->functionSpace_->getDofNo(elementNoLocal[vcComponent],
                                             dofIndex);
        // LOG(DEBUG) << "    element " << elementNoLocal[vcComponent] << " dof
        // " << dofIndex << ": indices[" << dofIndex*nVcComponents + vcComponent
        // << "]: " << indices[dofIndex*nVcComponents + vcComponent];
      }
    }
    ...
    // LOG(DEBUG) << "component " << componentIndex << ", get " <<
    // nDofsPerElement << " values at " << indices << ", result (starting at
    // index " << componentIndex*nDofsPerElement << "): " << result;
  }
  ...
  for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
    for (int componentIndex = 0; componentIndex < nComponents;
      ...
      // LOG(DEBUG) << "dof " << dofIndex << " component " << componentIndex <<
      // " value " << result[componentIndex*nDofsPerElement + dofIndex] << " ->
      // " << values[dofIndex][componentIndex];
    }
  }
}
...
template <typename FunctionSpaceType, int nComponents>
double FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::getValue(
...
FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::getValue(
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  assert(this->values_);
  this->values_->extractComponentCopy(
      componentNo, extractedFieldVariable->partitionedPetscVec());
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  assert(extractedFieldVariable->partitionedPetscVec());
  assert(this->values_);
  this->values_->extractComponentShared(
      componentNo, extractedFieldVariable->partitionedPetscVec());
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
    restoreExtractedComponent(
        std::shared_ptr<PartitionedPetscVec<FunctionSpaceType, nComponents2>>
            extractedVec,
        int componentNo) {
  assert(this->values_);
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::setValues(
  ...
  if (dofNosLocal.size() != values.size()) {
    LOG(DEBUG) << "dofNosLocal.size(): " << dofNosLocal.size()
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::setValues(
  ...
  if (values.size() < dofNosLocal.size()) {
    LOG(FATAL) << "FieldVariable::setValues: trying to set "
               << dofNosLocal.size() << " dofs but only " << values.size()
               << " values are given.";
  }
...
//! set values for all components for dofs, only nValues values will be set
//! despite potentially more dofNosLocal, after all calls to setValue(s),
//! finishGhostManipulation has to be called to apply the cached changes
template <typename FunctionSpaceType, int nComponents>
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::setValues(
    int nValues, const std::vector<dof_no_t> &dofNosLocal,
    const std::vector<std::array<double, nComponents>> &values,
    InsertMode petscInsertMode) {
  if (nValues == 0)
    return;
  ...
  if (dofNosLocal.size() < nValues) {
    LOG(ERROR) << "dofNosLocal.size()=" << dofNosLocal.size()
               << ", nValues=" << nValues;
  }
...
//! finishGhostManipulation has to be called to apply the cached changes
template <typename FunctionSpaceType, int nComponents>
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::setValue(
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::setValue(
  ...
  this->values_->setValues(componentNo, nEntries, indices.data(), data.data(),
                           petscInsertMode);
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  if (values.size() !=
      this->functionSpace_->meshPartition()->nDofsLocalWithGhosts())
    LOG(ERROR) << "setValuesWithGhosts, values.size(): " << values.size()
               << ", function space \"" << this->functionSpace_->meshName()
               << "\" has "
               << this->functionSpace_->meshPartition()->nDofsLocalWithGhosts()
               << " dofs local with ghosts.";
  assert(values.size() ==
         this->functionSpace_->meshPartition()->nDofsLocalWithGhosts());
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
  ...
  if (values.size() !=
      this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts()) {
    LOG(FATAL)
        << "setValuesWithoutGhosts: values.size: " << values.size() << " != "
        << this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts();
  }
  assert(values.size() ==
         this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts());
  ...
  this->setValues(values.size(),
                  this->functionSpace_->meshPartition()->dofNosLocal(), values,
                  petscInsertMode);
}
...
void FieldVariableSetGetStructured<FunctionSpaceType, nComponents>::
    setValuesWithoutGhosts(
        const std::array<std::vector<double>, nComponents> &values,
        InsertMode petscInsertMode) {
  for (int componentIndex = 0; componentIndex < nComponents; componentIndex++) {
    if (values[componentIndex].size() !=
        this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts()) {
      LOG(FATAL)
          << "setValuesWithoutGhosts: values.size: "
          << values[componentIndex].size() << " != "
          << this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts();
    }
    assert(values[componentIndex].size() ==
           this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts());
    ...
    this->values_->setValues(
        ...
        this->functionSpace_->meshPartition()->nDofsLocalWithoutGhosts(),
        this->functionSpace_->meshPartition()->dofNosLocal().data(),
        values[componentIndex].data(), petscInsertMode);
  }
}

//! set value to zero for all dofs
template <typename FunctionSpaceType, int nComponents>
void FieldVariableSetGetStructured<FunctionSpaceType,
