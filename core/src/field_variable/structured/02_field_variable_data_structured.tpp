...
FieldVariableDataStructured<FunctionSpaceType,
...
FieldVariableDataStructured<FunctionSpaceType, nComponents>::
    FieldVariableDataStructured(
        FieldVariable<FunctionSpaceType, nComponents> &rhs, std::string name,
        bool reuseData)
    : FieldVariableComponents<FunctionSpaceType,
                              nComponents>::FieldVariableComponents() {
  ...
  if (rhs.partitionedPetscVec()) {
    // if rhs is not a geometry field and therefore has a partitionedPetscVec,
    // use that
    this->values_ =
        std::make_shared<PartitionedPetscVec<FunctionSpaceType, nComponents>>(
            *rhs.partitionedPetscVec(), name, reuseData);
  } else {
...
FieldVariableDataStructured<FunctionSpaceType, nComponents>::
    FieldVariableDataStructured(
        ...
        std::vector<std::string> componentNames, bool reuseData,
        int rhsComponentNoBegin)
    : FieldVariableComponents<FunctionSpaceType,
                              nComponents>::FieldVariableComponents() {
  // initialize everything from other field variable
  assert(componentNames.size() == nComponents);

  VLOG(1) << "construct field variable \"" << name
          << "\" from other field variable \"" << rhs.name()
          << "\", reuseData: " << reuseData
          << ", rhsComponentNoBegin: " << rhsComponentNoBegin;
  ...
  if (rhs.partitionedPetscVec()) {
    ...
    this->values_ =
        std::make_shared<PartitionedPetscVec<FunctionSpaceType, nComponents>>(
            *rhs.partitionedPetscVec(), name, reuseData, rhsComponentNoBegin);
  } else {
    // else create new values vector
    this->values_ =
        std::make_shared<PartitionedPetscVec<FunctionSpaceType, nComponents>>(
...
FieldVariableDataStructured<FunctionSpaceType, nComponents>::
    FieldVariableDataStructured(
        ...
        std::vector<std::string> componentNames, bool isGeometryField)
    : FieldVariableComponents<FunctionSpaceType,
                              nComponents>::FieldVariableComponents() {
  this->name_ = name;
  ...
  // std::shared_ptr<Mesh::Structured<FunctionSpaceType::dim()>> meshStructured
  // =
  // std::static_pointer_cast<Mesh::Structured<FunctionSpaceType::dim()>>(functionSpace);
  ...
  LOG(DEBUG) << "FieldVariableDataStructured constructor, name=" << this->name_
             ...
             << ", isGeometryField: " << this->isGeometryField_;

  bool isStructuredRegularFixed =
      std::is_same<typename FunctionSpaceType::Mesh,
  ...
  if (!this->isGeometryField_ || !isStructuredRegularFixed) {
    ...
    // LOG(DEBUG) << "create a geometry field variable without values_ vector
    // (because it isStructuredRegularFixed)";
  }
}
...
/*
template<typename FunctionSpaceType, int nComponents>
std::array<element_no_t, FunctionSpaceType::Mesh::dim()>
FieldVariableDataStructured<FunctionSpaceType,nComponents>::
...
{
  // this is the same as this->nEntries_ / nComponents only if it is not the
geometry mesh of StructuredRegularFixed mesh return
this->functionSpace_->nDofsLocalWithoutGhosts();
}*/

template <typename FunctionSpaceType, int nComponents>
Vec &FieldVariableDataStructured<FunctionSpaceType, nComponents>::valuesLocal(
    int componentNo) {
  assert(this->values_);
...
template <typename FunctionSpaceType, int nComponents>
Vec &FieldVariableDataStructured<FunctionSpaceType, nComponents>::valuesGlobal(
    int componentNo) {
  assert(this->values_);
...
template <typename FunctionSpaceType, int nComponents>
Vec &FieldVariableDataStructured<FunctionSpaceType,
...
template <typename FunctionSpaceType, int nComponents>
void FieldVariableDataStructured<FunctionSpaceType,
...
initialize(std::string name, std::vector<std::string> &componentNames, bool
isGeometryField)
{
...
this->componentNames_.begin());
  ...
  // if there is not yet a values vector, create PartitionedPetscVec (not for
RegularFixed mesh geometry fields) if (this->values_ == nullptr)
  {
...
template <typename FunctionSpaceType, int nComponents>
void FieldVariableDataStructured<FunctionSpaceType, nComponents>::
    outputHeaderExelem(std::ostream &stream,
                       element_no_t currentElementGlobalNo,
                       int fieldVariableNo) {
  // output first line of header
  stream << " " << (fieldVariableNo + 1) << ") " << this->name_ << ", "
  ...
  for (int componentNo = 0; componentNo < nComponents; componentNo++) {
    ...
    stream << " " << name << ".   "
           << StringUtility::multiply<FunctionSpaceType::dim()>(
                  basisFunction.str())
           << ", no modify, standard node based." << std::endl
           << "   #Nodes= " << FunctionSpaceType::nNodesPerElement()
           << std::endl;
    ...
    for (int nodeIndex = 0; nodeIndex < FunctionSpaceType::nNodesPerElement();
         nodeIndex++) {
      stream << "   " << (nodeIndex + 1)
             << ".  #Values=" << FunctionSpaceType::nDofsPerNode() << std::endl
             << "      Value indices: ";

      for (int dofIndex = 0; dofIndex < FunctionSpaceType::nDofsPerNode();
           dofIndex++) {
        stream << " " << dofIndex + 1;
      }
...
template <typename FunctionSpaceType, int nComponents>
void FieldVariableDataStructured<FunctionSpaceType, nComponents>::
    outputHeaderExnode(std::ostream &stream, node_no_t currentNodeGlobalNo,
  ...
  */
  ...
  for (int componentNo = 0; componentNo < nComponents; componentNo++) {
    std::string name = this->componentNames_[componentNo];
    stream << "  " << name << ".  Value index=" << valueIndex + 1
           << ", #Derivatives= 0" << std::endl;

    valueIndex += FunctionSpaceType::nDofsPerNode();
  }
}

//! tell if 2 elements have the same exfile representation, i.e. same number of
//! versions
template <typename FunctionSpaceType, int nComponents>
...
template <typename FunctionSpaceType, int nComponents>
void FieldVariableDataStructured<FunctionSpaceType, nComponents>::output(
    std::ostream &stream) const {
  // only output if on rank 0
  if (this->functionSpace_->meshPartition()->ownRankNo() == 0) {
    stream << "\"" << this->name_ << "\" on mesh \""
           << this->functionSpace_->meshName() << "\""
           << ", isGeometryField: " << std::boolalpha << this->isGeometryField_
           << ", " << this->componentNames_.size()
           << (this->componentNames_.size() == 1 ? " component: "
                                                 : " components: ");
