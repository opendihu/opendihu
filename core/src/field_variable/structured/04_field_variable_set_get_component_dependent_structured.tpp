...
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::getElementValues(
...
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::getElementValues(
  ...
  for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
    for (int vcComponentNo = 0; vcComponentNo < nVcComponents;
         vcComponentNo++) {
      if (elementNoLocal[vcComponentNo] == -1)
        indices[dofIndex * nVcComponents + vcComponentNo] =
            0; // set index to 0, then here the dof 0 is retrieved and also
               // further used in computation but it is discarded later in
               // setValue
      else
        indices[dofIndex * nVcComponents + vcComponentNo] =
            this->functionSpace_->getDofNo(elementNoLocal[vcComponentNo],
                                           dofIndex);
    }
  }
...
template <typename FunctionSpaceType>
double FieldVariableSetGetComponent<FunctionSpaceType, 1>::getValue(
...
FieldVariableSetGetComponent<FunctionSpaceType, nComponents>::getValue(
    node_no_t dofLocalNo) const {
  assert(this->values_);
...
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::getValues(
...
//! get all stored local values, for 1 component
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::getValuesWithGhosts(
...
//! get all stored local values, for 1 component
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::getValuesWithoutGhosts(
    std::vector<double> &values, bool onlyNodalValues) const {
...
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValue(
    dof_no_t dofLocalNo, double value, InsertMode petscInsertMode) {
  assert(this->values_);
  assert(dofLocalNo <
...
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValue(
    const Vc::int_v &dofLocalNo, const Vc::double_v &value,
  ...
  // this->values_->setValues(0, nEntries, indices.data(), data.data(),
  // petscInsertMode);
...
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValue(
    const Vc::int_v &dofLocalNo, double value, InsertMode petscInsertMode) {
  assert(this->values_);
  assert(dofLocalNo[0] <
         this->functionSpace_->meshPartition()->nDofsLocalWithGhosts());
...
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValues(
    Vec petscVector) {
  assert(this->values_);
...
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValues(
    const std::vector<dof_no_t> &dofNosLocal, std::vector<double> &values,
    InsertMode petscInsertMode) {
  assert(this->values_);
...
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValues(
    const std::array<dof_no_t, nValues> dofNosLocal,
    std::array<double, nValues> values, InsertMode petscInsertMode) {
  assert(this->values_);
...
//! changes
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValuesWithGhosts(
    const std::vector<double> &values, InsertMode petscInsertMode) {
  if (values.size() !=
      this->functionSpace_->meshPartition()->nDofsLocalWithGhosts()) {
    LOG(ERROR) << "FieldVariable<1>::setValuesWithGhosts() values size: "
               << values.size() << ", nDofsWithGhosts: "
               << this->functionSpace_->meshPartition()->nDofsLocalWithGhosts();
  }
  assert(values.size() ==
         this->functionSpace_->meshPartition()->nDofsLocalWithGhosts());
...
//! changes
template <typename FunctionSpaceType>
void FieldVariableSetGetComponent<FunctionSpaceType, 1>::setValuesWithoutGhosts(
    const std::vector<double> &values, InsertMode petscInsertMode) {
  assert(values.size() ==
