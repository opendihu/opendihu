...
NodeToDofMapping::NodeDofInformation &
NodeToDofMapping::getNodeDofInformation(node_no_t nodeGlobalNo) {
...
dof_no_t NodeToDofMapping::getNodeDofNo(node_no_t nodeGlobalNo,
                                        int dofIndex) const {
  assert(nodeDofInformation_.find(nodeGlobalNo) != nodeDofInformation_.end());
  assert(dofIndex >= 0);
...
std::vector<double> &
NodeToDofMapping::getNodeScaleFactors(node_no_t nodeGlobalNo) {
  assert(nodeDofInformation_.find(nodeGlobalNo) != nodeDofInformation_.end());
...
int NodeToDofMapping::nVersions(node_no_t nodeGlobalNo) {
  VLOG(1) << "  NodeToDofMapping::nVersions node " << nodeGlobalNo
          << ", max: " << nodeDofInformation_.size();

  if (nodeDofInformation_.find(nodeGlobalNo) == nodeDofInformation_.end()) {
    LOG(FATAL) << "Node global " << nodeGlobalNo
               << " is not contained in any element!";
  }
  assert(nodeDofInformation_.find(nodeGlobalNo) != nodeDofInformation_.end());
