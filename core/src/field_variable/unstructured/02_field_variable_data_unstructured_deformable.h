 ...
 *  The number of elements, nodes and dofs for unstructured meshes is stored by
 * the geometry field of the mesh and not directly at the mesh like for
 * structured meshes.
 */
...
public:
  ...
  //! contructor as data copy with a different name (component names are the
  //! same)
  FieldVariableData(FieldVariable<FunctionSpaceType, nComponents> &rhs,
                    std::string name);

  //! contructor as data copy with a different name and different components
  template <int nComponents2>
  FieldVariableData(FieldVariable<FunctionSpaceType, nComponents2> &rhs,
                    std::string name, std::vector<std::string> componentNames);

  //! constructor with functionSpace, name and components
  FieldVariableData(std::shared_ptr<FunctionSpaceType> functionSpace,
                    std::string name, std::vector<std::string> componentNames);
  ...
  void initializeFromFieldVariable(FieldVariableType &fieldVariable,
                                   std::string name,
                                   std::vector<std::string> componentNames);

  //! set all data but the components from precomputed mappings
  void initializeFromMappings(
  ...
  void unifyMappings(std::shared_ptr<::FieldVariable::ElementToNodeMapping>
  ...
  std::shared_ptr<Component<FunctionSpaceType, nComponents>>
  component(int componentNo);

  //! return the array of components
  std::array<Component<FunctionSpaceType, nComponents>, nComponents> &
  component();
  ...
  //! calls PETSc functions to "assemble" the vector, i.e. flush the cached
  //! changes
...
protected:
  ...
  void createElementToDofMapping(
      std::shared_ptr<::FieldVariable::ElementToNodeMapping>
          elementToNodeMapping,
      const int nDofsPerNode);

  //! initialize components
  void initializeComponents(std::vector<std::string> &componentNames,
                            std::string exfileBasisRepresentation);

  int exfileNo_; //< number of the fieldvariable in exelem file (index starts at
                 //1)
  element_no_t nElements_; //< number of elements
  std::array<Component<FunctionSpaceType, nComponents>, nComponents>
      component_; //< one or multiple components of which this field variable
                  //consists of. They correspond to the names in
                  //this->componentNames_ (derived from FieldVariableComponents)
  std::shared_ptr<ExfileRepresentation>
      exfileRepresentation_; //< the indexing given in the exelem file, this is
                             //the same for all components
  std::shared_ptr<ElementToDofMapping>
      elementToDofMapping_; //< the element to dof mapping of all components,
                            //this is the same for all components
  std::shared_ptr<ElementToNodeMapping>
      elementToNodeMapping_; //< mapping from element-local node indices to
                             //global node numbers
  std::shared_ptr<NodeToDofMapping>
      nodeToDofMapping_; //< the node to dof mapping of all components, this is
                         //the same for all components
  std::shared_ptr<PartitionedPetscVec<FunctionSpaceType, nComponents>>
      values_; //< the vector that contains all values, the entries of all
               //components are interleaved, e.g. (val1comp1, val1comp2,
               //val2comp1, val2comp2, ...)
};
