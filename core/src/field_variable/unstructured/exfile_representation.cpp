...
void ExfileRepresentation::parseHeaderFromExelemFile(std::string content) {
  ...
  currentElementRepresentation_ =
      std::make_shared<ExfileElementRepresentation>();
  currentElementRepresentation_->parseFromExelemFile(content);

  VLOG(1) << " size of representation: " << representation_.size();

  // replace by earlier representation object if there exists one that is equal
  // to the just created one
  for (std::vector<std::shared_ptr<ExfileElementRepresentation>>::iterator
           iter = representation_.begin();
       iter != representation_.end(); iter++) {
...
bool ExfileRepresentation::operator==(const ExfileRepresentation &rhs) {
  // VLOG(1) << " compare exfile representation, sizes: " <<
  // representation_.size() << "!=" << rhs.representation_.size();

  if (representation_.size() != rhs.representation_.size()) {
    VLOG(1) << " exfile representation size mismatch: "
            << representation_.size() << "!=" << rhs.representation_.size();
    return false;
  }
...
std::shared_ptr<ExfileElementRepresentation> &
ExfileRepresentation::getExfileElementRepresentation(element_no_t elementNo) {
...
void ExfileRepresentation::unifyExfileElementRepresentations() {
  ...
  for (element_no_t elementGlobalNo1 = 0;
       elementGlobalNo1 < representation_.size() - 1; elementGlobalNo1++) {
    for (element_no_t elementGlobalNo2 = elementGlobalNo1 + 1;
         elementGlobalNo2 < representation_.size(); elementGlobalNo2++) {
      if (*representation_[elementGlobalNo1] ==
          *representation_[elementGlobalNo2]) {
...
bool ExfileRepresentation::haveSameExfileRepresentation(element_no_t element1,
                                                        element_no_t element2) {
  assert(element1 < (element_no_t)representation_.size());
  assert(element2 < (element_no_t)representation_.size());

  VLOG(2) << "exfileRpers: " << representation_[element1] << ","
          << representation_[element2];
...
std::ostream &operator<<(std::ostream &stream,
                         const ExfileRepresentation &rhs) {
  rhs.output(stream);
  return stream;
}
