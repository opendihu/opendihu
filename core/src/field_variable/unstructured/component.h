...
public:
  ...
  //! assign values of all dofs for a node, valuesBegin is an iterator that can
  //! iterate over the needed number of values, the order is given by dofs
  //! numbering
  void setNodeValues(node_no_t nodeGlobalNo,
                     std::vector<double>::iterator valuesBegin);

  //! assign values of all dofs for a node, the values are given by valuesBegin
  //! iterator as they appear in the exnode block, valuesBegin points to the
  //! beginning of the subblock for the particular component
  void setNodeValuesFromBlock(node_no_t nodeGlobalNo,
                              std::vector<double>::iterator valuesBegin);
  ...
  void setDofMappings(std::shared_ptr<ElementToDofMapping> elementToDofMapping,
  ...
  //! non-derivative values are returned
  ...
  //! get values from their global dof no.s
  void getValues(std::vector<dof_no_t> dofGlobalNo,
...
private:
  std::shared_ptr<PartitionedPetscVec<FunctionSpaceType, nComponents>>
      values_; //< vector of all values, the first components of all dofs, then
               // the 2nd component of all dofs, etc.
  int componentIndex_; //< index of the current component for this field
  ...
  std::string
      exfileBasisFunctionSpecification_; //< the basis function specification in
                                         // the exelem file, e.g.
                                         // c.Hermite*c.Hermite*c.Hermite
  element_no_t nElements_;               //< number of elements
  std::shared_ptr<ElementToDofMapping>
      elementToDofMapping_; //< mapping from element-local dof-indices to dof
                            // numbers
  std::shared_ptr<NodeToDofMapping>
      nodeToDofMapping_; //< mapping from nodes to dof numbers
  std::shared_ptr<ExfileRepresentation>
      exfileRepresentation_; //< indexing for exelem file
};
