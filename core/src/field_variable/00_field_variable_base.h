...
/** Base class for a field variable that just stores the mesh the field variable
 * is defined on.
 */
template <typename FunctionSpaceType>
class FieldVariableBaseFunctionSpace : public FieldVariableBase,
...
public:
  ...
  void setIsGeometryField(bool isGeometryField);

  //! check if there are NaNs or high values in the current variable, if yes
  //! output a warning
  ...
  virtual void getValuesWithoutGhosts(int componentNo,
                                      std::vector<double> &values,
                                      bool onlyNodalValues = false) const = 0;
...
operator<<(std::ostream &stream,
           const FieldVariableBaseFunctionSpace<FunctionSpaceType> &rhs) {
  stream << rhs.name();
  return stream;
}
