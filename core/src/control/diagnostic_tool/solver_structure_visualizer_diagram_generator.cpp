...
const int VARIABLES_LINE_LENGTH =
    54; // number of characters for the solver structure and variables,
        // afterwards there will be connection lines
const int SLOT_NAME_LENGTH = 10;

//! iterate over all nested solvers
void SolverStructureVisualizer::DiagramGenerator::generateDiagramRecursion(
  ...
  if (isLastChildSolver && currentSolver_->outputSlots.empty() &&
      currentSolver_->slotsConnections.empty())
    result << "└";
  else
    result << "├";
  // result << "┌";
  ...
  if (!currentSolver_->outputSlots.empty()) {
    ...
    for (int i = 0; i < currentSolver_->outputSlots.size(); i++) {
      ...
      // if mesh has already been references and thus is present in
      // referencedMeshNames_
      if (std::find(referencedMeshNames_.begin(), referencedMeshNames_.end(),
                    meshDescription) != referencedMeshNames_.end()) {
        meshReferenceNo =
            std::find(referencedMeshNames_.begin(), referencedMeshNames_.end(),
                      meshDescription) -
            referencedMeshNames_.begin();
      } else {
      ...
      if (fieldVariableName != componentName) {
        ...
        // only add component name if there are multiple components or, or the
        // component is not not named "0" which is the standard
        if (currentSolver_->outputSlots[i].nComponents > 1 ||
            componentName != "0")
          s << "." << componentName;
      } else {
        s << fieldVariableName;
      ...
      if (!currentSolver_->mappingsWithinSolver.empty()) {
        for (const solver_t::SlotsConnectionRepresentation &mapping :
             currentSolver_->mappingsWithinSolver) {
          int fromSlotNo = mapping.fromSlot;
          int toSlotNo = mapping.toSlot;
          int type = mapping.type;
          if (type != solver_t::SlotsConnectionRepresentation::
                          internalBeforeComputation &&
              type != solver_t::SlotsConnectionRepresentation::
                          internalAfterComputation) {
            LOG(ERROR)
                << "Slot Mapping should have type `internal*Computation`. Got :"
                << type;
          }

          bool isBefore = type == solver_t::SlotsConnectionRepresentation::
                                      internalBeforeComputation;
      ...
      const int requiredLineLength =
          VARIABLES_LINE_LENGTH - mappingStringLength;

      // shorten string if neccessary, add to stringstream s
      if (currentLineLength >= requiredLineLength) {
        outputSlotString = outputSlotString.substr(
            0, outputSlotString.length() -
                   (currentLineLength - requiredLineLength));
      ...
      if (slotNameInitialLength > SLOT_NAME_LENGTH) {
        ...
        slotName = std::string(SLOT_NAME_LENGTH - slotNameInitialLength, ' ') +
                   slotName;
      }
      ...
      int currentLineNo =
          std::count(currentResult.begin(), currentResult.end(), '\n') - 1;

      // LOG(DEBUG) << "current Result [" << currentResult << "] contains " <<
      // currentLineNo << " line breaks";
  ...
  std::vector<solver_t::SlotsConnectionRepresentation> &slotsConnections =
      currentSolver_->slotsConnections;
  if (!slotsConnections.empty()) {
    result << lineStart.str() << "│  slot connections: \n";
    for (int i = 0; i < slotsConnections.size(); i++) {
      if (slotsConnections[i].type ==
          solver_t::SlotsConnectionRepresentation::slot_connection_t::ba) {
        result << lineStart.str() << "│  " << slotsConnections[i].toSlot
               << "¤ <─ ¤" << slotsConnections[i].fromSlot << "\n";
      } else {
        ...
        case solver_t::SlotsConnectionRepresentation::slot_connection_t::ab:
          result << " ─> ";
          break;
        case solver_t::SlotsConnectionRepresentation::slot_connection_t::
            bidirectionalReuse:
          result << " <═> "; // or use ⇔ ?
          break;
        case solver_t::SlotsConnectionRepresentation::slot_connection_t::
        ...
        case solver_t::SlotsConnectionRepresentation::slot_connection_t::
            internalAfterComputation:
          LOG(ERROR) << "slotsConnections should not have a connecion type of "
                        "`internal*Computation`. Got: "
                     << slotsConnections[i].type;
        default:
          LOG(ERROR) << "unknown slot connection type: "
                     << slotsConnections[i].type;
        };
  ...
  std::vector<std::vector<std::vector<int>>>
      internalConnectionLinesCurrentSolverTerm(children.size());
  ...
  for (std::vector<std::shared_ptr<solver_t>>::iterator iter = children.begin();
    ...
    generateDiagramRecursion(
        result, internalConnectionLinesCurrentSolverTerm[i], slotLineNosTerm[i],
        depth + 1, isFirstChildSolver, isLastChildSolver);

    VLOG(1) << std::string(depth * 2, ' ') << "child " << i
            << ", i:" << internalConnectionLinesCurrentSolverTerm[i];
    ...
    // internal connection lines of the the 2nd and further solvers are
    // finished, the one of the first are passed on to the parent solver
    if ((isFirstChildSolver &&
         ownSolver->hasInternalConnectionToFirstNestedSolver) ||
        (children.size() == 2 && isLastChildSolver &&
         ownSolver->hasInternalConnectionToSecondNestedSolver)) {
      VLOG(1) << std::string(depth * 2, ' ') << "(" << ownSolver->name
              << ") first child solver gives "
              << internalConnectionLinesCurrentSolverTerm[i]
              << " own are: " << internalConnectionLinesCurrentSolver;
      internalConnectionLinesCurrentSolver.insert(
          internalConnectionLinesCurrentSolver.end(),
          internalConnectionLinesCurrentSolverTerm[i].begin(),
          internalConnectionLinesCurrentSolverTerm[i].end());
      VLOG(1) << std::string(depth * 2, ' ')
              << "now own: " << internalConnectionLinesCurrentSolver;
    } else {
      VLOG(1) << std::string(depth * 2, ' ') << "(" << ownSolver->name << ") "
              << i << ". child solver gives "
              << internalConnectionLinesCurrentSolverTerm[i]
              << " all are: " << internalConnectionLinesAll_;
      internalConnectionLinesAll_.insert(
          internalConnectionLinesAll_.end(),
          internalConnectionLinesCurrentSolverTerm[i].begin(),
          internalConnectionLinesCurrentSolverTerm[i].end());
      VLOG(1) << std::string(depth * 2, ' ')
              << "now all: " << internalConnectionLinesAll_;
    }
  }
  ...
  for (int i = 0; i < ownSolver->outputSlots.size(); i++) {
    ...
    VLOG(1) << std::string(depth * 2, ' ') << "(" << ownSolver->name
            << ") add slot " << slotLineNos[i]
            << ", now i: " << internalConnectionLinesCurrentSolver;
  }

  VLOG(1) << std::string(depth * 2, ' ')
          << "internalConnectionLinesCurrentSolver:"
          << internalConnectionLinesCurrentSolver;
  VLOG(1) << std::string(depth * 2, ' ')
          << "internalConnectionLinesAll_:" << internalConnectionLinesAll_;

  // add connections between slots of nested solvers to externalConnectionLines
  for (int i = 0; i < slotsConnections.size(); i++) {
    if (slotLineNosTerm.size() >= 2) {
      if ((slotsConnections[i].type ==
               solver_t::SlotsConnectionRepresentation::slot_connection_t::ab ||
           slotsConnections[i].type ==
               solver_t::SlotsConnectionRepresentation::slot_connection_t::
                   bidirectionalCopy ||
           slotsConnections[i].type ==
               solver_t::SlotsConnectionRepresentation::slot_connection_t::
                   bidirectionalReuse) &&
        ...
        newExternalConnectionLine.lineNoFrom =
            slotLineNosTerm[0][slotsConnections[i].fromSlot];
        newExternalConnectionLine.lineNoTo =
            slotLineNosTerm[1][slotsConnections[i].toSlot];
        ...
        newExternalConnectionLine.involvesMapping =
            meshDescriptionsOfSlots_[newExternalConnectionLine.lineNoFrom] !=
            meshDescriptionsOfSlots_[newExternalConnectionLine.lineNoTo];

        externalConnectionLines_.push_back(newExternalConnectionLine);
      } else if (slotsConnections[i].type ==
                     solver_t::SlotsConnectionRepresentation::
                         slot_connection_t::ba &&
                 slotLineNosTerm[1].size() > slotsConnections[i].fromSlot &&
                 slotLineNosTerm[0].size() > slotsConnections[i].toSlot) {
        ExternalConnectionLine newExternalConnectionLine;
        newExternalConnectionLine.lineNoFrom =
            slotLineNosTerm[0][slotsConnections[i].toSlot];
        newExternalConnectionLine.lineNoTo =
            slotLineNosTerm[1][slotsConnections[i].fromSlot];
        newExternalConnectionLine.slotNoFrom = slotsConnections[i].fromSlot;
        newExternalConnectionLine.slotNoTo = slotsConnections[i].toSlot;
        newExternalConnectionLine.lineColumn = 0;
        newExternalConnectionLine.lineType = slotsConnections[i].type;
        ...
        VLOG(1) << "slotLineNosTerm is not good. fromSlot: "
                << slotsConnections[i].fromSlot
                << ", toSlot: " << slotsConnections[i].toSlot
                << ", slot line nos term0: " << slotLineNosTerm[0]
                << ", term1: " << slotLineNosTerm[1];
      }
    } else {
      VLOG(1) << "slotLineNosTerm is too short: " << slotLineNosTerm.size()
              << ", but there are " << slotsConnections.size()
              << " slotsConnections.";
    }
  }

  // if the own solver does not have output slots but it has only a single
  // child, use their output slots, this is true for MultipleInstances
  if (slotLineNos.empty() && children.size() == 1) {
    slotLineNos = slotLineNosTerm[0];
  }
  slotLineNosAll_.insert(slotLineNosAll_.end(), slotLineNos.begin(),
                         slotLineNos.end());
}

//! initialize the diagram generator, afterwards, generateDiagram can be called
void SolverStructureVisualizer::DiagramGenerator::initialize(
    std::shared_ptr<solver_t> solverRoot) {
...
std::string SolverStructureVisualizer::DiagramGenerator::
  ...
  generateDiagramRecursion(diagramWithoutConnectionLines,
                           internalConnectionLinesCurrentSolver, slotLineNos, 0,
                           false, false);

  // add the rest of the internal connection lines that are still storade under
  // internalConnectionLinesCurrentSolverTerm to internalConnectionLinesAll
  internalConnectionLinesAll_.insert(
      internalConnectionLinesAll_.end(),
      internalConnectionLinesCurrentSolver.begin(),
      internalConnectionLinesCurrentSolver.end());
...
std::string
SolverStructureVisualizer::DiagramGenerator::generateFinalDiagram() {
  std::string diagram;

  //! externalConnectionLines_ contains the following information: <lineNoFrom,
  //! lineNoTo, lineColumn, lineType>
  ...
  std::string diagramWithoutConnectionLinesString =
      generateDiagramWithoutConnectionLines();
  ...
  internalConnectionLinesAll_.erase(
      std::remove_if(
          internalConnectionLinesAll_.begin(),
          internalConnectionLinesAll_.end(),
          [](std::vector<int> connection) { return connection.size() == 1; }),
      internalConnectionLinesAll_.end());
  ...
  int nLinesDiagram =
      std::count(diagramWithoutConnectionLinesString.begin(),
                 diagramWithoutConnectionLinesString.end(), '\n');
  ...
  for (int currentLineNo = 0; currentLineNo < nLinesDiagram; currentLineNo++) {
    ...
    for (const std::vector<int> &connectionLine : internalConnectionLinesAll_) {
      int lineNoFirst =
          *std::min_element(connectionLine.begin(), connectionLine.end());
      int lineNoLast =
          *std::max_element(connectionLine.begin(), connectionLine.end());

      // if the currently considered connectionLine is active, i.e. present in
      // the current line "currentLineNo"
      if (lineNoFirst <= currentLineNo && currentLineNo <= lineNoLast) {
        // ensure that the connectionLine gets a column assigned if it does not
        // have one already

        // if the current activeLine has no assigned column yet
        if (internalConnectionLineColumns.find(activeLineIndex) ==
            internalConnectionLineColumns.end()) {
          VLOG(1) << "find new column for internal connection line with index "
                  << activeLineIndex << ": "
                  << internalConnectionLinesAll_[activeLineIndex];

          // loop over possible columns until a free column is found that can be
          // assign to the current activeLine
          for (int columnNo = 0;; columnNo++) {
            // check if current column is still free
            bool columnIsFree = true;
            for (std::pair<int, int> columnNoConnectionLine :
                 internalConnectionLineColumns) {
              if (columnNoConnectionLine.second == columnNo) {
                VLOG(1) << "column " << columnNo << " is occupied by line "
                        << columnNoConnectionLine.first << ": "
                        << internalConnectionLinesAll_[columnNoConnectionLine
                                                           .first];
                columnIsFree = false;
                break;
              }
            ...
            if (columnIsFree) {
              ...
              nColumnsForInternalConnectionLines =
                  std::max(nColumnsForInternalConnectionLines, columnNo + 1);
              break;
            }
          }
        }
  ...
  LOG(DEBUG) << "internalConnectionLineColumns: "
             << internalConnectionLineColumns;
  LOG(DEBUG) << "nColumnsForInternalConnectionLines: "
             << nColumnsForInternalConnectionLines;

  std::size_t pos = 0;
  diagram +=
      DihuContext::globalConnectionsBySlotName()->getDescriptionForDiagram();

  diagram += "Solver structure: \n\n";

  // print actual diagram, into std::string diagram
  // loop over lines
  for (int currentLineNo = 0;
       pos < diagramWithoutConnectionLinesString.length(); currentLineNo++) {
    ...
    for (int i = 0;
         i < VARIABLES_LINE_LENGTH + SLOT_NAME_LENGTH + 1 - lineLength; i++) {
      line += " ";
    }
    ...
    for (int currentColumnNo = 0;
         currentColumnNo < nColumnsForInternalConnectionLines;
         currentColumnNo++) {
      bool connectionLineFoundAtCurrentColumn = false;
      for (std::pair<int, int> internalConnectionLineColumn :
           internalConnectionLineColumns) {
        int columnNo = internalConnectionLineColumn.second;

        if (currentColumnNo == columnNo) {
          VLOG(1) << "currentColumnNo " << currentColumnNo
                  << ", connection line " << internalConnectionLineColumn.first
                  << "/" << internalConnectionLinesAll_.size();

          // vector of lines that are connected to the same connection line
          const std::vector<int> &connectionLine =
              internalConnectionLinesAll_[internalConnectionLineColumn.first];
          ...
          VLOG(1) << "line " << currentLineNo << ", connection [" << lineNoFirst
                  << "," << lineNoLast << "]";

          if (lineNoFirst <= currentLineNo && currentLineNo <= lineNoLast) {
            // check if it is connected to the current row
            if (std::find(connectionLine.begin(), connectionLine.end(),
                          currentLineNo) != connectionLine.end()) {
    ...
    */
    ...
    // partWithSlotName =
    // std::string(StringUtility::stringLength(partWithSlotName), 'x');
    ...
    line = firstPart + std::string(" ") + internalConnectionLinesColumns +
           partWithSlotName;
    ...
    // find out vertial data slot connection lines that go through this output
    // line
    struct VerticalLineInCurrentRow {
      int lineColumn; // the column in rhe current row where the vertical lines
                      // passes through
      bool lineStartsHere; // if this row is the start of the line
      bool lineEndsHere;   // if this row is the end of the line
      bool hasMhere; // if the current row contains the "m" that specifies a
                     // mapping
      solver_t::SlotsConnectionRepresentation::slot_connection_t
          type; // type of the line
    };

    std::vector<VerticalLineInCurrentRow>
        verticalLinesInCurrentRow; //! <lineColumn,thisLineStartsHere,thisLineEndsHere,type>,
                                   //! lineColumn
    bool lineStartsHere = false;
    bool lineEndsHere = false;
    bool unconnectedSlotHere = false;
    solver_t::SlotsConnectionRepresentation::slot_connection_t lineType =
        solver_t::SlotsConnectionRepresentation::slot_connection_t::ab;
    int lineColumn = 0;
#if 0
    ...
    // loop over all externalConnectionLines_ and see if they pass through the
    // current line collect all those rows into data structure
    // verticalLinesInCurrentRow
    for (int i = 0; i < externalConnectionLines_.size(); i++) {
      ...
      if (externalConnectionLines_[i].slotNoFrom > 0)
        nDigitsSlotNoFrom =
            int(log10(externalConnectionLines_[i].slotNoFrom)) + 1;

      // if the connection line starts or ends in the current row, there will be
      // a horizontal line
      if (currentLineNo == startLineNo) {
        ...
        solver_t::SlotsConnectionRepresentation::slot_connection_t lineType =
            externalConnectionLines_[i].lineType;
        int lineColumn = externalConnectionLines_[i].lineColumn;

        bool hasMhere = false;
        // if it involes a mapping and has the "m" here
        if (externalConnectionLines_[i].involvesMapping &&
            (int)(0.5 * (startLineNo + endLineNo)) == currentLineNo)
          hasMhere = true;
    ...
    std::sort(verticalLinesInCurrentRow.begin(),
              verticalLinesInCurrentRow.end(),
              [](const VerticalLineInCurrentRow &a,
                 const VerticalLineInCurrentRow &b) {
                return a.lineColumn < b.lineColumn;
              });

    // VLOG(1) << "line " << currentLineNo << ", currentLineNo " <<
    // currentLineNo << ", line=[" << line << "] has vertical lines: " <<
    // verticalLinesInCurrentRow;

    // fill current line with spaces such that it has the correct length
    const int requiredLineLength =
        VARIABLES_LINE_LENGTH + 4 + nColumnsForInternalConnectionLines +
        SLOT_NAME_LENGTH; //< line length for structure and variables
    ...
    if (lineStartsHere || lineEndsHere) {
      if (lineType == solver_t::SlotsConnectionRepresentation::
                          slot_connection_t::bidirectionalReuse) {
        fillCharacter = "═";
      } else {
        fillCharacter = "─";
      }
    } else if (std::find(slotLineNosAll_.begin(), slotLineNosAll_.end(),
                         currentLineNo) != slotLineNosAll_.end()) {
      unconnectedSlotHere = true;
    }

    // if the line is yet too short, add fillCharacter until is has the correct
    // length
    if (lineLength < requiredLineLength) {
      for (int i = 0; i < requiredLineLength - lineLength; i++) {
        // starting tip of arrow if the line starts or ends in the current row
        // and has the according type at i == 0 if the slot no has one digit, at
        // i == 1 if the slot no has two digits, etc.
        if (i == nDigitsSlotNoFrom - 1 && lineStartsHere &&
            (lineType == solver_t::SlotsConnectionRepresentation::
                             slot_connection_t::ba ||
             lineType == solver_t::SlotsConnectionRepresentation::
                             slot_connection_t::bidirectionalCopy)) {
          line += "<";
        } else if (i == nDigitsSlotNoTo - 1 && lineEndsHere &&
                   (lineType == solver_t::SlotsConnectionRepresentation::
                                    slot_connection_t::ab ||
                    lineType == solver_t::SlotsConnectionRepresentation::
                                    slot_connection_t::bidirectionalCopy)) {
          line += "<";
        } else {
    ...
    if (!verticalLinesInCurrentRow.empty()) {
      // loop over columns in the current row, current column is
      // requiredLineLength + 2*i, i.e. it advances always by 2 characters
      for (int i = 0; i <= verticalLinesInCurrentRow.back().lineColumn; i++) {
        ...
        // determine space character which is a horizontal line if the
        // connection starts or ends here
        std::string space = " ";
        if (lineCrosses ||
            ((lineStartsHere || lineEndsHere) && i <= lineColumn)) {
          if (lineType == solver_t::SlotsConnectionRepresentation::
                              slot_connection_t::bidirectionalReuse) {
            space = "═";
          } else {
        ...
        if (i == verticalLinesInCurrentRow[linesIndex].lineColumn) {
          ...
          case solver_t::SlotsConnectionRepresentation::slot_connection_t::
            ...
            if (lineCrosses &&
              ...
              // if the meshes of the connected slots are different, it is a
              // mapping, add an "m" in the center of the line
              line += "m";
            } else {
              line += "║";
            }
          ...
          case solver_t::SlotsConnectionRepresentation::slot_connection_t::
              internalAfterComputation:
            LOG(ERROR) << "slotsConnections should not have a connecion type "
                          "of `internal*Computation`. Got: "
                       << verticalLinesInCurrentRow[linesIndex].type;
            break;
          };
          linesIndex++;
        } else if (unconnectedSlotHere && i == 0) {
          ...
          // there is no vertical line passing through the current position, add
          // two spaces
          line += space;
          line += space;
        }
...
void SolverStructureVisualizer::writeDiagramFile(std::string filename) {
  ...
  // clear all values, this is required for unit tests where multiple different
  // setups are created after each other
  solverRoot_ = nullptr;
