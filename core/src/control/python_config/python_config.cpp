...
PythonConfig::PythonConfig(PyObject *specificSettings) {
  ...
  VLOG(1) << "PythonConfig::constructor "
          << PythonUtility::getString(pythonConfig_);
...
PythonConfig::PythonConfig(const PythonConfig &rhs, std::string key) {
  ...
  VLOG(1) << "PythonConfig::constructor(rhs,key=\"" << key << "\") "
          << PythonUtility::getString(pythonConfig_);
  Py_XINCREF(pythonConfig_);
...
PythonConfig::PythonConfig(const PythonConfig &rhs, int i) {
  ...
  if (PyList_Check(rhs.pyObject())) {
    ...
    if (i >= nEntries) {
      LOG(ERROR) << getStringPath() << " list has only " << nEntries
                 << " entries, but entry " << i << " is required.";
...
PythonConfig::PythonConfig(const PythonConfig &rhs, std::string key,
                           PyObject *config, int listIndex) {
  pythonConfig_ = config;
  VLOG(1) << "PythonConfig::constructor(rhs,key=\"" << key << "\",config) "
          << PythonUtility::getString(pythonConfig_);
  Py_XINCREF(pythonConfig_);
...
//! constructor directly from PyObject*, path from rhs + key
PythonConfig::PythonConfig(const PythonConfig &rhs, std::string key,
                           std::string key2, PyObject *config) {
  pythonConfig_ = config;
  VLOG(1) << "PythonConfig::constructor(rhs,key=\"" << key << "\",config) "
          << PythonUtility::getString(pythonConfig_);
  Py_XINCREF(pythonConfig_);
...
PythonConfig &PythonConfig::operator=(const PythonConfig &rhs) {
  ...
  VLOG(1) << "PythonConfig::operator=(rhs)"
          << PythonUtility::getString(pythonConfig_);
  Py_XINCREF(pythonConfig_);
...
PyObject *PythonConfig::pyObject() const { return pythonConfig_; }
...
std::string PythonConfig::getStringPath() const {
  ...
  pathString << "config";
  for (std::vector<std::string>::const_iterator iter = path_.begin();
       iter != path_.end(); iter++) {
    if (*iter == "...") {
      pathString << "[...]";
...
bool PythonConfig::hasKey(std::string key) const {
  ...
  if (!PyDict_Check(pythonConfig_)) {
    ...
    LOG(ERROR) << "" << pathString << " is not a dict in \""
...
void PythonConfig::getKeys(std::vector<std::string> &keys) {
  ...
  std::vector<std::pair<std::string, PyObject *>> list =
      PythonUtility::convertFromPython<
          std::vector<std::pair<std::string, PyObject *>>>::get(pyList);

  keys.clear();
  for (std::vector<std::pair<std::string, PyObject *>>::iterator iter =
           list.begin();
       iter != list.end(); iter++) {
...
PyObject *PythonConfig::getOptionPyObject(std::string key,
...
double PythonConfig::getOptionDouble(
    std::string key, double defaultValue,
    PythonUtility::ValidityCriterion validityCriterion) const {
  std::string pathString = getStringPath();
  return PythonUtility::getOptionDouble(this->pythonConfig_, key, pathString,
                                        defaultValue, validityCriterion);
}

//! return the option value given by key in the python dictionary settings. If
//! not found, return the defaultValue
int PythonConfig::getOptionInt(
    std::string key, int defaultValue,
    PythonUtility::ValidityCriterion validityCriterion) const {
  std::string pathString = getStringPath();
  return PythonUtility::getOptionInt(this->pythonConfig_, key, pathString,
                                     defaultValue, validityCriterion);
}
...
bool PythonConfig::getOptionBool(std::string key, bool defaultValue) const {
  std::string pathString = getStringPath();
  return PythonUtility::getOptionBool(this->pythonConfig_, key, pathString,
                                      defaultValue);
}

//! return the option value given by key in the python dictionary settings. If
//! not found, return the defaultValue
std::string PythonConfig::getOptionString(std::string key,
                                          std::string defaultValue) const {
  std::string pathString = getStringPath();
  return PythonUtility::getOptionString(this->pythonConfig_, key, pathString,
                                        defaultValue);
}
...
bool PythonConfig::getOptionDictEnd(std::string keyString) const {
  std::string pathString = getStringPath();
  return PythonUtility::getOptionDictEnd(this->pythonConfig_, keyString,
                                         pathString);
}
...
bool PythonConfig::getOptionListEnd(std::string keyString) const {
  std::string pathString = getStringPath();
  return PythonUtility::getOptionListEnd(this->pythonConfig_, keyString,
                                         pathString);
}
...
void PythonConfig::getOptionVector(std::string keyString,
                                   std::vector<PythonConfig> &values) const {
  ...
  if (settings) {
    ...
    if (PyDict_Contains((PyObject *)settings, key)) {
      ...
      if (PyList_Check(value)) {
        ...
        for (; !this->getOptionListEnd(keyString);
      ...
      // this is no warning
      std::string pathString = this->getStringPath();
      LOG(DEBUG) << "" << pathString << "[\"" << keyString
