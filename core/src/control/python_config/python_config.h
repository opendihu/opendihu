 ...
 *  It knows the path from the config variable, e.g.
 * config["Solver1"]["Solver2"]["Solver3"] and the contents, e.g., {"key1": 0,
 * "key2": "value"}
 */
class PythonConfig {
public:
  ...
  //! constructor directly from PyObject*, path from rhs + key + [listIndex] (if
  //! not -1)
  ...
  PythonConfig(const PythonConfig &rhs, std::string key1, std::string key2,
               PyObject *config);
  ...
  //! iterator to beginning of path vector, which contains the keys from the
  //! top-level config object to the current context
  std::vector<std::string>::const_iterator pathBegin() const;

  //! iterator to end of path vector, which contains the keys from the top-level
  //! config object to the current context
  ...
  PyObject *getOptionPyObject(std::string key,
                              PyObject *defaultValue = Py_None) const;

  //! return the option value given by key in the python dictionary settings. If
  //! not found, return the defaultValue
  double getOptionDouble(std::string key, double defaultValue,
                         PythonUtility::ValidityCriterion validityCriterion =
                             PythonUtility::ValidityCriterion::None) const;

  //! return the option value given by key in the python dictionary settings. If
  //! not found, return the defaultValue
  int getOptionInt(std::string key, int defaultValue,
                   PythonUtility::ValidityCriterion validityCriterion =
                       PythonUtility::ValidityCriterion::None) const;
  ...
  template <typename Key, typename Value>
  void getOptionDictNext(std::string keyString,
  ...
  void getOptionVector(std::string keyString, int nEntries,
                       std::vector<double> &values) const;

  //! extract a vector with exactly the specified number of nEntries, must be a
  //! list, not specified entries are set to {0}
  template <typename T>
  void getOptionVector(std::string keyString, int nEntries,
  ...
  //! extract a vector with unknown number of nEntries, must be a list
  void getOptionVector(std::string keyString,
...
protected:
  PyObject
      *pythonConfig_; //< the python config dictionary of the current context
                      //(i.e. may be a sub-dict of the global config)
  std::vector<std::string>
      path_; //< the key words of the python config down to the current scope
};
