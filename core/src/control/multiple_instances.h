...
/** This class holds multiple instances of the template type, e.g. for having
 * multiple fibers, which are each as in example electrophysiology
 */
template <typename TimeSteppingScheme>
class MultipleInstances : public Runnable, public Multipliable {
public:
  typedef std::vector<
      std::shared_ptr<typename TimeSteppingScheme::SlotConnectorDataType>>
  ...
  //! call the own output writer on the data object, output files will contain
  //! currentTime, with callCountIncrement !=1 output timesteps can be skipped
  void writeOwnOutput(int timeStepNo, double currentTime,
                      int callCountIncrement = 1);

  //! call the output writer on the data object and all nested solvers, output
  //! files will contain currentTime, with callCountIncrement !=1 output
  //! timesteps can be skipped
