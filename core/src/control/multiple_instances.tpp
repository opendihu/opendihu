...
MultipleInstances<TimeSteppingScheme>::MultipleInstances(DihuContext context)
    : context_(context["MultipleInstances"]),
      specificSettings_(context_.getPythonConfig()), data_(context_),
      outputInitializeThisInstance_(false) {
  ...
  if (specificSettings_.hasKey("durationLogKey") ||
      specificSettings_.hasKey("logKey")) {
  ...
  nInstances_ =
      specificSettings_.getOptionInt("nInstances", 1, PythonUtility::Positive);
  ...
  PyObject *instanceConfig =
      specificSettings_.getOptionListBegin<PyObject *>("instances");

  int i = 0;
  for (; !specificSettings_.getOptionListEnd("instances") && i < nInstances_;
       specificSettings_.template getOptionListNext<PyObject *>("instances",
                                                                instanceConfig),
       i++) {
    if (instanceConfig == Py_None) {
      instanceConfigs.push_back(nullptr);
    } else {
      instanceConfigs.push_back(std::make_shared<PythonConfig>(
          specificSettings_, "instances", instanceConfig, i));
    }
  ...
  if (i < nInstances_) {
    LOG(ERROR) << "Could only create " << i
               << " instances from the given instances config python list, but "
                  "nInstances = "
               << nInstances_;
    nInstances_ = i;
  }
  ...
  if (!specificSettings_.getOptionListEnd("instances")) {
    ...
    std::vector<PyObject *> vector =
        PythonUtility::convertFromPython<std::vector<PyObject *>>::get(
            instancesList);
    LOG(ERROR) << "Only " << nInstances_
               << " instances were created, but more (" << vector.size()
               << ") configurations are given.";
  }

  VLOG(1) << "MultipleInstances constructor, create Partitioning for "
          << nInstances_ << " instances";
  ...
  if (specificSettings_.hasKey("ranksAllComputedInstances")) {
    ...
    specificSettings_.getOptionVector("ranksAllComputedInstances",
                                      ranksAllComputedInstancesVector);
    ranksAllComputedInstances.insert(ranksAllComputedInstancesVector.begin(),
                                     ranksAllComputedInstancesVector.end());
  }

  nInstancesComputedGlobally_ = 0;
  std::vector<std::tuple<std::shared_ptr<Partition::RankSubset>, bool,
                         std::shared_ptr<PythonConfig>>>
      instanceData(
          nInstances_); // <rankSubset, computeOnThisRank, instanceConfig>

  int ownRankNo =
      this->context_.ownRankNo(); // this may not be from MPI_COMM_WORLD but the
                                  // context's communicator
  ...
  for (int instanceConfigNo = 0; instanceConfigNo < nInstances_;
       instanceConfigNo++) {
    std::shared_ptr<PythonConfig> instanceConfig =
        instanceConfigs[instanceConfigNo];
    ...
    if (!instanceConfig) {
      ...
      LOG(ERROR) << "Instance " << instanceConfigs
                 << " has no \"ranks\" settings.";
    } else {
      // extract rank list from config
      instanceConfig->getOptionVector("ranks", ranks);
      std::set<int> rankSet(ranks.begin(), ranks.end());
      LOG(DEBUG) << "instance no. " << instanceConfigNo
                 << " has ranks: " << ranks << ", " << rankSet;
      ...
      VLOG(2) << "compute on this rank: " << std::boolalpha
              << computeOnThisRank;

      // create rank subset
      if (computeOnThisRank) {
        LOG(DEBUG) << "instance " << instanceConfigNo
      ...
      for (int i = 0; i < rankSubsets.size(); i++) {
        if (rankSubsets[i]->equals(rankSet)) {
          ...
          LOG(DEBUG) << "reuse rank subset of instance " << i
                     << ", ranks: " << ranks;
          break;
        }
      }
    ...
    if (!rankSubset) {
      LOG(DEBUG) << "create new rank subset from ranks " << ranks
                 << " in context " << *this->context_.rankSubset();
      // The rank subsets have to be created collectively by all ranks in the
      // current context, even if they will not be part of the new communicator!
      rankSubset = std::make_shared<Partition::RankSubset>(
          ranks.begin(), ranks.end(), this->context_.rankSubset());
    }
  ...
  rankSubsetAllComputedInstances_ = std::make_shared<Partition::RankSubset>(
      ranksAllComputedInstances.begin(), ranksAllComputedInstances.end());

  VLOG(1) << "rankSubsetAllComputedInstances: "
          << *rankSubsetAllComputedInstances_;

  // store the rank subset of all instances to partition manager, such that it
  // can be retrived when the instances are generated
  this->context_.partitionManager()->setRankSubsetForCollectiveOperations(
      rankSubsetAllComputedInstances_);

  // log the number of instances that are computed by all ranks
  PerformanceMeasurement::setParameter("nInstancesComputedGlobally",
                                       nInstancesComputedGlobally_);
  ...
  for (int instanceConfigNo = 0; instanceConfigNo < nInstances_;
    ...
    std::shared_ptr<Partition::RankSubset> rankSubset =
        std::get<0>(instanceData[instanceConfigNo]);
    bool computeOnThisRank = std::get<1>(instanceData[instanceConfigNo]);
    std::shared_ptr<PythonConfig> instanceConfig =
        std::get<2>(instanceData[instanceConfigNo]);

    if (!computeOnThisRank) {
      continue;
    }

    // store the rank subset containing only the own rank for the mesh of the
    // current instance
    this->context_.partitionManager()->setRankSubsetForNextCreatedPartitioning(
        rankSubset);

    VLOG(1) << "create sub context for instance no " << instanceConfigNo
            << ", rankSubset: " << *rankSubset;
    VLOG(1) << "this rankSubset is also set via "
               "setRankSubsetForNextCreatedPartitioning";

    // add this instance to the instances that are computed locally
    instancesLocal_.emplace_back(
        context_.createSubContext(*instanceConfig, rankSubset));
  ...
  if (this->logKey_ != "") {
    ...
    Control::PerformanceMeasurement::setParameter(logKey.str(),
                                                  nInstancesLocal_);
  }
...
void MultipleInstances<TimeSteppingScheme>::advanceTimeSpan(
  ...
  // This method advances the simulation by the specified time span. It will be
  // needed when this MultipleInstances object is part of a parent control
  // element, like a coupling to 3D model.
  for (int i = 0; i < nInstancesLocal_; i++) {
    instancesLocal_[i].advanceTimeSpan(withOutputWritersEnabled, checkpointing);
  }
  ...
  LOG(DEBUG) << "multipleInstances::advanceTimeSpan() complete, now call "
                "writeOutput, hasOutputWriters: "
             << this->outputWriterManager_.hasOutputWriters();

  if (nInstancesLocal_ > 0) {
    writeOwnOutput(instancesLocal_[0].numberTimeSteps(),
                   instancesLocal_[0].endTime());
  }
}

template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::setTimeSpan(double startTime,
                                                        double endTime) {
  for (int i = 0; i < nInstancesLocal_; i++) {
...
template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::initialize() {
  LOG_SCOPE_FUNCTION;
  ...
  // initialize output of progress in %, it is only output for one instance and
  // then only for rank 0
  if (outputInitialize_) {
    outputInitializeThisInstance_ = true;
    outputInitialize_ = false;
    el::Loggers::removeFlag(el::LoggingFlag::NewLineForContainer);
    LOG(INFO) << "Initialize " << nInstancesComputedGlobally_
              << " global instances (" << nInstancesLocal_ << " local).";
  }
  ...
  for (int i = 0; i < nInstancesLocal_; i++) {
    ...
    if (i == 0) {
      LOG(DEBUG) << " in multiple_instances, i=1, endChild and disable "
  ...
  for (int i = 0; i < nInstancesLocal_; i++) {
    ...
    if (VLOG_IS_ON(1)) {
      VLOG(1) << "instance " << i << "/" << nInstancesLocal_ << " is "
              << (*slotConnectorData_)[i];
    }
  }
...
template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::run() {
  initialize();

  LOG(INFO) << "MultipleInstances: " << nInstancesComputedGlobally_
            << " instance" << (nInstancesComputedGlobally_ != 1 ? "s" : "")
  ...
  for (int i = 0; i < nInstancesLocal_; i++) {
    if (omp_get_thread_num() == 0) {
      ...
      msg << omp_get_thread_num() << ": running " << nInstancesLocal_
          << " instances with " << omp_get_num_threads() << " OpenMP threads";
      LOG(DEBUG) << msg.str();
    }
  ...
  LOG(DEBUG) << "multipleInstances::run() complete, now call writeOutput, "
                "hasOutputWriters: "
...
::Data::MultipleInstances<typename TimeSteppingScheme::FunctionSpace,
                          TimeSteppingScheme> &
MultipleInstances<TimeSteppingScheme>::data() {
  return data_;
}
...
template <typename TimeSteppingScheme>
std::shared_ptr<
    typename MultipleInstances<TimeSteppingScheme>::SlotConnectorDataType>
MultipleInstances<TimeSteppingScheme>::getSlotConnectorData() {
  // call getSlotConnectorData on all instances such that they can prepare
  // themselves (e.g. timestepping schemes call prepareForGetSlotConnectorData)
  for (int i = 0; i < nInstancesLocal_; i++) {
...
MultipleInstances<TimeSteppingScheme>::instancesLocal() {
  return instancesLocal_;
}

//! time of simulation
template <typename TimeSteppingScheme>
double MultipleInstances<TimeSteppingScheme>::startTime() {
  if (nInstancesLocal_ > 0)
    return instancesLocal_[0].startTime();
  return 0;
}

//! time of simulation
template <typename TimeSteppingScheme>
double MultipleInstances<TimeSteppingScheme>::endTime() {
  if (nInstancesLocal_ > 0)
...
template <typename TimeSteppingScheme>
int MultipleInstances<TimeSteppingScheme>::numberTimeSteps() {
  if (nInstancesLocal_ > 0)
...
template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::writeOwnOutput(
    int timeStepNo, double currentTime, int callCountIncrement) {
  if (nInstancesLocal_ > 0) {
    LOG(DEBUG) << "MultipleInstances::writeOutput, timeStepNo: " << timeStepNo
...
template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::callOutputWriter(
    int timeStepNo, double currentTime, int callCountIncrement) {
  ...
  for (int i = 0; i < nInstancesLocal_; i++) {
    instancesLocal_[i].callOutputWriter(timeStepNo, currentTime,
                                        callCountIncrement);
  }
}

template <typename TimeSteppingScheme>
std::string MultipleInstances<TimeSteppingScheme>::getString(
    std::shared_ptr<
        typename MultipleInstances<TimeSteppingScheme>::SlotConnectorDataType>
        data) {
  std::stringstream s;
...
template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::saveFiberDataCheckpoint() {}

template <typename TimeSteppingScheme>
void MultipleInstances<TimeSteppingScheme>::restoreFiberDataCheckpoint() {}
