...
/** This class contains global variables (mesh manager and solver manager) and a
 * python config dictionary which can be a sub-dict of the global config.
 * Different objects of this class are used for the methods throughout this
 * program which have a different context in terms of their config.
 */
class DihuContext {
public:
  //! constructor, initialize context, parse command line parameters and input
  //! file
  DihuContext(int argc, char *argv[], bool doNotFinalizeMpi = false,
              bool settingsFromFile = true);

  //! constructor for test cases
  DihuContext(int argc, char *argv[], std::string pythonSettings,
              bool doNotFinalizeMpi = true);

  //! constructor with explicit pythonConfig
  DihuContext(int argc, char *argv[], bool doNotFinalizeMpi,
  ...
  //! return a context object with config originated at child node with given
  //! key
  ...
  //! and rankSubset, if rankSubset is not given, reuse own rankSubset
  DihuContext
  ...
  //! return the mappingBetweenMeshesManager manager object that contains all
  //! mappings between meshes
  static std::shared_ptr<MappingBetweenMeshes::Manager>
  mappingBetweenMeshesManager();
  ...
  //! return the object that collects information about all nested solvers and
  //! produces a diagram, also with data connections
  static std::shared_ptr<SolverStructureVisualizer> solverStructureVisualizer();

  //! return the object that collects the connector slots connections given
  //! under setting "connectedSlots"
  ...
  //! get the rank subset of this context, this may not be the same as
  //! MPI_COMM_WORLD
...
private:
  ...
  static std::vector<char *>
      megamolArgv_; //< the arguments use for the megamol instance
  ...
  static std::shared_ptr<zmq::socket_t>
      zmqSocket_; //< a socket that is connected to one megamol
#endif
};
