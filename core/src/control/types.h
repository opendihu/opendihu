...
//  examples/solid_mechanics/dynamic_mooney_rivlin/muscle_with_fat runtime:
//  5:08.61 min -> 3:07.93 min, compile time: 2m28,546s+1m34,024s ->
//  2m16,100s+9m10,411s This option is set by the build system.
...
typedef node_no_t
    element_no_t; // type to hold value of element no or number of elements
...
// values. Then everything is computed for 4 consecutive elements at once
// - or use the normal, non-vectorized version. Then, normal double types are
// used.
#ifdef USE_VECTORIZED_FE_MATRIX_ASSEMBLY

// implementation with explicit SIMD vectorization
const int nVcComponents =
    Vc::double_v::size(); //< number of components that will be computed at once
...
template <int D>
using VecD_v_t =
    VecD<D, Vc::double_v>; //< instead of vectorized VecD use a normal VecD
...
const int nVcComponents =
    1; //< number of components that will be computed at once
...
template <int D>
using Tensor2_v_t =
    Tensor2<D>; //< instead of vectorized Tensor2 use a normal Tensor2

template <int D>
using Tensor4_v_t =
    Tensor4<D>; //< instead of vectorized Tensor4 use a normal Tensor4
