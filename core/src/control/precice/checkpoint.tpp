...
void PreciceAdapterCheckpoint<NestedSolver>::saveCheckpoint(
    double currentTime) {
  ...
  // if the variable savedState_ does not yet exist, create as duplicate from
  // the state vector
  if (savedState_ == PETSC_NULL) {
    ierr = VecDuplicate(this->currentState(this->nestedSolver_), &savedState_);
    CHKERRV(ierr);
  }

  // copy values
  ierr = VecCopy(this->currentState(this->nestedSolver_), savedState_);
  CHKERRV(ierr);
}
...
template <typename NestedSolver>
double PreciceAdapterCheckpoint<NestedSolver>::loadCheckpoint() {
