...
void ReadWriteDataBase::preciceReadVolumeData(std::shared_ptr<precice::Participant> &preciceParticipant) {
    ...
    // NestedSolver::SlotConnectorDataType;
    ...
    //         // fill the vector geometryValues_ with the geometry values of
    //         the
...
void ReadWriteDataBase::preciceWriteVolumeData(std::shared_ptr<precice::Participant> &preciceParticipant) {
  ...
  //           values.clear();
  //           // static void
  //           slotGetValues(std::shared_ptr<SlotConnectorDataType>
  //           // slotConnectorData,
...
void ReadWriteDataBase::preciceReadSurfaceData(std::shared_ptr<precice::Participant> &preciceParticipant){
    // LOG(DEBUG) << "read data from precice";
...
void ReadWriteDataBase::preciceWriteSurfaceData(std::shared_ptr<precice::Participant> &preciceParticipant) {
  //   // write data to precice
  //     LOG(DEBUG) << "write data to precice";
  ...
  //                     << tractionValues_[2];
  //     #endif
  //             // scale traction values, they are always scaled by the factor
  //             of -1 for (double &value : tractionValues_) { value *=
  //             this->scalingFactor_; value *= -1;
  //             }
  ...
  //     #ifndef NDEBUG
  //             LOG(DEBUG) << "write traction data to precice: " <<
  //             tractionValues_; std::stringstream s; for (int i = 2; i <
  //             tractionValues_.size(); i += 3) { s << " " <<
  //             tractionValues_[i];
  //             }
  ...
  //                 tractionValues_);
  //         } else {
  //             LOG(FATAL) << "Unknown precice data (write), none of
  //             displacements, "
  //                         "velocities or traction is set.";
  //         }
  //         }
...
// void ReadWriteDataBase<NestedSolver>::setNeumannBoundaryConditions(typename
// PreciceAdapterInitialize<NestedSolver>::PreciceSurfaceData &preciceData){
//    using FunctionSpace =
//                 typename
//                 PreciceAdapterNestedSolver<NestedSolver>::FunctionSpace;
//             using ElementWithFacesType =
//                 typename SpatialDiscretization::NeumannBoundaryConditions<
//                     FunctionSpace, Quadrature::Gauss<3>,
//                     3>::ElementWithFaces;
//             /*
...
//             VecD<nComponents>>> dofVectors;  //< <surface-local dof no,
//             value>, nComponents == FunctionSpaceType::dim() for traction
//             boundary condition or nComponents = 1 for flux BC
//             std::vector<dof_no_t> surfaceDofs; //< dof nos of the volume
//             element that correspond to the face / surface. These are
//             different from the dofs in dofsVector which are numbered for the
//             surface only, surfaceDofs are in the numbering of the volume
//             element.
//                 // note, for flux BC, dofVectors[i].second is a VecD<1>
//             };*/

//             std::vector<ElementWithFacesType>
...
//                 const int nNodesX =
//                 this->functionSpace_->nNodesLocalWithoutGhosts(0); const int
//                 nNodesY = this->functionSpace_->nNodesLocalWithoutGhosts(1);
//                 const int nNodesZ =
//                 this->functionSpace_->nNodesLocalWithoutGhosts(2);

//                 int nElementsX =
//                 this->functionSpace_->meshPartition()->nElementsLocal(0); int
//                 nElementsY =
//                 this->functionSpace_->meshPartition()->nElementsLocal(1); int
//                 nElementsZ =
//                 this->functionSpace_->meshPartition()->nElementsLocal(2);

//                 // set node and element indics in z direction for bottom
//                 surface int nodeIndexZ = 0; int elementalNodeIndexZ = 0; int
//                 elementIndexZ = 0;

//                 // for top surface
//                 if (preciceData.preciceMesh->face ==
//                 PreciceAdapterInitialize<NestedSolver>::PreciceSurfaceData::face2Plus)
//                 { nodeIndexZ = nNodesZ - 1; elementIndexZ = nElementsZ - 1;
//                 elementalNodeIndexZ = 2;
//                 }

//                 // loop over elements
//                 for (int elementIndexY = 0; elementIndexY < nElementsY;
//                 elementIndexY++) { for (int elementIndexX = 0; elementIndexX
//                 < nElementsX; elementIndexX++) {
//                     ElementWithFacesType elementWithFaces;
//                     element_no_t elementNoLocal = elementIndexZ * nElementsX
//                     * nElementsY +
...
//                     // get dofs indices within the numbering of the volume
//                     element that
...
//                             2, typename
//                             FunctionSpace::BasisFunction>::nNodesPerElement()
//                             *
//                         nDofsPerNode;
...
//                     for (int elementalNodeIndexY = 0; elementalNodeIndexY <
//                     3;
//                         elementalNodeIndexY++) {
//                     for (int elementalNodeIndexX = 0; elementalNodeIndexX <
//                     3;
//                         elementalNodeIndexX++) {
...
//                         // only iterate over local dofs, the ghost dofs are
//                         not considered
//                         // here (although it would be correct to communicate
//                         them with
//                         // precice)
//                         if (dofNoLocal >=
...
//                         int valueIndex = dofNoLocal - nodeIndexZ * nNodesX *
//                         nNodesY;
...
//                         assert(valueIndex >= 0);
//                         if (valueIndex >=
//                         preciceData.preciceMesh->nNodesLocal) { LOG(ERROR) <<
//                         "valueIndex: " << valueIndex
//                                     << ", dofNoLocal: " << dofNoLocal
//                                     << ", nNodes: " << nNodesX << "," <<
//                                     nNodesY
...
//                                                                 Quadrature::Gauss<3>,
//                                                                 3>;
//             std::shared_ptr<NeumannBoundaryConditionsType>
...
//             {
//                 s << "{el. " <<
//                 neumannBoundaryConditionElements[i].elementNoLocal << ", \""
//                 << Mesh::getString(neumannBoundaryConditionElements[i].face)
//                 << "\", dofVectors: [";
//                 for (int j = 0; j <
//                 neumannBoundaryConditionElements[i].dofVectors.size();
//                     j++) {
//                 s << "(" <<
//                 neumannBoundaryConditionElements[i].dofVectors[j].first
//                     << ": (";
//                 for (int k = 0;
//                     k <
...
//                     s << ",";
//                     s <<
//                     neumannBoundaryConditionElements[i].dofVectors[j].second[k];
//                 }
//                 s << ")),";
//                 }
//                 s << "], surfaceDofs: ";
//                 for (int j = 0; j <
//                 neumannBoundaryConditionElements[i].surfaceDofs.size();
//                     j++)
...
// template <typename NestedSolver>
// void ReadWriteDataBase<NestedSolver>::setDirichletBoundaryConditions(typename
// PreciceAdapterInitialize<NestedSolver>::PreciceSurfaceData &preciceData){
...
//                         nodeIndexZ * nNodesX * nNodesY + nodeIndexY * nNodesX
//                         + nodeIndexX;
...
//                     // assign received values to dirichlet bc vector of size
//                     6 std::array<double, 6> newDirichletBCValue;

//                     for (int i = 0; i < 3; i++) {
//                     newDirichletBCValue[i] = displacementValues_[3 *
//                     valueIndex + i]; newDirichletBCValue[3 + i] =
//                     velocityValues_[3 * valueIndex + i];
//                     }

//                     newDirichletBCValues.push_back(
//                         std::pair<global_no_t, std::array<double,
//                         6>>(dofNoGlobal,
//                                                                     newDirichletBCValue));
//                 }
//                 }

//                 LOG(DEBUG) << "read data from precice complete, displacement
//                 values: "
//                         << displacementValues_
//                         << ", velocityValues: " << velocityValues_;
//                 LOG(DEBUG) << "read and set Dirichlet BC: " <<
//                 newDirichletBCValues;
//             }
