    ...
    protected:
        ...
        void preciceReadData_impl(){
            ...
            // bool tmp = preciceData.ioType == ;
                ...
                if (preciceData.ioType == ReadWrite::ioRead) {
        ...
        void preciceWriteData_impl(){
            ...
            for (auto &preciceData : this->preciceSurfaceData_) {
                if (preciceData.ioType ==
                    decltype(preciceData.ioType)::ioWrite) {
                // if the data is displacements and velocities
                if (!preciceData.displacementsName.empty()) {
        ...
        void setNeumannBoundaryConditions_impl(auto &preciceData){
                        using FunctionSpace =
            ...
            if (preciceData.preciceMesh->nNodesLocal != 0) {
                ...
                // for top surface
                if (preciceData.preciceMesh->face ==decltype(preciceData.preciceMesh.ioType)::face2Plus) {
                ...
                for (int elementIndexX = 0; elementIndexX < nElementsX; elementIndexX++) {
                    ...
                    if (preciceData.preciceMesh->face ==
                        decltype(preciceData.preciceMesh.ioType)::face2Plus)
        ...
        void setDirichletBoundaryConditions_impl(auto &preciceData){
