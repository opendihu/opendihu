...
PreciceAdapterVolumeCouplingInitialize<
    NestedSolver>::PreciceAdapterVolumeCouplingInitialize(DihuContext context)
    : context_(context["PreciceAdapterVolumeCoupling"]),
      nestedSolver_(this->context_), maximumPreciceTimestepSize_(0),
      timeStepOutputInterval_(1), initialized_(false) {
  // get python settings object from context
  this->specificSettings_ = this->context_.getPythonConfig();
}

template <typename NestedSolver>
void PreciceAdapterVolumeCouplingInitialize<NestedSolver>::initialize() {
#ifdef HAVE_PRECICE

  LOG(DEBUG) << "initialize precice adapter (volume coupling), initialized_="
             << initialized_;
  ...
  // std::cout << "rank for precice: " << rankNo << "/" << nRanks << " " <<
  // DihuContext::ownRankNoCommWorld() << "/" << DihuContext::nRanksCommWorld()
  // << std::endl;
  ...
  LOG(FATAL) << "Failed to initialize PreciceAdapterVolumeCoupling because "
                "opendihu is not compiled with preCICE.";
#endif
}
...
template <typename NestedSolver>
void PreciceAdapterVolumeCouplingInitialize<
    NestedSolver>::initializePreciceData() {
  ...
  // loop over items of the list under "preciceData", the order of the items is
  ...
  for (int listIndex = 0; listIndex < list.size(); listIndex++) {
    ...
    // create a new preciceData instance that will be added to preciceData_ at
    ...
    if (iter == preciceMeshes_.end()) {
      ...
      if (preciceData.opendihuMeshName != "") {
        functionSpace =
            DihuContext::meshManager()->functionSpace<FunctionSpace>(
                preciceData.opendihuMeshName);
    ...
    if (nDofsLocalWithoutGhosts * nArrayItems !=
      ...
      LOG(FATAL)
          ...
          << "- Rename the preciceMesh to a unique name and update the precice "
