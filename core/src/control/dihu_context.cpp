...
std::shared_ptr<SolverStructureVisualizer>
    DihuContext::solverStructureVisualizer_ = nullptr;
std::shared_ptr<GlobalConnectionsBySlotName>
    DihuContext::globalConnectionsBySlotName_ = nullptr;

// other global variables that are needed in static methods
std::string DihuContext::solverStructureDiagramFile_ =
    ""; //< filename of the solver structure diagram file
std::string DihuContext::pythonScriptText_ = ""; //< the python settings text
DihuContext::logFormat_t DihuContext::logFormat_ =
    DihuContext::logFormat_t::logFormatCsv; //< format of lines in the log file
...
#ifdef HAVE_ADIOS
std::shared_ptr<adios2::ADIOS> DihuContext::adios_ =
    nullptr; //< adios context option
#endif
...
DihuContext::DihuContext(const DihuContext &rhs)
    : pythonConfig_(rhs.pythonConfig_), rankSubset_(rhs.rankSubset_) {
  nObjects_++;
...
DihuContext::DihuContext(int argc, char *argv[], bool doNotFinalizeMpi,
                         PythonConfig pythonConfig,
                         std::shared_ptr<Partition::RankSubset> rankSubset)
    : pythonConfig_(pythonConfig), rankSubset_(rankSubset),
      doNotFinalizeMpi_(doNotFinalizeMpi) {
  nObjects_++;
  ...
  if (!rankSubset_) {
    rankSubset_ =
        std::make_shared<Partition::RankSubset>(); // create rankSubset with all
                                                   // ranks, i.e. MPI_COMM_WORLD
  }
}

DihuContext::DihuContext(int argc, char *argv[], bool doNotFinalizeMpi,
                         bool settingsFromFile)
  ...
  if (!initialized_) {
    ...
    // initial global variables that will be returned by the static methods
    // nRanksCommWorld() and ownRankNoCommWorld()
    ...
    // warn if OpenMPI 4 is used, remove this warning if you know if the bug has
    // been fixed (try running fibers_emg with at least 64 ranks)
    if (mpiVersion.find("Open MPI v4") != std::string::npos &&
        metaText().find("ipvs-epyc") != std::string::npos) {
      LOG(WARNING)
          << "Using MPI 4 on ipvs-epyc, which might cause problems. \n"
          << "In 2020 we found there is a bug in at least OpenMPI 4.0.4. "
             "Everything works fine with OpenMPI 3 (e.g. version 3.1.6). \n"
          << "So, either use OpenMPI 3 or you might check if the issues have "
             "already be fixed in newer versions of OpenMPI 4 or higher. \n"
    ...
    PetscErrorCode ierr = PetscOptionsHasName(
        NULL, NULL, "-pause", (PetscBool *)&enableDebuggingBarrier);
    CHKERRV(ierr);
    ...
    // register signal handler functions on various signals. This enforces
    // dumping of the log file even if the program crashes.
    ...
    // check if the first command line argument is *.py, only then it is treated
    // as config file
    ...
    if (argc > 1 && settingsFromFile) {
      ...
      if (firstArgument.rfind(".py") == firstArgument.size() - 3) {
        ...
        LOG(ERROR) << "First command line argument does not have suffix *.py, "
                      "not considering it as config file!";
      }
    }
      ...
      if (!loadPythonScriptFromFile(Control::settingsFileName)) {
        ...
        if (explicitConfigFileGiven) {
          LOG(FATAL) << "Could not load settings file \""
                     << Control::settingsFileName << "\".";
        } else {
          // look for any other settings.py files to output a message with a
          // suggestion
          ...
          if (ret == 0) {
            ...
            if (file.is_open()) {
              while (!file.eof()) {
                ...
                commandSuggestions << "  " << argv[0] << " " << line
                                   << std::endl;
              }
            }
          ...
          // if no settings file was given (default file "settings.py" was tried
          // but not successful)
          LOG(FATAL) << "No settings file was specified!" << std::endl
                     << "Usually you run the executable from within a "
                        "\"build_release\" or \"build_debug\" directory "
                     << "and the settings file is located one directory "
                        "higher. Try a command like the following:"
                     << std::endl
                     << std::endl
  ...
  // if this is the first constructed DihuContext object, create global objects
  // partition manager, mesh manager and solver manager
  ...
  if (!meshManager_) {
    ...
    mappingBetweenMeshesManager_ =
        std::make_shared<MappingBetweenMeshes::Manager>(pythonConfig_);
  }
  ...
  if (pythonConfig_.hasKey("regularization")) {
    if (pythonConfig_.isEmpty("regularization")) {
      ...
      std::array<double, 2> regularization =
          pythonConfig_.getOptionArray<double, 2>(
              "regularization", 0,
              PythonUtility::ValidityCriterion::NonNegative);
      MathUtility::INVERSE_REGULARIZATION_TOLERANCE = regularization[0];
      MathUtility::INVERSE_REGULARIZATION_EPSILON = regularization[1];
      LOG(INFO) << "Note, setting regularization to [tol,eps] = ["
                << regularization[0] << "," << regularization[1] << "].";
    }
  } else {
    LOG(INFO) << "Note, setting regularization to [tol,eps] = [1e-2,1e-1]. Add "
                 "`\"regularization\": None` in the settings to disable.";
  }
}

DihuContext::DihuContext(int argc, char *argv[], std::string pythonSettings,
                         bool doNotFinalizeMpi)
    : DihuContext(argc, argv, doNotFinalizeMpi, false) {
  nObjects_++;
  VLOG(1) << "DihuContext(d), nObjects = " << nObjects_;
  // This constructor is called when creating the context object from unit
  // tests.
  ...
  // they are first set to nullptr to allow break points on their destruction
...
PythonConfig DihuContext::getPythonConfig() const { return pythonConfig_; }
...
std::string DihuContext::metaText() {
  ...
  // metaTextStr << "current time: " << std::put_time(&tm, "%Y/%m/%d %H:%M:%S")
  // << ", hostname: ";
...
int DihuContext::ownRankNoCommWorld() { return ownRankNoCommWorld_; }
...
std::shared_ptr<SolverStructureVisualizer>
DihuContext::solverStructureVisualizer() {
  return solverStructureVisualizer_;
}

std::shared_ptr<GlobalConnectionsBySlotName>
DihuContext::globalConnectionsBySlotName() {
...
#ifdef HAVE_ADIOS
std::shared_ptr<adios2::ADIOS> DihuContext::adios() const { return adios_; }
#endif
...
DihuContext::logFormat_t DihuContext::logFormat() { return logFormat_; }
...
DihuContext DihuContext::operator[](std::string keyString) {
  ...
  DihuContext dihuContext(argc, argv, doNotFinalizeMpi_,
                          PythonConfig(pythonConfig_, keyString), rankSubset_);

  return dihuContext;
}

//! create a context object, like with the operator[] but with given config
DihuContext DihuContext::createSubContext(
    PythonConfig config, std::shared_ptr<Partition::RankSubset> rankSubset) {
  int argc = 0;
  char **argv = NULL;
...
DihuContext::~DihuContext() {
  ...
  if (nObjects_ == 0) {
    ...
    // This is only a problem when the code is tested with the GoogleTest
    // framework, because then we want to run multiple tests in one executable.
    // In this case, do not finalize MPI, but call MPI_Barrier instead which
    // also syncs the ranks.
  ...
  // VLOG(4) << "PY_CLEAR(PYTHONCONFIG_)";  // note: calling VLOG in a
  // destructor is critical and can segfault Py_CLEAR(pythonConfig_);
  ...
  // do not finalize Petsc because otherwise there can't be multiple DihuContext
  // objects for testing
  // PetscErrorCode ierr;
