...
template <typename Solver> void RepeatedCall<Solver>::initialize() {
  if (initialized_)
    return;
...
void RepeatedCall<Solver>::advanceTimeSpan(
  ...
  LOG(DEBUG) << "RepeatedCall::advanceTimeSpan, timeSpan=" << timeSpan
  ...
  for (; timeStepNo < this->numberTimeSteps_;) {
    ...
    {
      LOG(INFO) << "RepeatedCall, timestep " << timeStepNo << "/"
