...
class CrankNicolson : public TimeSteppingImplicit<DiscretizableInTimeType> {
public:
  //! constructor
...
protected:
  ...
  //! precomputes the system matrix A=(I - dt/2 * M^(-1)K) for the implicit
  //! euler scheme
  void setSystemMatrix(double timeStepWidth);

private:
  //! precomputes the integration matrix for the right hand side, (I + dt/2 *
  //! M^(-1)K)
  void setIntegrationMatrixRightHandSide();

  //! multiplies the right hand side with the integration matrix, (I + dt/2 *
  //! M^(-1)K)
  void evaluateTimesteppingRightHandSideImplicit(Vec &input, Vec &output,
                                                 int timeStepNo,
                                                 double currentTime);
};
