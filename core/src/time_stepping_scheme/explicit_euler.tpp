...
ExplicitEuler<DiscretizableInTime>::ExplicitEuler(DihuContext context)
  ...
  this->data_ = std::make_shared<
      Data::TimeStepping<typename DiscretizableInTime::FunctionSpace,
                         DiscretizableInTime::nComponents()>>(
      this->context_); // create data object for explicit euler
}
...
void ExplicitEuler<DiscretizableInTime>::advanceTimeSpan(
  ...
  LOG(DEBUG) << "ExplicitEuler::advanceTimeSpan, timeSpan=" << timeSpan
  ...
  for (; timeStepNo < this->numberTimeSteps_;) {
    ...
    {
      LOG(INFO) << "Explicit Euler, timestep " << timeStepNo << "/"
