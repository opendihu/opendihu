...
StreamlineTracer<DiscretizableInTimeType>::StreamlineTracer(DihuContext context)
    : context_(context["StreamlineTracer"]), problem_(context_),
  ...
  targetElementLength_ = specificSettings_.getOptionDouble(
      "targetElementLength", 0.0, PythonUtility::Positive);
  targetLength_ = specificSettings_.getOptionDouble("targetLength", 0.0,
                                                    PythonUtility::Positive);
  discardRelativeLength_ = specificSettings_.getOptionDouble(
      "discardRelativeLength", 0.0, PythonUtility::Positive);
  csvFilename_ = specificSettings_.getOptionString("csvFilename", "");
  csvFilenameBeforePostprocessing_ =
      specificSettings_.getOptionString("csvFilenameBeforePostprocessing", "");

  // get the first seed position from the list
  PyObject *pySeedPositions =
      specificSettings_.getOptionListBegin<PyObject *>("seedPoints");

  // loop over other entries of list
  for (; !specificSettings_.getOptionListEnd("seedPoints");
       specificSettings_.getOptionListNext<PyObject *>("seedPoints",
                                                       pySeedPositions)) {
    Vec3 seedPosition =
        PythonUtility::convertFromPython<Vec3>::get(pySeedPositions);
    seedPositions_.push_back(seedPosition);
  }
}

template <typename DiscretizableInTimeType>
void StreamlineTracer<DiscretizableInTimeType>::initialize() {
...
template <typename DiscretizableInTimeType>
void StreamlineTracer<DiscretizableInTimeType>::run() {
  initialize();
...
void StreamlineTracer<DiscretizableInTimeType>::traceStreamlines() {
  ...
  for (int seedPointNo = 0; seedPointNo < nSeedPoints; seedPointNo++) {
    ...
    streamlines[seedPointNo].insert(streamlines[seedPointNo].begin(),
                                    backwardPoints.rbegin(),
                                    backwardPoints.rend());
    streamlines[seedPointNo].insert(streamlines[seedPointNo].end(),
                                    startingPoint);
    streamlines[seedPointNo].insert(streamlines[seedPointNo].end(),
                                    forwardPoints.begin(), forwardPoints.end());

    LOG(DEBUG) << " seed point " << seedPointNo << ", "
               << streamlines[seedPointNo].size() << " points";
  }
  ...
  if (!csvFilenameBeforePostprocessing_.empty()) {
    ...
    if (!file.is_open())
      LOG(WARNING) << "Could not open \"" << csvFilenameBeforePostprocessing_
                   << "\" for writing";

    for (int streamlineNo = 0; streamlineNo != streamlines.size();
         streamlineNo++) {
      for (std::vector<Vec3>::const_iterator iter =
               streamlines[streamlineNo].begin();
           iter != streamlines[streamlineNo].end(); iter++) {
        Vec3 point = *iter;
  ...
  LOG(DEBUG) << "number streamlines after postprocessStreamlines: "
             << streamlines.size();
  ...
  for (int streamlineNo = 0; streamlineNo != streamlines.size();
       streamlineNo++) {
    LOG(DEBUG) << "seed point " << streamlineNo << ", number node positions: "
               << streamlines[streamlineNo].size();
    this->data_.createFiberMesh(streamlines[streamlineNo]);
  }
}

template <typename DiscretizableInTimeType>
void StreamlineTracer<DiscretizableInTimeType>::postprocessStreamlines(
  ...
  for (std::vector<std::vector<Vec3>>::iterator streamlinesIter =
           streamlines.begin();
       streamlinesIter != streamlines.end(); streamlinesIter++, i++) {
    lengths[i] = 0.0;
    ...
    for (std::vector<Vec3>::iterator pointsIter = streamlinesIter->begin();
         pointsIter != streamlinesIter->end(); pointsIter++, pointNo++) {
      if (!firstPoint) {
  ...
  LOG(INFO) << nEmptyStreamlines << " of " << streamlines.size()
            << " streamlines are invalid.";
  ...
  LOG(INFO) << "The median length of the streamlines is " << medianLength
            << ", the maximum length of the " << lengthsSorted.size()
            << " streamlines is " << maximumLength << ".";
  ...
  if (discardRelativeLength_ != 0.0) {
    ...
    for (std::vector<std::vector<Vec3>>::iterator streamlinesIter =
      ...
      if (lengths[i] < discardRelativeLength_ * medianLength) {
        LOG(INFO) << "Discarding streamline no. " << i << " with length "
                  << lengths[i] << " (Threshold "
                  << discardRelativeLength_ * medianLength << ").";
        streamlinesIter->clear();
      }
    }

    auto lastValidStreamline = std::remove_if(
        streamlines.begin(), streamlines.end(),
        [](const std::vector<Vec3> &a) -> bool { return a.empty(); });

    // remove previously cleared streamlines
    streamlines.erase(lastValidStreamline, streamlines.end());
  }
  ...
  if (targetElementLength_ != 0.0 &&
      targetElementLength_ != this->lineStepWidth_) {
    // loop over streamlines
    for (int i = 0; i < streamlines.size(); i++) {
      ...
      if (currentStreamline.empty()) {
        ...
        int presumedLength =
            int(currentStreamline.size() * targetElementLength_ /
                    this->lineStepWidth_ +
                10);
        newStreamline.reserve(presumedLength);

        VLOG(1) << "streamline no " << i << ", reserve length "
                << presumedLength;
        VLOG(1) << "targetElementLength_: " << targetElementLength_
                << ", scalingFactor: " << scalingFactor;

        Vec3 lastPoint = currentStreamline.front() * scalingFactor;
        Vec3 previousStreamlinePoint =
            lastPoint; // last point that was inserted into the new streamline
        ...
        // loop over points of streamline
        for (std::vector<Vec3>::iterator pointsIter = currentStreamline.begin();
             pointsIter != currentStreamline.end(); pointsIter++) {
          if (!firstPoint) {
            ...
            VLOG(1) << "old streamline interval " << lastPoint << " - "
                    << currentPoint << ", new lentgh: " << length
                    << " (targetElementLength=" << targetElementLength_ << ")";

            if (length > targetElementLength_) {
              double alpha = targetElementLength_ / length;
              Vec3 point =
                  (1. - alpha) * previousStreamlinePoint + alpha * currentPoint;

              VLOG(1) << "  length is too big, alpha=" << alpha
                      << ", take algebraic point " << point
                      << ", distance to previous point "
                      << previousStreamlinePoint << ": "
                      << MathUtility::length(point - previousStreamlinePoint);
        ...
        LOG(DEBUG) << "Scaled streamline by factor " << scalingFactor
                   << ", resampled from lineStepWidth " << this->lineStepWidth_
                   << " to targetElementLength " << targetElementLength_
                   << ", now it has " << newStreamline.size()
                   << " points, length " << lengths[i] * scalingFactor;
        streamlines[i] = newStreamline;
      }
    }
  }

  LOG(DEBUG) << "Number of streamlines after resampling: "
             << streamlines.size();
}
