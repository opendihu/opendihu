...
void ParallelFiberEstimation<BasisFunctionType>::determineLevel() {
...
template <typename BasisFunctionType>
bool ParallelFiberEstimation<BasisFunctionType>::checkTraceFinalFibers() {
  determineLevel();

  int nRanksAvailable = this->context_.nRanksCommWorld();

  // decide if the algorithm should no more refine the subdomains but trace the
  // final fibers
  ...
  if (!traceFinalFibers) {
    if (nRanksAvailable < currentRankSubset_->size() * 8) {
      LOG(WARNING) << "Cannot run algorithm until level " << maxLevel_
                   << ", currently at level " << level_
                   << ", total number of ranks is " << nRanksAvailable
                   << ", number needed for next level would be "
                   << currentRankSubset_->size() * 8 << "." << std::endl
...
template <typename BasisFunctionType>
void ParallelFiberEstimation<BasisFunctionType>::traceResultFibers(
    double streamlineDirection, int seedPointsZIndex,
    const std::vector<Vec3> &nodePositions,
    const std::array<std::array<std::vector<std::vector<Vec3>>, 4>, 8>
        &boundaryPointsSubdomain,
    bool finalFile) {
  LOG(DEBUG) << "traceResultFibers, nBoundaryPointsXNew_: "
             << nBoundaryPointsXNew_
             << ", nFineGridFibers: " << nFineGridFibers_;

  // grid, x = key fibers which are traced, o = interpolated fibers for
  // nFineGridFibers_ = 1
  //  _ _ _ _
  // |x o x o x|
  // |o o o o o|
  ...
  // total number of fibers per coordinate direction, including the ones on the
  // left and right boundarys
  int nFibersX =
      (nBoundaryPointsXNew_ - 1) * nFineGridFibers_ + nBoundaryPointsXNew_;
  // int nFibersX = nBoundaryPointsXNew_*(nFineGridFibers_+1) -
  // nFineGridFibers_;
  ...
  for (int i = 0; i < subdomainIndices.size(); i++) {
    ...
    for (int pointIndex = 0; pointIndex != nBoundaryPointsX_ - 1;
         pointIndex++, fibersPointIndex += pointIndexStride) {
      for (int zLevelIndex = 0; zLevelIndex != nBoundaryPointsZ_;
           zLevelIndex++) {
        fibers[fibersPointIndex][zLevelIndex] =
            boundaryPointsSubdomain[subdomainIndex][face][zLevelIndex]
                                   [pointIndex];
        fibers[fibersPointIndex][zLevelIndex + (nBoundaryPointsZ_ - 1)] =
            boundaryPointsSubdomain[subdomainIndex + 4][face][zLevelIndex]
                                   [pointIndex];
      }
    }
  }
    ...
    for (int pointIndex = 0; pointIndex != nBoundaryPointsX_ - 1;
      ...
      for (int zLevelIndex = 0; zLevelIndex != nBoundaryPointsZ_;
        ...
        fibers[fibersPointIndex][zLevelIndex + (nBoundaryPointsZ_ - 1)] =
            boundaryPointsSubdomain[subdomainIndex + 4][face][zLevelIndex]
                                   [pointIndex];
        VLOG(1) << "set value at left boundary, pointIndexStride: "
                << pointIndexStride << ", fibers[" << fibersPointIndex << "][z"
                << zLevelIndex
                << "] =" << fibers[fibersPointIndex][zLevelIndex];
        VLOG(1)
            << "set value at left boundary, pointIndexStride: "
            << pointIndexStride << ", fibers[" << fibersPointIndex << "][z"
  ...
  for (int i = 0; i < subdomainIndices.size(); i++) {
    const int subdomainIndex = subdomainIndices[i];
    for (int pointIndex = 0; pointIndex != nBoundaryPointsX_ - (i == 1 ? 0 : 1);
         pointIndex++, fibersPointIndex += pointIndexStride) {
      for (int zLevelIndex = 0; zLevelIndex != nBoundaryPointsZ_;
           zLevelIndex++) {
        fibers[fibersPointIndex][zLevelIndex] =
        ...
        fibers[fibersPointIndex][zLevelIndex + (nBoundaryPointsZ_ - 1)] =
            boundaryPointsSubdomain[subdomainIndex + 4][face][zLevelIndex]
                                   [pointIndex];
        VLOG(1) << "set value at right boundary, pointIndexStride: "
                << pointIndexStride << ", fibers[" << fibersPointIndex << "][z"
                << zLevelIndex
                << "] =" << fibers[fibersPointIndex][zLevelIndex];
        VLOG(1)
            << "set value at right boundary, pointIndexStride: "
            << pointIndexStride << ", fibers[" << fibersPointIndex << "][z"
            << zLevelIndex + (nBoundaryPointsZ_ - 1) << "] ="
            << fibers[fibersPointIndex][zLevelIndex + (nBoundaryPointsZ_ - 1)];
      }
  ...
  PyObject_CallFunction(
      functionOutputStreamlines_, "s i i O f", "12_final_boundary",
  ...
  // nodePositions are the node positions of the (not refined) 3D mesh,
  // dimension nBoundaryPointsX_*nBoundaryPointsX_*nBoundaryPointsZ_,
  // corresponding seedPointsZIndex

  // determine own seed points
  std::vector<Vec3> seedPoints(nBoundaryPointsXNew_ * nBoundaryPointsXNew_,
                               Vec3({0.0, 0.0, 0.0}));
  ...
  for (int j = 1; j < nBoundaryPointsXNew_ - 1; j++) {
    for (int i = 1; i < nBoundaryPointsXNew_ - 1; i++) {
      ...
      Vec3 seedPoint = nodePositions[seedPointsZIndex * nBoundaryPointsXNew_ *
                                         nBoundaryPointsXNew_ +
                                     j * nBoundaryPointsXNew_ + i];
      seedPoints[j * nBoundaryPointsXNew_ + i] = seedPoint;
    }
  }

  // seedPointsZIndex is either 0 or nBoundaryPointsZ_-1, change to
  // nBoundaryPointsXNew_-1 in the second case
  ...
  for (int i = 0; i < nBoundaryPointsXNew_;
       i++, fibersPointIndex += pointIndexStride) {
    seedPoints[0 * nBoundaryPointsXNew_ + i] =
        fibers[fibersPointIndex][seedPointsZIndex];
  }
  ...
  for (int i = 0; i < nBoundaryPointsXNew_;
       i++, fibersPointIndex += pointIndexStride) {
    seedPoints[(nBoundaryPointsXNew_ - 1) * nBoundaryPointsXNew_ + i] =
        fibers[fibersPointIndex][seedPointsZIndex];
  }
  ...
  for (int i = 0; i < nBoundaryPointsXNew_;
       i++, fibersPointIndex += pointIndexStride) {
    seedPoints[i * nBoundaryPointsXNew_ + 0] =
        fibers[fibersPointIndex][seedPointsZIndex];
  }
  ...
  for (int i = 0; i < nBoundaryPointsXNew_;
       i++, fibersPointIndex += pointIndexStride) {
    seedPoints[i * nBoundaryPointsXNew_ + (nBoundaryPointsXNew_ - 1)] =
        fibers[fibersPointIndex][seedPointsZIndex];
  }

  LOG(DEBUG) << "determine " << nBoundaryPointsXNew_ << "x"
             << nBoundaryPointsXNew_ << " seed points: " << seedPoints.size();
  ...
  // The overall picture is that global streamlines begin at the center (at
  // rankZNo/2). Rank int(nRanksZ/2) send the initial seed points to the rank
  // below (int(nRanksZ/2)-1) Then every rank traces its streamlines and sends
  // the end points as new seed points to the next rank (lower or upper
  // neighbour, depending on streamlineDirection)

  // determine if previously set seedPoints are used or if they are received
  // from neighbouring rank, receive seed points or send them to lower
  // neighbour, if own rank is int(nRanksZ/2)
  exchangeSeedPointsBeforeTracingKeyFibers(
      nRanksZ, rankZNo, streamlineDirectionUpwards, seedPoints);

  LOG(DEBUG) << "number of seed points after exchange with neighbours: "
             << seedPoints.size();
  ...
  // get seed points for interior fibers and trace fibers, the boundary fibers
  // are already traced
  for (int j = 1; j < nBoundaryPointsXNew_ - 1; j++) {
    for (int i = 1; i < nBoundaryPointsXNew_ - 1; i++) {
      // determine seed point
      ...
      if (nRanksZ == 1) {
        ...
        // copy collected points to result vector, note avoiding this additional
        // copy-step is not really possible, since it would require a push_front
        // which is only efficient with lists, but we need a vector here
        streamlinePoints.insert(streamlinePoints.begin(),
                                backwardPoints.rbegin(), backwardPoints.rend());
        streamlinePoints.insert(streamlinePoints.end(), seedPoint);
        streamlinePoints.insert(streamlinePoints.end(), forwardPoints.begin(),
                                forwardPoints.end());
      ...
      // sample the streamline at equidistant z levels and store points in
      // fibers vector
      int fibersPointIndex =
          j * (nFineGridFibers_ + 1) * nFibersX + i * (nFineGridFibers_ + 1);
      sampleStreamlineAtEquidistantZPoints(
          streamlinePoints, seedPoint, bottomZClip, topZClip,
          fibers[fibersPointIndex], fibersPointIndex * 1000,
          rawSampledStreamlinesForDebugging); // last parameter is value for
                                              // debugging output

      LOG(DEBUG) << " (" << i << "," << j << ") seedPoint " << seedPoint
                 << ", sampled streamline (" << i << "," << j
                 << ") at equidistant z points";
    }
  }

  // check which of the key fibers are invalid
  std::vector<std::vector<bool>> keyFiberIsValid(
      nBoundaryPointsXNew_,
      std::vector<bool>(
          nBoundaryPointsXNew_,
          true)); // this only stores for key fibers if they are valid,
                  // dimensions are nBoundaryPointsXNew_ x nBoundaryPointsXNew_
  ...
  for (int j = 0; j < nBoundaryPointsXNew_; j++) {
    for (int i = 0; i < nBoundaryPointsXNew_; i++) {
      ...
      if (fibers[pointIndex].size() != nBoundaryPointsZNew_) {
        LOG(DEBUG) << "fiber[" << pointIndex << "] (" << i << "," << j
                   << ") of (" << nBoundaryPointsXNew_ << ","
                   << nBoundaryPointsXNew_ << ")"
                   << " is invalid: not long enough, size: "
                   << fibers[pointIndex].size()
                   << " != " << nBoundaryPointsZNew_;
        logMessage << "    fiber[" << pointIndex << "] (" << i << "," << j
                   << ") of (" << nBoundaryPointsXNew_ << ","
                   << nBoundaryPointsXNew_ << ")"
                   << " is not long enough (size: " << fibers[pointIndex].size()
                   << " != " << nBoundaryPointsZNew_ << ")" << std::endl;
        isValid = false;
        ...
        isValid = false;
      } else if (MathUtility::norm<3>(
                     fibers[pointIndex][nBoundaryPointsZNew_ / 2]) < 1e-4) {
        LOG(DEBUG) << "fiber[" << pointIndex << "] (" << i << "," << j
                   << ") of (" << nBoundaryPointsXNew_ << ","
                   << nBoundaryPointsXNew_ << ")"
  ...
  LOG(INFO) << "key fibers, number: " << MathUtility::sqr(nBoundaryPointsXNew_)
            << ", valid: " << nValid << ", invalid: " << nInvalid;

  // fix the invalid key fibers in the interior by interpolating from the
  // neighbouring fibers
  ...
  exchangeSeedPointsAfterTracingKeyFibers(nRanksZ, rankZNo,
                                          streamlineDirectionUpwards, nFibersX,
                                          seedPoints, fibers);
  ...
  MPI_Reduce(&nValid, &nValidGlobal, 1, MPI_INT, MPI_SUM, 0,
             currentRankSubset_->mpiCommunicator());
  MPI_Reduce(&nInvalid, &nInvalidGlobal, 1, MPI_INT, MPI_SUM, 0,
             currentRankSubset_->mpiCommunicator());
  MPI_Reduce(&nFibersFixed, &nFibersFixedGlobal, 1, MPI_INT, MPI_SUM, 0,
             currentRankSubset_->mpiCommunicator());

  if (currentRankSubset_->ownRankNo() == 0) {
    LOG(INFO) << "total number of key fibers, initially valid: " << nValidGlobal
              << ", initially invalid: " << nInvalidGlobal
              << ", fixed: " << nFibersFixedGlobal
              << ", finally invalid: " << nInvalidGlobal - nFibersFixedGlobal
              << ", finally valid: " << nValidGlobal + nFibersFixedGlobal;
  }
...
#ifdef STL_OUTPUT
  PyObject_CallFunction(
      functionOutputPoints_, "s i i O f", "13_final_seed_points",
      currentRankSubset_->ownRankNo(), level_,
...
#ifndef NDEBUG
  PyObject_CallFunction(
      functionOutputStreamlines_, "s i i O f", "13_final_interior",
      currentRankSubset_->ownRankNo(), level_,
      PythonUtility::convertToPython<std::vector<std::vector<Vec3>>>::get(
          fibers),
      0.1);
  ...
  LOG(DEBUG) << "interpolate fibers in between key fibers, nFineGridFibers_: "
             << nFineGridFibers_;
  ...
  for (int j = 0; j < nBoundaryPointsXNew_ - 1; j++) {
    for (int i = 0; i < nBoundaryPointsXNew_ - 1; i++) {
      ...
      // |o o
      // |x_o_x
      int keyFiberPointIndex0 =
          j * (nFineGridFibers_ + 1) * nFibersX + i * (nFineGridFibers_ + 1);
      int keyFiberPointIndex1 = j * (nFineGridFibers_ + 1) * nFibersX +
                                (i + 1) * (nFineGridFibers_ + 1);
      int keyFiberPointIndex2 = (j + 1) * (nFineGridFibers_ + 1) * nFibersX +
                                i * (nFineGridFibers_ + 1);
      int keyFiberPointIndex3 = (j + 1) * (nFineGridFibers_ + 1) * nFibersX +
                                (i + 1) * (nFineGridFibers_ + 1);

      // determine if key fibers are all valid
      bool keyFibersInvalid = false;
      if (fibers[keyFiberPointIndex0].size() != nBoundaryPointsZNew_ ||
          fibers[keyFiberPointIndex1].size() != nBoundaryPointsZNew_ ||
          fibers[keyFiberPointIndex2].size() != nBoundaryPointsZNew_ ||
          fibers[keyFiberPointIndex3].size() != nBoundaryPointsZNew_) {
        LOG(DEBUG) << "fibers at (" << i << "," << j
                   << ") are incomplete, sizes: "
                   << fibers[keyFiberPointIndex0].size() << ","
      ...
      if (!keyFibersInvalid) {
        if (MathUtility::norm<3>(
                fibers[keyFiberPointIndex0][nBoundaryPointsZNew_ / 2]) < 1e-4 ||
            MathUtility::norm<3>(
                fibers[keyFiberPointIndex1][nBoundaryPointsZNew_ / 2]) < 1e-4 ||
            MathUtility::norm<3>(
                fibers[keyFiberPointIndex2][nBoundaryPointsZNew_ / 2]) < 1e-4 ||
            MathUtility::norm<3>(
                fibers[keyFiberPointIndex3][nBoundaryPointsZNew_ / 2]) < 1e-4) {
          LOG(DEBUG) << "fibers at (" << i << "," << j
      ...
      if (!keyFibersInvalid) {
        VLOG(1) << "interpolate fine fibers at (" << i << "," << j
                << "), index " << keyFiberPointIndex0;
      }
      ...
      for (int fineGridJ = 0; fineGridJ < nFineGridFibers_ + 1; fineGridJ++) {
        for (int fineGridI = 0; fineGridI < nFineGridFibers_ + 1; fineGridI++) {
          ...
          int fibersPointIndex =
              keyFiberPointIndex0 + fineGridJ * nFibersX + fineGridI;

          // if key fibers are invalid, set interpolated fibers to 0
          if (keyFibersInvalid) {
            for (int zLevelIndex = 0; zLevelIndex != nBoundaryPointsZNew_;
                 zLevelIndex++) {
              fibers[fibersPointIndex][zLevelIndex] = Vec3({0.0, 0.0, 0.0});
            }
          } else {
            // interpolate fibers using key fibers
            Vec2 xi({fineGridI / (nFineGridFibers_ + 1.0),
                     fineGridJ / (nFineGridFibers_ + 1.0)});

            VLOG(1) << "  fine fiber (" << fineGridI << "," << fineGridJ
                    << ") index " << fibersPointIndex << ", xi: " << xi;
            ...
            for (int zLevelIndex = 0; zLevelIndex != nBoundaryPointsZNew_;
              ...
              fibers[fibersPointIndex][zLevelIndex] =
                  (1.0 - xi[0]) * (1.0 - xi[1]) *
                      fibers[keyFiberPointIndex0][zLevelIndex] +
                  xi[0] * (1.0 - xi[1]) *
                      fibers[keyFiberPointIndex1][zLevelIndex] +
                  (1.0 - xi[0]) * xi[1] *
                      fibers[keyFiberPointIndex2][zLevelIndex] +
  ...
  PythonUtility::checkForError();
#endif
  LOG(DEBUG) << "invalid fibers: " << nFibersNotInterpolated
             << ", valid fibers: " << nFibers - nFibersNotInterpolated;

  int nFibersGlobal = 0;
  int nFibersNotInterpolatedGlobal = 0;
  MPI_Reduce(&nFibers, &nFibersGlobal, 1, MPI_INT, MPI_SUM, 0,
             currentRankSubset_->mpiCommunicator());
  MPI_Reduce(&nFibersNotInterpolated, &nFibersNotInterpolatedGlobal, 1, MPI_INT,
             MPI_SUM, 0, currentRankSubset_->mpiCommunicator());

  if (currentRankSubset_->ownRankNo() == 0) {
    LOG(INFO) << "total number of fibers, valid: "
              << nFibersGlobal - nFibersNotInterpolatedGlobal
              << ", invalid: " << nFibersNotInterpolatedGlobal;
  }
  ...
  if (finalFile) {
    ...
    filenameStr << "out/level_" << level_ << "/result_0x0_level_" << level_
                << ".bin";
  }
  ...
  // the (x+,x-,y+,y-) boundary fibers of the global boundarys are not written
  // to the file because they were not traced but estimated from the boundary
  // mesh which may be of bad quality

  filename = filenameStr.str();
  filenameStr.str("");
  if (filename.find(".bin") != std::string::npos) {
    filenameStr << filename.substr(0, filename.find(".bin")) << ".no_boundary"
                << filename.substr(filename.find(".bin"));
  } else {
  ...
  for (std::pair<int, int> nFibersFixedOnLevel : nFibersFixed_) {
    if (nFibersFixedOnLevel.second != 0)
      LOG(INFO) << nFibersFixedOnLevel.second
                << " fibers were invalid and then fixed on recursion level "
                << nFibersFixedOnLevel.first;
  }
}

template <typename BasisFunctionType>
void ParallelFiberEstimation<BasisFunctionType>::writeToFile(
    std::string filename, std::vector<std::vector<Vec3>> &fibers, int nFibersX,
    bool withBoundaryLayer) {
  ...
  int nPointsWholeFiber =
      meshPartition_->nRanks(2) * (nBoundaryPointsZNew_ - 1) + 1;

  LOG(DEBUG) << "open file \"" << filename << "\".";
  // open file
  MPI_File fileHandle;
  MPIUtility::handleReturnValue(
      MPI_File_open(currentRankSubset_->mpiCommunicator(), filename.c_str(),
  ...
  const int nParameters =
      10; // if I change this, also change the constant header length further
          // down in resampleFibersInFile
  if (ownRankNo == 0) {
    std::string writeBuffer(
        "opendihu binary fibers file     "); // 32 characters
    ...
    LOG(DEBUG) << "nFibersTotal: " << nFibersTotal
               << ", nPointsWholeFiber: " << nPointsWholeFiber;

    writeBuffer += std::string(c, nParameters * sizeof(int32_t));

    MPI_Status status;
    MPIUtility::handleReturnValue(
        MPI_File_write(fileHandle, writeBuffer.c_str(), writeBuffer.length(),
                       MPI_BYTE, &status),
        "MPI_File_write", &status);
  ...
  // write fibers
  LOG(DEBUG) << "write fibers to \"" << filename << "\", nFibersX: " << nFibersX
             << ", nRanks: (" << meshPartition_->nRanks(0) << ","
             << meshPartition_->nRanks(1) << "," << meshPartition_->nRanks(2)
             << "), withBoundaryLayer: " << withBoundaryLayer;
  ...
  for (int j = 0; j < nFibersXToWrite; j++) {
    ...
    if (!withBoundaryLayer && j == 0 &&
        meshPartition_->ownRankPartitioningIndex(1) == 0)
      continue;

    for (int i = 0; i < nFibersXToWrite; i++) {
      // only consider last fiber in right column
      if (!withBoundaryLayer && i == 0 &&
          meshPartition_->ownRankPartitioningIndex(0) == 0)
        continue;
      ...
      // fill values vector
      for (int zLevelIndex = 0; zLevelIndex < nPointsCurrentFiber;
           zLevelIndex++) {
        ...
        if (fibers[fibersPointIndex].size() < nPointsCurrentFiber) {
          ...
          fibers[fibersPointIndex].resize(nPointsCurrentFiber,
                                          Vec3({0.0, 0.0, 0.0}));
        }
        if (fibers[fibersPointIndex][int(nPointsCurrentFiber / 2)][0] == 0.0 &&
            fibers[fibersPointIndex][int(nPointsCurrentFiber / 2)][1] == 0.0 &&
        ...
        for (int k = 0; k < 3; k++) {
          values[zLevelIndex * 3 + k] =
              fibers[fibersPointIndex][zLevelIndex][k];
        }
      }
      char *writeBuffer = reinterpret_cast<char *>(values.data());

      // compute offset in file
      int nFibersPreviousRanks0 =
          meshPartition_->ownRankPartitioningIndex(0) * (nFibersX - 1);
      int nFibersPreviousRanks1 =
          meshPartition_->ownRankPartitioningIndex(1) * (nFibersX - 1);
      ...
      int pointOffset = fiberIndex * nPointsWholeFiber +
                        meshPartition_->ownRankPartitioningIndex(2) *
                            (nBoundaryPointsZNew_ - 1);

      int offset = headerOffset + pointOffset * 3 * sizeof(double);

      VLOG(1) << "write fiber (" << i << "," << j << "), global ("
              << fiberIndex0 << "," << fiberIndex1 << "), global index "
              << fiberIndex << ", nValues: " << nValues
              << ", nBytes: " << nBytes << ", pointOffset: " << pointOffset
              << " (" << nPointsWholeFiber
              << " points per fiber), headerOffset: " << headerOffset
              << ", offset: " << offset;

      // write to file
      MPI_Status status;
      MPIUtility::handleReturnValue(MPI_File_write_at(fileHandle, offset,
                                                      writeBuffer, nBytes,
                                                      MPI_BYTE, &status),
                                    "MPI_File_write_at", &status);
    }
  }

  MPIUtility::handleReturnValue(MPI_File_close(&fileHandle), "MPI_File_close");

  LOG(DEBUG) << nFibersRow0 << "x" << nFibersRow0 << " fibers with "
             << nPointsWholeFiber << " points per fiber written to file \""
             << filename << "\".";
  ...
  if (currentRankSubset_->ownRankNo() == 0) {
    ...
    if (nPointsWholeFiber != nNodesPerFiber_ ||
        finalBottomZClip_ != bottomZClip_ || finalTopZClip_ != topZClip_) {
