...
void ParallelFiberEstimation<BasisFunctionType>::resampleFibersInFile(
    int nPointsPerFiber, std::string filename) {
  // create a new file with all the fibers from the old file but resampled such
  // that they have nNodesPerFiber_ nodes
  LOG(DEBUG) << "resample fibers in file, this is completely serial, "
  ...
  if (nPointsPerFiber != bufferNPointsPerFiber.i) {
    LOG(FATAL) << "Invalid number of points per fiber: file \"" << filename
               << "\" contains " << bufferNPointsPerFiber.i << ", but "
               << nPointsPerFiber << " are required.";
  }
  file.close();
  ...
  // if filename does not change, i.e. filename does not depend on number of
  // fibers
  if (filename == filenameExistingFile) {
    ...
    ret++;
    LOG(INFO) << nFibersX << "x" << nFibersX << " fibers with "
              << nPointsPerFiber << " points per fiber written to file \""
              << filenameExistingFile << "\".";
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
  }

  // open existing file to read
  std::ifstream fileOld(filenameExistingFile.c_str(),
  ...
  double oldZIncrement =
      double(topZClip_ - bottomZClip_) / (nPointsPerFiber - 1);
  ...
  for (int fiberIndex = 0; fiberIndex != nFibers; fiberIndex++) {
    ...
    int oldZIndexPrevious0 =
        -1; /// z index of the point that is currently loaded in previousPoint
    int oldZIndexNext0 =
        -1; /// z index of the point that is currently loaded in nextPoint
    ...
    for (int zIndex = 0; zIndex < nNodesPerFiber_; zIndex++) {
      ...
      double currentZ = finalBottomZClip_ +
                        zIndex * double(finalTopZClip_ - finalBottomZClip_) /
                            (nNodesPerFiber_ - 1);

      // LOG(DEBUG) << "clip: " << finalBottomZClip_ << "," << finalTopZClip_ <<
      // " zIndex: " << zIndex << "/" << nNodesPerFiber_ << ", currentZ: " <<
      // currentZ;

      // compute z indices of the point in the old fiber between which the new
      // point will be
      ...
      if (oldZIndexPrevious != oldZIndexPrevious0) {
        fileOld.seekg(32 + headerLength + fiberIndex * fiberDataSize +
                      oldZIndexPrevious * 3 * sizeof(double));
      ...
      double alpha =
          (currentZ - (bottomZClip_ + oldZIndexPrevious * oldZIncrement)) /
          oldZIncrement;
      Vec3 newPoint = (1. - alpha) * previousPoint + alpha * nextPoint;

      // if (fiberIndex < 10 || fiberIndex > nFibers-10)
      if (false) {
        LOG(DEBUG) << "f" << fiberIndex << " z" << zIndex << "(" << currentZ
                   << ") indices " << oldZIndexPrevious << "," << oldZIndexNext
                   << ", points " << previousPoint << nextPoint
                   << ", alpha: " << alpha << ", newPoint: " << newPoint;
      }
  ...
  LOG(INFO) << nFibersX << "x" << nFibersX << " fibers with " << nNodesPerFiber_
            << " points per fiber written to file \"" << filename << "\"."
            << std::endl;
  LOG(INFO) << "Use the following command to convert the file to various "
               "formats for inspection:\n  examine_bin_fibers.py \""
            << filenameExistingFile << "\"" << std::endl;
}

template <typename BasisFunctionType>
void ParallelFiberEstimation<BasisFunctionType>::stretchMeshAtCornersInFile(
    std::string filename) {
  ...
  for (int zIndex = 0; zIndex < nPointsPerFiber; zIndex++) {
    ...
    for (int fiberIndexJ = 0; fiberIndexJ != nFibersX; fiberIndexJ++) {
      for (int fiberIndexI = 0; fiberIndexI != nFibersX; fiberIndexI++) {
        ...
        // the following computations calculate two values: a in [0,1] and
        // factor around 1 a specifies how much the factor is used and how much
        // the normal radius is used, a = 0 means normal radius, i.e., no mesh
        // change, a=1 means full changed mesh

        double h = 0.9; // parameter, how far the area of mesh changes reaches
                        // in circumferential direction, 0<h<=1, smaller is more
                        // concentrated around corners
        double a = (1 - sin(2 * M_PI * ((a1 - 0.5) / h + 0.5) + M_PI_2)) /
                   2; // 0 -> 0  0.5 -> 1  1 -> 0
        ...
        double maxR =
            fabs((nFibersX - 1 - indexCenter) /
                 cos(indexPhi)); // cos(phi) = a/h = x/maxR => maxR = x/cos(phi)
        if ((indexPhi >= M_PI_4 && indexPhi < 3 * M_PI_4) ||
            (indexPhi <= -M_PI_4 && indexPhi > -3 * M_PI_4))
          maxR = fabs(
              (nFibersX - 1 - indexCenter) /
        ...
        double s = 0.95; // [1-s, 1]    // parameter 0 < s << 1, how far the
                         // area of mesh changes reaches in radial direction,
                         // smaller is more concentrated around the corners
        double alpha =
            0.7; // 0 << alpha < 1, parameter how much the mesh is pulled
                 // towards the center, lower value = more, 1 = not at all
        ...
        if (relativeR > 1 - s) {
          ...
          double x = relativeR;
          scalingFunction2 =
              4 * alpha - 8 * alpha / s + 4 * alpha / s2 +
              x * (-4 * alpha + 16 * alpha / s - 12 * alpha / s2 + 5 - 16 / s +
                   12 / s2) -
              4 + 8 / s + std::pow(x, 3) * (4 - 4 * alpha) / s2 +
              std::pow(x, 2) * (-8 * alpha * s + 12 * alpha + 8 * s - 12) / s2 -
              4 / s2;
        }
        ...
        if (fiberIndexI == 0 || fiberIndexI == nFibersX - 1 ||
            fiberIndexJ == 0 || fiberIndexJ == nFibersX - 1) {
          a = 0;
          factor = 1;
        }
