...
void ParallelFiberEstimation<
    BasisFunctionType>::interpolateFineFibersFromFile() {
  // open existing file to read
  bool waitIfFileGetsBig =
      specificSettings_.getOptionBool("waitIfFileGetsBig", true);
  ...
  std::ifstream fileOld(resultFilename_.c_str(),
  ...
  LOG(DEBUG) << "read header: nFibers: " << nFibersOld
             << ", nFibersX: " << nFibersOldX
  ...
  std::ofstream fileNew(newFilename.str().c_str(),
  ...
  LOG(INFO) << "\"" << resultFilename_ << "\" -> \"" << newFilename.str()
            << "\"";
  LOG(INFO) << "interpolate from " << nFibersOldX << " x " << nFibersOldX
            << " = " << nFibersOld << " to " << nFibersNewX << " x "
            << nFibersNewX << " = " << nFibersNew
            << ", (nFineGridFibers: " << nFineGridFibers_ << ")";

  const long long fiberDataSize = nPointsPerFiber * 3 * sizeof(double);
  long long nBytes =
      (32 + headerLength) + (long long)(nFibersNew)*fiberDataSize;
  long long nGibiBytes =
      nBytes / (long long)(1024) / (long long)(1024) / (long long)(1024);
  ...
  if (int((fileOldSize - (32 + headerLength)) / fiberDataSize) != nFibersOld) {
    LOG(DEBUG) << "File \"" << resultFilename_ << "\" states to have "
               << nFibersOld << " fibers in header, but actually has "
               << int((fileOldSize - (32 + headerLength)) / fiberDataSize)
               << " fibers!";
  }
  ...
  for (int fiberIndexY = 0; fiberIndexY != nFibersNewX; fiberIndexY++) {
    for (int fiberIndexX = 0; fiberIndexX != nFibersNewX; fiberIndexX++) {
      ...
      VLOG(1) << "fiber " << fiberIndexX << "," << fiberIndexY
              << ", oldFiberIndex: " << oldFiberIndexX << "," << oldFiberIndexY;

      // if fiber is key fiber
      if (fiberIndexX % (nFineGridFibers_ + 1) == 0 &&
          fiberIndexY % (nFineGridFibers_ + 1) == 0) {
        // copy fiber data from old file
        int oldFiberIndex = oldFiberIndexY * nFibersOldX + oldFiberIndexX;

        // assert that file is long enough
        if (32 + headerLength + (oldFiberIndex + 1) * fiberDataSize >
            fileOldSize) {
          LOG(ERROR) << "Read past end of file, oldFiberIndex: "
                     << oldFiberIndex;
        }
        assert(32 + headerLength + (oldFiberIndex + 1) * fiberDataSize <=
               fileOldSize);
        ...
        */
        ...
        for (int i = 0; i < 4; i++) {
          ...
          std::size_t oldFiberEndPosInFile =
              32 + headerLength + (oldFiberIndex[i] + 1) * fiberDataSize;
        ...
        double alpha0 =
            double(fiberIndexX - oldFiberIndexX * (nFineGridFibers_ + 1)) /
            (nFineGridFibers_ + 1);
        double alpha1 =
            double(fiberIndexY - oldFiberIndexY * (nFineGridFibers_ + 1)) /
            (nFineGridFibers_ + 1);
        ...
        VLOG(1) << "(" << fiberIndexX << "," << fiberIndexY
                << ") alpha: " << alpha0 << "," << alpha1;
        ...
        for (int zPointIndex = 0; zPointIndex < nPointsPerFiber;
          ...
          for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 3; j++) {
              double *memoryLocation = reinterpret_cast<double *>(
                  &buffer[i][(zPointIndex * 3 + j) * sizeof(double)]);
              point[i][j] = *memoryLocation;
            }
          }

          Vec3 resultPoint = (1. - alpha0) * (1. - alpha1) * point[0] +
