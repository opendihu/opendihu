...
bool ParallelFiberEstimation<BasisFunctionType>::adjustFilename(
    std::string &filename, int nFibersX) {
  ...
  LOG(DEBUG) << "adjust filename [" << filename << "], nFibersX: " << nFibersX
             << ", path: [" << path << "], fileBase: [" << fileBase << "]";
  ...
  if (filenameHasXFormat) {
    ...
    newFilename << path << fileBase.substr(0, xNumbersBeginPos) << nFibersX
                << "x" << nFibersX << fileBase.substr(suffixPos);
