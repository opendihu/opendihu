...
void ParallelFiberEstimation<BasisFunctionType>::
    createNeumannBoundaryConditions(
        std::shared_ptr<SpatialDiscretization::NeumannBoundaryConditions<
            FunctionSpaceType, Quadrature::Gauss<3>, 1>>
            &neumannBoundaryConditions) {
  typedef SpatialDiscretization::NeumannBoundaryConditions<
      FunctionSpaceType, Quadrature::Gauss<3>, 1>
      NeumannBoundaryConditionsType;
  ...
  if (meshPartition_->ownRankPartitioningIndex(2) == 0) {
    ...
    for (element_no_t elementIndexX = 0;
      ...
      for (element_no_t elementIndexY = 0;
        ...
        this->functionSpace_->getElementGeometry(elementNoLocal,
                                                 geometryValues);

        if (std::is_same<BasisFunctionType,
          ...
          // compute surface
          surfaceBottomLocal +=
              0.5 * MathUtility::norm<3>(MathUtility::cross(a, b));
        } else if (std::is_same<BasisFunctionType,
                                BasisFunction::LagrangeOfOrder<2>>::value) {
          // 6 7 8
          // 3 4 5
          // 0 1 2
    ...
    for (element_no_t elementIndexX = 0;
      ...
      for (element_no_t elementIndexY = 0;
        ...
        if (std::is_same<BasisFunctionType,
          ...
          surfaceTopLocal +=
              0.5 * MathUtility::norm<3>(MathUtility::cross(a, b));
          LOG(DEBUG) << "surfaceTopLocal += 0.5*|" << a << " x " << b << "| = "
                     << 0.5 * MathUtility::norm<3>(MathUtility::cross(a, b))
                     << " g: " << geometryValues;
        }
      }
  ...
  MPIUtility::handleReturnValue(
      MPI_Allreduce(&surfaceBottomLocal, &surfaceBottomGlobal, 1, MPI_DOUBLE,
                    MPI_SUM, this->currentRankSubset_->mpiCommunicator()),
      "MPI_Allreduce (1)");
  MPIUtility::handleReturnValue(
      MPI_Allreduce(&surfaceTopLocal, &surfaceTopGlobal, 1, MPI_DOUBLE, MPI_SUM,
                    this->currentRankSubset_->mpiCommunicator()),
      "MPI_Allreduce (2)");
  ...
  LOG(DEBUG) << "surfaceTop local: " << surfaceTopLocal
             << ", global: " << surfaceTopGlobal << ", fluxTop: " << fluxTop;
  LOG(DEBUG) << "surfaceBottom local: " << surfaceBottomLocal
             << ", global: " << surfaceBottomGlobal
             << ", fluxBottom: " << fluxBottom;
  ...
  struct ElementWithFaces
  {
    ...
    std::vector<std::pair<dof_no_t, VecD<nComponents>>> dofVectors;  //<
  <surface-local dof no, value>, nComponents == FunctionSpaceType::dim() for
  traction boundary condition or nComponents = 1 for flux BC
    std::vector<dof_no_t> surfaceDofs;    //< dof nos of the volume element that
  correspond to the face / surface. These are different from the dofs in
  dofsVector which are numbered for the surface only, surfaceDofs are in the
  numbering of the volume element.
    // note, for flux BC, dofVectors[i].second is a VecD<1>
  };*/

  typedef
      typename NeumannBoundaryConditionsType::ElementWithFaces ElementWithFaces;
      ...
      for (element_no_t elementIndexY = 0;
        ...
        for (int i = 0; i < 4; i++) {
          elementWithFaces.dofVectors.push_back(
              std::pair<dof_no_t, VecD<1>>(i, VecD<1>({fluxBottom})));
        }
  ...
  if (meshPartition_->ownRankPartitioningIndex(2) ==
    ...
    for (element_no_t elementIndexX = 0;
      ...
      for (element_no_t elementIndexY = 0;
        ...
        for (int i = 0; i < 4; i++) {
          elementWithFaces.dofVectors.push_back(
              std::pair<dof_no_t, VecD<1>>(i, VecD<1>({fluxTop})));
        }
