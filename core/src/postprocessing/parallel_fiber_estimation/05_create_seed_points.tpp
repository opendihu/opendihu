...
void ParallelFiberEstimation<BasisFunctionType>::createSeedPoints(
    const std::array<bool, 4> &subdomainIsAtBoundary, int seedPointsZIndex,
    const std::vector<Vec3> &nodePositions, std::vector<Vec3> &seedPoints) {
  ...
  LOG(DEBUG) << "createSeedPoints, seedPointsZIndex: " << seedPointsZIndex
             << ", subdomainIsAtBoundary: " << std::boolalpha
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face0Minus]) {
    for (int i = iBeginVertical; i < iEndVertical; i++) {
      seedPoints.push_back(
          nodePositions[seedPointsZIndex * subdomainNNodesX * subdomainNNodesY +
                        i * subdomainNNodesX + 0]);
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face0Plus]) {
    for (int i = iBeginVertical; i < iEndVertical; i++) {
      seedPoints.push_back(
          nodePositions[seedPointsZIndex * subdomainNNodesX * subdomainNNodesY +
                        i * subdomainNNodesX + (subdomainNNodesX - 1)]);
      seedPointPositionDebug.push_back(
          std::array<int, 2>({subdomainNNodesX - 1, i}));
    }
  }
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face1Plus]) {
    for (int i = iBeginHorizontal; i < iEndHorizontal; i++) {
      seedPoints.push_back(
          nodePositions[seedPointsZIndex * subdomainNNodesX * subdomainNNodesY +
                        (subdomainNNodesY - 1) * subdomainNNodesX + i]);
      seedPointPositionDebug.push_back(
          std::array<int, 2>({i, subdomainNNodesY - 1}));
    }
  }

  LOG(DEBUG)
      << "seedPoints: starting with horizontal center line, streamlineIndex = "
      << seedPoints.size();
  ...
  for (int i = iBeginHorizontal; i < iEndHorizontal; i++) {
    seedPoints.push_back(
        nodePositions[seedPointsZIndex * subdomainNNodesX * subdomainNNodesY +
                      int(subdomainNNodesY / 2) * subdomainNNodesX + i]);
    seedPointPositionDebug.push_back(
        std::array<int, 2>({i, int(subdomainNNodesY / 2)}));
  }

  LOG(DEBUG)
      << "seedPoints: starting with vertical center line, streamlineIndex = "
      << seedPoints.size();
  ...
  for (int i = iBeginVertical; i < iEndVertical; i++) {
    seedPoints.push_back(
        nodePositions[seedPointsZIndex * subdomainNNodesX * subdomainNNodesY +
                      i * subdomainNNodesX + int(subdomainNNodesX / 2)]);
    seedPointPositionDebug.push_back(
        std::array<int, 2>({int(subdomainNNodesX / 2), i}));
  }
  ...
  for (std::vector<std::pair<int, int>>::iterator iter = coordinates.begin();
    ...
    int i = iter->first;
    int j = iter->second;
    LOG(DEBUG) << "(i,j) = " << i << "," << j << ", index: "
               << seedPointsZIndex * subdomainNNodesX * subdomainNNodesY +
                      j * subdomainNNodesX + i
  ...
  LOG(DEBUG) << "seedPoints: after cornerStreamlines, streamlineIndex = "
             << seedPoints.size();

#ifndef NDEBUG
  LOG(DEBUG) << "seed point positions: ";
  for (int i = 0; i < seedPointPositionDebug.size(); i++) {
    LOG(DEBUG) << "  " << i << " at " << seedPointPositionDebug[i] << ": "
               << seedPoints[i];
  }
#endif
}
...
void ParallelFiberEstimation<BasisFunctionType>::
    extractSeedPointsFromBoundaryPoints(
  ...
  // extract the seed points from the fixed boundary points, they are needed to
  // be send to the neighbor process in z direction to continue the streamline
  // tracing
  ...
  //                                     <--+ nBoundaryPointsXNew_   (=9) =
  //                                     nBoundaryPointsX_*2-1
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face0Minus]) {
    for (int i = iBeginVertical; i < iEndVertical; i++) {
      int subdomainIndex = (i < nBoundaryPointsX_ - 1 ? 0 : 2) +
                           (streamlineDirectionUpwards ? 4 : 0);
      int streamlineIndex =
          (i < nBoundaryPointsX_ - 1 ? i : i - (nBoundaryPointsX_ - 1));
      seedPoints.push_back(boundaryPointsSubdomain[subdomainIndex][(
          int)Mesh::face_t::face0Minus][seedPointsZIndex][streamlineIndex]);
    }
  }
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face0Plus]) {
    for (int i = iBeginVertical; i < iEndVertical; i++) {
      int subdomainIndex = (i < nBoundaryPointsX_ - 1 ? 1 : 3) +
                           (streamlineDirectionUpwards ? 4 : 0);
      int streamlineIndex =
          (i < nBoundaryPointsX_ - 1 ? i : i - (nBoundaryPointsX_ - 1));
      seedPoints.push_back(boundaryPointsSubdomain[subdomainIndex][(
          int)Mesh::face_t::face0Plus][seedPointsZIndex][streamlineIndex]);
    }
  }
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face1Minus]) {
    for (int i = iBeginHorizontal; i < iEndHorizontal; i++) {
      int subdomainIndex = (i < nBoundaryPointsX_ - 1 ? 0 : 1) +
                           (streamlineDirectionUpwards ? 4 : 0);
      int streamlineIndex =
          (i < nBoundaryPointsX_ - 1 ? i : i - (nBoundaryPointsX_ - 1));
      seedPoints.push_back(boundaryPointsSubdomain[subdomainIndex][(
          int)Mesh::face_t::face1Minus][seedPointsZIndex][streamlineIndex]);
    }
  }
  ...
  if (!subdomainIsAtBoundary[(int)Mesh::face_t::face1Plus]) {
    for (int i = iBeginHorizontal; i < iEndHorizontal; i++) {
      int subdomainIndex = (i < nBoundaryPointsX_ - 1 ? 2 : 3) +
                           (streamlineDirectionUpwards ? 4 : 0);
      int streamlineIndex =
          (i < nBoundaryPointsX_ - 1 ? i : i - (nBoundaryPointsX_ - 1));
      seedPoints.push_back(boundaryPointsSubdomain[subdomainIndex][(
          int)Mesh::face_t::face1Plus][seedPointsZIndex][streamlineIndex]);
    }
  }
