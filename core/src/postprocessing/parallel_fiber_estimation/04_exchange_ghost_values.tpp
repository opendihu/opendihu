...
bool ParallelFiberEstimation<BasisFunctionType>::neighbourExists(
  ...
  if (faceOrEdge < 4) {
    ...
    // it is an edge
    if ((faceOrEdge == Mesh::face_or_edge_t::edge0Minus1Minus &&
         !subdomainIsAtBoundary[Mesh::face_t::face0Minus] &&
         !subdomainIsAtBoundary[Mesh::face_t::face1Minus]) ||
        (faceOrEdge == Mesh::face_or_edge_t::edge0Plus1Minus &&
         !subdomainIsAtBoundary[Mesh::face_t::face0Plus] &&
         !subdomainIsAtBoundary[Mesh::face_t::face1Minus]) ||
        (faceOrEdge == Mesh::face_or_edge_t::edge0Minus1Plus &&
         !subdomainIsAtBoundary[Mesh::face_t::face0Minus] &&
         !subdomainIsAtBoundary[Mesh::face_t::face1Plus]) ||
        (faceOrEdge == Mesh::face_or_edge_t::edge0Plus1Plus &&
         !subdomainIsAtBoundary[Mesh::face_t::face0Plus] &&
         !subdomainIsAtBoundary[Mesh::face_t::face1Plus])) {
      return true;
    }
...
template <typename BasisFunctionType>
void ParallelFiberEstimation<BasisFunctionType>::printRanksInNeighbourhood(
    const std::array<bool, 4> &subdomainIsAtBoundary) {
  std::stringstream s;
  ...
  if (subdomainIsAtBoundary[Mesh::face_t::face1Plus]) {
    ...
    if (subdomainIsAtBoundary[Mesh::face_t::face0Minus]) {
      ...
      int neighbourRankNo =
          meshPartition_->neighbourRank(Mesh::face_or_edge_t::edge0Minus1Plus);
      s << " " << std::setw(2) << neighbourRankNo;
    }

    // center
    int neighbourRankNo =
        meshPartition_->neighbourRank(Mesh::face_or_edge_t::faceEdge1Plus);
    s << " " << std::setw(2) << neighbourRankNo;
    ...
    if (subdomainIsAtBoundary[Mesh::face_t::face0Plus]) {
      ...
      int neighbourRankNo =
          meshPartition_->neighbourRank(Mesh::face_or_edge_t::edge0Plus1Plus);
      s << " " << std::setw(2) << neighbourRankNo;
    }
  }
  ...
  if (subdomainIsAtBoundary[Mesh::face_t::face0Minus]) {
    ...
    int neighbourRankNo =
        meshPartition_->neighbourRank(Mesh::face_or_edge_t::faceEdge0Minus);
    s << " " << std::setw(2) << neighbourRankNo;
  }
  ...
  if (subdomainIsAtBoundary[Mesh::face_t::face0Plus]) {
    ...
    int neighbourRankNo =
        meshPartition_->neighbourRank(Mesh::face_or_edge_t::faceEdge0Plus);
    s << " " << std::setw(2) << neighbourRankNo;
  }
  ...
  if (subdomainIsAtBoundary[Mesh::face_t::face1Minus]) {
    ...
    if (subdomainIsAtBoundary[Mesh::face_t::face0Minus]) {
      ...
      int neighbourRankNo =
          meshPartition_->neighbourRank(Mesh::face_or_edge_t::edge0Minus1Minus);
      s << " " << std::setw(2) << neighbourRankNo;
    }

    // center
    int neighbourRankNo =
        meshPartition_->neighbourRank(Mesh::face_or_edge_t::faceEdge1Minus);
    s << " " << std::setw(2) << neighbourRankNo;
    ...
    if (subdomainIsAtBoundary[Mesh::face_t::face0Plus]) {
      ...
      int neighbourRankNo =
          meshPartition_->neighbourRank(Mesh::face_or_edge_t::edge0Plus1Minus);
      s << " " << std::setw(2) << neighbourRankNo;
    }
  }
...
template <typename BasisFunctionType>
void ParallelFiberEstimation<BasisFunctionType>::exchangeGhostValues(
    const std::array<bool, 4> &subdomainIsAtBoundary) {
  struct GhostValues {
    std::vector<double>
  ...
  std::vector<Mesh::face_or_edge_t> faces = {
      Mesh::face_or_edge_t::faceEdge0Minus,
      Mesh::face_or_edge_t::faceEdge0Plus,
      Mesh::face_or_edge_t::faceEdge1Minus,
      Mesh::face_or_edge_t::faceEdge1Plus,
      Mesh::face_or_edge_t::edge0Minus1Minus,
      Mesh::face_or_edge_t::edge0Plus1Plus,
      Mesh::face_or_edge_t::edge0Plus1Minus,
      Mesh::face_or_edge_t::edge0Minus1Plus};

  // communicate ghost elements to neighbouring subdomains
  // determine elements on own domain that are to be send to the neighbouring
  // domain loop over faces: face0Minus = 0, face0Plus, face1Minus, face1Plus
  // and edges: edge0Minus1Minus, edge0Plus1Minus, edge0Minus1Plus,
  // edge0Plus1Plus
  for (int faceIndex = 0; faceIndex != 8; faceIndex++) {
    ...
    if (neighbourExists(subdomainIsAtBoundary, faceOrEdge)) {
      LOG(DEBUG) << "determine ghost elements for face "
                 << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge);

      // get information about neighbouring rank and boundary elements for face
      int neighbourRankNo;
      std::vector<dof_no_t> dofNos;
      meshPartition_->getBoundaryElements(
          (Mesh::face_or_edge_t)faceOrEdge, ghostLayerWidth_, neighbourRankNo,
          ghostValuesBuffer[faceIndex].nElementsPerCoordinateDirection, dofNos);

      LOG(DEBUG) << "getBoundaryElements for face "
                 << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge)
                 << " returned neighbourRankNo=" << neighbourRankNo
                 << ", nElementsPerCoordinateDirection: "
                 << ghostValuesBuffer[faceIndex].nElementsPerCoordinateDirection
                 << ", " << dofNos.size() << " dofs: " << dofNos;

      // get relevant values in own domain that will be send to the neighbouring
      // domain
      problem_->data().functionSpace()->geometryField().getValues(
          dofNos, boundaryValues[faceIndex].nodePositionValues);
      problem_->data().solution()->getValues(
          dofNos, boundaryValues[faceIndex].solutionValues);
      data_.gradient()->getValues(dofNos,
                                  boundaryValues[faceIndex].gradientValues);
    }
  }

  // this barrier is needed such that VecGhostUpdate does not interfere with the
...
// non-blocking communication, does not work with 64 processes
  ...
  for (int faceIndex = 0; faceIndex != 8; faceIndex++) {
    ...
    if (neighbourExists(subdomainIsAtBoundary, faceOrEdge)) {
      int neighbourRankNo =
          meshPartition_->neighbourRank((Mesh::face_or_edge_t)faceOrEdge);
      assert(neighbourRankNo != -1);
      ...
      int nNodePositionValues =
          boundaryValues[faceIndex].nodePositionValues.size();
      int nSolutionValues = boundaryValues[faceIndex].solutionValues.size();
      int nGradientValues = boundaryValues[faceIndex].gradientValues.size();
      assert(nSolutionValues * 3 == nGradientValues);
      assert(nNodePositionValues == nGradientValues);

      LOG(DEBUG) << "exchange ghosts with neighbour " << neighbourRankNo << " ("
                 << Mesh::getString(faceOrEdge)
                 << ") : nNodePositionValues=" << nNodePositionValues
                 << ", nSolutionValues=" << nSolutionValues
                 << ", nGradientValues=" << nGradientValues;
      ...
      ghostValuesBuffer[faceIndex].nodePositionValues.resize(
          nNodePositionValues);
      MPIUtility::handleReturnValue(
          MPI_Irecv(ghostValuesBuffer[faceIndex].nodePositionValues.data(),
                    nNodePositionValues, MPI_DOUBLE, neighbourRankNo, 0,
      ...
      ghostValuesBuffer[faceIndex].solutionValues.resize(nSolutionValues);
      MPIUtility::handleReturnValue(
          MPI_Irecv(ghostValuesBuffer[faceIndex].solutionValues.data(),
                    nSolutionValues, MPI_DOUBLE, neighbourRankNo, 0,
                    currentRankSubset_->mpiCommunicator(), &receiveRequest),
          "MPI_Irecv");
      receiveRequests.push_back(receiveRequest);
      ...
      ghostValuesBuffer[faceIndex].gradientValues.resize(nGradientValues);
      MPIUtility::handleReturnValue(
          MPI_Irecv(ghostValuesBuffer[faceIndex].gradientValues.data(),
                    nGradientValues, MPI_DOUBLE, neighbourRankNo, 0,
                    currentRankSubset_->mpiCommunicator(), &receiveRequest),
          "MPI_Irecv");
      receiveRequests.push_back(receiveRequest);

      LOG(DEBUG) << "receive from rank " << neighbourRankNo << " ("
                 << Mesh::getString(faceOrEdge) << ") completed";
      ...
      MPIUtility::handleReturnValue(
          MPI_Isend(boundaryValues[faceIndex].nodePositionValues.data(),
                    nNodePositionValues, MPI_DOUBLE, neighbourRankNo, 0,
      ...
      // post non-blocking send call to send solution values
      MPIUtility::handleReturnValue(
          MPI_Isend(boundaryValues[faceIndex].solutionValues.data(),
                    nSolutionValues, MPI_DOUBLE, neighbourRankNo, 0,
                    currentRankSubset_->mpiCommunicator(), &sendRequest),
          "MPI_Isend");
      sendRequests.push_back(sendRequest);
      ...
      MPIUtility::handleReturnValue(
          MPI_Isend(boundaryValues[faceIndex].gradientValues.data(),
                    nGradientValues, MPI_DOUBLE, neighbourRankNo, 0,
                    currentRankSubset_->mpiCommunicator(), &sendRequest),
          "MPI_Isend");
      sendRequests.push_back(sendRequest);
  ...
  LOG(DEBUG) << "waitall (" << sendRequests.size() << " send requests, "
             << receiveRequests.size() << " receiveRefquests) complete";
#endif
  LOG(DEBUG) << "ghost exchange communication done, currentRankSubset_: "
             << *currentRankSubset_;
  ...
  for (int faceIndex = 0; faceIndex != 8; faceIndex++) {
    ...
    if (neighbourExists(subdomainIsAtBoundary, faceOrEdge)) {
#if defined(WRITE_CHECKPOINT_GHOST_MESH) ||                                    \
    defined(USE_CHECKPOINT_GHOST_MESH) || !defined(NDEBUG)
      int neighbourRankNo =
          meshPartition_->neighbourRank((Mesh::face_or_edge_t)faceOrEdge);
#endif
      ...
      filenameOut << "checkpoints/checkpoint_ghost_mesh_l" << level_ << "_"
                  << neighbourRankNo << "_"
                  << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge)
                  << ".txt";
      ...
      LOG(DEBUG) << " output "
                 << ghostValuesBuffer[faceIndex].nodePositionValues.size()
                 << " node position values, "
                 << ghostValuesBuffer[faceIndex].solutionValues.size()
                 << " solution values, "
                 << ghostValuesBuffer[faceIndex].gradientValues.size()
                 << " gradient values to file " << filenameOut.str();
      LOG(DEBUG) << "ghostValuesBuffer[faceIndex].nodePositionValues: "
                 << ghostValuesBuffer[faceIndex].nodePositionValues;
      LOG(DEBUG) << "ghostValuesBuffer[faceIndex].solutionValues: "
                 << ghostValuesBuffer[faceIndex].solutionValues;
      LOG(DEBUG) << "ghostValuesBuffer[faceIndex].gradientValues: "
                 << ghostValuesBuffer[faceIndex].gradientValues;

      for (int i = 0;
           i < ghostValuesBuffer[faceIndex].nodePositionValues.size(); i++) {
        if (i % int(ghostValuesBuffer[faceIndex].nodePositionValues.size() /
                    3) ==
            0)
          fileOut << std::endl;
      ...
      for (int i = 0; i < ghostValuesBuffer[faceIndex].solutionValues.size();
           i++)
        fileOut << ghostValuesBuffer[faceIndex].solutionValues[i] << " ";

      fileOut << std::endl;

      for (int i = 0; i < ghostValuesBuffer[faceIndex].gradientValues.size();
           i++)
      ...
      fileOut.close();
      LOG(DEBUG) << "Wrote ghost meshes ("
                 << ghostValuesBuffer[faceIndex].nodePositionValues.size()
                 << " node position values, "
                 << ghostValuesBuffer[faceIndex].solutionValues.size()
                 << " solution values, "
                 << ghostValuesBuffer[faceIndex].gradientValues.size()
                 << " gradient values) to checkpoint file "
                 << filenameOut.str();
#endif

#ifdef USE_CHECKPOINT_GHOST_MESH
      // load all data from a checkpoint as this point in the code (for
      // debugging)
      std::stringstream filenameIn;
      filenameIn << "checkpoints/checkpoint_ghost_mesh_l" << level_ << "_"
                 << neighbourRankNo << "_"
                 << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge) << ".txt";
      std::ifstream fileIn;
      ...
      fileIn.close();
      LOG(DEBUG) << "Loaded ghost meshes ("
                 << ghostValuesBuffer[faceIndex].nodePositionValues.size()
                 << " node position values, "
                 << ghostValuesBuffer[faceIndex].solutionValues.size()
                 << " solution values, "
                 << ghostValuesBuffer[faceIndex].gradientValues.size()
                 << " gradient values) from checkpoint file "
                 << filenameIn.str();

      int elementNo = 392;
      // element coordinates 0,0,49
      if (neighbourRankNo == 1 &&
          Mesh::getString((Mesh::face_or_edge_t)faceOrEdge) == "0+") {
        LOG(DEBUG) << "special element " << elementNo;
        // int
        //  0+
        //  element coordinates: (0,0,49) / (1,8,50)
        int dof[8] = {49 * (2 * 9),         49 * (2 * 9) + 1,
                      49 * (2 * 9) + 1 * 2, 49 * (2 * 9) + 1 * 2 + 1,
                      50 * (2 * 9),         50 * (2 * 9) + 1,
                      50 * (2 * 9) + 1 * 2, 50 * (2 * 9) + 1 * 2 + 1};
        for (int i = 0; i < 8; i++) {
          LOG(DEBUG)
              << ghostValuesBuffer[faceIndex].nodePositionValues[dof[i]] << ","
              << ghostValuesBuffer[faceIndex].nodePositionValues[918 + dof[i]]
              << ","
              << ghostValuesBuffer[faceIndex]
                     .nodePositionValues[918 * 2 + dof[i]];
        }
      }
      ...
      if (neighbourRankNo != -1) {
        std::stringstream s;
        s << "05_received_ghost_elements_face_"
          << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge);
        PyObject_CallFunction(
            functionOutputGhostElements_, "s i i O O f", s.str().c_str(),
            currentRankSubset_->ownRankNo(), level_,
            PythonUtility::convertToPython<std::vector<double>>::get(
                ghostValuesBuffer[faceIndex].nodePositionValues),
            PythonUtility::convertToPython<std::array<element_no_t, 3>>::get(
                ghostValuesBuffer[faceIndex].nElementsPerCoordinateDirection),
            0.05);
        PythonUtility::checkForError();
      }
  ...
  // create a new communicator that contains only the current rank, because the
  // ghost mesh is not partitioned
  LOG(DEBUG) << "split rank subset, own rank: "
             << currentRankSubset_->ownRankNo();
  std::shared_ptr<Partition::RankSubset> rankSubsetSingleRank =
      std::make_shared<Partition::RankSubset>(currentRankSubset_->ownRankNo(),
                                              currentRankSubset_);
  ...
  for (int faceIndex = 0; faceIndex != 8; faceIndex++) {
    ...
    if (!ghostValuesBuffer[faceIndex].nodePositionValues.empty()) {
      ...
      meshName << this->functionSpace_->meshName() << "_ghost_"
               << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge);

      // transform the node position data from vector of double to vector of
      // Vec3
      ...
      for (node_no_t nodeIndex = 0; nodeIndex < nNodes; nodeIndex++) {
        nodePositions[nodeIndex][0] =
            ghostValuesBuffer[faceIndex]
                .nodePositionValues[0 * nNodes + nodeIndex];
        nodePositions[nodeIndex][1] =
            ghostValuesBuffer[faceIndex]
                .nodePositionValues[1 * nNodes + nodeIndex];
        nodePositions[nodeIndex][2] =
            ghostValuesBuffer[faceIndex]
                .nodePositionValues[2 * nNodes + nodeIndex];
      }

      // the number of elements per coordinate direction was set earlier by
      // getBoundaryElements
      LOG(DEBUG)
          << "create ghost mesh with nElementsPerCoordinateDirection: "
          << ghostValuesBuffer[faceIndex].nElementsPerCoordinateDirection;

      // create ghost mesh
      std::array<int, 3> nRanks({1, 1, 1});
      context_.partitionManager()->setRankSubsetForNextCreatedPartitioning(
          rankSubsetSingleRank);
      ghostMesh =
          context_.meshManager()
              ->template createFunctionSpace<FunctionSpaceType>(
                  meshName.str(), nodePositions,
                  ghostValuesBuffer[faceIndex].nElementsPerCoordinateDirection,
                  nRanks);

      LOG(DEBUG) << "created ghost mesh for face "
                 << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge) << ".";

      // create solution field variable for ghost mesh
      LOG(DEBUG) << "create solution field variable on ghost mesh";
      this->ghostMeshSolution_[faceOrEdge] =
          ghostMesh->template createFieldVariable<1>("solution");
      this->ghostMeshSolution_[faceOrEdge]->setValuesWithGhosts(
          ghostValuesBuffer[faceIndex].solutionValues);

      // create gradient field variable for ghost mesh
      LOG(DEBUG) << "create gradient field variable on ghost mesh";
      this->ghostMeshGradient_[faceOrEdge] =
          ghostMesh->template createFieldVariable<3>("gradient");
      int nGradientValues =
          ghostValuesBuffer[faceIndex].gradientValues.size() / 3;
      ...
      for (int i = 0; i < nGradientValues; i++) {
        gradientValues[i][0] =
            ghostValuesBuffer[faceIndex].gradientValues[3 * i + 0];
        gradientValues[i][1] =
            ghostValuesBuffer[faceIndex].gradientValues[3 * i + 1];
        gradientValues[i][2] =
            ghostValuesBuffer[faceIndex].gradientValues[3 * i + 2];
      }
      ...
      LOG(DEBUG)
          ...
          << Mesh::getString((Mesh::face_or_edge_t)faceOrEdge)
          << " is null, because "
             "ghostValuesBuffer[faceIndex].nodePositionValues is empty()";
    }

    this->functionSpace_->setGhostMesh((Mesh::face_or_edge_t)faceOrEdge,
                                       ghostMesh);
    // LOG(DEBUG) << "after settings ghost mesh for face " <<
    // Mesh::getString((Mesh::face_or_edge_t)faceOrEdge) << ": ";
