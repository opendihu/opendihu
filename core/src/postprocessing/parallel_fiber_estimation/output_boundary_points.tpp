...
void ParallelFiberEstimation<BasisFunctionType>::outputBoundaryPoints(
    ...
    for (int face = Mesh::face_t::face0Minus; face <= Mesh::face_t::face1Plus;
      ...
      for (int zLevelIndex = 0; zLevelIndex < nBoundaryPointsZ_;
        ...
        for (int pointIndex = 0;
             pointIndex !=
             boundaryPointsSubdomain[subdomainIndex][face][zLevelIndex].size();
             pointIndex++) {
          points.push_back(boundaryPointsSubdomain[subdomainIndex][face]
  ...
  PyObject_CallFunction(
      functionOutputPoints_, "s i i O f", filename.str().c_str(),
...
template <typename BasisFunctionType>
void ParallelFiberEstimation<BasisFunctionType>::outputStreamlines(
    std::array<std::array<std::vector<std::vector<Vec3>>, 4>, 8>
        &boundaryPointsSubdomain,
    std::string name) {
#ifndef NDEBUG
  ...
  for (int subdomainIndex = 0; subdomainIndex < 8; subdomainIndex++) {
    for (int face = Mesh::face_t::face0Minus; face <= Mesh::face_t::face1Plus;
         face++) {
      // LOG(DEBUG) << "subdomainIndex: " << subdomainIndex << ", face: " <<
      // Mesh::getString((Mesh::face_t)face);

      for (int pointIndex = 0;
           pointIndex !=
           boundaryPointsSubdomain[subdomainIndex][face][0].size();
           pointIndex++) {
        std::vector<Vec3> points;
        for (int zLevelIndex = 0; zLevelIndex < nBoundaryPointsZ_;
             zLevelIndex++) {
  ...
  filename << name;
  PyObject_CallFunction(
      functionOutputStreamlines_, "s i i O f", filename.str().c_str(),
      currentRankSubset_->ownRankNo(), level_,
