...
void scaleFibersInFile(std::string inputFilename, std::string outputFilename,
  ...
  std::ofstream fileNew(outputFilename.c_str(),
  ...
  std::cout << "Scaling factor " << scalingFactor << ", " << nFibersX << "x"
            << nFibersX << " fibers with " << nPointsPerFiber
            << " points per fiber." << std::endl;
  ...
  for (int fiberIndex = 0; fiberIndex != nFibers; fiberIndex++) {
    ...
    for (int zIndex = 0; zIndex < nPointsPerFiber; zIndex++) {
      int fileIndex = 32 + headerLength + fiberIndex * fiberDataSize +
      ...
      for (int i = 0; i < 3; i++) {
        oldBoundingBoxMinimum[i] =
            std::min(oldBoundingBoxMinimum[i], oldPoint[i]);
        oldBoundingBoxMaximum[i] =
            std::max(oldBoundingBoxMaximum[i], oldPoint[i]);
        newBoundingBoxMinimum[i] =
            std::min(newBoundingBoxMinimum[i], newPoint[i]);
        newBoundingBoxMaximum[i] =
            std::max(newBoundingBoxMaximum[i], newPoint[i]);
      }
    }
  }
  ...
  std::cout << "Output file \"" << outputFilename << "\",\n  bounding box "
            ...
            << " x " << newBoundingBoxMaximum[2] - newBoundingBoxMinimum[2]
            << std::endl;
}
