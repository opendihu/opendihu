...
template <int nStates, int nAlgebraics_, typename FunctionSpaceType>
void RhsRoutineHandler<nStates, nAlgebraics_,
                       FunctionSpaceType>::initializeRhsRoutine() {
  ...
  if (this->specificSettings_.hasKey("forceRecompileRhs")) {
    LOG(WARNING) << "Option \"forceRecompileRhs\" is no longer used, simply "
                    "specify \"sourceFilename\" to compile the function.";
  }
  if (this->specificSettings_.hasKey("useGivenLibrary")) {
    LOG(WARNING) << "Option \"useGivenLibrary\" is no longer used, simply "
                    "specify \"libraryFilename\" to use an existing library.";
  }
  if (this->specificSettings_.hasKey("simdSourceFilename")) {
    LOG(WARNING) << "Option \"simdSourceFilename\" is no longer used, simply "
                    "specify \"sourceFilename\" to create a simd version and "
                    "compile the function.";
  }

  // load type
  optimizationType_ =
      this->specificSettings_.getOptionString("optimizationType", "vc");

  if (optimizationType_ == "comp") {
    LOG(WARNING) << "Option \"optimizationType\"=\"comp\" has been renamed to "
                    "\"simd\"! Using \"simd\" instead.";
    optimizationType_ = "simd";
  }

  if (optimizationType_ != "simd" && optimizationType_ != "vc" &&
      optimizationType_ != "openmp" && optimizationType_ != "gpu") {
    LOG(ERROR)
        << "Option \"optimizationType\" is \"" << optimizationType_
        << "\" but valid values are \"simd\", \"vc\", \"openmp\" or \"gpu\"."
        << " Now setting to \"vc\".";
    optimizationType_ = "vc";
  }

  // for vc optimization, the exponential function can be approximated which is
  // faster than the exact exp function
  if (optimizationType_ == "vc") {
    approximateExponentialFunction_ = this->specificSettings_.getOptionBool(
        "approximateExponentialFunction", true);
    useAoVSMemoryLayout_ = true;
    if (this->specificSettings_.hasKey("useAoVSMemoryLayout"))
      useAoVSMemoryLayout_ =
          this->specificSettings_.getOptionBool("useAoVSMemoryLayout", true);
  } else if (optimizationType_ == "openmp") {
    // default value 0 means no restriction
    maximumNumberOfThreads_ = this->specificSettings_.getOptionInt(
        "maximumNumberOfThreads", 0, PythonUtility::NonNegative);
  }
  ...
  if (this->specificSettings_.hasKey("libraryFilename")) {
    libraryFilename =
        this->specificSettings_.getOptionString("libraryFilename", "lib.so");
    ...
    baseFilename << StringUtility::extractBasename(
                        this->cellmlSourceCodeGenerator_.sourceFilename())
                 << "_" << this->cellmlSourceCodeGenerator_.nParameters()
                 << "_";
    ...
    const std::vector<int> &algebraicsForTransfer =
        this->data_.algebraicsForTransfer();
    for (int i = 0; i < algebraicsForTransfer.size(); i++)
      baseFilename << algebraicsForTransfer[i];
    baseFilename << "_";

    const std::vector<int> &parametersForTransfer =
        this->data_.parametersForTransfer();
    for (int i = 0; i < parametersForTransfer.size(); i++)
      baseFilename << parametersForTransfer[i];

    baseFilename << "_" << optimizationType_ << "_" << this->nInstances_;
    ...
    s.str("");
    s << "src/" << baseFilename.str() << "." << rankNoWorldCommunicator
      << this->cellmlSourceCodeGenerator_.sourceFileSuffix();
    ...
    int ownRankNoCommunicator =
        this->functionSpace_->meshPartition()->ownRankNo();
    std::vector<int> nInstancesRanks(nRanksCommunicator);
    nInstancesRanks[ownRankNoCommunicator] = this->nInstances_;

    // std::cout << "ownRankNoCommunicator: " << ownRankNoCommunicator << ",
    // Communicator has " << nRanksCommunicator << " ranks, nInstancesRanks: "
    // << nInstancesRanks << std::endl;

    MPIUtility::handleReturnValue(
        MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL,
                      nInstancesRanks.data(), 1, MPI_INT,
...
void *RhsRoutineHandler<nStates, nAlgebraics_, FunctionSpaceType>::
  ...
  void *handle = NULL;
  for (int i = 0; handle == NULL && i < 50;
       i++) // wait maximum 2.5 ms for rank 0 to finish
  {
    // load dynamic library
    handle = dlopen((currentWorkingDirectory + libraryFilename).c_str(),
  ...
  if (handle == NULL) {
    LOG(ERROR) << "Could not load library \""
               << (currentWorkingDirectory + libraryFilename) << "\":\n\n"
               << dlerror();
  }
...
bool RhsRoutineHandler<nStates, nAlgebraics_, FunctionSpaceType>::
  ...
  if (handle) {
    ...
    rhsRoutine_ =
        (void (*)(void *, double, double *, double *, double *, double *))dlsym(
            handle, "computeCellMLRightHandSide");
    rhsRoutineGPU_ =
        (void (*)(void *, double, double *, double *, double *, double *))dlsym(
            handle, "computeGPUCellMLRightHandSide");
    rhsRoutineSingleInstance_ =
        (void (*)(void *, double, double *, double *, double *, double *))dlsym(
            handle, "computeCellMLRightHandSideSingleInstance");
    initConstsOpenCOR_ =
        (void (*)(double *, double *, double *))dlsym(handle, "initConsts");
    computeRatesOpenCOR_ = (void (*)(double, double *, double *, double *,
                                     double *))dlsym(handle, "computeRates");
    computeVariablesOpenCOR_ =
        (void (*)(double, double *, double *, double *, double *))dlsym(
            handle, "computeVariables");

    LOG(DEBUG) << "Library \"" << libraryFilename << "\" loaded. "
               << "rhsRoutine_: " << (rhsRoutine_ == NULL ? "NULL" : "yes")
               << ", rhsRoutineGPU_: "
               << (rhsRoutineGPU_ == NULL ? "NULL" : "yes")
               << ", rhsRoutineSingleInstance: "
               << (rhsRoutineSingleInstance_ == NULL ? "NULL" : "yes")
               << ", initConstsOpenCOR_: "
               << (initConstsOpenCOR_ == NULL ? "NULL" : "yes")
               << ", computeRatesOpenCOR_: "
               << (computeRatesOpenCOR_ == NULL ? "NULL" : "yes")
               << ", computeVariablesOpenCOR_: "
               << (computeVariablesOpenCOR_ == NULL ? "NULL" : "yes");

    if (rhsRoutine_) {
      // if the opendihu-generated rhs function (with openmp pragmas) is present
      // in the library, we can directly use it and we are done in this method.
      return true;
    } else if (rhsRoutineGPU_) {
      // if the gpu-processed rhs function (with openmp 4.5 pragmas) is present
      // in the library, we can directly use it and we are done in this method.
      rhsRoutine_ = rhsRoutineGPU_;
      return true;
    } else if (initConstsOpenCOR_) {
      LOG(FATAL) << "Could not load rhs routine from dynamic library \""
                 << libraryFilename
                 << "\", but a computeRates method from OpenCOR was detected. "
                 << "Please provide the sourceFilename of the source instead "
                    "of the library.";
      return false;
    } else {
      LOG(FATAL) << "Could not load rhs routine from dynamic library \""
...
template <int nStates, int nAlgebraics_, typename FunctionSpaceType>
void RhsRoutineHandler<nStates, nAlgebraics_, FunctionSpaceType>::
    createLibraryOnOneRank(std::string libraryFilename,
                           const std::vector<int> &nInstancesRanks) {
  // get the global rank no, needed for the output filenames
  int rankNoWorldCommunicator = DihuContext::ownRankNoCommWorld();

  // determine if this rank should do compilation, such that each nInstances is
  // compiled only once, by the rank with lowest number
  int i = 0;
  int rankWhichCompilesLibrary = 0;
  for (std::vector<int>::const_iterator iter = nInstancesRanks.begin();
       iter != nInstancesRanks.end(); iter++, i++) {
  ...
  if (rankWhichCompilesLibrary == ownRankNoCommunicator) {
    ...
    // create source file
    this->cellmlSourceCodeGenerator_.generateSourceFile(
        sourceToCompileFilename_, optimizationType_,
    ...
    // load compiler flags
    std::string compilerFlags = this->specificSettings_.getOptionString(
        "compilerFlags",
    ...
    std::stringstream s;
    s << this->cellmlSourceCodeGenerator_.compilerCommand() << " "
      << sourceToCompileFilename_ << " " << compilerFlags << " "
      << this->cellmlSourceCodeGenerator_.additionalCompileFlags() << " ";

    std::string compileCommandOptions = s.str();

    // compile library to filename with "*.rankNoWorldCommunicator", then wait
    // (different wait times for ranks), then rename file to without
    // "*.rankNoWorldCommunicator"
    compileCommand << compileCommandOptions << " -o " << libraryFilename << "."
                   << rankNoWorldCommunicator
                   << " "
                   //<< " && sleep " << int((rankNoWorldCommunicator%100)/10+1)
                   << " && mv " << libraryFilename << "."
                   << rankNoWorldCommunicator << " " << libraryFilename;
    ...
    if (ret != 0) {
      ...
      try {
        ...
        LOG(ERROR) << "Could not modify compile command. Command: \""
                   << compileCommand.str() << "\".";
      }
    } else {
...
template <int nStates, int nAlgebraics_, typename FunctionSpaceType>
bool RhsRoutineHandler<nStates, nAlgebraics_,
                       FunctionSpaceType>::approximateExponentialFunction() {
  return approximateExponentialFunction_;
}

template <int nStates, int nAlgebraics_, typename FunctionSpaceType>
std::string RhsRoutineHandler<nStates, nAlgebraics_,
                              FunctionSpaceType>::optimizationType() {
