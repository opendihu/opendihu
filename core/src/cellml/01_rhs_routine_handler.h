...
protected:
  ...
  std::string optimizationType_; //< type of generated file, e.g. "simd", "gpu",
                                 //"openmp"
  bool
      approximateExponentialFunction_; //< when using "vc" as
  ...
  void (*rhsRoutine_)(
      ...
      double *algebraics,
      double *parameters); //< function pointer to the rhs routine that can
                           // compute several instances of the problem in
                           // parallel. Data is assumed to contain values for a
                           // state contiguously, e.g. (state[1], state[1],
                           // state[1], state[2], state[2], state[2], ...). The
                           // first parameter is a this pointer.
