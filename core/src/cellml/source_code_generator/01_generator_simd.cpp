...
void CellmlSourceCodeGeneratorSimd::generateSourceFileSimd(
  ...
  simdSource << std::endl
             ...
             << " * The \"optimizationType\" is \"simd\". (Other options are "
                "\"vc\" and \"openmp\".) */"
             << std::endl
  ...
  simdSource << "  double VOI = t;   /* current simulation time */"
             << std::endl;
  simdSource << std::endl
  ...
  for (code_expression_t &codeExpression : cellMLCode_.lines) {
    if (codeExpression.type != code_expression_t::commented_out) {
      ...
      codeExpression.visitLeafs(
          [&simdSource,
           this](CellmlSourceCodeGeneratorSimd::code_expression_t &expression,
            ...
            case code_expression_t::commented_out:
              simdSource << "  // (not assigning to a parameter) "
                         << expression.code;
              break;
