...
void CellmlSourceCodeGeneratorBase::initializeNames(std::string inputFilename,
                                                    int nInstances, int nStates,
                                                    int nAlgebraics) {
...
void CellmlSourceCodeGeneratorBase::initializeSourceCode(
    const std::vector<int> &parametersUsedAsAlgebraic,
    const std::vector<int> &parametersUsedAsConstant,
    std::vector<double> &parametersInitialValues, int maximumNumberOfParameters,
    double *parameterValues) {
  // parametersInitialValues is the list of initial parameter values as given in
  // the settings

  parametersUsedAsAlgebraic_.assign(parametersUsedAsAlgebraic.begin(),
                                    parametersUsedAsAlgebraic.end());
  parametersUsedAsConstant_.assign(parametersUsedAsConstant.begin(),
                                   parametersUsedAsConstant.end());

  nParameters_ =
      parametersUsedAsAlgebraic_.size() + parametersUsedAsConstant_.size();

  // nParameters_ is the number of parameters per instance as determined from
  // mappings
  ...
  if (nParameters_ > maximumNumberOfParameters) {
    LOG(FATAL)
        ...
        << "Now you have (" << nParameters_
        << " parameters and the maximum possible number is "
        << maximumNumberOfParameters << ").";
  }

  LOG(DEBUG) << "nParameters_: " << nParameters_
             << ", parametersInitialValues.size(): "
             << parametersInitialValues.size()
             << ", nAlgebraics_: " << nAlgebraics_
             << ", nInstances_: " << nInstances_;

  if (parametersInitialValues.size() != nParameters_ &&
      parametersInitialValues.size() != nParameters_ * nInstances_) {
    // the given number of parameters does not match the number of initial
    ...
    if (parametersInitialValues.size() < nParameters_)
      LOG(WARNING) << "  Using '0' as default value for "
                   << (nParameters_ - parametersInitialValues.size())
                   << "undefiend parameters.";
    else
      LOG(WARNING) << "  Using only the first " << nParameters_
                   << " parameters";
  ...
  if (parametersInitialValues.size() == nParameters_) {
    ...
    LOG(DEBUG)
        << "copy parameters which were given only for one instance to all "
        << nInstances_ << " instances";
    for (int instanceNo = 0; instanceNo < nInstances_; instanceNo++) {
      for (int j = 0; j < nParameters_; j++) {
        // parameterValues has struct of array memory layout with space for a
        // total of nAlgebraics_ parameters [i0p0, i1p0, i2p0, ... i0p1, i1p1,
        // i2p1, ...]
        parameterValues[j * nInstances_ + instanceNo] =
            parametersInitialValues[j];

        VLOG(1) << "  " << instanceNo << "," << j << " set index "
                << (j * nInstances_ + instanceNo) << " to value "
                << parametersInitialValues[j] << "="
                << parameterValues[j * nInstances_ + instanceNo];
      }
    }
  } else // parametersInitialValues.size() == nParameters_ * nInstances_
  {
    ...
    // contiguous array of struct: inst1p0, inst1p1, ... instNp0, instNp1...
    // instNpM
    LOG(DEBUG) << "parameters size is matching for all instances";
    for (int instanceNo = 0; instanceNo < nInstances_; instanceNo++) {
      for (int j = 0; j < nParameters_; j++) {
        // parameters are given in array of struct ordering with nParameters_
        // parameters per instance: [inst0p0, inst0p1, ... inst0pn, inst1p0,
        // inst1p1, ...]
        parameterValues[j * nInstances_ + instanceNo] =
            parametersInitialValues[instanceNo * nParameters_ + j];
      }
    }
  }
  ...
  cellmlMessage << "CellML file \"" << sourceFilename_ << "\" with " << nStates_
                << " state" << (nStates_ != 1 ? "s" : "") << ", "
                << nAlgebraicsInSource_ << " algebraic"
                << (nAlgebraicsInSource_ != 1 ? "s" : "") << ", specified "
                << nParameters_ << " parameter"
                << (nParameters_ != 1 ? "s" : "") << ": " << s.str() << "\n";

  // only print message if it has not already been printed
  static std::vector<std::string> cellmlMessages;
  if (std::find(cellmlMessages.begin(), cellmlMessages.end(),
                cellmlMessage.str()) == cellmlMessages.end()) {
  ...
  for (int instanceNo = 0; instanceNo < nInstances_; instanceNo++) {
    for (int j = 0; j < nParameters_; j++) {
      // parameters are given in array of struct ordering: [inst0p0, inst0p1,
      // ... inst0pn, inst1p0, inst1p1, ...]
      message << parameterValues[j * nInstances_ + instanceNo] << " ";
    }
    message << ",";
  }
  LOG(DEBUG) << "parameterValues (only up to nParameters=" << nParameters_
             << " parameter, allocated space is for " << nAlgebraics_
             << " parameters: \n"
             << message.str();
#endif
...
void CellmlSourceCodeGeneratorBase::convertFromXmlToC() {
  ...
  if (DihuContext::ownRankNoCommWorld() == 0) {
    ...
    if (cFile.is_open()) {
      ...
      useOpenCor = true;
#else
      LOG(FATAL) << "OpenCOR is not available to convert XML to c files, but "
                    "CellMLAdapter has an XML file as input: \""
                 << sourceFilename_ << "\".";
#endif
    }
  }

  LOG(DEBUG) << "useOpenCor=" << useOpenCor << ", cFilename: " << cFilename
  ...
  if (useOpenCor) {
    ...
    if (DihuContext::ownRankNoCommWorld() == 0) {
      ...
      command << "\"" << OPENCOR_BINARY << "\" -c CellMLTools::export \""
              << sourceFilename_ << "\" \"" << OPENCOR_FORMATS_DIRECTORY
              << "/C.xml\" > \"" << cFilename << "\"";
      LOG(DEBUG) << "Use opencor to convert file: \n" << command.str();
      int ret = system(command.str().c_str());
      if (ret != 0) {
        LOG(FATAL) << "Could not convert CellML XML file \"" << sourceFilename_
                   << "\" to c file \"" << cFilename << "\" using OpenCOR: \n"
                   << command.str();
      }
    }
  }
  ...
  // LOG(DEBUG) << "MPI barrier, wait on all ranks until conversion is finished,
  // n ranks: " <<
  // DihuContext::partitionManager()->rankSubsetForCollectiveOperations()->size();

  // wait on all ranks until conversion is finished, this fails with multiple
  // ranks in x,y and z direction
  // MPIUtility::handleReturnValue(MPI_Barrier(DihuContext::partitionManager()->rankSubsetForCollectiveOperations()->mpiCommunicator()),
  // "MPI_Barrier");
...
void CellmlSourceCodeGeneratorBase::generateSingleInstanceCode() {
  ...
  sourceCode << std::endl
             << "// compute the rhs for a single instance, this can be used "
                "for computation of the equilibrium values of the states"
             << std::endl
             << "#ifdef __cplusplus\n"
             << "extern \"C\"\n"
             << "#endif\n"
  ...
  for (code_expression_t &codeExpression : cellMLCode_.lines) {
    ...
    if (codeExpression.type != code_expression_t::commented_out) {
      sourceCode << "  ";
      codeExpression.visitLeafs([&sourceCode,
                                 this](code_expression_t &expression,
...
CellmlSourceCodeGeneratorBase::algebraicNames() const {
  return algebraicNames_;
}

const std::vector<std::string> &
CellmlSourceCodeGeneratorBase::stateNames() const {
  return stateNames_;
}

const std::vector<std::string> &
CellmlSourceCodeGeneratorBase::constantNames() const {
