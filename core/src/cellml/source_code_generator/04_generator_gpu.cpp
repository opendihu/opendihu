...
void CellmlSourceCodeGeneratorGpu::generateSourceFileGpu(
  ...
  sourceCode << std::endl
             ...
             << " * The \"optimizationType\" is \"gpu\". (Other options are "
                "\"vc\", \"openmp\" and \"simd\".) */"
             << std::endl
  ...
  sourceCode << std::endl
             << "  #pragma omp target parallel for map(to:states,t,parameters) "
                "map(from:rates,algebraics)"
             << std::endl
...
void CellmlSourceCodeGeneratorGpu::generateSourceFastMonodomainGpu(
    bool approximateExponentialFunction, int nFibersToCompute,
    int nInstancesToComputePerFiber, int nParametersPerInstance,
    bool hasAlgebraicsForTransfer, std::string &headerCode,
    std::string &mainCode) {
...
#pragma omp \
  ...
  auto t = std::time(nullptr);
  auto tm = *std::localtime(&t);
  sourceCodeHeader << std::endl
                   << "/* This file was created by opendihu at "
                   << StringUtility::timeToString(
                          &tm) // std::put_time(&tm, "%d/%m/%Y %H:%M:%S")
                   << ".\n * It is designed for the FastMonodomainSolver and "
                      "contains code for offloading to GPU.\n "
                   << " */\n";
  ...
  sourceCodeHeader << defineHelperFunctions(
      helperFunctions, approximateExponentialFunction, false, useReal);
  ...
  for (std::string constantAssignmentsLine : constantAssignments_) {
    ...
    sourceCodeMain << indent << "const real " << constantAssignmentsLine
                   << std::endl;
  }
  ...
  for (code_expression_t &codeExpression : cellMLCode_.lines) {
    if (codeExpression.type != code_expression_t::commented_out) {
      ...
      codeExpression.visitLeafs(
          [&sourceCodeLine, &isCommentedOut, nInstancesToCompute, &isFirst,
              ...
              if (expression.code == "CONSTANTS") {
                ...
                if (expression.code == "states") {
                  if (expression.arrayIndex == 0) {
                    ...
                    sourceCodeLine
                        << "states["
                        << expression.arrayIndex * nInstancesToCompute
                        << "+instanceToComputeNo]";
                  }
  ...
  sourceCodeMain << "\n"
                 << "          // algebraic step\n"
                 << "          // compute y* = y_n + dt*rhs(y_n), y_n = state, "
                    "rhs(y_n) = rate, y* = intermediateState\n";

  sourceCodeMain << indent
                 << "states[0+instanceToComputeNo] = "
                    "vmValues[instanceToComputeNo] + dt0D*rate0;\n";

  for (int stateNo = 1; stateNo < this->nStates_; stateNo++) {
    sourceCodeMain << indent << "states[" << stateNo * nInstancesToCompute
                   << "+instanceToComputeNo] = states["
                   << stateNo * nInstancesToCompute
                   << "+instanceToComputeNo] + dt0D*rate" << stateNo << ";\n";
  }
            ...
            case code_expression_t::variableName:
              ...
              if (expression.code == "CONSTANTS") {
                ...
                if (expression.code == "states") {
                  ...
                  if (isFirst)
                    sourceCodeLine << "const real ";
                  sourceCodeLine << "intermediateAlgebraic"
  ...
  sourceCodeMain << indent
                 << "vmValues[instanceToComputeNo] += 0.5*dt0D*(rate0 + "
                    "intermediateRate0);\n";

  for (int stateNo = 1; stateNo < this->nStates_; stateNo++) {
    sourceCodeMain << indent << "states[" << stateNo * nInstancesToCompute
                   << "+instanceToComputeNo] += 0.5*dt0D*(rate" << stateNo
                   << " + intermediateRate" << stateNo << ");\n";
  }
  ...
  if (hasAlgebraicsForTransfer) {
    ...
    // loop over algebraics and generate code to copy the updated algebraic
    // values to the algebraics
    for (int algebraicNo = 0; algebraicNo < this->nAlgebraics_; algebraicNo++) {
      ...
      if (std::find(this->parametersUsedAsAlgebraic_.begin(),
        ...
        sourceCodeMain << indent << "      case " << algebraicNo << ":\n"
                       << indent
                       << "        algebraicsForTransfer[i*nInstancesToCompute "
                          "+ instanceToComputeNo] = intermediateAlgebraic"
                       << algebraicNo << ";\n"
                       << indent << "        break;\n";
      }
    }
  ...
  // loop over states and generate code to copy the updated state values to the
  // statesForTransfer
  for (int stateNo = 1; stateNo < this->nStates_; stateNo++) {
    // only of the algebraic was computed and not replaced by a parameter
    sourceCodeMain << indent << "      case " << stateNo << ":\n"
                   << indent
                   << "        statesForTransfer[i*nInstancesToCompute + "
                      "instanceToComputeNo] = states["
                   << stateNo * nInstancesToCompute
                   << "+instanceToComputeNo];\n"
                   << indent << "        break;\n";
