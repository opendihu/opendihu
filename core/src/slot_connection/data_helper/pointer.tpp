...
int SlotConnectorDataHelper<SlotConnectorDataType>::nSlots(
...
template <typename SlotConnectorDataType>
int SlotConnectorDataHelper<SlotConnectorDataType>::nArrayItems(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData, int slotNo) {
  if (!slotConnectorData)
    return 0;
  return 1;
}
...
SlotConnectorDataHelper<SlotConnectorDataType>::getMeshPartitionBase(
  ...
  if (slotNo < nSlotsVariable1) {
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable1Type>
        fieldVariable = slotConnectorData->variable1[slotNo].values;
    ...
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable2Type>
        fieldVariable = slotConnectorData->variable2[index].values;
...
template <typename SlotConnectorDataType>
std::string SlotConnectorDataHelper<SlotConnectorDataType>::getMeshName(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData, int slotNo) {
  if (!slotConnectorData)
    return std::string();
  int nSlotsVariable1 = slotConnectorData->variable1.size();
...
template <typename SlotConnectorDataType>
bool SlotConnectorDataHelper<SlotConnectorDataType>::slotSetValues(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData, int slotNo,
    int arrayIndex, const std::vector<dof_no_t> &dofNosLocal,
...
template <typename SlotConnectorDataType>
void SlotConnectorDataHelper<SlotConnectorDataType>::slotGetValues(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData, int slotNo,
    int arrayIndex, const std::vector<dof_no_t> &dofNosLocal,
  ...
  if (slotNo < nSlotsVariable1) {
    ...
    LOG(DEBUG) << "slot " << slotNo << ": from fieldVariable \""
               << fieldVariable->name() << "\", component " << componentNo
...
template <typename SlotConnectorDataType>
void SlotConnectorDataHelper<SlotConnectorDataType>::slotSetGeometryValues(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData, int slotNo,
    int arrayIndex, const std::vector<dof_no_t> &dofNosLocal,
  ...
  using FunctionSpaceType =
      typename SlotConnectorDataType::FieldVariable1Type::FunctionSpace;
  ...
  if (slotNo < nSlotsVariable1) {
    ...
    // add the geometry field in the slot connector data, such that it will be
    // automatically transferred to the connected slots
    slotConnectorData->addGeometryField(std::make_shared<GeometryFieldType>(
        fieldVariable->functionSpace()->geometryField()));
  } else {
    // if the slot no corresponds to a field variables stored under variable2
    int index = slotNo - nSlotsVariable1;
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable2Type>
...
//! variable given by slotNo
template <typename SlotConnectorDataType>
void SlotConnectorDataHelper<SlotConnectorDataType>::slotGetGeometryValues(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData, int slotNo,
    int arrayIndex, const std::vector<dof_no_t> &dofNosLocal,
...
template <typename SlotConnectorDataType>
void SlotConnectorDataHelper<SlotConnectorDataType>::getSlotNames(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData,
    std::vector<std::string> &slotNames) {
  if (!slotConnectorData)
    return;

  slotNames.insert(slotNames.end(), slotConnectorData->slotNames.begin(),
                   slotConnectorData->slotNames.end());
}

//! get a string representation of the slot connector data for debugging
template <typename SlotConnectorDataType>
std::string SlotConnectorDataHelper<SlotConnectorDataType>::getString(
    std::shared_ptr<SlotConnectorDataType> slotConnectorData) {
  if (!slotConnectorData)
    return std::string("x");
