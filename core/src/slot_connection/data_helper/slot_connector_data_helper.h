...
/** Helper class that gives the number of connector slots and allows to access
 * the slots' field variables. `arrayIndex` refers to the item of an array,
 * e.g., resulting from MultipleInstances (e.g., multiple fibers)
 */
template <typename SlotConnectorDataType> struct SlotConnectorDataHelper {
  ...
  static std::shared_ptr<Partition::MeshPartitionBase>
  getMeshPartitionBase(std::shared_ptr<SlotConnectorDataType> slotConnectorData,
                       int slotNo, int arrayIndex);

  //! get the mesh name of the field variable at the slot
  static std::string
  getMeshName(std::shared_ptr<SlotConnectorDataType> slotConnectorData,
              int slotNo);

  //! set the values at given dofs at the field variable given by slotNo
  static bool
  slotSetValues(std::shared_ptr<SlotConnectorDataType> slotConnectorData,
                int slotNo, int arrayIndex,
  ...
  static void
  slotGetValues(std::shared_ptr<SlotConnectorDataType> slotConnectorData,
                int slotNo, int arrayIndex,
                const std::vector<dof_no_t> &dofNosLocal,
  ...
  static void
  getSlotNames(std::shared_ptr<SlotConnectorDataType> slotConnectorData,
...
struct SlotConnectorDataHelper<
  ...
  //! get the number of slots
  static int
  nSlots(std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>
             slotConnectorData);

  //! get the number of items if the slotConnector is organized in an array,
  //! `arrayIndex` can then be chosen in [0,nArrayItems]
  static int nArrayItems(
  ...
  //! a, b, [(c,d),(c,d),(c,d)], e -> [a, b, c, d, e] and not [a, b, c, d, c, d,
  //! c, d, e]
  ...
  //! get a string representation of the slot connector data for debugging
  static std::string
  getString(std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>
                slotConnectorData);
};

template <typename SlotConnectorDataType>
struct SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>> {
  //! get the number of slots
  static int nSlots(
...
struct SlotConnectorDataHelper<
  ...
  //! get a string representation of the slot connector data for debugging
  static std::string
  getString(std::shared_ptr<std::tuple<std::shared_ptr<SlotConnectorDataType1>,
                                       std::shared_ptr<SlotConnectorDataType2>>>
                slotConnectorData);
};
