...
int SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    nSlots(std::shared_ptr<std::vector<std::shared_ptr<
               std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
               slotConnectorData) {
...
int SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    nArrayItems(std::shared_ptr<std::vector<std::shared_ptr<
                    std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
                    slotConnectorData,
                int slotNo) {
  if (!slotConnectorData)
...
SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
  ...
  int nSlotsVariable1 =
      (*(*slotConnectorData)[arrayIndex1])[arrayIndex2]->variable1.size();

  // if the slot no corresponds to a field variables stored under variable1
  if (slotNo < nSlotsVariable1) {
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable1Type>
        fieldVariable =
    ...
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable2Type>
        fieldVariable =
...
template <typename SlotConnectorDataType>
std::string SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    getMeshName(std::shared_ptr<std::vector<std::shared_ptr<
                    std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
                    slotConnectorData,
                int slotNo) {
  if (!slotConnectorData)
  ...
  if (slotNo < nSlotsVariable1) {
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable1Type>
        fieldVariable = (*(*slotConnectorData)[0])->variable1[slotNo].values;
    ...
    int index = slotNo - nSlotsVariable1;
    std::shared_ptr<typename SlotConnectorDataType::FieldVariable2Type>
        fieldVariable = (*(*slotConnectorData)[0])->variable2[index].values;
...
template <typename SlotConnectorDataType>
bool SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    slotSetValues(std::shared_ptr<std::vector<std::shared_ptr<
                      std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
                      slotConnectorData,
                  int slotNo, int arrayIndex,
  ...
  if (slotNo < nSlotsVariable1) {
    ...
    int componentNo =
        (*(*slotConnectorData)[arrayIndex1])->variable1[slotNo].componentNo;
    ...
    int componentNo =
        (*(*slotConnectorData)[arrayIndex1])->variable2[index].componentNo;
...
template <typename SlotConnectorDataType>
void SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    slotGetValues(std::shared_ptr<std::vector<std::shared_ptr<
                      std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
                      slotConnectorData,
                  int slotNo, int arrayIndex,
                  const std::vector<dof_no_t> &dofNosLocal,
...
void SlotConnectorDataHelper<std::vector<
  ...
  if (!slotConnectorData) {
    LOG(DEBUG)
        << "slotSetGeometryValues (vector2), slotConnectorData is not set";
    return;
  }
  if (slotConnectorData->empty()) {
  ...
  if (arrayIndex >= sizeFirstVector * sizeSecondVector) {
    LOG(DEBUG) << "slotSetGeometryValues (vector2), size: " << sizeFirstVector
               << "*" << sizeSecondVector << " = "
               << sizeFirstVector * sizeSecondVector << " <= arrayIndex "
               << arrayIndex;
    return;
  }
  ...
  if (slotNo < nSlotsVariable1) {
    ...
    fieldVariable->functionSpace()->geometryField().setValues(dofNosLocal,
                                                              values);
  } else {
    // if the slot no corresponds to a field variables stored under variable2
    int index = slotNo - nSlotsVariable1;
...
void SlotConnectorDataHelper<std::vector<
  ...
  if (!slotConnectorData) {
    LOG(DEBUG)
        << "slotGetGeometryValues (vector2), slotConnectorData is not set";
    return;
  }
  if (slotConnectorData->empty()) {
  ...
  if (arrayIndex >= sizeFirstVector * sizeSecondVector) {
    LOG(DEBUG) << "slotGetGeometryValues (vector2), size: " << sizeFirstVector
               << "*" << sizeSecondVector << " = "
               << sizeFirstVector * sizeSecondVector << " <= arrayIndex "
               << arrayIndex;
    return;
  ...
  if (slotNo < nSlotsVariable1) {
    ...
    // get the actual values
    fieldVariable->functionSpace()->geometryField().getValues(dofNosLocal,
                                                              values);
...
void SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    getSlotNames(std::shared_ptr<std::vector<std::shared_ptr<
                     std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
                     slotConnectorData,
  ...
  if (!(*slotConnectorData)[0]->empty()) {
    // only collect the slot names from the first vector item, because all items
    // should have the same slots
    SlotConnectorDataHelper<SlotConnectorDataType>::getSlotNames(
        (*(*slotConnectorData)[0])[0], slotNames);
  }
}
...
std::string SlotConnectorDataHelper<std::vector<
    std::shared_ptr<std::vector<std::shared_ptr<SlotConnectorDataType>>>>>::
    getString(std::shared_ptr<std::vector<std::shared_ptr<
                  std::vector<std::shared_ptr<SlotConnectorDataType>>>>>
                  slotConnectorData) {
  if (!slotConnectorData)
    return std::string("[[x]]");
    ...
    if (!(*slotConnectorData)[0]->empty()) {
      ...
      for (int j = 0; j < slotConnectorData->size(); j++) {
        ...
        for (int i = 0; i < (*slotConnectorData)[0]->size(); i++) {
          if (i != 0)
            s << ", ";
          s << SlotConnectorDataHelper<SlotConnectorDataType>::getString(
              (*(*slotConnectorData)[0])[0]);
        }
        s << "]";
      }
      s << "]";
