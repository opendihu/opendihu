...
int SlotConnectorDataHelper<
  ...
  if (!slotConnectorData->empty())
    return SlotConnectorDataHelper<SlotConnectorDataType>::nSlots(
        (*slotConnectorData)[0]);
  return 0;
}
...
SlotConnectorDataHelper<std::vector<std::shared_ptr<SlotConnectorDataType>>>::
  ...
  return SlotConnectorDataHelper<SlotConnectorDataType>::getMeshPartitionBase(
      (*slotConnectorData)[itemNo], slotNo, arrayIndexInItem);
}
...
SlotConnectorDataHelper<std::vector<std::shared_ptr<SlotConnectorDataType>>>::
  ...
  return SlotConnectorDataHelper<SlotConnectorDataType>::getMeshName(
      (*slotConnectorData)[0], slotNo);
}
...
bool SlotConnectorDataHelper<
  ...
  return SlotConnectorDataHelper<SlotConnectorDataType>::slotSetValues(
      (*slotConnectorData)[itemNo], slotNo, arrayIndexInItem, dofNosLocal,
...
void SlotConnectorDataHelper<
  ...
  if (!slotConnectorData) {
    LOG(DEBUG)
        << "slotSetGeometryValues (vector), slotConnectorData is not set";
    return;
  }
  if (slotConnectorData->empty()) {
...
void SlotConnectorDataHelper<
  ...
  if (!slotConnectorData) {
    LOG(DEBUG)
        << "slotGetGeometryValues (vector), slotConnectorData is not set";
    return;
  }
  if (slotConnectorData->empty()) {
  ...
  LOG(DEBUG) << "slotGetGeometryValues on item " << arrayIndex << " -> "
             << arrayIndexInItem << "(" << nArrayItemsPerItem << ")";
...
void SlotConnectorDataHelper<
  ...
  SlotConnectorDataHelper<SlotConnectorDataType>::slotSetRepresentationGlobal(
      (*slotConnectorData)[itemNo], slotNo, arrayIndexInItem);
}
...
void SlotConnectorDataHelper<
  ...
  if (!slotConnectorData->empty()) {
    // only collect the slot names from the first item of the vector, as they
    // should be all the same
    SlotConnectorDataHelper<SlotConnectorDataType>::getSlotNames(
        (*slotConnectorData)[0], slotNames);
  }
}
