...
void GlobalConnectionsBySlotName::addConnections(
  ...
  if (!slotConnectorData) {
    LOG(ERROR) << "In GlobalConnectionsBySlotName::addConnections, "
  ...
  SlotConnectorDataHelper<SlotConnectorDataType1>::getSlotNames(
      std::get<0>(*slotConnectorData), slotNamesTerm1);
  SlotConnectorDataHelper<SlotConnectorDataType2>::getSlotNames(
      std::get<1>(*slotConnectorData), slotNamesTerm2);
  ...
  for (std::pair<std::string, std::string> connectedSlotNames : connections_) {
    ...
    std::vector<std::string>::iterator iterFrom = std::find(
        slotNamesTerm1.begin(), slotNamesTerm1.end(), connectedSlotNames.first);
    std::vector<std::string>::iterator iterTo =
        std::find(slotNamesTerm2.begin(), slotNamesTerm2.end(),
                  connectedSlotNames.second);

    LOG(DEBUG) << "connectedSlotNames " << connectedSlotNames
               << " slotNamesTerm1: " << slotNamesTerm1
               << ", slotNamesTerm2: " << slotNamesTerm2;
    LOG(DEBUG) << "  Term1->Term2, Term1 found: "
               << (iterFrom != slotNamesTerm1.end())
               << ", Term2 found: " << (iterTo != slotNamesTerm2.end());
    ...
    if (iterFrom != slotNamesTerm1.end() && iterTo != slotNamesTerm2.end()) {
      ...
      LOG(DEBUG) << "Term1->Term2, add connectionn for slots " << slotNoFrom
                 << " -> " << slotNoTo;
    ...
    // determine slot no.s for current slotConnectorData
    iterFrom = std::find(slotNamesTerm2.begin(), slotNamesTerm2.end(),
                         connectedSlotNames.first);
    iterTo = std::find(slotNamesTerm1.begin(), slotNamesTerm1.end(),
                       connectedSlotNames.second);

    LOG(DEBUG) << "  Term2->Term1, Term2 found: "
               << (iterFrom != slotNamesTerm2.end())
               << ", Term1 found: " << (iterTo != slotNamesTerm1.end());
    ...
    if (iterFrom != slotNamesTerm2.end() && iterTo != slotNamesTerm1.end()) {
      ...
      LOG(DEBUG) << "Term2->Term1, add connectionn for slots " << slotNoFrom
                 << " -> " << slotNoTo;
  ...
  // check for slots that have the same slot name in both Term1 and Term2, these
  // will also be connected
  for (int slotNoTerm1 = 0; slotNoTerm1 < slotNamesTerm1.size();
       slotNoTerm1++) {
    ...
    std::vector<std::string>::iterator iter =
        std::find(slotNamesTerm2.begin(), slotNamesTerm2.end(), slotNameTerm1);

    if (iter != slotNamesTerm2.end()) {
      int slotNoTerm2 = std::distance(slotNamesTerm2.begin(), iter);

      LOG(DEBUG) << "add two-way connections because of matching slot name "
                 << slotNameTerm1;

      // if connection does not yet exist
      if (std::find(connectionsFromSameSlotNames_.begin(),
                    connectionsFromSameSlotNames_.end(),
                    slotNameTerm1) == connectionsFromSameSlotNames_.end()) {
