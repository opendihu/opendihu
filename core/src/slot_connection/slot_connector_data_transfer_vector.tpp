...
void SlotConnectorDataTransfer<
  ...
  VLOG(1)
      << "Solution vector mapping (slot_connector_data_transfer_vector.tpp)";
  int nTransferableVariables = std::min(transferableSolutionData1->size(),
  ...
  if (transferableSolutionData1->size() != transferableSolutionData2->size()) {
    LOG(ERROR) << "Trying to transfer data from "
               ...
               << StringUtility::demangle(
                      typeid(SlotConnectorDataType2).name());
  }

  // loop over the two vectors A and B and transfer A[i] -> B[i] for every
  // component
  for (int i = 0; i < nTransferableVariables; i++) {
    SlotConnectorDataTransfer<SlotConnectorDataType1, SlotConnectorDataType2>::
        transfer((*transferableSolutionData1)[i],
                 (*transferableSolutionData2)[i], slotsConnection,
                 offsetSlotNoData1, offsetSlotNoData2);
  }

  VLOG(1) << "at the end of std::vector transfer: transferableSolutionData2: "
          << (*transferableSolutionData2);
}
...
void SlotConnectorDataTransfer<
  ...
  LOG(DEBUG) << "transferableSolutionData1: "
             << SlotConnectorDataHelper<
                    ...
                    getString(transferableSolutionData1);
  LOG(DEBUG) << "transferableSolutionData2: "
             << SlotConnectorDataHelper<Data::SlotConnectorData<
                    FunctionSpaceType2, nComponents2a,
                    nComponents2b>>::getString(transferableSolutionData2);
#endif
  ...
  if (transferableSolutionData1->empty()) {
    LOG(ERROR)
        << "Trying to transfer data from an empty entry to a single vector. "
           "Types: "
        << std::endl
        << StringUtility::demangle(
               typeid(Data::SlotConnectorData<FunctionSpaceType1, nComponents1a,
                                              nComponents1b>)
                   .name())
        << std::endl
        << StringUtility::demangle(
               typeid(Data::SlotConnectorData<FunctionSpaceType2, nComponents2a,
                                              nComponents2b>)
                   .name());
    return;
  }
   ...
   * vector<field variables>. This results e.g. from multiple fibers.
   *  "transferableSolutionData2" is just normal field variables without a
   * vector. Each object of type SlotConnectorData contains possibly two
  ...
  */
  ...
  std::shared_ptr<
      ...
      transferableSolutionData1Front = (*transferableSolutionData1)[0];
  ...
  for (int i = 0; i < transferableSolutionData1Front->variable1.size(); i++) {
    ...
    if (toVectorNo == 0) {
      ...
      for (int fiberIndex = 0; fiberIndex < transferableSolutionData1->size();
           fiberIndex++) {
        ...
        typedef FieldVariable::FieldVariable<FunctionSpaceType1, nComponents1a>
            FieldVariable1;
        std::shared_ptr<FieldVariable1> fieldVariable1 =
            (*transferableSolutionData1)[fiberIndex]
                ->variable1[fromVectorIndex]
                .values;
        int componentNo1 = (*transferableSolutionData1)[fiberIndex]
                               ->variable1[fromVectorIndex]
                               .componentNo;
...
void SlotConnectorDataTransfer<
  ...
  LOG(DEBUG) << "transferableSolutionData1: "
             << SlotConnectorDataHelper<Data::SlotConnectorData<
                    FunctionSpaceType1, nComponents1a,
                    nComponents1b>>::getString(transferableSolutionData1);
  LOG(DEBUG) << "transferableSolutionData2: "
             << SlotConnectorDataHelper<
                    std::vector<std::shared_ptr<Data::SlotConnectorData<
                        FunctionSpaceType2, nComponents2a, nComponents2b>>>>::
  ...
  if (transferableSolutionData2->empty()) {
    LOG(ERROR)
        << "Trying to transfer data from a single entry to an empty vector. "
           "Types: "
        << std::endl
        << StringUtility::demangle(
               typeid(Data::SlotConnectorData<FunctionSpaceType1, nComponents1a,
        ...
        << std::endl
        << StringUtility::demangle(
               typeid(std::vector<std::shared_ptr<Data::SlotConnectorData<
                          FunctionSpaceType2, nComponents2a, nComponents2b>>>)
                   .name());
    return;
  }
  ...
  // the following is analogous to the code in
  // slot_connector_data_transfer_fibers_emg.tpp where there are two nested
  // vectors, here it is only one

  assert(transferableSolutionData2->size() > 0);

  std::shared_ptr<
      Data::SlotConnectorData<FunctionSpaceType2, nComponents2a, nComponents2b>>
      transferableSolutionData2Front = (*transferableSolutionData2)[0];
  ...
  for (int i = 0; i < transferableSolutionData1->variable1.size(); i++) {
    ...
    if (toVectorNo == 0) {
      ...
      // loop over vector of fibers
      for (int fiberIndex = 0; fiberIndex < transferableSolutionData2->size();
           fiberIndex++) {
        // map from transferableSolutionData1->variable1 to
        // transferableSolutionData2->variable1
        ...
        typedef FieldVariable::FieldVariable<FunctionSpaceType2, nComponents2a>
            FieldVariable2;
        std::shared_ptr<FieldVariable2> &fieldVariable2 =
            (*transferableSolutionData2)[fiberIndex]
                ->variable1[fromVectorIndex]
                .values;
        int componentNo2 = (*transferableSolutionData2)[fiberIndex]
                               ->variable1[fromVectorIndex]
                               .componentNo;
