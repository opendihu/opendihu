...
GlobalConnectionsBySlotName::GlobalConnectionsBySlotName(
    PythonConfig settings) {
  // parse the connectedSlots
  if (settings.hasKey("connectedSlots")) {
    settings.getOptionVector<std::pair<std::string, std::string>>(
        "connectedSlots", connections_);
  }
}
...
std::string GlobalConnectionsBySlotName::getDescriptionForDiagram() {
  ...
  if (!connections_.empty()) {
    s << "The following data slot connection were given by the setting "
         "\"connectedSlots\":\n";
    for (std::vector<std::pair<std::string, std::string>>::iterator iter =
             connections_.begin();
      ...
      for (std::vector<std::pair<std::string, std::string>>::iterator iter2 =
               connections_.begin();
           iter2 != connections_.end(); iter2++) {
      ...
      if (fromSlotName.length() < 6) {
        fromSlotName =
            std::string(6 - fromSlotName.length(), ' ') + fromSlotName;
      }
  ...
  if (!connectionsFromSameSlotNames_.empty()) {
    s << "The following data slots were connected because the names appeared "
         "in both terms of a coupling or splitting scheme:\n";
