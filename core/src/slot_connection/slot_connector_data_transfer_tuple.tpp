...
/** Transfer between a tuple and another field variables with given component
 * number
 */
template <typename SlotConnectorDataType1, typename SlotConnectorDataType2,
          typename SlotConnectorDataType3>
void SlotConnectorDataTransfer<
  ...
  /* /!\ always use subSlotsConnection1 for slot argument and
     ...
     wrong */
  LOG(DEBUG) << "transfer tuple (1a), offsetSlotNoData1: " << offsetSlotNoData1
             << ", offsetSlotNoData2: " << offsetSlotNoData2;
  ...
  LOG(DEBUG) << "transferableSolutionData2: "
             << SlotConnectorDataHelper<SlotConnectorDataType3>::getString(
                    transferableSolutionData3);
#endif

  // copy slotsConnection for second tuple
  if (!slotsConnection.subSlotsConnection1()) {
    slotsConnection.subSlotsConnection1() =
        std::make_shared<SlotsConnection>(slotsConnection);
  }

  // do transfer with slots in first tuple part
  SlotConnectorDataTransfer<SlotConnectorDataType1, SlotConnectorDataType3>::
      transfer(std::get<0>(*transferableSolutionData1),
               transferableSolutionData3,
               *slotsConnection.subSlotsConnection1(), offsetSlotNoData1,
               offsetSlotNoData2);

  LOG(DEBUG) << "transfer tuple (1b), offsetSlotNoData1: " << offsetSlotNoData1
             << ", offsetSlotNoData2: " << offsetSlotNoData2;
#ifdef SOLUTION_VECTOR_MAPPING_DEBUGGING_OUTPUT
  LOG(DEBUG) << "transferableSolutionData1: "
             << SlotConnectorDataHelper<
  ...
  if (!slotsConnection.subSlotsConnection2()) {
    slotsConnection.subSlotsConnection2() =
        std::make_shared<SlotsConnection>(slotsConnection);
  }

  // first offset changes by number of slots that were present in the first
  // tuple entry
  offsetSlotNoData1 += SlotConnectorDataHelper<SlotConnectorDataType1>::nSlots(
      std::get<0>(*transferableSolutionData1));

  // second offset does not change
  offsetSlotNoData2 += 0;

  // do transfer with slots in second tuple part
  SlotConnectorDataTransfer<SlotConnectorDataType2, SlotConnectorDataType3>::
      transfer(std::get<1>(*transferableSolutionData1),
               transferableSolutionData3,
               *slotsConnection.subSlotsConnection2(), offsetSlotNoData1,
               offsetSlotNoData2);
}
...
void SlotConnectorDataTransfer<
    ...
    std::tuple<std::shared_ptr<SlotConnectorDataType2>,
               std::shared_ptr<SlotConnectorDataType3>>>::
    transfer(
        const std::shared_ptr<SlotConnectorDataType1> transferableSolutionData1,
        std::shared_ptr<std::tuple<std::shared_ptr<SlotConnectorDataType2>,
                                   std::shared_ptr<SlotConnectorDataType3>>>
  ...
  // do transfer with slots in first tuple part
  SlotConnectorDataTransfer<SlotConnectorDataType1, SlotConnectorDataType2>::
      transfer(transferableSolutionData1,
               std::get<0>(*transferableSolutionData2),
               *slotsConnection.subSlotsConnection1(), offsetSlotNoData1,
               offsetSlotNoData2);
  ...
  // second offset changes by number of slots that were present in the first
  // tuple entry
  offsetSlotNoData2 += SlotConnectorDataHelper<SlotConnectorDataType2>::nSlots(
      std::get<0>(*transferableSolutionData2));

  // do transfer with slots in second tuple part
  SlotConnectorDataTransfer<SlotConnectorDataType1, SlotConnectorDataType3>::
      transfer(transferableSolutionData1,
               std::get<1>(*transferableSolutionData2),
               *slotsConnection.subSlotsConnection2(), offsetSlotNoData1,
               offsetSlotNoData2);
}

template <typename SlotConnectorDataType1, typename SlotConnectorDataType2,
          typename SlotConnectorDataType3, typename SlotConnectorDataType4>
void SlotConnectorDataTransfer<
    std::tuple<std::shared_ptr<SlotConnectorDataType1>,
               std::shared_ptr<SlotConnectorDataType2>>,
    std::tuple<std::shared_ptr<SlotConnectorDataType3>,
    ...
    transfer(
        const std::shared_ptr<
            std::tuple<std::shared_ptr<SlotConnectorDataType1>,
                       std::shared_ptr<SlotConnectorDataType2>>>
            transferableSolutionData1,
        std::shared_ptr<std::tuple<std::shared_ptr<SlotConnectorDataType3>,
                                   std::shared_ptr<SlotConnectorDataType4>>>
            transferableSolutionData2,
        SlotsConnection &slotsConnection, int offsetSlotNoData1,
        int offsetSlotNoData2) {
  ...
  // second offset changes by number of slots that were present in the first
  // tuple entry
  offsetSlotNoData2 += SlotConnectorDataHelper<SlotConnectorDataType3>::nSlots(
      std::get<0>(*transferableSolutionData2));
  ...
  if (originalTransferDirection) {
    ...
    SlotConnectorDataTransfer<SlotConnectorDataType1, SlotConnectorDataType4>::
        transfer(std::get<0>(*transferableSolutionData1),
                 std::get<1>(*transferableSolutionData2),
                 *slotsConnection.subSlotsConnection2(), offsetSlotNoData1,
                 offsetSlotNoData2);
    ...
    if (!slotsConnection.subSlotsConnection3()) {
      slotsConnection.subSlotsConnection3() =
          std::make_shared<SlotsConnection>(slotsConnection);
    }
    ...
    SlotConnectorDataTransfer<SlotConnectorDataType1, SlotConnectorDataType4>::
        transfer(std::get<0>(*transferableSolutionData1),
                 std::get<1>(*transferableSolutionData2),
                 *slotsConnection.subSlotsConnection3(), offsetSlotNoData1,
                 offsetSlotNoData2);
  }
  ...
  // first offset changes by number of slots that were present in the first
  // tuple entry
  offsetSlotNoData1 = initialOffsetSlotNoData1 +
  ...
  if (!slotsConnection.subSlotsConnection4()) {
    slotsConnection.subSlotsConnection4() =
        std::make_shared<SlotsConnection>(slotsConnection);
  }

  // do transfer with slots in second tuple part
  SlotConnectorDataTransfer<SlotConnectorDataType2, SlotConnectorDataType4>::
      transfer(std::get<1>(*transferableSolutionData1),
               std::get<1>(*transferableSolutionData2),
               *slotsConnection.subSlotsConnection4(), offsetSlotNoData1,
               offsetSlotNoData2);
}
