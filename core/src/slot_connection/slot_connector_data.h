 ...
 *  This is the data that will be transferred to connected classes in the nested
 * structure.
 */
...
struct ComponentOfFieldVariable {
  ...
  void setValue(dof_no_t dofNoLocal, double value,
...
/** This is a general data type that can contain multiple field variables with
 * designated components. They are used for the slotConnector of different
 * solvers. This particular struct is, for example, used for the CellMLAdapter,
 * where the two variables store states and algebraics to be passed to the next
 * solver.
 */
...
struct SlotConnectorData {
  ...
  typedef FieldVariable::FieldVariable<FunctionSpaceType, nComponents1>
      FieldVariable1Type;
  typedef FieldVariable::FieldVariable<FunctionSpaceType, nComponents2>
      FieldVariable2Type;

  //! add a component of a field variable to the vector
  void addFieldVariable(
      std::shared_ptr<
  ...
  void addGeometryField(
      std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 3>>
          geometryField);

  //! get the number of slots that is contained in this SlotConnectorData, this
  //! is equal to variable1.size() + variable2.size()
  int nSlots();
};
...
std::ostream &operator<<(
    ...
    const std::shared_ptr<
        SlotConnectorData<FunctionSpaceType, nComponents1, nComponents2>> &rhs);
