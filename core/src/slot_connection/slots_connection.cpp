...
SlotsConnection::SlotsConnection(PythonConfig settings)
    : fieldVariableNamesInitialized_(false), transferDirectionTerm1To2_(true),
      slotInformationInitialized_(false), settings_(settings),
      subSlotsConnection1_(nullptr), subSlotsConnection2_(nullptr),
      subSlotsConnection3_(nullptr), subSlotsConnection4_(nullptr) {
  ...
  // set indices in connectorForVisualizerTerm1To2_ and
  // connectorForVisualizerTerm1To2_
  for (std::vector<int>::iterator iter = indicesTerm1To2.begin();
       iter != indicesTerm1To2.end(); iter++) {
    Connector connector;
  ...
  for (std::vector<int>::iterator iter = indicesTerm2To1.begin();
       iter != indicesTerm2To1.end(); iter++) {
    Connector connector;
    connector.index = *iter;
    connector.avoidCopyIfPossible = false;
...
void SlotsConnection::addConnectionTerm1ToTerm2(int slotNoFrom, int slotNoTo) {
  ...
  if (connectorForVisualizerTerm1To2_.size() > slotNoFrom) {
    if (connectorForVisualizerTerm1To2_[slotNoFrom].index == slotNoTo) {
      LOG(DEBUG) << "addConnectionTerm1ToTerm2(" << slotNoFrom << ","
                 << slotNoTo << "), is already contained";
  ...
  LOG(DEBUG) << "set new connection Term1->Term2 " << slotNoFrom << ":"
             << slotNoTo;
  ...
  for (int i = 0; i < connectorForVisualizerTerm1To2_.size(); i++) {
    s << "  " << std::setw(2) << i << "->" << std::setw(2)
      << connectorForVisualizerTerm1To2_[i].index;
...
void SlotsConnection::addConnectionTerm2ToTerm1(int slotNoFrom, int slotNoTo) {
  ...
  if (connectorForVisualizerTerm2To1_.size() > slotNoFrom) {
    if (connectorForVisualizerTerm2To1_[slotNoFrom].index == slotNoTo) {
      LOG(DEBUG) << "addConnectionTerm2ToTerm1(" << slotNoFrom << ","
                 << slotNoTo << "), is already contained";
  ...
  LOG(DEBUG) << "set new connection Term2->Term1 " << slotNoFrom << ":"
             << slotNoTo;
  ...
  for (int i = 0; i < connectorForVisualizerTerm2To1_.size(); i++) {
    s << "  " << std::setw(2) << i << "->" << std::setw(2)
      << connectorForVisualizerTerm2To1_[i].index;
...
void SlotsConnection::updateAvoidCopyIfPossible() {
  // if field variable gets mapped in both directions, set avoidCopyIfPossible
  // to true
  for (int i = 0; i < connectorForVisualizerTerm1To2_.size(); i++) {
...
const std::vector<SlotsConnection::Connector> &
SlotsConnection::connectorForVisualizerTerm1To2() const {
  return connectorForVisualizerTerm1To2_;
}

//! get the connectors from term 2 to term 1
const std::vector<SlotsConnection::Connector> &
SlotsConnection::connectorForVisualizerTerm2To1() const {
...
std::string SlotsConnection::getDebugInformation() const {
  ...
  int nSlotsTerm1 = fieldVariableNamesTerm1Vector1_.size() +
                    fieldVariableNamesTerm1Vector2_.size();
  ...
  for (int i = 0; i < fieldVariableNamesTerm1Vector1_.size(); i++, slotNo++) {
    result << "  slot " << slotNo << ": " << fieldVariableNamesTerm1Vector1_[i]
           << " (stored in variable1[" << i << "])" << std::endl;
  }
  for (int i = 0; i < fieldVariableNamesTerm1Vector2_.size(); i++, slotNo++) {
    result << "  slot " << slotNo << ": " << fieldVariableNamesTerm1Vector2_[i]
           << " (stored in variable2[" << i << "])" << std::endl;
  }

  // output slots of Term 2
  int nSlotsTerm2 = fieldVariableNamesTerm2Vector1_.size() +
                    fieldVariableNamesTerm2Vector2_.size();
  ...
  for (int i = 0; i < fieldVariableNamesTerm2Vector1_.size(); i++, slotNo++) {
    result << "  slot " << slotNo << ": " << fieldVariableNamesTerm2Vector1_[i]
           << " (stored in variable1[" << i << "])" << std::endl;
  }
  for (int i = 0; i < fieldVariableNamesTerm2Vector2_.size(); i++, slotNo++) {
    result << "  slot " << slotNo << ": " << fieldVariableNamesTerm2Vector2_[i]
           << " (stored in variable2[" << i << "])" << std::endl;
  }

  // output connections of Term 1
  result << "Term 1 has the following " << connectorTerm1To2_.size()
         << " connection" << (connectorTerm1To2_.size() == 1 ? "" : "s") << ":"
         << std::endl;
  ...
  for (int i = 0; i < connectorTerm1To2_.size(); i++) {
    ...
    if (vectorNo == 0) {
      if (vectorIndex >= 0 &&
          vectorIndex < fieldVariableNamesTerm1Vector1_.size()) {
      ...
      if (vectorIndex >= 0 &&
          vectorIndex < fieldVariableNamesTerm1Vector2_.size()) {
    ...
    if (connectorTerm1To2_[i].index == -1) {
      ...
      vectorNo =
          connectorTerm1To2_[i].index >= nFieldVariablesTerm2Vector1_ ? 1 : 0;
      vectorIndex =
          connectorTerm1To2_[i].index - vectorNo * nFieldVariablesTerm2Vector1_;

      result << "Term2.variable" << vectorNo + 1 << "[" << vectorIndex << "], ";

      if (vectorNo == 0) {
        if (vectorIndex >= 0 &&
            vectorIndex < fieldVariableNamesTerm2Vector1_.size()) {
        ...
        if (vectorIndex >= 0 &&
            vectorIndex < fieldVariableNamesTerm2Vector2_.size()) {
  ...
  result << "Term 2 has the following " << connectorTerm2To1_.size()
         << " connection" << (connectorTerm2To1_.size() == 1 ? "" : "s") << ":"
         << std::endl;
  ...
  for (int i = 0; i < connectorTerm2To1_.size(); i++) {
    ...
    if (connectorTerm2To1_[i].index == -1) {
      ...
      vectorNo =
          connectorTerm2To1_[i].index >= nFieldVariablesTerm1Vector1_ ? 1 : 0;
      vectorIndex =
          connectorTerm2To1_[i].index - vectorNo * nFieldVariablesTerm1Vector1_;
...
bool SlotsConnection::getSlotInformationUncached(
    int fromVectorNo, int fromVectorIndex, int &toVectorNo, int &toVectorIndex,
    bool &avoidCopyIfPossible, bool disableWarnings) const {

  disableWarnings =
      true; // do not show warnings, they would also appear if
            // SlotsConnectionDataType is a tuple, this is the case for MapDofs
  ...
  if (transferDirectionTerm1To2_) {
    ...
    if (fromIndex >= connectorTerm1To2_.size()) {
      if (!disableWarnings)
        LOG(DEBUG)
            << "(0) Unconnected slot " << fromIndex << " of Term1 in "
            << settings_ << ", fromIndex (" << fromIndex << "=" << fromVectorNo
            << "*" << nFieldVariablesTerm1Vector1_ << "+" << fromVectorIndex
            << ") >= " << connectorTerm1To2_.size() << "\nThere are only "
            << connectorTerm1To2_.size()
            << " slots to connect to, but a connection for slot no "
            << fromIndex << " is needed."
            << "\nMaybe not enough entries are given for "
               "\"connectedSlotsTerm1To2\" or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
               "cellml that are not used further)."
            << getDebugInformation()
    ...
    if (fromVectorNo == 0 && fromVectorIndex >= nFieldVariablesTerm1Vector1_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(1) Unconnected slot " << fromIndex << " of Term1 in "
            << settings_ << ", from vector 0 (variable1), index "
            << fromVectorIndex << ", but this vector has only "
            << nFieldVariablesTerm1Vector1_ << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
               "cellml that are not used further)."
            << getDebugInformation()
            << DihuContext::solverStructureVisualizer()->getDiagram();
      return false;
    } else if (fromVectorNo == 1 &&
               fromVectorIndex >= nFieldVariablesTerm1Vector2_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(2) Unconnected slot " << fromIndex << " of Term1 in "
            << settings_ << ", from vector 1 (variable2), index "
            << fromVectorIndex << ", but this vector has only "
            << nFieldVariablesTerm1Vector2_ << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
               "cellml that are not used further)."
    ...
    // if toIndex is set to None in the python config, this means we do not want
    // to have a connection in this slot and it is no warning
    if (toIndex <= -1) {
      return false;
    }
    ...
    if (toVectorNo == 0 && toVectorIndex >= nFieldVariablesTerm2Vector1_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(3) Unconnected slot " << toIndex << " of Term2 in "
            << settings_ << ", from vector " << fromVectorNo << " (variable"
            << fromVectorNo + 1 << "), index " << fromVectorIndex
            << " to vector 0 (variable1), index " << toVectorIndex
            << ", but this vector has only " << nFieldVariablesTerm2Vector1_
            << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
            ...
            << DihuContext::solverStructureVisualizer()->getDiagram();
      return false;
    } else if (toVectorNo == 1 &&
               toVectorIndex >= nFieldVariablesTerm2Vector2_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(4) Unconnected slot " << toIndex << " of Term2 in "
            << settings_ << ", from vector " << fromVectorNo << " (variable"
            << fromVectorNo + 1 << "), index " << fromVectorIndex
            << " to vector 1 (variable2), index " << toVectorIndex
            << ", but this vector has only " << nFieldVariablesTerm2Vector2_
            << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
    ...
    if (fromIndex >= connectorTerm2To1_.size()) {
      if (!disableWarnings)
        LOG(DEBUG)
            << "(5) Unconnected slot " << fromIndex << " of Term2 in "
            << settings_ << ", fromIndex (" << fromIndex
            << ") >= " << connectorTerm2To1_.size() << "\nThere are only "
            << connectorTerm2To1_.size()
            << " slots to connect from, but a connection from slot no "
            << fromIndex << " is needed."
            << "\nMaybe not enough entries are given for "
               "\"connectedSlotsTerm1To2\" or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
    ...
    if (fromVectorNo == 0 && fromVectorIndex >= nFieldVariablesTerm2Vector1_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(6) Unconnected slot " << fromIndex << " of Term2 in "
            << settings_ << ", from vector 0 (variable1), index "
            << fromVectorIndex << ", but this vector has only "
            << nFieldVariablesTerm2Vector1_ << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
               "cellml that are not used further)."
      ...
      return false;
               ...
               fromVectorIndex >= nFieldVariablesTerm2Vector2_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(7) Unconnected slot " << fromIndex << " of Term2 in "
            << settings_ << ", from vector 1 (variable2), index "
            << fromVectorIndex << ", but this vector has only "
            << nFieldVariablesTerm2Vector2_ << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
               "cellml that are not used further)."
    ...
    if (toVectorNo == 0 && toVectorIndex >= nFieldVariablesTerm1Vector1_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(8) Unconnected slot " << toIndex << " of Term1 in "
            << settings_ << ", from vector " << fromVectorNo << " (variable"
            << fromVectorNo + 1 << "), index " << fromVectorIndex
            << " to vector 0 (variable1), index " << toVectorIndex
            << ", but this vector has only " << nFieldVariablesTerm1Vector1_
            << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
      ...
      return false;
               ...
               toVectorIndex >= nFieldVariablesTerm1Vector2_) {
      if (!disableWarnings)
        LOG(WARNING)
            << "(9) Unconnected slot " << toIndex << " of Term1 in "
            << settings_ << ", from vector " << fromVectorNo << " (variable"
            << fromVectorNo + 1 << "), index " << fromVectorIndex
            << " to vector 1 (variable2), index " << toVectorIndex
            << ", but this vector has only " << nFieldVariablesTerm1Vector2_
            << " entries." << std::endl
            << "\nMaybe wrong numbers are given for \"connectedSlotsTerm1To2\" "
               "or \"connectedSlotsTerm2To1\" "
            << "or unneccessary slots have been created (e.g. algebraics in "
...
bool SlotsConnection::getSlotInformation(int fromVectorNo, int fromVectorIndex,
...
#ifndef NDEBUG
  VLOG(1) << "getSlotInformation(" << fromVectorNo << "," << fromVectorIndex
          << ")" << getDebugInformation();
  LOG(DEBUG) << "getSlotInformation(" << fromVectorNo << "," << fromVectorIndex
  ...
  if (slotInformationInitialized_) {
    ...
    bool check_success = getSlotInformationUncached(
        fromVectorNo, fromVectorIndex, check_toVectorNo, check_toVectorIndex,
        check_avoidCopyIfPossible, disableWarnings);
#endif

    if (fromVectorIndex >=
        slotInformation_[transferDirectionTerm1To2_][fromVectorNo].size()) {
#ifndef NDEBUG
      if (check_success != false) {
        LOG(FATAL)
            << "lookup table does not match recomputed getSlotInformation: ("
            << &slotInformation_
            << ")\n"
               "  slotInformation_["
            << (transferDirectionTerm1To2_ ? "1->2" : "2->1") << "]["
            << fromVectorNo << "][" << fromVectorIndex
            << "]:\n"
               "    return:     "
            << false
            << " (fromVectorIndex is too large)\n"
               "  getSlotInformationUncached("
            << fromVectorNo << "," << fromVectorIndex
            << "):\n"
               "    successful: "
            << check_success << "\n"
    ...
    const Result &result = slotInformation_[transferDirectionTerm1To2_]
                                           [fromVectorNo][fromVectorIndex];
    ...
    return getSlotInformationUncached(fromVectorNo, fromVectorIndex, toVectorNo,
