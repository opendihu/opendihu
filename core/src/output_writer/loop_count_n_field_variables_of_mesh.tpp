...
template <typename FieldVariablesForOutputWriterType, int i>
    ...
    loopCountNFieldVariablesOfMesh(
        const FieldVariablesForOutputWriterType &fieldVariables,
        std::string meshName, int &nFieldVariablesOfMesh) {
  // call what to do in the loop body
  if (countNFieldVariablesOfMesh<typename std::tuple_element<
          i, FieldVariablesForOutputWriterType>::type>(
          std::get<i>(fieldVariables), meshName, nFieldVariablesOfMesh))
    return;

  // advance iteration to next tuple element
  loopCountNFieldVariablesOfMesh<FieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, meshName, nFieldVariablesOfMesh);
}
...
typename std::enable_if<
    ...
    bool>::type
countNFieldVariablesOfMesh(CurrentFieldVariableType currentFieldVariable,
                           std::string meshName, int &nFieldVariablesOfMesh) {
  VLOG(2) << "count number of field variables in type "
          << StringUtility::demangle(typeid(CurrentFieldVariableType).name())
...
countNFieldVariablesOfMesh(VectorType currentFieldVariableGradient,
                           std::string meshName, int &nFieldVariablesOfMesh) {
  VLOG(2) << "count number of field variables in vector with size "
          << currentFieldVariableGradient.size();
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    // call function on all vector entries
    if (countNFieldVariablesOfMesh<typename VectorType::value_type>(
            currentFieldVariable, meshName, nFieldVariablesOfMesh))
      return true;
  }
...
countNFieldVariablesOfMesh(TupleType currentFieldVariableTuple,
                           std::string meshName, int &nFieldVariablesOfMesh) {
  // call for tuple element
  loopCountNFieldVariablesOfMesh<TupleType>(currentFieldVariableTuple, meshName,
                                            nFieldVariablesOfMesh);
...
countNFieldVariablesOfMesh(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (countNFieldVariablesOfMesh<std::shared_ptr<SubFieldVariableType>>(
            currentSubFieldVariable, meshName, nFieldVariablesOfMesh))
      return true;
  }
