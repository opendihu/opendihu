...
bool Generic::prepareWrite(const DataType &data, int timeStepNo,
                           double currentTime, int callCountIncrement) {
  VLOG(2) << "Generic::prepareWrite (\"" << filenameBase_
  ...
  The first column is an (optional) call to write the initial values with
  callCountIncrement=0. If the call is not made, the remaining table does not
  change.
  ...
  */
  ...
  int lastCallCountToWrite =
      writeCallCount_ - (writeCallCount_ % outputInterval_);

  // if there was a call count between the last and the current where the file
  // should have been written
  if ((oldWriteCallCount < lastCallCountToWrite &&
       lastCallCountToWrite <= writeCallCount_) ||
      callCountIncrement == 0) {
    ...
    if (timeStepNo != -1) {
      ...
      case fileNumberingByTimeStepIndex:
        ...
        //   For time integrators it corresponds to the data after the
        //   first,...,n-th time step
        s << "_" << std::setw(7) << std::setfill('0') << writeCallCount_;
        break;
      default:
        LOG(ERROR) << "BUG: Unknown file numbering '" << fileNumbering_
    ...
    if (data.functionSpace()->meshPartition()->nRanks() > 1) {
      appendRankNo(s, data.functionSpace()->meshPartition()->nRanks(),
                   data.functionSpace()->meshPartition()->ownRankNo());
    }

    // set the current filename and return true, which indicates that the output
    // writer will write the file
    filename_ = s.str();
    return true;
  } else {
