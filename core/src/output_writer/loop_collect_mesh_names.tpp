...
typename std::enable_if<
    ...
    bool>::type
collectMeshNames(CurrentFieldVariableType currentFieldVariable,
                 std::set<std::string> &meshNames) {
...
collectMeshNames(VectorType currentFieldVariableGradient,
  ...
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    ...
    if (collectMeshNames<typename VectorType::value_type>(currentFieldVariable,
                                                          meshNames))
      return true;
  }
...
collectMeshNames(TupleType currentFieldVariableTuple,
                 std::set<std::string> &meshNames) {
...
collectMeshNames(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (collectMeshNames<std::shared_ptr<SubFieldVariableType>>(
            currentSubFieldVariable, meshNames))
      return true;
  }
