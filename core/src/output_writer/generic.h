...
class Generic {
public:
  //! constructor, if rankSubset is not given, use the
...
protected:
  ...
  //! i.e. set filename_ from config
  //! @param callCountIncrement number of times the callCount is incremented,
  //! affects the frequency in which output files are written (outputInterval_)
  template <typename DataType>
  bool prepareWrite(const DataType &data, int timeStepNo = -1,
                    double currentTime = 0.0, int callCountIncrement = 1);

  DihuContext context_; //< the context object

  std::string
      filenameBaseWithNo_;   //< beginning of the file with "_<fileNo>" appended
  ...
  int outputInterval_ =
      0; //< the interval in which calls to write actually write data

  std::shared_ptr<Partition::RankSubset>
      rankSubset_; //< the ranks that collectively call Paraview::write
