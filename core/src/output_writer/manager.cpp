...
void Manager::initialize(DihuContext context, PythonConfig settings,
  ...
  if (settings.hasKey("OutputWriter")) {
    LOG(DEBUG)
        << "OutputWriter::Manager::initialize(), settings has key OutputWriter";

    // get the first value from the list
    PyObject *writerSettings =
        settings.getOptionListBegin<PyObject *>("OutputWriter");

    // loop over other values
    for (int listIndex = 0; !settings.getOptionListEnd("OutputWriter");
         settings.getOptionListNext<PyObject *>("OutputWriter", writerSettings),
             listIndex++) {
      if (VLOG_IS_ON(1)) {
        VLOG(1) << "parse outputWriter, settings: " << settings.pyObject()
  ...
  // if there were outputFileNo stored from earlier output writers, restore
  // these numbers
...
void Manager::createOutputWriterFromSettings(
  ...
  if (settings.hasKey("format")) {
    ...
    if (typeString == "Paraview") {
      outputWriter_.push_back(
          std::make_shared<Paraview>(context, settings, rankSubset));
    } else if (typeString == "PythonCallback") {
      outputWriter_.push_back(
          std::make_shared<PythonCallback>(context, settings, rankSubset));
    } else if (typeString == "PythonFile") {
      outputWriter_.push_back(
          std::make_shared<PythonFile>(context, settings, rankSubset));
    } else if (typeString == "Exfile" || typeString == "ExFile") {
      outputWriter_.push_back(
          std::make_shared<Exfile>(context, settings, rankSubset));
    } else if (typeString == "MegaMol") {
#ifdef HAVE_ADIOS
      outputWriter_.push_back(
          std::make_shared<MegaMol>(context, settings, rankSubset));
#else
...
bool Manager::hasOutputWriters() { return !outputWriter_.empty(); }
