...
/** Base class of ExfileWriter that writes exelem and exnode files of given
 * field variables. FieldVariablesForOutputWriterType is a
 * std::tuple<std::shared_ptr<>, std::shared_ptr<>, ...> of field variables.
 *  Only field variables which are defined on the specified mesh will be output.
 *  The FunctionSpaceType has to be the type of the field variables given in
 * meshName.
 */
...
public:
  ...
  static void outputExelem(std::ostream &stream,
                           FieldVariablesForOutputWriterType fieldVariables,
                           std::string meshName,
                           std::shared_ptr<FunctionSpaceType> mesh,
                           int nFieldVariablesOfMesh);

  //! write exnode file to given stream, only output fieldVariables that are on
  //! a mesh with the given meshName
  static void outputExnode(std::ostream &stream,
                           FieldVariablesForOutputWriterType fieldVariables,
                           std::string meshName,
                           std::shared_ptr<FunctionSpaceType> mesh,
                           int nFieldVariablesOfMesh);
