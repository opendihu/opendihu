...
template <typename FieldVariablesForOutputWriterType, int i>
    ...
    loopGetValuesAtNode(const FieldVariablesForOutputWriterType &fieldVariables,
                        std::string meshName, element_no_t currentNodeGlobalNo,
                        std::vector<double> &valuesAtNode) {
  // call what to do in the loop body
  if (getValuesAtNode<typename std::tuple_element<
  ...
  loopGetValuesAtNode<FieldVariablesForOutputWriterType, i + 1>(
...
getValuesAtNode(CurrentFieldVariableType currentFieldVariable,
  ...
  VLOG(1) << "  meshName: " << meshName
          << ", get dofsAtNode for currentNodeGlobalNo: " << currentNodeGlobalNo
          << ", n components: " << currentFieldVariable->nComponents();

  std::vector<dof_no_t> &dofsAtNode =
      currentFieldVariable->nodeToDofMapping()->getNodeDofs(
          currentNodeGlobalNo);

  // loop over components
  for (int componentNo = 0; componentNo < currentFieldVariable->nComponents();
       componentNo++) {
...
getValuesAtNode(TupleType currentFieldVariableTuple, std::string meshName,
  ...
  loopGetValuesAtNode<TupleType>(currentFieldVariableTuple, meshName,
                                 currentNodeGlobalNo, valuesAtNode);
...
getValuesAtNode(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (getValuesAtNode<std::shared_ptr<SubFieldVariableType>>(
