...
template <typename FieldVariablesForOutputWriterType, int i>
    ...
    loopOutputHeaderExnode(
        const FieldVariablesForOutputWriterType &fieldVariables,
        int &fieldVariableIndex, std::string meshName, std::ostream &stream,
        node_no_t currentNodeGlobalNo, int &valueIndex) {
  // call what to do in the loop body
  if (outputHeaderExnode<typename std::tuple_element<
          i, FieldVariablesForOutputWriterType>::type>(
          std::get<i>(fieldVariables), fieldVariableIndex, meshName, stream,
          currentNodeGlobalNo, valueIndex))
    return;

  // advance iteration to next tuple element
  loopOutputHeaderExnode<FieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, fieldVariableIndex, meshName, stream, currentNodeGlobalNo,
      valueIndex);
}
...
typename std::enable_if<
    ...
    bool>::type
outputHeaderExnode(CurrentFieldVariableType currentFieldVariable,
  ...
  currentFieldVariable->outputHeaderExnode(stream, currentNodeGlobalNo,
                                           valueIndex, fieldVariableIndex);
  fieldVariableIndex++;
...
outputHeaderExnode(VectorType currentFieldVariableGradient,
                   int &fieldVariableIndex, std::string meshName,
                   std::ostream &stream, node_no_t currentNodeGlobalNo,
                   int &valueIndex) {
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    // call function on all vector entries
    if (outputHeaderExnode<typename VectorType::value_type>(
            currentFieldVariable, fieldVariableIndex, meshName, stream,
            currentNodeGlobalNo, valueIndex))
      return true;
  }
  return false; // do not break iteration
...
outputHeaderExnode(TupleType currentFieldVariableTuple, int &fieldVariableIndex,
                   std::string meshName, std::ostream &stream,
                   node_no_t currentNodeGlobalNo, int &valueIndex) {
  // call for tuple element
  loopOutputHeaderExnode<TupleType>(currentFieldVariableTuple,
                                    fieldVariableIndex, meshName, stream,
                                    currentNodeGlobalNo, valueIndex);
...
outputHeaderExnode(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (outputHeaderExnode<std::shared_ptr<SubFieldVariableType>>(
            currentSubFieldVariable, fieldVariableIndex, meshName, stream,
            currentNodeGlobalNo, valueIndex))
      return true;
  }
