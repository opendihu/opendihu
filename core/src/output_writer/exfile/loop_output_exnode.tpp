...
template <typename FieldVariablesForOutputWriterType,
  ...
  if (outputExnode<typename std::tuple_element<
  ...
  loopOutputExnode<FieldVariablesForOutputWriterType,
                   AllFieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, allFieldVariables, meshName, file);
}
...
typename std::enable_if<
    ...
    bool>::type
outputExnode(CurrentFieldVariableType currentFieldVariable,
             const FieldVariablesForOutputWriterType &fieldVariables,
             std::string meshName, std::ofstream &file) {
  ...
  if (currentFieldVariable->functionSpace()->meshName() == meshName) {
    ...
    ExfileWriter<FunctionSpace, FieldVariablesForOutputWriterType>::
        outputExnode(file, fieldVariables, meshName,
...
outputExnode(VectorType currentFieldVariableGradient,
             const FieldVariablesForOutputWriterType &fieldVariables,
             std::string meshName, std::ofstream &file) {
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    // call function on all vector entries
    if (outputExnode<typename VectorType::value_type,
                     FieldVariablesForOutputWriterType>(
            currentFieldVariable, fieldVariables, meshName, file))
      return true; // break iteration
  }
  return false; // do not break iteration
}
...
outputExnode(TupleType currentFieldVariableTuple,
             const AllFieldVariablesForOutputWriterType &fieldVariables,
             std::string meshName, std::ofstream &file) {
  // call for tuple element
  loopOutputExnode<TupleType, AllFieldVariablesForOutputWriterType>(
      currentFieldVariableTuple, fieldVariables, meshName, file);
...
outputExnode(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (outputExnode<std::shared_ptr<SubFieldVariableType>,
                     AllFieldVariablesForOutputWriterType>(
