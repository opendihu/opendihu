...
template <typename FieldVariablesForOutputWriterType,
  ...
  if (outputExelem<typename std::tuple_element<
  ...
  loopOutputExelem<FieldVariablesForOutputWriterType,
                   AllFieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, allFieldVariables, meshName, file, mesh);
}
...
typename std::enable_if<
    ...
    bool>::type
outputExelem(CurrentFieldVariableType currentFieldVariable,
             const FieldVariablesForOutputWriterType &fieldVariables,
             std::string meshName, std::ofstream &file,
             std::shared_ptr<Mesh::Mesh> &mesh) {
  ...
  if (currentFieldVariable->functionSpace()->meshName() == meshName) {
    ...
    ExfileWriter<FunctionSpace, FieldVariablesForOutputWriterType>::
        outputExelem(file, fieldVariables, meshName,
...
outputExelem(VectorType currentFieldVariableGradient,
             const FieldVariablesForOutputWriterType &fieldVariables,
             std::string meshName, std::ofstream &file,
             std::shared_ptr<Mesh::Mesh> &mesh) {
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    // call function on all vector entries
    if (outputExelem<typename VectorType::value_type,
                     FieldVariablesForOutputWriterType>(
            currentFieldVariable, fieldVariables, meshName, file, mesh))
      return true; // break iteration
  }
  return false; // do not break iteration
}
...
outputExelem(TupleType currentFieldVariableTuple,
             const AllFieldVariablesForOutputWriterType &fieldVariables,
             std::string meshName, std::ofstream &file,
             std::shared_ptr<Mesh::Mesh> &mesh) {
  // call for tuple element
  loopOutputExelem<TupleType, AllFieldVariablesForOutputWriterType>(
      currentFieldVariableTuple, fieldVariables, meshName, file, mesh);
...
outputExelem(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (outputExelem<std::shared_ptr<SubFieldVariableType>,
                     AllFieldVariablesForOutputWriterType>(
            currentSubFieldVariable, fieldVariables, meshName, file, mesh))
      return true;
  }
