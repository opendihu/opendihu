...
template <typename FieldVariablesForOutputWriterType, int i>
  ...
  if (checkIfNewExnodeHeaderNecessary<typename std::tuple_element<
          i, FieldVariablesForOutputWriterType>::type>(
          std::get<i>(fieldVariables), meshName, currentNodeGlobalNo,
          newHeaderNecessary))
    return;

  // advance iteration to next tuple element
  loopCheckIfNewExnodeHeaderNecessary<FieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, meshName, currentNodeGlobalNo, newHeaderNecessary);
}
...
checkIfNewExnodeHeaderNecessary(CurrentFieldVariableType currentFieldVariable,
  ...
  int previousNumberVersions =
      currentFieldVariable->nodeToDofMapping()->nVersions(currentNodeGlobalNo -
                                                          1);
  int currentNumberVersions =
      currentFieldVariable->nodeToDofMapping()->nVersions(currentNodeGlobalNo);
...
checkIfNewExnodeHeaderNecessary(VectorType currentFieldVariableGradient,
  ...
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    ...
    if (checkIfNewExnodeHeaderNecessary<typename VectorType::value_type>(
            currentFieldVariable, meshName, currentNodeGlobalNo,
            newHeaderNecessary))
      return true;
  }
  return false; // do not break iteration
...
checkIfNewExnodeHeaderNecessary(TupleType currentFieldVariableTuple,
                                std::string meshName,
                                element_no_t currentNodeGlobalNo,
                                bool &newHeaderNecessary) {
  // call for tuple element
  loopCheckIfNewExnodeHeaderNecessary<TupleType>(currentFieldVariableTuple,
                                                 meshName, currentNodeGlobalNo,
                                                 newHeaderNecessary);
...
checkIfNewExnodeHeaderNecessary(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (checkIfNewExnodeHeaderNecessary<std::shared_ptr<SubFieldVariableType>>(
            currentSubFieldVariable, meshName, currentNodeGlobalNo,
            newHeaderNecessary))
      return true;
  }
