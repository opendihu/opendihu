...
void ExfileWriter<
    ...
    outputExelem(
        std::ostream &stream, FieldVariablesForOutputWriterType fieldVariables,
        std::string meshName,
  ...
  // loop over elements
  for (element_no_t currentElementGlobalNo = 0;
       currentElementGlobalNo < nElements; currentElementGlobalNo++) {
    // check if a new header is necessary
    if (currentElementGlobalNo > 0) {
      outputHeader = false;
      ExfileLoopOverTuple::loopCheckIfNewExelemHeaderNecessary<
          FieldVariablesForOutputWriterType>(
          fieldVariables, meshName, currentElementGlobalNo, outputHeader);
    }
    ...
    if (outputHeader) {
      ...
      if (nScaleFactors == 0) {
        ...
        stream << " "
               << BasisFunction::getBasisRepresentationString<
                      D, BasisFunctionType>();
      ...
      ExfileLoopOverTuple::loopOutputHeaderExelem<
          FieldVariablesForOutputWriterType>(fieldVariables, fieldVariableIndex,
                                             meshName, stream,
                                             currentElementGlobalNo);
    }

    mesh->elementToNodeMapping()->outputElementExelem(stream,
                                                      currentElementGlobalNo);
  }
}
...
void ExfileWriter<
  ...
  // loop over all nodes
  for (node_no_t currentNodeGlobalNo = 0; currentNodeGlobalNo < nNodes;
       currentNodeGlobalNo++) {
    // check if a new header is necessary
    if (currentNodeGlobalNo > 0) {
      outputHeader = false;
      ExfileLoopOverTuple::loopCheckIfNewExnodeHeaderNecessary<
          FieldVariablesForOutputWriterType>(fieldVariables, meshName,
                                             currentNodeGlobalNo, outputHeader);
    }
    ...
    if (outputHeader) {
      ...
      int valueIndex =
          0; // an index that runs over values of components of field variables
             // and corresponds to the index in the values block for a node
    ...
    ExfileLoopOverTuple::loopGetValuesAtNode<FieldVariablesForOutputWriterType>(
        fieldVariables, meshName, currentNodeGlobalNo, valuesAtNode);

    StringUtility::outputValuesBlock(stream, valuesAtNode.begin(),
                                     valuesAtNode.end(), 8);
  }
}
