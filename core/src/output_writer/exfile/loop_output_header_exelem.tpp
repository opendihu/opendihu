...
template <typename FieldVariablesForOutputWriterType, int i>
  ...
  if (outputHeaderExelem<typename std::tuple_element<
  ...
  loopOutputHeaderExelem<FieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, fieldVariableIndex, meshName, stream,
      currentFieldVariableGlobalNo);
}
...
typename std::enable_if<
    ...
    bool>::type
outputHeaderExelem(CurrentFieldVariableType currentFieldVariable,
  ...
  currentFieldVariable->outputHeaderExelem(stream, currentFieldVariableGlobalNo,
                                           fieldVariableIndex);
  fieldVariableIndex++;
...
outputHeaderExelem(VectorType currentFieldVariableGradient,
                   int &fieldVariableIndex, std::string meshName,
                   std::ostream &stream,
                   element_no_t currentFieldVariableGlobalNo) {
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    // call function on all vector entries
    if (outputHeaderExelem<typename VectorType::value_type>(
...
outputHeaderExelem(TupleType currentFieldVariableTuple, int &fieldVariableIndex,
                   std::string meshName, std::ostream &stream,
                   element_no_t currentFieldVariableGlobalNo) {
  // call for tuple element
  loopOutputHeaderExelem<TupleType>(currentFieldVariableTuple,
...
outputHeaderExelem(CurrentFieldVariableType currentFieldVariable,
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (outputHeaderExelem<std::shared_ptr<SubFieldVariableType>>(
