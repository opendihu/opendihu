...
getNodalValues(CurrentFieldVariableType currentFieldVariable,
  ...
  VLOG(1) << "field variable "
          ...
          << "\", is geometry: " << currentFieldVariable->isGeometryField()
          << ", values size: " << values.size();

  if (!currentFieldVariable->functionSpace())
    return false;

  // if mesh name is one of the specified meshNames (and it is not a geometry
  // field)
  if (meshNames.find(currentFieldVariable->functionSpace()->meshName()) !=
    ...
    for (int componentNo = 0; componentNo < nComponents; componentNo++) {
      ...
      const std::vector<dof_no_t> &dofNosLocalNaturalOrdering =
          currentFieldVariable->functionSpace()
              ->meshPartition()
              ->dofNosLocalNaturalOrdering();
      currentFieldVariable->getValues(componentNo, dofNosLocalNaturalOrdering,
    ...
    values[fieldVariableName].reserve(values[fieldVariableName].size() +
                                      componentValues[0].size() * nComponents);
    LOG(DEBUG) << "get nodal values of \"" << fieldVariableName << "\" ("
