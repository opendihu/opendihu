...
template <typename FieldVariablesForOutputWriterType, int i>
  ...
  if (collectFieldVariablesNames<
          typename std::tuple_element<i,
                                      FieldVariablesForOutputWriterType>::type,
          FieldVariablesForOutputWriterType>(std::get<i>(fieldVariables),
                                             fieldVariables, meshName, scalars,
                                             vectors))
    return;

  // advance iteration to next tuple element
  loopCollectFieldVariablesNames<FieldVariablesForOutputWriterType, i + 1>(
      fieldVariables, meshName, scalars, vectors);
}
...
collectFieldVariablesNames(
  ...
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    ...
    if (collectFieldVariablesNames<typename VectorType::value_type,
                                   FieldVariablesForOutputWriterType>(
            currentFieldVariable, fieldVariables, meshName, scalars, vectors))
      return true; // break iteration
  }
  return false; // do not break iteration
}
...
collectFieldVariablesNames(
  ...
  loopCollectFieldVariablesNames<TupleType>(currentFieldVariableTuple, meshName,
                                            scalars, vectors);
...
collectFieldVariablesNames(
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (collectFieldVariablesNames<std::shared_ptr<SubFieldVariableType>,
                                   FieldVariablesForOutputWriterType>(
            currentSubFieldVariable, fieldVariables, meshName, scalars,
            vectors))
      return true;
  }
