...
void Paraview::writePolyDataFile(
  ...
  {
    int dimensionality;    //< D=1: object is a VTK "Line", D=2, D=3: object is
  a VTK "Poly" global_no_t nPointsLocal;   //< the number of points needed for
  representing the mesh, local value of rank global_no_t nCellsLocal;    //< the
  number of VTK "cells", i.e. "Lines" or "Polys", which is the opendihu number
  of "elements", local value of rank global_no_t nPointsGlobal;   //< the number
  of points needed for representing the mesh, global value of all rank
    global_no_t nCellsGlobal;    //< the number of VTK "cells", i.e. "Lines" or
  "Polys", which is the opendihu number of "elements", global value of all ranks

    std::vector<PolyDataPropertiesForMesh::DataArrayName> pointDataArrays;   //<
  <name,nComponents> of PointData DataArray elements
  };
*/
  ...
  struct VTKPiece
  {
    std::set<std::string> meshNamesCombinedMeshes;   //< the meshNames of the
  combined meshes, or only one meshName if it is not a merged mesh
    PolyDataPropertiesForMesh properties;   //< the properties of the merged
  mesh

    std::string firstScalarName;   //< name of the first scalar field variable
  of the mesh std::string firstVectorName;   //< name of the first non-scalar
  field variable of the mesh
  ...
  if properties has been set void setVTKValues()
    {
      // set values for firstScalarName and firstVectorName from the values in
  pointDataArrays for (auto pointDataArray : properties.pointDataArrays)
      {
  ...
  */
  ...
  if (!meshPropertiesInitialized) {
    ...
    for (std::map<std::string, PolyDataPropertiesForMesh>::iterator iter =
         ...
         iter != meshPropertiesPolyDataFile_.end(); iter++) {
                         ...
                         // mesh
      {
        if (vtkPiece1D_.properties.pointDataArrays.size() !=
          ...
          LOG(DEBUG) << "Mesh " << meshName << " cannot be combined with "
                     ...
                     << meshName << " (is "
                     << iter->second.pointDataArrays.size() << " instead of "
  ...
  if (!meshPropertiesInitialized) {
    ...
    MPIUtility::handleReturnValue(
        ...
        "MPI_Exscan");
    MPIUtility::handleReturnValue(
        MPI_Reduce(&vtkPiece1D_.properties.nPointsLocal, &nPointsGlobal1D_, 1,
                   MPI_INT, MPI_SUM, 0, this->rankSubset_->mpiCommunicator()),
        "MPI_Reduce");
    MPIUtility::handleReturnValue(
        MPI_Reduce(&vtkPiece1D_.properties.nCellsLocal, &nLinesGlobal1D_, 1,
                   MPI_INT, MPI_SUM, 0, this->rankSubset_->mpiCommunicator()),
        "MPI_Reduce");
  ...
  for (std::map<std::string, std::vector<double>>::iterator iter =
       ...
       iter != fieldVariableValues.end(); iter++) {
  ...
  for (std::vector<PolyDataPropertiesForMesh::DataArrayName>::iterator
           pointDataArrayIter = vtkPiece1D_.properties.pointDataArrays.begin();
       pointDataArrayIter != vtkPiece1D_.properties.pointDataArrays.end();
       pointDataArrayIter++) {
  ...
  if (vtkPiece1D_.firstScalarName != "") {
    outputFileParts[outputFilePartNo] << " Scalars=\""
                                      << vtkPiece1D_.firstScalarName << "\"";
  }
  if (vtkPiece1D_.firstVectorName != "") {
    outputFileParts[outputFilePartNo] << " Vectors=\""
                                      << vtkPiece1D_.firstVectorName << "\"";
  }
  ...
  for (std::vector<PolyDataPropertiesForMesh::DataArrayName>::iterator
    ...
    writeCombinedValuesVector(fileHandle, ownRankNo,
                              ...
                              fieldVariableNo, writeFloatsAsInt);
