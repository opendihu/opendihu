...
template <typename T>
void Paraview::writeCombinedValuesVector(MPI_File fileHandle, int ownRankNo,
  ...
  if (binaryOutput_) {
    VLOG(1) << "Paraview::writeCombinedValuesVector, " << values.size()
            << " values: " << values;
    ...
    if (globalValuesSize_.size() < identifier + 1) {
      ...
      MPIUtility::handleReturnValue(
          MPI_Reduce(&localValuesSize, &globalValuesSize, 1, MPI_INT, MPI_SUM,
                     0, this->rankSubset_->mpiCommunicator()),
          "MPI_Reduce");

      globalValuesSize_[identifier] =
          globalValuesSize; // value only set on rank 0, all other ranks have
                            // value 0
      ...
      MPIUtility::handleReturnValue(
          MPI_Exscan(&nLocalValues, &nPreviousValues, 1, MPI_INT, MPI_SUM,
                     this->rankSubset_->mpiCommunicator()),
          "MPI_Exscan");
      nPreviousValues_[identifier] = nPreviousValues;

      VLOG(1) << "reduce data length of total vector, localValuesSize: "
              << localValuesSize << ", global size: " << globalValuesSize
              << ", nPreviousValues: " << nPreviousValues;
    }
    ...
    if (std::is_same<T, double>::value || std::is_same<T, float>::value) {
      if (writeFloatsAsInt) {
        ...
        // values are float type, put 4 byte data per float in list of int32_t
        // datatype
        for (int i = 0; i < values.size(); i++) {
          union {
            float f;
    ...
    if (valuesVector.size() < 1) {
      LOG(FATAL) << "No values for rank " << ownRankNo
                 << " in paraview binary output. This is not supported for "
                    "binary output, so set binary to False.";
    }
    ...
    if (ownRankNo != 0) {
      MPIUtility::handleReturnValue(
          MPI_Isend(&firstValue, 1, MPI_FLOAT, ownRankNo - 1, 0,
                    this->rankSubset_->mpiCommunicator(), &sendRequest),
          "MPI_Isend");
    }
    ...
    if (ownRankNo != this->rankSubset_->size() - 1) {
      MPIUtility::handleReturnValue(
          MPI_Irecv(&firstValueNextRank, 1, MPI_FLOAT, ownRankNo + 1, 0,
                    this->rankSubset_->mpiCommunicator(), &receiveRequest),
          "MPI_Irecv");
      MPI_Status status;
      MPIUtility::handleReturnValue(MPI_Wait(&receiveRequest, &status),
                                    "MPI_Wait", &status);
    }
    if (ownRankNo != 0) {
      MPI_Status status;
      MPIUtility::handleReturnValue(MPI_Wait(&sendRequest, &status), "MPI_Wait",
                                    &status);
    }

    VLOG(1) << "firstValueNextRank: " << firstValueNextRank
            << ", nPreviousValues_[" << identifier
            << "]: " << nPreviousValues_[identifier]
            << ", %3=" << (nPreviousValues_[identifier] % 3);

    // the end of the own values are contained in the own written data plus some
    // part of the first value of the next rank
    if (nPreviousValues_[identifier] % 3 == 0) {
      valuesVector.push_back(firstValueNextRank);
      writeBuffer =
          Paraview::encodeBase64Int32(valuesVector.begin(), valuesVector.end(),
                                      false); // without leading dataset size

      // remove last bytes that encode the additional last value
      int offset = 0;
      int nBits =
          (std::distance(valuesVector.begin(), valuesVector.end()) - 1) * 4 *
              8 -
          offset;
      int nBytes = nBits / 6;
      if (nBits % 6 != 0) {
      ...
      int offset = 4;
      int nBits =
          (std::distance(valuesVector.begin(), valuesVector.end()) - 2) * 4 *
              8 -
          offset;
      int nBytes = nBits / 6;
      if (nBits % 6 != 0) {
      ...
      stringData = stringData.substr(
          6); // discard first six bytes (discards padding 4 bytes and gives
              // offset of 4 bits (6*6=36 = 4*8+4))
      if (ownRankNo != this->rankSubset_->size() - 1) {
        stringData = stringData.substr(0, nBytes);
      }

      VLOG(1) << "padding removed [" << stringData
              << "] (size=" << this->rankSubset_->size() << ")";
      // info << ", padding removed [" << stringData << "] nBits: " <<
      // (std::distance(valuesVector.begin(), valuesVector.end())-2)
      //   << "*4*8 - " << offset << " = " << nBits << ", nBits % 6 = " << nBits
      //   % 6 << ", nBytes = " << nBytes << "  ";
      ...
      int offset = 2;
      int nBits =
          (std::distance(valuesVector.begin(), valuesVector.end()) - 3) * 4 *
              8 -
          offset;
      int nBytes = nBits / 6;
      ...
      stringData = stringData.substr(
          11); // discard first 11 bytes (discards padding 2*4=8 bytes and gives
               // offset of 2 bits (11*6=66 = 8*8+2))
    ...
    if (VLOG_IS_ON(3)) {
      ...
      for (std::list<int32_t>::iterator iter = valuesVector.begin();
           iter != valuesVector.end(); iter++)
        VLOG(3) << *iter << " ";
    }

    // info << " (rank " << ownRankNo << ", values:";
    // for (std::list<int32_t>::iterator iter=valuesVector.begin(); iter !=
    // valuesVector.end(); iter++)
    //   info << " " << *iter;
    // info << ") ";

    // determine global number of bytes and make sure that it is a multiple of 4
    // by adding padding "="s
    int nBytesLocal = writeBuffer.length();
    int nBytesGlobal = 0;
    MPIUtility::handleReturnValue(
        MPI_Reduce(&nBytesLocal, &nBytesGlobal, 1, MPI_INT, MPI_SUM,
                   this->rankSubset_->size() - 1,
                   this->rankSubset_->mpiCommunicator()),
        "MPI_Reduce");

    // add base64 padding with "=" characters such that global size is a factor
    // of 4
    if (ownRankNo == this->rankSubset_->size() - 1) {
      ...
      if (nBytesGlobal % 4 == 1) {
        ...
        LOG(FATAL) << "Base64 encoding in parallel has a bug, padding needs 3 "
                      "'=' signs, but only 1 or 2 should be ever needed";
  ...
  MPIUtility::handleReturnValue(
      MPI_File_write_ordered(fileHandle, writeBuffer.c_str(),
                             writeBuffer.length(), MPI_BYTE, MPI_STATUS_IGNORE),
