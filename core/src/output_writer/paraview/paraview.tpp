...
void Paraview::write(DataType &data, int timeStepNo, double currentTime,
  ...
  for (std::string meshName : meshesToOutput) {
    ...
    ParaviewLoopOverTuple::loopOutput(data.getFieldVariablesForOutputWriter(),
                                      data.getFieldVariablesForOutputWriter(),
                                      meshName, filenameStart.str(),
...
void Paraview::writeParaviewFieldVariable(FieldVariableType &fieldVariable,
  ...
  if (onlyParallelDatasetElement) {
    ...
    for (int componentNo = 0; componentNo < nComponents; componentNo++) {
      ...
      const node_no_t nNodesLocal = fieldVariable.functionSpace()
                                        ->meshPartition()
    ...
    for (int i = 0; i < componentValues[0].size(); i++) {
      for (int componentNo = 0; componentNo < nComponentsParaview;
           componentNo++) {
...
template <typename FieldVariableType>
void Paraview::writeParaviewPartitionFieldVariable(
...
std::string Paraview::encodeBase64Float(Iter iterBegin, Iter iterEnd,
  ...
  bool success =
      Base64::Encode(raw.data(), rawLength, encoded.data(), encodedLength);
  if (!success)
...
template <typename Iter>
std::string Paraview::encodeBase64Int32(Iter iterBegin, Iter iterEnd,
                                        bool withEncodedSizePrefix) {
  ...
  bool success = Base64::Encode(raw.data(), rawLength, (char *)encoded.data(),
                                encodedLength);
  if (!success)
    LOG(WARNING) << "Base64 encoding failed";
...
template <typename Iter>
std::string Paraview::encodeBase64UInt8(Iter iterBegin, Iter iterEnd,
                                        bool withEncodedSizePrefix) {
