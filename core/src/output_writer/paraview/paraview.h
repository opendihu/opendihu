...
public:
  ...
  static void writeParaviewFieldVariable(
      FieldVariableType &fieldVariable, std::ofstream &file, bool binaryOutput,
      bool fixedFormat, bool onlyParallelDatasetElement = false);

  //! write the a field variable indicating which ranks own which portion of the
  //! domain as VTK <DataArray> element to file, if onlyParallelDatasetElement
  //! write the <PDataArray> element
  template <typename FieldVariableType>
  static void writeParaviewPartitionFieldVariable(
      FieldVariableType &geometryField, std::ofstream &file, bool binaryOutput,
      bool fixedFormat, bool onlyParallelDatasetElement = false);

  //! write a single *.vtp file that contains all data of all 1D field
  //! variables. This is uses MPI IO. It can be enabled with the "combineFiles"
  //! option. on return, combinedMeshesOut contains the 1D mesh names that were
  //! written to the vtp file.
  ...
  //! enabled with the "combineFiles" option. on return, combinedMeshesOut
  //! contains the 3D or 2D mesh names that were written to the vtu file.
  template <typename FieldVariablesForOutputWriterType>
  ...
  //! as encoded prefix
  static std::string encodeBase64Vec(const Vec &vector,
                                     bool withEncodedSizePrefix = true);

  //! encode a std::vector<double> or std::list<double> as base64
  template <typename Iter>
  static std::string encodeBase64Float(Iter iterBegin, Iter iterEnd,
                                       bool withEncodedSizePrefix = true);

  //! encode a std::vector<int> as 32bit base64 values
  template <typename Iter>
  static std::string encodeBase64Int32(Iter iterBegin, Iter iterEnd,
                                       bool withEncodedSizePrefix = true);

  //! encode a vector as UInt64 values
  template <typename Iter>
  std::string encodeBase64UInt8(Iter iterBegin, Iter iterEnd,
                                bool withEncodedSizePrefix = true);
  ...
  //! convert to a string with space separated values
  static std::string convertToAscii(const std::vector<double> &vector,
                                    bool humanReadable);

  //! convert to a string with space separated values
  static std::string convertToAscii(const std::vector<element_no_t> &vector,
                                    bool humanReadable);

  // only if Petsc uses long long int for PetscInt, element_no_t is not the same
  // as int and we need to define convertToAscii for int vectors
#ifdef PETSC_USE_64BIT_INDICES
  //! convert to a string with space separated values
  static std::string convertToAscii(const std::vector<int> &vector,
                                    bool humanReadable);
#endif
...
protected:
  ...
  //! write some ascii data to the file as a collective shared operation. Only
  //! rank 0 writes, but the other ranks wait and the shared file pointer is
  //! incremented.
  void writeAsciiDataShared(MPI_File fileHandle, int ownRankNo,
                            std::string writeBuffer);

  //! write the values vector combined to the file, correctly encoded,
  //! identifier is an id to access cached values
  template <typename T>
  void writeCombinedValuesVector(MPI_File fileHandle, int ownRankNo,
                                 const std::vector<T> &values, int identifier,
  ...
  void writeCombinedTypesVector(MPI_File fileHandle, int ownRankNo, int nValues,
                                bool output3DMeshes, int identifier);
  ...
  std::vector<int>
      nPreviousValues_; //< cached values used in writeCombinedValuesVector
