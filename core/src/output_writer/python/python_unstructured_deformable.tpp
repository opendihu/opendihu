...
PyObject *
Python<FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                    BasisFunctionType>,
  ...
  LOG(DEBUG) << "build pyData, "
             << std::tuple_size<FieldVariablesForOutputWriterType>::value;
  ...
  PyObject *data = Py_BuildValue(
      "{s s, s s, s s, s i, s i, s s, s i, s O, s i, s i, s O, s O, s i, s d}",
      "version", DihuContext::versionText().c_str(), "meta",
      DihuContext::metaText().c_str(), "meshType", "UnstructuredDeformable",
      "dimension", D, "nElements", mesh->nElementsLocal(), "basisFunction",
      basisFunction.c_str(), "basisOrder", basisOrder, "onlyNodalValues",
      onlyNodalValues ? Py_True : Py_False, "nRanks", nRanks, "ownRankNo",
      ownRankNo, "data", pyData, "elementalDofs", pyElementalDofs, "timeStepNo",
      timeStepNo, "currentTime", currentTime);
...
Python<FunctionSpace::FunctionSpace<Mesh::UnstructuredDeformableOfDimension<D>,
                                    BasisFunctionType>,
       FieldVariablesForOutputWriterType>::
    buildPyElementalDofsObject(std::shared_ptr<Mesh::Mesh> meshBase,
                               bool onlyNodalValues) {
  typedef FunctionSpace::FunctionSpace<
      Mesh::UnstructuredDeformableOfDimension<D>, BasisFunctionType>
      FunctionSpace;
  std::shared_ptr<FunctionSpace> mesh =
      std::static_pointer_cast<FunctionSpace>(meshBase);

  // create a list of lists for each element the node numbers (if
  // onlyNodalValues) or the dofs
  ...
  for (element_no_t elementNo = 0; elementNo < mesh->nElementsLocal();
    ...
    // add to list
    PyList_SetItem(pyElementalDofs, (Py_ssize_t)elementNo,
                   pyDofs); // steals reference to pyComponent
  }
