 ...
 * std::shared_ptr<>, ...> of field variables that will be output.
 */
template <typename FunctionSpaceType,
          typename FieldVariablesForOutputWriterType>
class Python {};

/* Specialization for RegularFixed. This also outputs rhs matrix and stiffness
 * matrix for laplace problems.
 *
 */
template <int D, typename BasisFunctionType,
          typename FieldVariablesForOutputWriterType>
class Python<FunctionSpace::FunctionSpace<
...
template <int D, typename BasisFunctionType,
          typename FieldVariablesForOutputWriterType>
class Python<FunctionSpace::FunctionSpace<Mesh::CompositeOfDimension<D>,
                                          BasisFunctionType>,
             FieldVariablesForOutputWriterType>
    : public PythonBase<FieldVariablesForOutputWriterType> {
public:
...
private:
  //! create a list of list where for each element the dofs are listed (if
  //! !onlyNodalValues) or the node numbers (if onlyNodalValues)
  static PyObject *
