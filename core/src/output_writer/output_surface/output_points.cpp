...
void OutputPoints::writeCsvFile(std::string filename, double currentTime,
  ...
  if (currentTime <= 1e-5) {
    if (!writeGeometry) {
      ...
      for (int i = 0; i < nPointsGlobal; i++) {
        file << ";" << geometry[3 * i + 0] << ";" << geometry[3 * i + 1] << ";"
             << geometry[3 * i + 2];
      }
      file << std::endl;
    }
    ...
    if (writeGeometry) {
      for (int pointNo = 0; pointNo < nPointsGlobal; pointNo++) {
        file << ";p" << pointNo << "_x;p" << pointNo << "_y;p" << pointNo
             << "_z";
      }
    }
  ...
  file << StringUtility::timeToString(&tm) << ";" << currentTime << ";"
       << nPointsGlobal;
...
void OutputPoints::writeVtpFile(std::string filename, double currentTime,
                                const std::vector<double> &geometry,
                                const std::vector<double> &values,
                                int nComponents,
  ...
  std::string stringTime =
      Paraview::encodeBase64Float(time.begin(), time.end());

  file << "<?xml version=\"1.0\"?>" << std::endl
       << "<!-- " << DihuContext::versionText() << " "
       << DihuContext::metaText() << ", currentTime: " << currentTime << " -->"
       << std::endl
  ...
  file << std::string(2, '\t') << "<Piece NumberOfPoints=\"" << nPointsGlobal
       << "\" NumberOfVerts=\"0\" "
