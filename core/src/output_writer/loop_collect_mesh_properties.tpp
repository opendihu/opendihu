...
template <typename FieldVariablesForOutputWriterType, int i>
  ...
  if (collectMeshProperties<typename std::tuple_element<
...
collectMeshProperties(
  ...
  std::set<std::string> meshNamesSet(meshNamesVector.begin(),
                                     meshNamesVector.end());
  if (meshNamesSet.find(meshName) == meshNamesSet.end())
    meshNamesVector.push_back(meshName);
  // LOG(DEBUG) << "field variable \"" << currentFieldVariable->name() << "\",
  // mesh \"" << meshName << "\".";

  /*
  int dimensionality;    //< D=1: object is a VTK "Line", D=2, D=3: object
  should be represented by an unstructured grid global_no_t nPoints;   //< the
  number of points needed for representing the mesh global_no_t nCells;    //<
  the number of VTK "cells", i.e. "Lines" or "Polys", which is the opendihu
  number of "elements" std::vector<node_no_t> nNodesLocalWithGhosts;   //< local
  number of nodes including ghosts, for all dimensions

  std::vector<PointData> pointDataArrays;   //< <name,nComponents> of PointData
  DataArray elements
  */
  int dimensionality = currentFieldVariable->functionSpace()->dim();
  meshProperties[meshName].dimensionality = dimensionality;
  meshProperties[meshName].nPointsLocal =
      currentFieldVariable->functionSpace()->nNodesLocalWithGhosts();
  meshProperties[meshName].nPointsGlobal =
      currentFieldVariable->functionSpace()->nNodesGlobal();
  ...
  int nNodesPerElement1D =
      FunctionSpace::FunctionSpaceBaseDim<1, BasisFunction>::nNodesPerElement();
  int nCellsPerElement = pow(nNodesPerElement1D - 1, dimensionality);

  meshProperties[meshName].nCellsGlobal =
      currentFieldVariable->functionSpace()->nElementsGlobal() *
      nCellsPerElement;
  meshProperties[meshName].nCellsLocal =
      currentFieldVariable->functionSpace()->nElementsLocal() *
      nCellsPerElement;

  meshProperties[meshName].nNodesLocalWithGhosts.resize(dimensionality);

  for (int dimensionIndex = 0; dimensionIndex < dimensionality;
       dimensionIndex++) {
    meshProperties[meshName].nNodesLocalWithGhosts[dimensionIndex] =
        currentFieldVariable->functionSpace()
            ->meshPartition()
            ->nNodesLocalWithGhosts(dimensionIndex);
  }
  ...
  if (meshProperties[meshName].unstructuredMeshConnectivityValues.empty()) {
    GetConnectivityValuesUnstructuredMesh<
        typename CurrentFieldVariableType::element_type::FunctionSpace>::
        get(currentFieldVariable->functionSpace(),
            meshProperties[meshName].unstructuredMeshConnectivityValues);
  }
  ...
  if (!currentFieldVariable->isGeometryField()) {
    ...
    dataArrayName.componentNames =
        std::vector<std::string>(currentFieldVariable->componentNames().begin(),
                                 currentFieldVariable->componentNames().end());
