...
#ifdef HAVE_ADIOS
std::map<std::string, std::array<std::shared_ptr<MegaMol::adios_writer_t>, 2>>
    MegaMol::adiosWriters_;
std::map<std::string, std::shared_ptr<MegaMol::adios_writer_t>>
    MegaMol::adiosWriter_;
#endif

BoundingBox::BoundingBox()
    : min(Vec3({0.0, 0.0, 0.0})), max(Vec3({0.0, 0.0, 0.0})) {}
...
MegaMol::MegaMol(DihuContext context, PythonConfig settings,
  ...
  useFrontBackBuffer_ =
      specificSettings_.getOptionBool("useFrontBackBuffer", true);
}
#else
...
void MegaMol::writeAdiosVariables() {
  ...
  std::shared_ptr<adios_writer_t> &currentWriter =
      adiosWriter_[this->filenameBase_];

  // reduce global number of nodes
  int nNodesLocal =
      currentWriter->megaMolWriterContext.geometryFieldVectorValues.size();
  nNodesGlobal_ = 0;
  MPI_Reduce(&nNodesLocal, &nNodesGlobal_, 1, MPI_INT, MPI_SUM, 0,
  ...
  if (ownRankNo == 0) {
    ...
    if (!adiosNPointsPerCoordinateDirection_) {
      adiosNPointsPerCoordinateDirection_ =
          std::make_shared<adios2::Variable<int>>(
              currentWriter->adiosIo->DefineVariable<int>(
                  "nPointsPerCoordinateDirection", {3}, {0}, {3},
                  adios2::ConstantDims));
    }

    std::array<int, 3> &nPointsPerCoordinateDirection =
        currentWriter->megaMolWriterContext.nPointsPerCoordinateDirection;

    LOG(DEBUG) << "write nPointsPerCoordinateDirection: "
               << nPointsPerCoordinateDirection;
    currentWriter->engine->Put<int>(*adiosNPointsPerCoordinateDirection_.get(),
                                    nPointsPerCoordinateDirection.data());

    // write radius of spheres
    globalRadius_ = 0.1;
    double approximateDistanceBetweenFibers =
        currentWriter->megaMolWriterContext.approximateDistanceBetweenFibers;
    if (approximateDistanceBetweenFibers > 1e-3) {
      globalRadius_ = approximateDistanceBetweenFibers * 0.1;
    }
    if (!globalRadiusVariable_) {
      globalRadiusVariable_ = std::make_shared<adios2::Variable<double>>(
          currentWriter->adiosIo->DefineVariable<double>("global_radius"));
    }
    currentWriter->engine->Put<double>(*globalRadiusVariable_.get(),
                                       &globalRadius_);

    // write global number of nodes
    if (!globalNumberOfNodesVariable_) {
      globalNumberOfNodesVariable_ = std::make_shared<adios2::Variable<int>>(
          currentWriter->adiosIo->DefineVariable<int>("node_count"));
    }
    currentWriter->engine->Put<int>(*globalNumberOfNodesVariable_.get(),
                                    &nNodesGlobal_);
  }
...
void MegaMol::writeGeometryField() {
  ...
  std::vector<Vec3> &geometryFieldVectorValues =
      currentWriter->megaMolWriterContext.geometryFieldVectorValues;
  ...
  std::vector<double> &geometryFieldValues =
      currentWriter->megaMolWriterContext.geometryFieldValues;
  ...
  if (!adiosFieldVariableGeometry_) {
    ...
    MPI_Exscan(&localSize, &offset, 1, MPI_LONG_LONG, MPI_SUM,
               rankSubset->mpiCommunicator());
    MPI_Allreduce(&localSize, &globalSize, 1, MPI_LONG_LONG, MPI_SUM,
                  rankSubset->mpiCommunicator());
    ...
    adiosFieldVariableGeometry_ = std::make_shared<adios2::Variable<double>>(
        currentWriter->adiosIo->DefineVariable<double>(
            "xyz", {(long unsigned int)globalSize}, {(long unsigned int)offset},
            {(long unsigned int)localSize}, adios2::ConstantDims));
  }

  // write data to file
  currentWriter->engine->Put<double>(*adiosFieldVariableGeometry_.get(),
                                     geometryFieldValues.data());
}
...
void MegaMol::writeBoundaryBoxes() {
  ...
  for (std::vector<Vec3>::iterator iter = geometryFieldVectorValues.begin();
       iter != geometryFieldVectorValues.end(); iter++) {
    for (int i = 0; i < 3; i++) {
  ...
  MPI_Reduce(localBoundingBox.min.data(), globalBoundingBox.min.data(), 3,
             MPI_DOUBLE, MPI_MIN, 0, rankSubset->mpiCommunicator());
  MPI_Reduce(localBoundingBox.max.data(), globalBoundingBox.max.data(), 3,
             MPI_DOUBLE, MPI_MAX, 0, rankSubset->mpiCommunicator());

  LOG(DEBUG) << "reduced global bounding box: " << globalBoundingBox.min << ", "
             << globalBoundingBox.max;

  // transfer bounding box values to format that is compatible with MegaMol
  std::array<double, 6> &globalBoundingBoxValues =
      currentWriter->megaMolWriterContext.globalBoundingBoxValues;
  ...
  std::array<double, 6> &localBoundingBoxValues =
      currentWriter->megaMolWriterContext.localBoundingBoxValues;
  ...
  if (!localBoundingBoxVariable_) {
    localBoundingBoxVariable_ = std::make_shared<adios2::Variable<double>>(
        currentWriter->adiosIo->DefineVariable<double>(
            "localBoundingBox", {(long unsigned int)nRanks, 6},
            {(long unsigned int)ownRankNo, 6}, {1, 6}, adios2::ConstantDims));
  }

  LOG(DEBUG) << "write localBoundingBoxValues: " << localBoundingBoxValues;
  currentWriter->engine->Put<double>(*localBoundingBoxVariable_.get(),
                                     localBoundingBoxValues.data());
  ...
  if (ownRankNo == 0) {
    ...
    if (!globalBoundingBoxVariable_) {
      globalBoundingBoxVariable_ = std::make_shared<adios2::Variable<double>>(
          currentWriter->adiosIo->DefineVariable<double>(
              "globalBoundingBox", {6}, {0}, {6}, adios2::ConstantDims));
    }

    LOG(DEBUG) << "write globalBoundingBoxValues: " << globalBoundingBoxValues;
    currentWriter->engine->Put<double>(*globalBoundingBoxVariable_.get(),
                                       globalBoundingBoxValues.data());
  }
}
...
void MegaMol::writeScalarFields() {
  ...
  std::vector<double> &emgValues =
      currentWriter->megaMolWriterContext.emgValues;
  std::vector<double> &transmembraneFlowValues =
      currentWriter->megaMolWriterContext.transmembraneFlowValues;
  ...
  MPI_Exscan(&localSize, &offset, 1, MPI_INT, MPI_SUM,
             rankSubset->mpiCommunicator());
  MPI_Allreduce(&localSize, &globalSize, 1, MPI_INT, MPI_SUM,
                rankSubset->mpiCommunicator());

  // define variables
  if (!adiosFieldVariableVm_) {
    // name, global size, offset, local size
    adiosFieldVariableVm_ = std::make_shared<adios2::Variable<double>>(
        currentWriter->adiosIo->DefineVariable<double>(
            "vm", {(long unsigned int)globalSize}, {(long unsigned int)offset},
            {(long unsigned int)localSize}, adios2::ConstantDims));

    LOG(DEBUG) << "define variable \"vm\", localSize: " << localSize
               << ", offset: " << offset << ", globalSize: " << globalSize;

    if (!emgValues.empty()) {
      LOG(DEBUG) << "n values: " << vmValues.size() << ", " << emgValues.size()
                 << ", " << transmembraneFlowValues.size();

      assert(vmValues.size() == emgValues.size());
      assert(vmValues.size() == transmembraneFlowValues.size());

      // name, global size, offset, local size
      adiosFieldVariableEmg_ = std::make_shared<adios2::Variable<double>>(
          currentWriter->adiosIo->DefineVariable<double>(
              "emg", {(long unsigned int)globalSize},
              {(long unsigned int)offset}, {(long unsigned int)localSize},
              adios2::ConstantDims));

      // name, global size, offset, local size
      adiosFieldVariableTransmembraneFlow_ =
          std::make_shared<adios2::Variable<double>>(
              currentWriter->adiosIo->DefineVariable<double>(
                  "transmembraneFlow", {(long unsigned int)globalSize},
                  {(long unsigned int)offset}, {(long unsigned int)localSize},
                  adios2::ConstantDims));
    }

    offsetVariable_ = std::make_shared<adios2::Variable<int>>(
        currentWriter->adiosIo->DefineVariable<int>(
            "nodeOffsetOnOwnComputeNode", {(long unsigned int)nRanks},
            {(long unsigned int)ownRankNo}, {1}, adios2::ConstantDims));

    rankNoVariable_ = std::make_shared<adios2::Variable<int>>(
        currentWriter->adiosIo->DefineVariable<int>(
            "rankNo", {(long unsigned int)globalSize},
            {(long unsigned int)offset}, {(long unsigned int)localSize},
            adios2::ConstantDims));
  }

  // write data to file
  currentWriter->engine->Put<double>(*adiosFieldVariableVm_.get(),
                                     vmValues.data());

  if (!emgValues.empty()) {
    // write data to file
    currentWriter->engine->Put<double>(*adiosFieldVariableEmg_.get(),
                                       emgValues.data());

    // write data to file
    currentWriter->engine->Put<double>(
        *adiosFieldVariableTransmembraneFlow_.get(),
        transmembraneFlowValues.data());
  }
  ...
  // write data to file
  currentWriter->engine->Put<int>(
      *rankNoVariable_.get(),
      currentWriter->megaMolWriterContext.rankNo.data());
}
...
void MegaMol::notifyMegaMol() {
  if (DihuContext::ownRankNoCommWorld() == 0 && context_.zmqSocket()) {
    ...
    message << "mmSetParamValue(\"::dat::filename\", \""
            << adiosOutputFilename.str() << "\")";
