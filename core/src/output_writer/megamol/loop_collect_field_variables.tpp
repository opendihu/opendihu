...
template <typename FieldVariablesForOutputWriterType,
  ...
  if (collectFieldVariables<typename std::tuple_element<
                                i, FieldVariablesForOutputWriterType>::type,
                            FunctionSpaceType>(std::get<i>(fieldVariables),
                                               meshName, geometryField,
                                               scalarFieldVariables))
    return;

  // advance iteration to next tuple element
  loopCollectFieldVariables<FieldVariablesForOutputWriterType,
                            FunctionSpaceType, i + 1>(
...
typename std::enable_if<
    !TypeUtility::isTuple<CurrentFieldVariableType>::value &&
        ...
        std::is_same<
            ...
            FunctionSpaceType>::value,
    bool>::type
collectFieldVariables(
    CurrentFieldVariableType currentFieldVariable, std::string meshName,
    std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 3>>
        &geometryField,
    std::vector<
        std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 1>>>
  ...
  geometryField =
      std::make_shared<FieldVariable::FieldVariable<FunctionSpaceType, 3>>(
          currentFieldVariable->functionSpace()->geometryField());
...
collectFieldVariables(
    ...
    std::vector<
        std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 1>>>
        &scalarFieldVariables) {
  // FunctionSpaceType is different from the function space that is used in the
  // currentFieldVariable, therefore, the geometry field cannot be extracted
  // here
  return false; // do not break iteration
}
...
collectFieldVariables(
    TupleType currentFieldVariableTuple, std::string meshName,
    std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 3>>
        &geometryField,
    std::vector<
        std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 1>>>
  ...
  loopCollectFieldVariables<TupleType, FunctionSpaceType>(
      currentFieldVariableTuple, meshName, geometryField, scalarFieldVariables);
...
collectFieldVariables(
    VectorType currentFieldVariableGradient, std::string meshName,
    std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 3>>
        &geometryField,
    std::vector<
        std::shared_ptr<FieldVariable::FieldVariable<FunctionSpaceType, 1>>>
  ...
  for (auto &currentFieldVariable : currentFieldVariableGradient) {
    ...
    if (collectFieldVariables<typename VectorType::value_type>(
            currentFieldVariable, meshName, geometryField,
            scalarFieldVariables))
...
collectFieldVariables(
  ...
  for (auto &currentSubFieldVariable : subFieldVariables) {
    // call function on all vector entries
    if (collectFieldVariables<std::shared_ptr<SubFieldVariableType>,
                              FunctionSpaceType>(currentSubFieldVariable,
                                                 meshName, geometryField,
                                                 scalarFieldVariables))
      return true;
  }
