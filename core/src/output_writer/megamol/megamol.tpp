...
void MegaMol::write(DataType &data, int timeStepNo, double currentTime,
  ...
  std::shared_ptr<Partition::RankSubset> rankSubset =
      context_.partitionManager()->rankSubsetForCollectiveOperations();
  ...
  try {
    ...
    if (useFrontBackBuffer_) {
      ...
      if (adiosWriters_.find(this->filenameBase_) == adiosWriters_.end()) {
        adiosWriters_[this->filenameBase_] =
            std::array<std::shared_ptr<MegaMol::adios_writer_t>, 2>(
                {nullptr, nullptr});
      }

      if (!adiosWriters_[this->filenameBase_][currentOpenWriterIndex_]) {
        adiosWriters_[this->filenameBase_][currentOpenWriterIndex_] =
            std::make_shared<adios_writer_t>();
        adiosWriters_[this->filenameBase_][currentOpenWriterIndex_]
            ->nOpenWriters = 0;
        adiosWriters_[this->filenameBase_][currentOpenWriterIndex_]->adiosIo =
            std::make_shared<adios2::IO>(context_.adios()->DeclareIO(
                this->filenameBase_ +
                std::string(1, '0' + currentOpenWriterIndex_)));
        assert(adiosWriters_[this->filenameBase_][currentOpenWriterIndex_]
                   ->adiosIo);
      }
      ...
      if (!(adiosWriter_[this->filenameBase_])) {
        ...
        adiosWriter_[this->filenameBase_]->adiosIo =
            std::make_shared<adios2::IO>(
    ...
    if (currentWriter->nOpenWriters == 0) {
      ...
      if (!currentWriter->engine) {
        LOG(DEBUG) << "create new engine, on outputFileName: \""
                   << outputFileName.str() << "\"";

        // create new writer
        adios2::Engine engine = currentWriter->adiosIo->Open(
            outputFileName.str(), adios2::Mode::Write,
        ...
        currentWriter->megaMolWriterContext.approximateDistanceBetweenFibers =
            -1; // initialize to not set

        // write python script and meta data as attribute
        currentWriter->adiosIo->DefineAttribute<std::string>(
            "config", this->context_.pythonScriptText());
        currentWriter->adiosIo->DefineAttribute<std::string>(
            "version", this->context_.versionText());
        currentWriter->adiosIo->DefineAttribute<std::string>(
            "meta", this->context_.metaText());
      }
    ...
    LOG(DEBUG) << "in MegaMol::write \"" << this->filenameBase_ << "\", "
               << "opening time step, currentOpenWriterIndex_: "
               << currentOpenWriterIndex_
               << ", nOpenWriters: " << currentWriter->nOpenWriters;
    ...
    for (std::string meshName : meshNames) {
      ...
      MegaMolLoopOverTuple::loopOutput(data.getFieldVariablesForOutputWriter(),
                                       data.getFieldVariablesForOutputWriter(),
                                       meshName, specificSettings_,
    ...
    // LOG(DEBUG) << "timeStepCloseInterval: " << timeStepCloseInterval << ",
    // writeCallCount_: " << writeCallCount_;

    // int timeStepCloseInterval =
    // specificSettings_.getOptionInt("timeStepCloseInterval",1); if
    // (writeCallCount_ % timeStepCloseInterval == 0 && writeCallCount_ > 0)

    LOG(DEBUG) << "in MegaMol::write, \"" << this->filenameBase_
               << " closing time step, currentOpenWriterIndex_: "
               << currentOpenWriterIndex_
               << ", nOpenWriters: " << currentWriter->nOpenWriters
               << ", combineNInstances_: " << combineNInstances_;

    if (currentWriter->nOpenWriters > combineNInstances_) {
      LOG(FATAL) << "More open MegaMol writers (" << currentWriter->nOpenWriters
                 << ") on file \"" << this->filenameBase_
                 << "\" than allowed (combineNInstances_ = "
                 << combineNInstances_ << ")";
    }
    ...
    LOG(ERROR) << "Writer \"" << this->filenameBase_
