...
void MegaMolWriter<FunctionSpaceType, FieldVariablesForOutputWriterType>::
    outputData(FieldVariablesForOutputWriterType fieldVariables,
               std::string meshName,
  ...
  // collect the geometryField and all scalar field variables for the current
  // mesh
  MegaMolLoopOverTuple::loopCollectFieldVariables(
      fieldVariables, meshName, geometryField, scalarFieldVariables);

  // retrieve the new geometry field values
  int oldSize = megaMolWriterContext.geometryFieldVectorValues.size();
  geometryField->getValuesWithoutGhosts(
      megaMolWriterContext.geometryFieldVectorValues);

  LOG(DEBUG) << "nPointsPerCoordinateDirection: "
             << megaMolWriterContext.nPointsPerCoordinateDirection;
  LOG(DEBUG) << "number of scalar field variables: "
             << scalarFieldVariables.size();

  // get first other scalar field variable
  if (scalarFieldVariables.size() > 0 && scalarFieldVariables.size() <= 1) {
    scalarFieldVariables[0]->getValuesWithoutGhosts(
        megaMolWriterContext.vmValues);
    LOG(DEBUG) << "mesh \"" << meshName << "\", retrieve field variable \""
               << scalarFieldVariables[0]->name()
               << "\", n values: " << megaMolWriterContext.vmValues.size();

    megaMolWriterContext.nPointsPerCoordinateDirection[0] =
        geometryField->functionSpace()->meshPartition()->nNodesGlobal(0);
    ...
    if (geometryField->functionSpace()->dim() >= 2)
      megaMolWriterContext.nPointsPerCoordinateDirection[1] =
          geometryField->functionSpace()->meshPartition()->nNodesGlobal(1);
    else
      megaMolWriterContext.nPointsPerCoordinateDirection[1] = 1;

    if (geometryField->functionSpace()->dim() >= 3)
      megaMolWriterContext.nPointsPerCoordinateDirection[2] =
          geometryField->functionSpace()->meshPartition()->nNodesGlobal(2);
    else
      megaMolWriterContext.nPointsPerCoordinateDirection[1] = 1;

    LOG(DEBUG) << "evaluate names of field variables";
    for (typename std::vector<std::shared_ptr<
             FieldVariable::FieldVariable<FunctionSpaceType, 1>>>::iterator
             iter = scalarFieldVariables.begin();
      ...
      if ((*iter)->name() == "Vm") {
        ...
        (*iter)->getValuesWithoutGhosts(
            megaMolWriterContext.transmembraneFlowValues);
        LOG(DEBUG) << "->trans";
        LOG(DEBUG) << "n values: "
                   << megaMolWriterContext.transmembraneFlowValues.size();
      }
    }
    // LOG(INFO) << "mesh \"" << meshName << "\", retrieve field variable \"" <<
    // scalarFieldVariables[0]->name() << "\", n values: " <<
    // megaMolWriterContext.scalarFieldVariableValues.size();
  }
  ...
  double approximateDistanceBetweenFibers = MathUtility::distance<3>(
      ...
      megaMolWriterContext.geometryFieldVectorValues
          [megaMolWriterContext.geometryFieldVectorValues.size() - 1]);

  // store value as minimum to approximateDistanceBetweenFibers
  if (megaMolWriterContext.approximateDistanceBetweenFibers == -1) {
    megaMolWriterContext.approximateDistanceBetweenFibers =
        approximateDistanceBetweenFibers;
  } else {
    megaMolWriterContext.approximateDistanceBetweenFibers =
        std::min(megaMolWriterContext.approximateDistanceBetweenFibers,
                 approximateDistanceBetweenFibers);
  }
}
#endif
