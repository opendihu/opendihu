...
/**
 * A nonlinear Newton solver configuration that contains all parameters for
 * PETSc and the SNES object.
 */
class Nonlinear : public Linear {
public:
  //! construct solver from python settings
  Nonlinear(PythonConfig specificSettings, MPI_Comm mpiCommunicator,
            std::string name);

  //! return the SNES object that is used for solving (nonlinear solver context)
  std::shared_ptr<SNES> snes();

  //! do nothing for the nonlinear solver, the initialization is already done in
  //! the constructor
  void initialize();

protected:
  std::shared_ptr<SNES>
      snes_; //< the PETSc SNES (scalable nonlinear equations solvers) object
