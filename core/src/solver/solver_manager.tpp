...
std::shared_ptr<SolverType> Manager::solver(PythonConfig settings,
                                            MPI_Comm mpiCommunicator,
                                            std::string solverNameKey) {
  ...
  if (solvers_.find(mpiCommunicator) == solvers_.end()) {
    solvers_[mpiCommunicator] =
        std::map<std::string, std::shared_ptr<Solver>>();
  }
  ...
  if (settings.hasKey(solverNameKey)) {
    ...
    if (hasSolver(solverName, mpiCommunicator)) {
      LOG(DEBUG) << "Solver with solverName \"" << solverName
                 << "\" requested and found, type is "
                 << StringUtility::demangle(
                        typeid(solvers_[mpiCommunicator][solverName]).name());

      return std::static_pointer_cast<SolverType>(
          solvers_[mpiCommunicator][solverName]);
    } else if (solverConfiguration_.find(solverName) !=
               solverConfiguration_.end()) {
      ...
      std::shared_ptr<SolverType> solver = std::make_shared<SolverType>(
      ...
      LOG(ERROR) << "Config contains reference to solver with solverName \""
                 << solverName << "\" but no such solver was defined.";
    }
  } else {
  ...
  LOG(DEBUG) << "Create new solver with type "
             << StringUtility::demangle(typeid(SolverType).name())
             << " and name \"" << anonymousName.str() << "\".";

  // create new solver object
  std::shared_ptr<SolverType> solver = std::make_shared<SolverType>(
      settings, mpiCommunicator, anonymousName.str());
...
template <typename SolverType>
void Manager::deleteSolver(PythonConfig settings, MPI_Comm mpiCommunicator,
                           std::string solverNameKey) {
  ...
  // loop over all stored solvers
  for (std::map<std::string, std::shared_ptr<Solver>>::iterator iter =
           solvers_[mpiCommunicator].begin();
       iter != solvers_[mpiCommunicator].end(); iter++) {
    // check if type matches
