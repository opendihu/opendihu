...
PyObject *PythonUtility::getOptionPyObject(const PyObject *settings,
  ...
  if (PyDict_Contains((PyObject *)settings, key)) {
    ...
    LOG(WARNING) << "" << pathString << "[\"" << keyString
                 ...
                 << "\". Assuming default value " << getString(defaultValue);
    Py_CLEAR(key);
    return defaultValue;
  }
...
double PythonUtility::getOptionDouble(const PyObject *settings,
                                      std::string keyString,
                                      std::string pathString,
                                      double defaultValue,
  ...
  if (PyDict_Contains((PyObject *)settings, key)) {
    ...
    if (PyList_Check(value)) {
      ...
      if (listNEntries >= 1) {
        ...
        if (listNEntries > 1) {
          LOG(WARNING) << "Only using first value " << result
                       << " of list of length " << listNEntries << " at "
                       << pathString << "[\"" << keyString << "\"]."
                       << std::endl;
        }
      } else {
        LOG(WARNING) << "Empty list for " << pathString << "[\"" << keyString
                     << "\"]." << std::endl;
      }
    } else {
  ...
  case Positive:
    if (result <= 0.0) {
      LOG(WARNING) << "Value " << result << " of " << pathString << "[\""
                   << keyString
  ...
  case Between1And3:
    if (result < 1.0) {
      LOG(WARNING) << "Value " << result << " of " << pathString << "[\""
                   << keyString << "\"] is invalid (<1). Using default value "
                   << defaultValue << ".";
      result = defaultValue;
    } else if (result > 3.0) {
      LOG(WARNING) << "Value " << result << " of " << pathString << "[\""
                   << keyString << "\"] is invalid (>3). Using default value "
                   << defaultValue << ".";
      result = defaultValue;
    }
    break;
...
int PythonUtility::getOptionInt(const PyObject *settings, std::string keyString,
  ...
  if (PyDict_Contains((PyObject *)settings, key)) {
    ...
    if (PyList_Check(value)) {
      ...
      if (listNEntries >= 1) {
        ...
        LOG(WARNING) << "Empty list in " << pathString << "[\"" << keyString
                     << "\"]." << std::endl;
      }
    } else {
...
bool PythonUtility::getOptionBool(const PyObject *settings,
                                  std::string keyString, std::string pathString,
                                  bool defaultValue) {
  int result = defaultValue;
...
std::string PythonUtility::getOptionString(const PyObject *settings,
                                           std::string keyString,
                                           std::string pathString,
...
PyObject *PythonUtility::getOptionFunction(const PyObject *settings,
                                           std::string keyString,
                                           std::string pathString) {
  ...
  if (PyDict_Contains((PyObject *)settings, key)) {
    ...
    if (PyFunction_Check(function)) {
      ...
      if (PyCallable_Check(function)) {
        ...
        LOG(WARNING) << "Value for " << pathString << "[\"" << keyString
...
std::string PythonUtility::getString(PyObject *object, int indent,
                                     int first_indent) {
  if (!object)
    return "NULL";
...
bool PythonUtility::getOptionDictEnd(const PyObject *settings,
                                     std::string keyString,
                                     std::string pathString) {
  if (!itemList)
    return true;
...
bool PythonUtility::getOptionListEnd(const PyObject *settings,
                                     std::string keyString,
                                     std::string pathString) {
  if (!list)
    return true;
  ...
  if (settings) {
    ...
    if (PyDict_Contains((PyObject *)settings, key)) {
      ...
      if (PyList_Check(value)) {
        ...
        double value = PythonUtility::getOptionListBegin<double>(
            settings, keyString, pathString);
        int i = 0;

        // loop over other values
        for (; !PythonUtility::getOptionListEnd(settings, keyString,
                                                pathString) &&
               i < nEntries;
             PythonUtility::getOptionListNext<double>(settings, keyString,
                                                      pathString, value),
             i++) {
          values[i] = value;
        }
        ...
        for (;
             !PythonUtility::getOptionDictEnd(settings, keyString, pathString);
             PythonUtility::getOptionDictNext<int, double>(
                 settings, keyString, pathString, dictItem)) {
          ...
          if (index >= 0 && index < nEntries) {
            ...
            LOG(WARNING) << "In " << pathString << "[\"" << keyString
                         << "\"], ignoring key " << index << ", maximum key is "
                         << nEntries - 1;
          }
        }
      } else {
        double value = PythonUtility::getOptionDouble(settings, keyString,
                                                      pathString, 0.0);
      ...
      LOG(WARNING) << "" << pathString << "[\"" << keyString
                   << "\"] not set in \"" << Control::settingsFileName
                   << "\", assuming list with " << nEntries << " zeros";
    }
    Py_CLEAR(key);
  }
}
...
void PythonUtility::checkForError() {
  ...
  if (PyErr_Occurred()) {
    ...
    if (strType != NULL && reprType != NULL)
      LOG(DEBUG) << "type: " << convertFromPython<std::string>::get(strType)
                 << ", " << convertFromPython<std::string>::get(reprType);
    ...
    static PyObject *functionFormatException =
    ...
    PyObject *returnValue = PyObject_CallFunctionObjArgs(
        functionFormatException, type, value, traceback, NULL);
    if (returnValue != NULL) {
      std::vector<std::string> result =
          convertFromPython<std::vector<std::string>>::get(returnValue);
      std::stringstream str;
      for (std::vector<std::string>::iterator resultIter = result.begin();
           resultIter != result.end(); resultIter++) {
        str << *resultIter;
      }
      LOG(ERROR) << str.str();
...
PyObject *PythonUtility::convertToPythonList(unsigned int nEntries,
                                             double *data) {
  // start critical section for python API calls
  // PythonUtility::GlobalInterpreterLock lock;
...
// python GIL handling is only needed for multi-threading. It does not work
// properly with openmp.
// int PythonUtility::GlobalInterpreterLock::nGILS_ = 0;
/*std::recursive_mutex PythonUtility::GlobalInterpreterLock::mutex_;
std::unique_lock<std::recursive_mutex>
PythonUtility::GlobalInterpreterLock::lock_(
