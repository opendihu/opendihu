...
struct PythonUtility::convertFromPython<std::array<ValueType, nComponents>> {
  ...
  static std::array<ValueType, nComponents>
  get(PyObject *object, std::array<ValueType, nComponents> defaultValue,
    ...
    if (PyList_Check(object)) {
      ...
      for (; i < iEnd; i++) {
        result[i] = PythonUtility::convertFromPython<ValueType>::get(
            PyList_GetItem(object, (Py_ssize_t)i), defaultValue[i]);
      }
      ...
      for (; i < iEnd; i++) {
        result[i] = PythonUtility::convertFromPython<ValueType>::get(
            PyTuple_GetItem(object, (Py_ssize_t)i), defaultValue[i]);
      }
      ...
      for (int itemListIndex = 0; itemListIndex < PyList_Size(itemList);
           itemListIndex++) {
      ...
      ValueType valueDouble = PythonUtility::convertFromPython<ValueType>::get(
          object, defaultValue[0]);

      result[0] = valueDouble;
      std::copy(defaultValue.begin() + 1, defaultValue.end(),
                result.begin() + 1);
  ...
  static std::array<ValueType, nComponents> get(PyObject *object) {
    ...
    return convertFromPython<std::array<ValueType, nComponents>>::get(
        object, defaultValue);
  }
};
...
struct PythonUtility::convertFromPython<std::array<ValueType, nComponents>> {
  ...
  get(PyObject *object, std::array<ValueType, nComponents> defaultValue,
    ...
    if (PyList_Check(object)) {
      global_no_t i = 0;
      global_no_t iEnd =
          std::min((global_no_t)PyList_Size(object), nComponents);
...
struct PythonUtility::convertFromPython<std::array<ValueType, nComponents>> {
  ...
  get(PyObject *object, std::array<ValueType, nComponents> defaultValue,
    ...
    if (PyList_Check(object)) {
      unsigned long i = 0;
      unsigned long iEnd =
          std::min((unsigned long)PyList_Size(object), nComponents);

      for (; i < iEnd; i++) {
        result[i] = PythonUtility::convertFromPython<ValueType>::get(
            PyList_GetItem(object, (Py_ssize_t)i), (ValueType)defaultValue[i]);
      }
...
struct PythonUtility::convertFromPython<MathUtility::Matrix<nRows, nColumns>> {
  //! convert a python object to its corresponding c type, with type checking,
  //! if conversion is not possible, use defaultValue
  static MathUtility::Matrix<nRows, nColumns>
  get(PyObject *object, MathUtility::Matrix<nRows, nColumns> defaultValue) {
    return PythonUtility::convertFromPython<
        std::array<double, nRows * nColumns>>::get(object, defaultValue);
  }
...
struct PythonUtility::convertFromPython<std::vector<ValueType>> {
  ...
  static std::vector<ValueType> get(PyObject *object,
                                    std::vector<ValueType> defaultValue) {
    // start critical section for python API calls
    // PythonUtility::GlobalInterpreterLock lock;
    ...
    if (PyList_Check(object)) {
      ...
      for (int i = 0; i < nEntries; i++) {
        ...
        result[i] = PythonUtility::convertFromPython<ValueType>::get(
            PyList_GetItem(object, (Py_ssize_t)i), defaultValueItem);
      }
      return result;
    } else if (PyTuple_Check(object)) {
  ...
  static std::vector<ValueType> get(PyObject *object, ValueType defaultValue) {
    ...
    if (PyList_Check(object)) {
      ...
      for (int i = 0; i < nEntries; i++) {
        result[i] = PythonUtility::convertFromPython<ValueType>::get(
            PyList_GetItem(object, (Py_ssize_t)i));
      }
      return result;
    } else if (PyTuple_Check(object)) {
      int nEntries = PyTuple_Size(object);
      ...
      for (int i = 0; i < nEntries; i++) {
        result[i] = PythonUtility::convertFromPython<ValueType>::get(
            PyTuple_GetItem(object, (Py_ssize_t)i));
      }
      return result;
    } else if (PyDict_Check(object)) {
      PyObject *itemList = PyDict_Items(object);
      ...
      ValueType valueDouble =
          PythonUtility::convertFromPython<ValueType>::get(object);
...
// partial specialization for std::vector<std::pair<>>, this can convert tuples
// to [<key,value>,...] vectors
template <typename KeyType, typename ValueType>
struct PythonUtility::convertFromPython<
  ...
  static std::vector<std::pair<KeyType, ValueType>> get(PyObject *object) {
    ...
    if (PyDict_Check(object)) {
      ...
      for (int i = 0; i < nEntries; i++) {
        result[i] = PythonUtility::convertFromPython<
            std::pair<KeyType, ValueType>>::get(PyList_GetItem(object,
                                                               (Py_ssize_t)i));
      }
      return result;
    } else if (PyTuple_Check(object)) {
      ...
      for (int i = 0; i < nEntries; i++) {
        result[i] = PythonUtility::convertFromPython<
            std::pair<KeyType, ValueType>>::get(PyTuple_GetItem(object,
                                                                (Py_ssize_t)i));
      }
      return result;
    } else {
...
struct PythonUtility::convertFromPython<std::pair<ValueType1, ValueType2>> {
  //! convert a python object to its corresponding c type, with type checking,
  //! if conversion is not possible, use defaultValue
  static std::pair<ValueType1, ValueType2>
  get(PyObject *object, std::pair<ValueType1, ValueType2> defaultValue) {
    // start critical section for python API calls
    // PythonUtility::GlobalInterpreterLock lock;
    ...
    if (PyTuple_Check(object)) {
      ...
      if (nEntries != 2) {
        LOG(WARNING) << "Converting python tuple to pair, but the tuple has "
                     << nEntries << " != 2 entries.";
      }

      result.first = PythonUtility::convertFromPython<ValueType1>::get(
          PyTuple_GetItem(object, (Py_ssize_t)0), defaultValue.first);
      result.second = PythonUtility::convertFromPython<ValueType2>::get(
          PyTuple_GetItem(object, (Py_ssize_t)1), defaultValue.second);
  ...
  static std::pair<ValueType1, ValueType2> get(PyObject *object) {
    ...
    return PythonUtility::convertFromPython<
        std::pair<ValueType1, ValueType2>>::get(object, defaultValue);
  }
};
...
ExtractTuple(PyObject *object, int nEntries,
...
ExtractTuple(PyObject *object, int nEntries,
             std::tuple<ValueTypes...> &result) {
  typedef
      typename std::tuple_element<index, std::tuple<ValueTypes...>>::type Type;
  if (index < nEntries) {
    ...
    // if the current index is higher than the number of entries in the python
    // config, default values are used
  ...
  static std::tuple<ValueTypes...> get(PyObject *object) {
    ...
    if (PyTuple_Check(object)) {
      ...
      if (nEntriesTuple < nEntries) {
        LOG(WARNING) << "Python tuple has " << nEntries
                     << " entries, expected are only " << nEntriesTuple << ".";
      } else if (nEntriesTuple > nEntries) {
        LOG(WARNING) << "Python tuple has only " << nEntries << " entries, but "
                     << nEntriesTuple
                     << " are required, using 0 for the other values.";
      }
...
struct PythonUtility::convertToPython<std::map<KeyType, ValueType>> {
  ...
  static PyObject *get(const std::map<KeyType, ValueType> &value) {
    ...
    for (const std::pair<const KeyType, ValueType> &entry : value) {
      ...
      PyObject *value =
          PythonUtility::convertToPython<ValueType>::get(entry.second);
