...
double_v_t computeApproximateMeshWidth(
  ...
  for (int componentNo = 0; componentNo < 3; componentNo++) {
    result += sqr(geometryValues[0][componentNo] -
                  geometryValues[nNodes - 1][componentNo]);
  }
  // LOG(INFO) << "h=" << result;
  return result;
}
...
computeIntegrationFactor(const std::array<VecD<3, double_v_t>, 2> &jacobian) {
  ...
  /*
  LOG(DEBUG) << "2D integration factor, m11:" << m11 << ", m21: " << m21 << ",
  m32: " << m32 << ", sqr(m11*m22): " << sqr(m11*m22) << ", sqrt "  <<
  sqr(m11*m22) + sqr(m11*m32) - 2*m11*m12*m21*m22 - 2*m11*m12*m31*m32
    + sqr(m12*m21) + sqr(m12*m31) + sqr(m21*m32) - 2*m21*m22*m31*m32 +
  sqr(m22*m31) << ", result: " << sqrt(sqr(m11*m22) + sqr(m11*m32) -
  2*m11*m12*m21*m22 - 2*m11*m12*m31*m32
    + sqr(m12*m21) + sqr(m12*m31) + sqr(m21*m32) - 2*m21*m22*m31*m32 +
  sqr(m22*m31));
    */

  return sqrt(sqr(m11 * m22) + sqr(m11 * m32) - 2 * m11 * m12 * m21 * m22 -
              2 * m11 * m12 * m31 * m32 + sqr(m12 * m21) + sqr(m12 * m31) +
              sqr(m21 * m32) - 2 * m21 * m22 * m31 * m32 + sqr(m22 * m31));
}
...
computeIntegrationFactor(const std::array<VecD<3, double_v_t>, 3> &jacobian) {
  ...
  double_v_t determinant = m11 * m22 * m33 - m11 * m23 * m32 - m12 * m21 * m33 +
...
double_v_t computeDeterminant(const Tensor2<3, double_v_t> &jacobian) {
  ...
  return m11 * m22 * m33 - m11 * m23 * m32 - m12 * m21 * m33 + m12 * m23 * m31 +
...
computeSymmetricInverse(const Tensor2<3, double_v_t> &matrix,
  ...
  determinant = m11 * m22 * m33 - m11 * sqr(m32) - sqr(m21) * m33 +
                2 * m21 * m31 * m32 - m22 * sqr(m31);

  // regularize matrix if near singular, by adding ε*h*I (small values on
  // diagonal), scaling with h is for consistency
  double_v_t regularizationEpsilon = 0;
  // where operator is documented here:
  // https://vcdevel.github.io/Vc-1.4.1/group__Utilities.html#gaa18ac68167ac7614731134de7364a1d5
  Vc::where(abs(determinant) < INVERSE_REGULARIZATION_TOLERANCE,
            regularizationEpsilon) =
      INVERSE_REGULARIZATION_EPSILON * approximateMeshWidth;
...
Tensor2<1, double_v_t> computeInverse(const Tensor2<1, double_v_t> &matrix,
  ...
  // where operator is documented here:
  // https://vcdevel.github.io/Vc-1.4.1/group__Utilities.html#gaa18ac68167ac7614731134de7364a1d5
  Vc::where(abs(matrix[0][0]) < INVERSE_REGULARIZATION_TOLERANCE,
            regularizationEpsilon) =
      INVERSE_REGULARIZATION_EPSILON * approximateMeshWidth;
...
Tensor2<3, double_v_t> computeInverse(const Tensor2<3, double_v_t> &matrix,
  ...
  determinant = m11 * m22 * m33 - m11 * m23 * m32 - m12 * m21 * m33 +
                m12 * m23 * m31 + m13 * m21 * m32 - m13 * m22 * m31;
...
std::array<double_v_t, 9> computeTransformationMatrixAndDeterminant(
  ...
  result[0] =
      prefactor * (sqr(m12 * m23 - m13 * m22) + sqr(m12 * m33 - m13 * m32) +
                   sqr(m22 * m33 - m23 * m32));
  result[1] = prefactor * (-(m11 * m23 - m13 * m21) * (m12 * m23 - m13 * m22) -
                           (m11 * m33 - m13 * m31) * (m12 * m33 - m13 * m32) -
                           (m21 * m33 - m23 * m31) * (m22 * m33 - m23 * m32));
  result[2] = prefactor * ((m11 * m22 - m12 * m21) * (m12 * m23 - m13 * m22) +
                           (m11 * m32 - m12 * m31) * (m12 * m33 - m13 * m32) +
                           (m21 * m32 - m22 * m31) * (m22 * m33 - m23 * m32));
  result[3] = result[1];
  result[4] =
      prefactor * (sqr(m11 * m23 - m13 * m21) + sqr(m11 * m33 - m13 * m31) +
                   sqr(m21 * m33 - m23 * m31));
  result[5] = prefactor * (-(m11 * m22 - m12 * m21) * (m11 * m23 - m13 * m21) -
                           (m11 * m32 - m12 * m31) * (m11 * m33 - m13 * m31) -
                           (m21 * m32 - m22 * m31) * (m21 * m33 - m23 * m31));
  result[6] = result[2];
  result[7] = result[5];
  result[8] =
      prefactor * (sqr(m11 * m22 - m12 * m21) + sqr(m11 * m32 - m12 * m31) +
                   sqr(m21 * m32 - m22 * m31));
...
std::array<double_v_t, 9> computeTransformationDiffusionMatrixAndDeterminant(
  ...
  // compute matrix entries of symmetric matrix  J^-1 A J^-T  assuming that A is
  // symmetric
  std::array<double_v_t, 9> result;
  result[0] =
      prefactor * ((m12 * m23 - m13 * m22) * (a13 * (m22 * m33 - m23 * m32) -
                                              a23 * (m12 * m33 - m13 * m32) +
                                              a33 * (m12 * m23 - m13 * m22)) -
                   (m12 * m33 - m13 * m32) * (a12 * (m22 * m33 - m23 * m32) -
                                              a22 * (m12 * m33 - m13 * m32) +
                                              a32 * (m12 * m23 - m13 * m22)) +
                   (m22 * m33 - m23 * m32) * (a11 * (m22 * m33 - m23 * m32) -
                                              a21 * (m12 * m33 - m13 * m32) +
                                              a31 * (m12 * m23 - m13 * m22)));
  result[1] =
      prefactor * (-(m11 * m23 - m13 * m21) * (a13 * (m22 * m33 - m23 * m32) -
                                               a23 * (m12 * m33 - m13 * m32) +
                                               a33 * (m12 * m23 - m13 * m22)) +
                   (m11 * m33 - m13 * m31) * (a12 * (m22 * m33 - m23 * m32) -
                                              a22 * (m12 * m33 - m13 * m32) +
                                              a32 * (m12 * m23 - m13 * m22)) -
                   (m21 * m33 - m23 * m31) * (a11 * (m22 * m33 - m23 * m32) -
                                              a21 * (m12 * m33 - m13 * m32) +
                                              a31 * (m12 * m23 - m13 * m22)));
  result[2] =
      prefactor * ((m11 * m22 - m12 * m21) * (a13 * (m22 * m33 - m23 * m32) -
                                              a23 * (m12 * m33 - m13 * m32) +
                                              a33 * (m12 * m23 - m13 * m22)) -
                   (m11 * m32 - m12 * m31) * (a12 * (m22 * m33 - m23 * m32) -
                                              a22 * (m12 * m33 - m13 * m32) +
                                              a32 * (m12 * m23 - m13 * m22)) +
                   (m21 * m32 - m22 * m31) * (a11 * (m22 * m33 - m23 * m32) -
                                              a21 * (m12 * m33 - m13 * m32) +
                                              a31 * (m12 * m23 - m13 * m22)));
  result[4] =
      prefactor * ((m11 * m23 - m13 * m21) * (a13 * (m21 * m33 - m23 * m31) -
                                              a23 * (m11 * m33 - m13 * m31) +
                                              a33 * (m11 * m23 - m13 * m21)) -
                   (m11 * m33 - m13 * m31) * (a12 * (m21 * m33 - m23 * m31) -
                                              a22 * (m11 * m33 - m13 * m31) +
                                              a32 * (m11 * m23 - m13 * m21)) +
                   (m21 * m33 - m23 * m31) * (a11 * (m21 * m33 - m23 * m31) -
                                              a21 * (m11 * m33 - m13 * m31) +
                                              a31 * (m11 * m23 - m13 * m21)));
  result[5] =
      prefactor * (-(m11 * m22 - m12 * m21) * (a13 * (m21 * m33 - m23 * m31) -
                                               a23 * (m11 * m33 - m13 * m31) +
                                               a33 * (m11 * m23 - m13 * m21)) +
                   (m11 * m32 - m12 * m31) * (a12 * (m21 * m33 - m23 * m31) -
                                              a22 * (m11 * m33 - m13 * m31) +
                                              a32 * (m11 * m23 - m13 * m21)) -
                   (m21 * m32 - m22 * m31) * (a11 * (m21 * m33 - m23 * m31) -
                                              a21 * (m11 * m33 - m13 * m31) +
                                              a31 * (m11 * m23 - m13 * m21)));
  result[8] =
      prefactor * ((m11 * m22 - m12 * m21) * (a13 * (m21 * m32 - m22 * m31) -
                                              a23 * (m11 * m32 - m12 * m31) +
                                              a33 * (m11 * m22 - m12 * m21)) -
                   (m11 * m32 - m12 * m31) * (a12 * (m21 * m32 - m22 * m31) -
                                              a22 * (m11 * m32 - m12 * m31) +
                                              a32 * (m11 * m22 - m12 * m21)) +
                   (m21 * m32 - m22 * m31) * (a11 * (m21 * m32 - m22 * m31) -
                                              a21 * (m11 * m32 - m12 * m31) +
                                              a31 * (m11 * m22 - m12 * m21)));
  // we know that the diffusion tensor is symmetric. Therefore  J^-1 A J^-T  is
  // symmetric, too.
  result[3] = result[1];
  result[6] = result[2];
  result[7] = result[5];
  // result[3] = prefactor * (-(m12*m23 - m13*m22)*(a13*(m21*m33 - m23*m31) -
  // a23*(m11*m33 - m13*m31) + a33*(m11*m23 - m13*m21)) + (m12*m33 -
  // m13*m32)*(a12*(m21*m33 - m23*m31) - a22*(m11*m33 - m13*m31) + a32*(m11*m23
  // - m13*m21)) - (m22*m33 - m23*m32)*(a11*(m21*m33 - m23*m31) - a21*(m11*m33 -
  // m13*m31) + a31*(m11*m23 - m13*m21))); result[6] = prefactor * ((m12*m23 -
  // m13*m22)*(a13*(m21*m32 - m22*m31) - a23*(m11*m32 - m12*m31) + a33*(m11*m22
  // - m12*m21)) - (m12*m33 - m13*m32)*(a12*(m21*m32 - m22*m31) - a22*(m11*m32 -
  // m12*m31) + a32*(m11*m22 - m12*m21)) + (m22*m33 - m23*m32)*(a11*(m21*m32 -
  // m22*m31) - a21*(m11*m32 - m12*m31) + a31*(m11*m22 - m12*m21))); result[7] =
  // prefactor * (-(m11*m23 - m13*m21)*(a13*(m21*m32 - m22*m31) - a23*(m11*m32 -
  // m12*m31) + a33*(m11*m22 - m12*m21)) + (m11*m33 - m13*m31)*(a12*(m21*m32 -
  // m22*m31) - a22*(m11*m32 - m12*m31) + a32*(m11*m22 - m12*m21)) - (m21*m33 -
  // m23*m31)*(a11*(m21*m32 - m22*m31) - a21*(m11*m32 - m12*m31) + a31*(m11*m22
  // - m12*m21)));
...
applyTransformation(const std::array<double_v1_t, 9> &transformationMatrix,
  ...
  // compute result
  result = v21 * (m11 * v11 + m12 * v12 + m13 * v13) +
           v22 * (m12 * v11 + m22 * v12 + m23 * v13) +
           v23 * (m13 * v11 + m23 * v12 + m33 * v13);
  return result;
}
...
void rotateMatrix(Matrix<3, 3, double_v_t> &matrix,
  ...
  const double_v_t be3bNorm = sqrt(sqr(b1) * sqr(b3) + sqr(b2) * sqr(b3) +
                                   sqr(sqr(b1) + sqr(b2))); // |b x (e_3 x b)|

  if (Vc::any_of(MathUtility::abs(bNorm) < 1e-15)) {
    LOG(ERROR) << "Trying to transform a matrix to direction vector "
               << directionVector;
    return;
  }

  // if one of the new basis vectors is zero (this happens, if directionVector
  // == [0,0,1]), use e2 instead of e3
  if (Vc::any_of(MathUtility::abs(e3bNorm) < 1e-15) ||
      Vc::any_of(MathUtility::abs(be3bNorm) < 1e-15)) {
    const double_v_t e2bNorm = sqrt(sqr(b3) + sqr(b1)); // |e_2 x b|
    const double_v_t be2bNorm =
        sqrt(sqr(b1) * sqr(b2) + sqr(sqr(b1) * sqr(b3)) + sqr(b2) +
             sqr(b3)); // |b x (e_3 x b)|

    if (Vc::any_of(MathUtility::abs(e2bNorm) < 1e-15) ||
        Vc::any_of(MathUtility::abs(be2bNorm) < 1e-15)) {
      LOG(ERROR) << "Trying to transform a matrix to direction vector "
                 << directionVector;
      return;
    }
    ...
    Matrix<3, 3, double_v_t> basis(
        {// normalized(b)    normalized(e_2 x b)   normalized(b x e_2 x b)
         b1 / bNorm, b3 / e2bNorm, -b1 * b2 / be2bNorm, b2 / bNorm, 0,
         (sqr(b1) + sqr(b3)) / be2bNorm, b3 / bNorm, -b1 / e2bNorm,
         -b2 * b3 / be2bNorm});

    Matrix<3, 3, double_v_t> basis_T({basis(0, 0), basis(1, 0), basis(2, 0),
    ...
    Matrix<3, 3, double_v_t> basis(
        {// normalized(b)    normalized(e_3 x b)   normalized(b x e_3 x b)
         b1 / bNorm, -b2 / e3bNorm, -b1 * b3 / be3bNorm, b2 / bNorm,
         b1 / e3bNorm, -b2 * b3 / be3bNorm, b3 / bNorm, 0,
         (sqr(b1) + sqr(b2)) / be3bNorm});
