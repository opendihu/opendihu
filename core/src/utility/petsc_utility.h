...
//! extract all entries of the PETSc matrix and store values row-major to
//! std::vector. The vector is resized to the needed size.
...
//! create a Vec Petsc object, this function is to be removed, currently only
//! needed because of solid mechanics
...
//! check if the matrix and vector number of entries are correct such that
//! stiffnessMatrix can be multiplied to rhs
...
void dumpVector(std::string filename, std::string format, Vec &vector,
                MPI_Comm mpiCommunicator, int componentNo = 0,
                int nComponents = 1);

//! dump the matrix to a file using PetscViewer, format is "default", "ascii" or
//! "matlab" if the matrix is nested, only one file is created that contains the
//! whole matrix data.
