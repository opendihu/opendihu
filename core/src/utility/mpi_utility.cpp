...
void handleReturnValue(int returnValue, std::string descriptor,
  ...
  if (status != MPI_STATUS_IGNORE && status != nullptr) {
    ...
    */
    // LOG(DEBUG) << "MPI status: count=" << status->_ucount << ", cancelled="
    // << status->_cancelled << ", MPI_SOURCE=" << status->MPI_SOURCE
    //   << ", MPI_TAG=" << status->MPI_TAG << ", MPI_ERROR=" <<
    //   status->MPI_ERROR;
  }

  // in order for the following line to work, enable initialization of this
  // logger in control/initialization/initialize_logging.cpp
  ...
  if (!descriptor.empty()) {
    LOG(ERROR) << "Error in " << descriptor << ": "
               << std::string(errorString.begin(),
                              errorString.begin() + stringLength);
  } else {
    LOG(ERROR) << "Error in MPI function: "
               << std::string(errorString.begin(),
                              errorString.begin() + stringLength);
  }
}
...
void gdbParallelDebuggingBarrier() {
  ...
  if (nRanks > 0) {
#ifdef NDEBUG
    int pid = getpid();
    LOG(INFO) << "Rank " << rankNo << ", PID " << pid
              << " is waiting for gdbResume=" << gdbResume << " to become 1 "
              << std::endl
              << std::endl
              << "gdb -p " << pid << std::endl
              << std::endl
    ...
    LOG(INFO) << "Rank " << rankNo << ", PID " << pid
              << " resumes because gdbResume=" << gdbResume;
  }
}
...
std::string loadFile(std::string filename, MPI_Comm mpiCommunicator) {
  ...
  handleReturnValue(
      MPI_Bcast(&fileLength, 1, MPI_UNSIGNED_LONG_LONG, 0, mpiCommunicator),
      "MPI_Bcast (10)");
  ...
  int offset = 0;
  handleReturnValue(MPI_File_read_at_all(fileHandle, offset,
                                         fileContents.data(), fileLength,
