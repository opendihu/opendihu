 ...
 *
 * This solver encapsulates the DynamicHyperelasticitySolver or
 * HyperelasticitySolver (static). Which one to use can be chosen at runtime in
 * the config. This class adds functionality to compute and transfer active
 * stresses, fiber stretches and contraction velocity and so on.
 */
template <typename MeshType = Mesh::StructuredDeformableOfDimension<3>,
          typename Term = Equation::SolidMechanics::
              TransverselyIsotropicMooneyRivlinIncompressibleActive3D,
          bool withLargeOutputFiles = true>
class MuscleContractionSolver : public Runnable,
...
public:
  ...
  //! make the DisplacementsFunctionSpace of the DynamicHyperelasticitySolver
  //! class available
  typedef typename DynamicHyperelasticitySolverType::DisplacementsFunctionSpace
      FunctionSpace;
...
protected:
  ...
  //! update the geometry at the given meshes, map the own geometry field to the
  //! meshes
  ...
  bool isDynamic_; //< if the dynamic formulation or the quasi-static
                   // formulation is used
  bool enableForceLengthRelation_; //< if the force-length relation factor
                                   // f_l(λ_f) should be multiplied
  double lambdaDotScalingFactor_;  //< scaling factor for the computation of
                                   // lambdaDot
  std::vector<std::string>
      meshNamesOfGeometryToMapTo_; //< a list of mesh names which will get
                                   // updated with the geometry
