...
/** A specialized solver for the multidomain equation, as formulated by Thomas
 * Klotz (2017)
 *
 ...
 * reaction term. V_mk^(i+1) is the output and should be connected to the input
 * of the reaction term.
 *
 * There are two additional output connection slots, active_stress_k (input of
 * active stress from subcellular solver) and active_stress_total (output of
 * total active stress)
 */
template <typename FiniteElementMethodPotentialFlow,
          typename FiniteElementMethodDiffusion>
class MultidomainSolver : public TimeSteppingScheme, public Runnable {
public:
  typedef typename FiniteElementMethodDiffusion::FunctionSpace FunctionSpace;
  typedef typename Data::Multidomain<
      typename FiniteElementMethodDiffusion::FunctionSpace>::FieldVariableType
      FieldVariableType;
  typedef typename Data::Multidomain<
...
protected:
  ...
  //! assemble the sub matrices of the system matrix, that is a block matrix
  //! containing stiffness matrices of the diffusion sub problems
  virtual void setSystemMatrixSubmatrices(double timeStepWidth);

  //! create the nested and single system matrix from the initialized
  //! submatrices
  ...
  //! initialize all information for the linearSolver_ object, also set
  //! information to preconditioner
