 ...
 *  Includes Vc::double_v::size() instances of the CellML problem (usually 4
 * when using AVX-2). These will be computed at once using vector instructions.
 */
template <int nStates> struct FiberPointBuffers {
...
/** Specialize the default allocator for the FiberPointBuffers struct to use the
 * aligned allocated provided by Vc. This could also be done by
 * Vc_DECLARE_ALLOCATOR(<class>), but not here because of the template parameter
 * nStates.
 */
namespace std {
...
/** The implementation of a monodomain solver as used in the fibers_emg example,
 * number of states and algebraics is templated. This class contains all
 * functionality except the reaction term. Deriving classes only need to
 * implement compute0D.
 */
...
public:
  ...
  typedef CellmlAdapter<nStates, nAlgebraics, // nStates,nAlgebraics: 57,1 =
  ...
  //! save the current state of the locally computed fibers in a checkpoint
  //! variable, to be later restored by restoreFiberDataCheckpoint()
  void saveFiberDataCheckpoint();

  //! restore the checkpoint created by saveFiberDataCheckpoint() and set the
  //! local fiber's state to the saved state in the checkpoint
...
protected:
  //! load the firing times file and initialize the firingEvents_ and
  //! motorUnitNo_ variables
  ...
  //! create a source file with compute0D function from the CellML model, using
  //! the vc optimization type
  void initializeCellMLSourceFileVc();

  //! create a source file with compute0D function from the CellML model, using
  //! the gpu optimization type
  ...
  //! solve the 0D problem, starting from startTime. This is the part that is
  //! usually provided by the cellml file
  void compute0D(double startTime, double timeStepWidth, int nTimeSteps,
                 bool storeAlgebraicsForTransfer);

  //! compute one time step of the right hand side for a single simd vector of
  //! instances
  virtual void
  compute0DInstance(Vc::double_v states[],
  ...
  //! method to be called after the compute0D, updates the information in
  //! fiberPointBuffersStatesAreCloseToEquilibrium_
  void equilibriumAccelerationUpdate(const Vc::double_v statesPreviousValues[],
                                     int pointBuffersNo);

  //! check if the 0D computations for the current point are disabled because
  //! the states are in equilibrium
  bool isEquilibriumAccelerationCurrentPointDisabled(bool stimulateCurrentPoint,
                                                     int pointBuffersNo);
  ...
  //! generate source code to solve the monodomain equation with offloading to
  //! gpu
  void generateMonodomainSolverGpuSource(std::string filename,
  ...
  //! find out the currently used version of GCC, return as a string in the
  //! format, e.g., "10.2.0"
