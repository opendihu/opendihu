...
/** This is a template class that developers can copy and adjust to create their
 * own solver. This solver is a timestepping scheme. This means that something
 * is done iteratively in time. For a static solver, refer to the other
 * template, "my_new_static_solver.h". There are also the files
 * "data_management/my_new_timestepping_solver.{h,cpp}" that need to be
 * adjusted. At the end, add an #include to this file in "opendihu.h".
 *
 *  Briefly explain what your solver does in this comment section.
 */
template <typename FiniteElementMethod>
class DiffusionAdvectionSolver : public Runnable,
...
public:
  ...
  // typedef typename FiniteElementMethod::Data Data;   // either, if you do not
  // need your own data object, use the data object of FiniteElementMethod
  typedef ::Data::DiffusionAdvectionSolver<
      typename FiniteElementMethod::FunctionSpace>
