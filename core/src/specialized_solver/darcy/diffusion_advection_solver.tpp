...
DiffusionAdvectionSolver<FiniteElementMethod>::DiffusionAdvectionSolver(
    DihuContext context)
    : Runnable(),
      ::TimeSteppingScheme::TimeSteppingScheme(
          context["DiffusionAdvectionSolver"]), // replace
                                                // "DiffusionAdvectionSolver" by
                                                // the name of your solver, this
                                                // will be the key for the dict
                                                // in settings
      finiteElementMethod_(this->context_), data_(this->context_) {
  ...
  // parse options
  advectionVelocity_ = this->specificSettings_.getOptionArray<double, 2>(
      "advectionVelocity", std::array<double, 2>{0, 0});
...
void DiffusionAdvectionSolver<FiniteElementMethod>::advanceTimeSpan(
  ...
  LOG(DEBUG) << "DiffusionAdvectionSolver::advanceTimeSpan, timeSpan="
  ...
  for (; timeStepNo < this->numberTimeSteps_;) {
    ...
    {
      LOG(INFO) << "DiffusionAdvectionSolver, timestep " << timeStepNo << "/"
    ...
    // Set timespan for finiteElementMethod_, the nested solver should advance
    // its simulation by our timeStepWidth_. This, in turn, may lead to multiple
    // timesteps in the finiteElementMethod_.
    // this->finiteElementMethod_.setTimeSpan(currentTime,
    ...
    Mat stiffnessMatrix =
        this->finiteElementMethod_.data().stiffnessMatrix()->valuesGlobal();
    // Mat massMatrix =
    // this->finiteElementMethod_.data().massMatrix()->valuesGlobal();
    Mat inverseLumpedMassMatrix = this->finiteElementMethod_.data()
    ...
    Mat KminusV; // = K - V
    PetscErrorCode ierr;
    ierr = MatDuplicate(stiffnessMatrix, MAT_COPY_VALUES, &KminusV);
    CHKERRV(ierr);
    ierr = MatAXPY(KminusV, -1, vMatrix, SAME_NONZERO_PATTERN);
    CHKERRV(ierr);

    // compute M^-1*(K - V)
    Mat inverseMKminusV;

    ierr = MatMatMult(inverseLumpedMassMatrix, KminusV, MAT_INITIAL_MATRIX,
                      PETSC_DEFAULT, &inverseMKminusV);
    CHKERRV(ierr);

    // increment = (M^-1*(K - V)) * u
    ierr = MatMult(inverseMKminusV, solution, increment);
    CHKERRV(ierr);

    // u = u + dt * increment
    // y = alpha x + y.
    ierr = VecAXPY(solution, this->timeStepWidth_, increment);
    CHKERRV(ierr);
...
template <typename FiniteElementMethod>
void DiffusionAdvectionSolver<FiniteElementMethod>::initialize() {
  ...
  // basis functions). The finiteElementMethod_ solver already created a
  // function space that we should use. We already have a typedef
  // "FunctionSpace" that is the class of finiteElementMethod_'s function space
  // type.
  std::shared_ptr<FunctionSpace> functionSpace =
      finiteElementMethod_.data().functionSpace();
...
template <typename FiniteElementMethod>
void DiffusionAdvectionSolver<FiniteElementMethod>::run() {
...
template <typename FiniteElementMethod>
void DiffusionAdvectionSolver<FiniteElementMethod>::computeVMatrix() {

  typedef Quadrature::Gauss<2> QuadratureType;
  const int nComponents = 1;

  const int D = FunctionSpace::dim();
  LOG(TRACE) << "setStiffnessMatrix " << D
             << "D using integration, FunctionSpace: "
             << StringUtility::demangle(typeid(FunctionSpace).name())
             << ", QuadratureType: "
             << StringUtility::demangle(typeid(QuadratureType).name());
  ...
  typedef MathUtility::Matrix<nUnknownsPerElement, nUnknownsPerElement,
                              double_v_t>
      EvaluationsType;
  ...
  std::array<std::array<double, D>, QuadratureDD::numberEvaluations()>
      samplingPoints = QuadratureDD::samplingPoints();
  EvaluationsArrayType evaluationsArray{};

  LOG(DEBUG) << "1D integration with " << QuadratureType::numberEvaluations()
             << " evaluations";
  LOG(DEBUG) << D << "D integration with " << QuadratureDD::numberEvaluations()
             << " evaluations";

  // initialize variables
  std::shared_ptr<PartitionedPetscMat<FunctionSpace>> vMatrix =
      this->data_.vMatrix();

  std::shared_ptr<FunctionSpace> functionSpace =
      std::static_pointer_cast<FunctionSpace>(this->data_.functionSpace());
  ...
  for (int elementNoLocal = 0; elementNoLocal < nElementsLocal;
    ...
    std::array<dof_no_v_t, nDofsPerElement> dofNosLocal =
    ...
    for (int i = 0; i < nDofsPerElement; i++) {
      for (int j = 0; j < nDofsPerElement; j++) {
        ...
        for (int rowComponentNo = 0; rowComponentNo < nComponents;
             rowComponentNo++) {
          for (int columnComponentNo = 0; columnComponentNo < nComponents;
               columnComponentNo++) {
            int componentNo = rowComponentNo * nComponents + columnComponentNo;

            // LOG(DEBUG) << " initialize vMatrix entry ( " << dofNosLocal[i] <<
            // "," << dofNosLocal[j] << ") (no. " << cntr++ << ")";
            vMatrix->setValue(componentNo, dofNosLocal[i], dofNosLocal[j], 0,
                              INSERT_VALUES);
          }
        }
      }
  ...
  for (int elementNoLocal = 0; elementNoLocal < nElementsLocal;
    ...
    for (unsigned int samplingPointIndex = 0;
      ...
      std::array<Vec3_v_t, D> jacobian =
          FunctionSpace::computeJacobian(geometry, xi);

      // get the factor in the integral that arises from the change in
      // integration domain from world to coordinate space
      ...
      VLOG(2) << "samplingPointIndex=" << samplingPointIndex << ", xi=" << xi
              << ", geometry: " << geometry << ", jac: " << jacobian;

      std::array<std::array<double, D>, FunctionSpace::nDofsPerElement()>
          gradPhi = data_.functionSpace()->getGradPhi(xi);

      // get evaluations of integrand at xi for all (i,j)-dof pairs, integrand
      // is defined in another class
      for (int i = 0; i < nDofsPerElement; i++) {
        for (int j = 0; j < nDofsPerElement; j++) {
          // v * phi[i] * gradPhi[j](xi)
          evaluationsArray[samplingPointIndex](i, j) =
              FunctionSpace::phi(i, xi) *
              (gradPhi[j][0] * advectionVelocity_[0] +
               gradPhi[j][1] * advectionVelocity_[1]) *
              integrationFactor;
        }
      }
        ...
        for (int rowComponentNo = 0; rowComponentNo < nComponents;
          ...
          for (int columnComponentNo = 0; columnComponentNo < nComponents;
            ...
            double_v_t integratedValue =
                integratedValues(i * nComponents + rowComponentNo,
                                 j * nComponents + columnComponentNo);
            int componentNo = rowComponentNo * nComponents + columnComponentNo;

            VLOG(2) << "  dof pair (" << i << "," << j << ") dofs ("
                    << dofNosLocal[i] << "," << dofNosLocal[j] << "), "
                    << "component (" << rowComponentNo << ","
                    << columnComponentNo << "), " << componentNo
            ...
            // add the entry in the stiffness matrix, for all dofs of the
            // vectorized values at once, i.e. K_dofINoLocal[0],dofJNoLocal[0] =
            // value[0] i.e. K_dofINoLocal[1],dofJNoLocal[1] = value[1], etc.
            // Note that K_dofINoLocal[0],dofJNoLocal[1] would be potentially
            // zero, the contributions are considered element-wise
            vMatrix->setValue(componentNo, dofINoLocal, dofJNoLocal,
...
template <typename FiniteElementMethod>
void DiffusionAdvectionSolver<FiniteElementMethod>::reset() {
...
template <typename FiniteElementMethod>
void DiffusionAdvectionSolver<FiniteElementMethod>::callOutputWriter(
...
template <typename FiniteElementMethod>
void DiffusionAdvectionSolver<FiniteElementMethod>::executeMyHelperMethod() {
...
template <typename FiniteElementMethod>
std::shared_ptr<typename DiffusionAdvectionSolver<
    FiniteElementMethod>::SlotConnectorDataType>
DiffusionAdvectionSolver<FiniteElementMethod>::getSlotConnectorData() {
