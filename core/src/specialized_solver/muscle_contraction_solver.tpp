...
MuscleContractionSolver<MeshType, Term, withLargeOutputFiles>::
    ...
    : Runnable(), ::TimeSteppingScheme::TimeSteppingScheme(
                      context["MuscleContractionSolver"]),
  ...
  if (isDynamic_) {
    ...
    dynamicHyperelasticitySolver_ =
        std::make_shared<DynamicHyperelasticitySolverType>(this->context_);
  } else {
    LOG(DEBUG) << "use static hyperelasticity solver";
    staticHyperelasticitySolver_ =
        std::make_shared<StaticHyperelasticitySolverType>(this->context_);
  }
  ...
  // parse options
  pmax_ = this->specificSettings_.getOptionDouble("Pmax", 1.0,
                                                  PythonUtility::Positive);
  enableForceLengthRelation_ =
      this->specificSettings_.getOptionBool("enableForceLengthRelation", true);
  lambdaDotScalingFactor_ =
      this->specificSettings_.getOptionDouble("lambdaDotScalingFactor", 1.0);

  this->specificSettings_.template getOptionVector<std::string>(
      "mapGeometryToMeshes", meshNamesOfGeometryToMapTo_);
}
...
void MuscleContractionSolver<MeshType, Term, withLargeOutputFiles>::
  ...
  // time are all set by the parent class, TimeSteppingScheme.

  // create mapping between meshes for the geometry field mapping, do this here
  // already at the beginning where the meshes are not yet deformed
  ...
  LOG_N_TIMES(3, INFO) << "durationComputeMappingBetweenMeshes: "
                       << Control::PerformanceMeasurement::getDuration(
                              "durationComputeMappingBetweenMeshes");

  // output for debugging
  LOG(DEBUG) << "MuscleContractionSolver<MeshType,Term,withLargeOutputFiles>::"
                "advanceTimeSpan, timeSpan="
  ...
  for (; timeStepNo < this->numberTimeSteps_;) {
    ...
    {
      LOG(INFO) << "MuscleContractionSolver, timestep " << timeStepNo << "/"
    ...
    if (isDynamic_) {
      this->dynamicHyperelasticitySolver_->setTimeSpan(
          currentTime, currentTime + this->timeStepWidth_);
    ...
    // compute new values of λ and λ_dot, to be transferred to the CellML
    // solvers
    computeLambda();
...
void MuscleContractionSolver<MeshType, Term,
                             withLargeOutputFiles>::initialize() {
  LOG_SCOPE_FUNCTION;
  ...
  if (isDynamic_) {
    dynamicHyperelasticitySolver_->setUniqueDataPrefix(
        StringUtility::optionalConcat(this->uniqueDataPrefix_,
                                      "muscle_contraction_solver"));
    dynamicHyperelasticitySolver_->initialize();
  } else {
    staticHyperelasticitySolver_->setUniqueDataPrefix(
        StringUtility::optionalConcat(this->uniqueDataPrefix_,
                                      "muscle_contraction_solver"));
  ...
  // basis functions). The dynamicHyperelasticitySolver_ solver already created
  // a function space that we should use. We already have a typedef
  // "FunctionSpace" that is the class of dynamicHyperelasticitySolver_'s
  // function space type.
  ...
  data_.setUniquePrefix(StringUtility::optionalConcat(
      this->uniqueDataPrefix_, "muscle_contraction_solver"));
  data_.initialize();

  bool setGeometryFieldForTransfer = meshNamesOfGeometryToMapTo_.empty();

  if (isDynamic_) {
    typename DynamicHyperelasticitySolverType::HyperelasticitySolverType
        &hyperelasticitySolver =
            dynamicHyperelasticitySolver_->hyperelasticitySolver();

    // set field variables from dynamicHyperelasticitySolver in data_ such that
    // they can be output by the output writer
    data_.setFieldVariables(
        dynamicHyperelasticitySolver_->data().displacements(),
    ...
    // they can be output by the output writer
    data_.setFieldVariables(
        staticHyperelasticitySolver_->data().displacements(),
...
template <typename MeshType, typename Term, bool withLargeOutputFiles>
void MuscleContractionSolver<MeshType, Term, withLargeOutputFiles>::reset() {
  if (isDynamic_)
...
void MuscleContractionSolver<MeshType, Term, withLargeOutputFiles>::
  ...
  if (isDynamic_) {
    this->dynamicHyperelasticitySolver_->callOutputWriter(
        timeStepNo, currentTime, callCountIncrement);
  } else {
    staticHyperelasticitySolver_->callOutputWriter(timeStepNo, currentTime,
                                                   callCountIncrement);
  }
...
void MuscleContractionSolver<
    MeshType, Term, withLargeOutputFiles>::initializeMappingBetweenMeshes() {
  if (this->durationLogKey_ != "")
    Control::PerformanceMeasurement::stop(this->durationLogKey_ +
  ...
  if (!meshNamesOfGeometryToMapTo_.empty()) {
    bool reverseMappingOrder =
        this->specificSettings_.getOptionBool("reverseMappingOrder", true);

    using SourceFunctionSpaceType =
        typename StaticHyperelasticitySolverType::DisplacementsFunctionSpace;
    // using SourceFieldVariableType =
    // FieldVariable::FieldVariable<SourceFunctionSpaceType,3>;

    assert(data_.functionSpace());

    // get source function space
    std::shared_ptr<SourceFunctionSpaceType> functionSpaceSource =
        data_.functionSpace();
    ...
    for (std::string meshName : meshNamesOfGeometryToMapTo_) {
      ...
      LOG(DEBUG) << "mesh \"" << meshName << "\", test if "
                 << StringUtility::demangle(
                        typeid(TargetFunctionSpaceType1).name());

      // if the mesh name corresponds to a linear mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType1>(meshName)) {
        // get target function space
        std::shared_ptr<TargetFunctionSpaceType1> functionSpaceTarget =
        ...
        if (reverseMappingOrder)
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<TargetFunctionSpaceType1,
                                              SourceFunctionSpaceType>(
                  functionSpaceTarget, functionSpaceSource);
        else
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<SourceFunctionSpaceType,
      ...
      LOG(DEBUG) << "mesh \"" << meshName << "\", test if "
                 << StringUtility::demangle(
                        typeid(TargetFunctionSpaceType2).name());

      // if the mesh name corresponds to a quadratic mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType2>(meshName)) {
        // get target function space
        std::shared_ptr<TargetFunctionSpaceType2> functionSpaceTarget =
            this->context_.meshManager()
        ...
        if (reverseMappingOrder)
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<TargetFunctionSpaceType2,
                                              SourceFunctionSpaceType>(
                  functionSpaceTarget, functionSpaceSource);
        else
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<SourceFunctionSpaceType,
                                              TargetFunctionSpaceType2>(
                  functionSpaceSource, functionSpaceTarget);
      ...
      using TargetFunctionSpaceType3 =
          ::FunctionSpace::FunctionSpace<Mesh::CompositeOfDimension<3>,
                                         BasisFunction::LagrangeOfOrder<1>>;
      LOG(DEBUG) << "mesh \"" << meshName << "\", test if "
                 << StringUtility::demangle(
                        typeid(TargetFunctionSpaceType3).name());

      // if the mesh name corresponds to a linear mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType3>(meshName)) {
        // get target function space
        std::shared_ptr<TargetFunctionSpaceType3> functionSpaceTarget =
            this->context_.meshManager()
        ...
        if (reverseMappingOrder)
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<TargetFunctionSpaceType3,
                                              SourceFunctionSpaceType>(
                  functionSpaceTarget, functionSpaceSource);
        else
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<SourceFunctionSpaceType,
                                              TargetFunctionSpaceType3>(
                  functionSpaceSource, functionSpaceTarget);
      } else
        LOG(DEBUG) << "no";

      // for second order composite meshes
      using TargetFunctionSpaceType4 =
          ::FunctionSpace::FunctionSpace<Mesh::CompositeOfDimension<3>,
                                         BasisFunction::LagrangeOfOrder<2>>;
      LOG(DEBUG) << "mesh \"" << meshName << "\", test if "
                 << StringUtility::demangle(
                        typeid(TargetFunctionSpaceType4).name());

      // if the mesh name corresponds to a quadratic mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType4>(meshName)) {
        // get target function space
        std::shared_ptr<TargetFunctionSpaceType4> functionSpaceTarget =
            this->context_.meshManager()
        ...
        if (reverseMappingOrder)
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<TargetFunctionSpaceType4,
                                              SourceFunctionSpaceType>(
                  functionSpaceTarget, functionSpaceSource);
        else
          DihuContext::mappingBetweenMeshesManager()
              ->template mappingBetweenMeshes<SourceFunctionSpaceType,
                                              TargetFunctionSpaceType4>(
                  functionSpaceSource, functionSpaceTarget);
      } else
        LOG(DEBUG) << "no";
    }
...
template <typename MeshType, typename Term, bool withLargeOutputFiles>
void MuscleContractionSolver<MeshType, Term,
                             withLargeOutputFiles>::mapGeometryToGivenMeshes() {
  if (this->durationLogKey_ != "")
    Control::PerformanceMeasurement::stop(this->durationLogKey_ +
                                          std::string("_map_geometry"));
  ...
  if (!meshNamesOfGeometryToMapTo_.empty()) {
    ...
    std::shared_ptr<SourceFieldVariableType> geometryFieldSource =
        std::make_shared<SourceFieldVariableType>(
            data_.functionSpace()->geometryField());
    ...
    for (std::string meshName : meshNamesOfGeometryToMapTo_) {
      ...
      using TargetFunctionSpaceType1 = ::FunctionSpace::FunctionSpace<
          Mesh::StructuredDeformableOfDimension<3>,
          BasisFunction::LagrangeOfOrder<1>>;
      using TargetFieldVariableType1 =
          FieldVariable::FieldVariable<TargetFunctionSpaceType1, 3>;

      // if the mesh name corresponds to a linear mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType1>(meshName)) {
        // get target geometry field variable
        std::shared_ptr<TargetFieldVariableType1> geometryFieldTarget =
            std::make_shared<TargetFieldVariableType1>(
                this->context_.meshManager()
                    ->functionSpace<TargetFunctionSpaceType1>(meshName)
                    ->geometryField());
      ...
      using TargetFunctionSpaceType2 = ::FunctionSpace::FunctionSpace<
          Mesh::StructuredDeformableOfDimension<3>,
          BasisFunction::LagrangeOfOrder<2>>;
      using TargetFieldVariableType2 =
          FieldVariable::FieldVariable<TargetFunctionSpaceType2, 3>;

      // if the mesh name corresponds to a quadratic mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType2>(meshName)) {
        // get target geometry field variable
        std::shared_ptr<TargetFieldVariableType2> geometryFieldTarget =
            std::make_shared<TargetFieldVariableType2>(
                this->context_.meshManager()
                    ->functionSpace<TargetFunctionSpaceType2>(meshName)
                    ->geometryField());

        LOG(DEBUG) << "transfer geometry field to quadratic mesh, "
                   << geometryFieldSource->functionSpace()->meshName() << " -> "
                   << geometryFieldTarget->functionSpace()->meshName();
        LOG(DEBUG)
            << StringUtility::demangle(typeid(SourceFunctionSpaceType).name())
            << " -> "
            << StringUtility::demangle(typeid(TargetFunctionSpaceType2).name());

        // perform the mapping
        DihuContext::mappingBetweenMeshesManager()
            ->template prepareMapping<SourceFieldVariableType,
                                      TargetFieldVariableType2>(
                geometryFieldSource, geometryFieldTarget, -1);

        // map the whole geometry field (all components), do not avoid copy
        DihuContext::mappingBetweenMeshesManager()
            ->template map<SourceFieldVariableType, TargetFieldVariableType2>(
                geometryFieldSource, geometryFieldTarget, -1, -1, false);
        DihuContext::mappingBetweenMeshesManager()
            ->template finalizeMapping<SourceFieldVariableType,
                                       TargetFieldVariableType2>(
      ...
      using TargetFunctionSpaceType3 =
          ::FunctionSpace::FunctionSpace<Mesh::CompositeOfDimension<3>,
                                         BasisFunction::LagrangeOfOrder<1>>;
      using TargetFieldVariableType3 =
          FieldVariable::FieldVariable<TargetFunctionSpaceType3, 3>;

      // if the mesh name corresponds to a linear mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType3>(meshName)) {
        // get target geometry field variable
        std::shared_ptr<TargetFieldVariableType3> geometryFieldTarget =
            std::make_shared<TargetFieldVariableType3>(
                this->context_.meshManager()
                    ->functionSpace<TargetFunctionSpaceType3>(meshName)
                    ->geometryField());
        ...
        LOG(DEBUG)
            << StringUtility::demangle(typeid(SourceFunctionSpaceType).name())
            << " -> "
            << StringUtility::demangle(typeid(TargetFunctionSpaceType3).name());

        // perform the mapping
        DihuContext::mappingBetweenMeshesManager()
            ->template prepareMapping<SourceFieldVariableType,
                                      TargetFieldVariableType3>(
                geometryFieldSource, geometryFieldTarget, -1);

        // map the whole geometry field (all components), do not avoid copy
        DihuContext::mappingBetweenMeshesManager()
            ->template map<SourceFieldVariableType, TargetFieldVariableType3>(
                geometryFieldSource, geometryFieldTarget, -1, -1, false);
        DihuContext::mappingBetweenMeshesManager()
            ->template finalizeMapping<SourceFieldVariableType,
                                       TargetFieldVariableType3>(
      ...
      using TargetFunctionSpaceType4 =
          ::FunctionSpace::FunctionSpace<Mesh::CompositeOfDimension<3>,
                                         BasisFunction::LagrangeOfOrder<2>>;
      using TargetFieldVariableType4 =
          FieldVariable::FieldVariable<TargetFunctionSpaceType4, 3>;

      // if the mesh name corresponds to a quadratic mesh
      if (this->context_.meshManager()
              ->hasFunctionSpaceOfType<TargetFunctionSpaceType4>(meshName)) {
        // get target geometry field variable
        std::shared_ptr<TargetFieldVariableType4> geometryFieldTarget =
            std::make_shared<TargetFieldVariableType4>(
                this->context_.meshManager()
                    ->functionSpace<TargetFunctionSpaceType4>(meshName)
                    ->geometryField());
        ...
        LOG(DEBUG)
            << StringUtility::demangle(typeid(SourceFunctionSpaceType).name())
            << " -> "
            << StringUtility::demangle(typeid(TargetFunctionSpaceType4).name());

        // perform the mapping
        DihuContext::mappingBetweenMeshesManager()
            ->template prepareMapping<SourceFieldVariableType,
                                      TargetFieldVariableType4>(
                geometryFieldSource, geometryFieldTarget, -1);

        // map the whole geometry field (all components), do not avoid copy
        DihuContext::mappingBetweenMeshesManager()
            ->template map<SourceFieldVariableType, TargetFieldVariableType4>(
                geometryFieldSource, geometryFieldTarget, -1, -1, false);
        DihuContext::mappingBetweenMeshesManager()
            ->template finalizeMapping<SourceFieldVariableType,
                                       TargetFieldVariableType4>(
...
template <typename MeshType, typename Term, bool withLargeOutputFiles>
std::shared_ptr<typename MuscleContractionSolver<
    MeshType, Term, withLargeOutputFiles>::DynamicHyperelasticitySolverType>
MuscleContractionSolver<MeshType, Term,
                        withLargeOutputFiles>::dynamicHyperelasticitySolver() {
...
template <typename MeshType, typename Term, bool withLargeOutputFiles>
typename MuscleContractionSolver<MeshType, Term, withLargeOutputFiles>::Data &
MuscleContractionSolver<MeshType, Term, withLargeOutputFiles>::data() {
  return data_;
}
...
template <typename MeshType, typename Term, bool withLargeOutputFiles>
std::shared_ptr<typename MuscleContractionSolver<
    MeshType, Term, withLargeOutputFiles>::SlotConnectorDataType>
MuscleContractionSolver<MeshType, Term,
...
FullStaticDataForCheckpointing<
  ...
  return std::tuple_cat(staticHyperelasticitySolver_->fullData()
...
bool FullStaticDataForCheckpointing<MeshType, Term, withLargeOutputFiles>::
  ...
  return staticHyperelasticitySolver_->fullData().restoreState(r) &&
...
FullDynamicDataForCheckpointing<
    MeshType, Term, withLargeOutputFiles>::getFieldVariablesForCheckpointing() {
  return std::tuple_cat(dynamicHyperelasticitySolver_->fullData()
                            .getFieldVariablesForCheckpointing(),
                        data_.getFieldVariablesForCheckpointing());
}
...
bool FullDynamicDataForCheckpointing<MeshType, Term, withLargeOutputFiles>::
    restoreState(const InputReader::Generic &r) {
  return dynamicHyperelasticitySolver_->fullData().restoreState(r) &&
         data_.restoreState(r);
}
