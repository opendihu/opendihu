...
QuasiStaticLinearElasticitySolver<
    FiniteElementMethod>::QuasiStaticLinearElasticitySolver(DihuContext context)
    : context_(context["QuasiStaticLinearElasticitySolver"]), data_(context_),
      ...
      finiteElementMethodLinearElasticity_(this->context_),
      initialized_(false) {
  // get python config
  ...
  LOG(DEBUG) << "QuasiStaticLinearElasticitySolver: parsed parameters "
                "maximumActiveStress: "
...
void QuasiStaticLinearElasticitySolver<FiniteElementMethod>::advanceTimeSpan(
  ...
  computeActiveStress(); // this->data_.activation(); to
                         // this->data_.activeStress() =
                         // finiteElementMethodLinearElasticity_.data().activeStress()
...
void QuasiStaticLinearElasticitySolver<
    FiniteElementMethod>::computeActiveStress() {
  // this->data_.activation(); to this->data_.activeStress();, use
  // data_.fiberDirection()
  ...
  for (element_no_t elementNoLocal = 0;
    ...
    this->data_.activation()->getElementValues(elementNoLocal,
                                               activationValues);

    std::array<Vec3, nDofsPerElement> fiberDirectionValues;
    this->data_.fiberDirection()->getElementValues(elementNoLocal,
                                                   fiberDirectionValues);
    ...
    for (int dofIndex = 0; dofIndex < nDofsPerElement; dofIndex++) {
      ...
      double directionalStress =
          maximumActiveStress_ * activationValues[dofIndex];

      // scale stress by strain to obtain a characteristic force-length
      // relationship
#if 0
      ...
      MathUtility::Matrix<D, D> stressTensor =
          directionalStress * anisotropyTensor_.value(elementNoLocal);
...
template <typename FiniteElementMethod>
void QuasiStaticLinearElasticitySolver<FiniteElementMethod>::run() {
...
template <typename FiniteElementMethod>
void QuasiStaticLinearElasticitySolver<FiniteElementMethod>::setTimeSpan(
    double startTime, double endTime) {
  endTime_ = endTime;
}

template <typename FiniteElementMethod>
void QuasiStaticLinearElasticitySolver<FiniteElementMethod>::initialize() {
  ...
  finiteElementMethodLinearElasticity_.data().setActiveStress(
      data_.activeStress());
  finiteElementMethodLinearElasticity_.data().setRightHandSideActive(
  ...
  data_.setData(std::make_shared<DataLinearElasticityType>(
      finiteElementMethodLinearElasticity_.data()));
  ...
  if (specificSettings_.hasKey("fiberDirection")) {
    fiberDirection_ = specificSettings_.getOptionArray<double, 3>(
        "fiberDirection", Vec3{0, 0, 1});

    // set fiber direction in field variable data_.fiberDirection()
    std::vector<Vec3> fiberDirectionValues(
        data_.functionSpace()->nDofsLocalWithoutGhosts(), fiberDirection_);
    data_.fiberDirection()->setValuesWithoutGhosts(fiberDirectionValues);

    usePotentialFlowForFiberDirection_ = false;
  } else {
    // fiberDirection is not given in settings, use a potential flow simulation
    // (Laplace equation) and get the fiber direction from the streamlines
    ...
    // avoid that solver structure file is created, this should only be done
    // after the whole simulation has finished
    DihuContext::solverStructureVisualizer()->disable();
  ...
  MathUtility::Matrix<3, 3, double> defaultValue(
      std::array<double, 9>{1, 0, 0, 0, 0, 0, 0, 0, 0});
  this->anisotropyTensor_.initialize(this->specificSettings_,
                                     "anisotropyTensor", defaultValue,
...
template <typename FiniteElementMethod>
void QuasiStaticLinearElasticitySolver<FiniteElementMethod>::callOutputWriter(
...
template <typename FiniteElementMethod>
std::shared_ptr<typename QuasiStaticLinearElasticitySolver<
    FiniteElementMethod>::SlotConnectorDataType>
QuasiStaticLinearElasticitySolver<FiniteElementMethod>::getSlotConnectorData() {
  return this->data_.getSlotConnectorData();
}

//! output the given data for debugging
template <typename FiniteElementMethod>
std::string QuasiStaticLinearElasticitySolver<FiniteElementMethod>::getString(
    std::shared_ptr<typename QuasiStaticLinearElasticitySolver<
        FiniteElementMethod>::SlotConnectorDataType>
        data) {
  std::stringstream s;
