...
QuasiStaticNonlinearElasticitySolverFebio::
    ...
    : NonlinearElasticitySolverFebio(
          context, "QuasiStaticNonlinearElasticitySolverFebio") {
  // load settings
  this->activationFactor_ = this->specificSettings_.getOptionDouble(
      "activationFactor", 1e-5, PythonUtility::NonNegative);
  this->force_ = this->specificSettings_.getOptionDouble(
      "force", 100, PythonUtility::NonNegative);
...
void QuasiStaticNonlinearElasticitySolverFebio::createFebioInputFile() {
  ...
  if (ownRankNo == 0) {
    ...
    fileContents << "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>" << "\n"
                 ...
                 << "\t\t<max_refs>15</max_refs> <!-- Max number of stiffness "
                    "reformations -->"
                 << "\n"
                 << "\t\t<max_ups>10</max_ups>   <!-- Max number of "
                    "BFGS/Broyden stiffness updates --> "
                 << "\n"
                 << "\t\t<dtol>0.001</dtol>      <!-- Convergence tolerance on "
                    "displacement -->"
                 << "\n"
                 << "\t\t<etol>0.01</etol>       <!-- Convergence tolerance on "
                    "energy -->"
                 << "\n"
                 << "\t\t<rtol>0</rtol>          <!-- Convergence tolerance on "
                    "residual, 0=disabled-->"
                 << "\n"
                 << "\t\t<lstol>0.9</lstol>      <!-- Convergence tolerance on "
                    "line search -->"
                 << "\n"
    ...
    // loop over global elements and add one material per element with the
    // activation value of this element
    for (global_no_t elementNoGlobalPetsc = 0;
      ...
      fileContents
          << "\t\t<material id=\"" << elementNoGlobalPetsc + 1
          << "\" name=\"Material for element " << elementNoGlobalPetsc
          ...
          << "\t\t\t<lam_max>1.4</lam_max>" << "\n"
          << "\t\t\t<k>1e6</k>" << "\n"
          << "\t\t\t<fiber type=\"local\">1, 5</fiber>    <!-- fiber direction "
             "is from local node 1 to 5 in every element, i.e. upwards -->"
          << "\n"
    ...
    for (global_no_t elementNoGlobalPetsc = 0;
      ...
      fileContents << "\t\t\t<hex8 id=\"" << elementNoGlobalPetsc + 1
                   << "\" mat=\"" << elementNoGlobalPetsc + 1 << "\"> ";
    ...
    for (int j = 0; j < nNodesY; j++) {
      for (int i = 0; i < nNodesX; i++) {
        dof_no_t dofNoLocal =
            (nNodesZ - 1) * nNodesY * nNodesX + j * nNodesX + i;
    ...
    fileContents << "\t<LoadData>" << "\n"
                 ...
                 << "\n"
                 << "\t\t</loadcurve>" << "\n"
                 << "\t\t<loadcurve id=\"2\"> <!-- for external load that "
                 ...
                 << "\t\t\t<node_data file=\"febio_geometry_output.txt\" "
                    "format=\"%i,%g,%g,%g,%g,%g,%g,%g,%g,%g\" "
                    "data=\"x;y;z;ux;uy;uz;Rx;Ry;Rz\"/>"
                 << "\n"
                 << "\t\t\t<element_data file=\"febio_stress_output.txt\" "
