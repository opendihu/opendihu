...
#if 1
NonlinearElasticitySolverFebio::NonlinearElasticitySolverFebio(
    DihuContext context, std::string solverName)
    : context_(context[solverName]), data_(context_), solverName_(solverName),
      initialized_(false) {
  ...
  if (solverName_ ==
      "NonlinearElasticitySolverFebio") // if the base class solver is
                                        // referenced in settings
  {
    specificSettings_.getOptionVector("materialParameters",
                                      materialParameters_);

    if (materialParameters_.size() < 3) {
      LOG(FATAL) << specificSettings_
                 << "[\"materialParameters\"]: 3 material parameters, [c0, c1, "
                    "k], are required, but only "
                 << materialParameters_.size() << " are given.";
    }
  }
  ...
  tractionVector_ =
      specificSettings_.getOptionArray<double, 3>("tractionVector", 0);
...
bool NonlinearElasticitySolverFebio::isFebioAvailable() {
  // to see if febio2 is installed, run it with a non-existing command line
  // argument "a" this produces the output "FATAL ERROR: Invalid command line
  // option" and exits with status 0
...
void NonlinearElasticitySolverFebio::runFebio() {
  ...
  if (ownRankNo == 0) {
    ...
    int returnValue = system(
        "rm -f febio3_input.log febio3_stress_output.txt "
        "febio3_geometry_output.txt; febio3 -i febio3_input.feb > /dev/null");
...
#ifndef NDEBUG
    // copy contents of log file from the febio execution to the opendihu.log
    // file
...
void NonlinearElasticitySolverFebio::createFebioInputFile() {
  ...
  if (ownRankNo == 0) {
    ...
    fileContents
        ...
        << "<!--\n"
        << "Problem Description:\n"
        << "\t" << problemDescription_ << " at time " << endTime_
        << ", created by opendihu\n"
        << "-->\n"
        ...
        << "\t\t<max_refs>100</max_refs> <!-- (15) Max number of stiffness "
           "reformations -->\n"
        << "\t\t<max_ups>10</max_ups>   <!-- (10) Max number of BFGS/Broyden "
           "stiffness updates --> \n"
        << "\t\t<dtol>1e-10</dtol>      <!-- (0.001) Convergence tolerance on "
           "displacement -->\n"
        << "\t\t<etol>1e-10</etol>       <!-- (0.01) Convergence tolerance on "
           "energy -->\n"
        << "\t\t<rtol>0</rtol>          <!-- Convergence tolerance on "
           "residual, 0=disabled-->\n"
        << "\t\t<lstol>1e-10</lstol>      <!-- (0.9) Convergence tolerance on "
           "line search -->\n"
    ...
    // loop over nodes
    for (global_no_t nodeNoGlobalPetsc = 0;
         nodeNoGlobalPetsc < data_.functionSpace()->nNodesGlobal();
         nodeNoGlobalPetsc++) {
      double nodePositionX =
          nodePositionValuesGlobal[3 * nodeNoGlobalPetsc + 0];
      double nodePositionY =
          nodePositionValuesGlobal[3 * nodeNoGlobalPetsc + 1];
      double nodePositionZ =
          nodePositionValuesGlobal[3 * nodeNoGlobalPetsc + 2];

      fileContents << "\t\t\t<node id=\"" << nodeNoGlobalPetsc + 1 << "\">"
                   << nodePositionX << "," << nodePositionY << ","
                   << nodePositionZ << "</node>\n";
    }
    ...
    for (global_no_t elementNoGlobalPetsc = 0;
         elementNoGlobalPetsc < data_.functionSpace()->nElementsGlobal();
         elementNoGlobalPetsc++) {
      nodeNosGlobal[elementNoGlobalPetsc * 8 + 0];

      // elements types:
      // https://help.febio.org/FEBio/FEBio_um_2_8/FEBio_um_2-8-3.8.2.1.html
      fileContents << "\t\t\t<elem id=\"" << elementNoGlobalPetsc + 1 << "\"> ";
      fileContents << nodeNosGlobal[elementNoGlobalPetsc * 8 + 0] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 1] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 2] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 3] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 4] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 5] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 6] << ", "
                   << nodeNosGlobal[elementNoGlobalPetsc * 8 + 7];
      fileContents << "</elem>\n";
    }
    ...
    std::string dirichletBoundaryConditionsModeString =
        this->specificSettings_.getOptionString(
            "dirichletBoundaryConditionsMode", "fix_all");
    ...
    for (element_no_t elementNoLocal : tractionElementNos_) {
      ...
      for (int dofIndex : dofIndices) {
        ...
        dof_no_t dofNoLocal =
            this->data_.functionSpace()->getDofNo(elementNoLocal, dofIndex);
        global_no_t dofNoGlobalPetsc =
            this->data_.functionSpace()->meshPartition()->getDofNoGlobalPetsc(
                dofNoLocal);
    ...
    fileContents
        ...
        << "\t\t\t<scale lc=\"2\">1</scale>\n"
        << "\t\t\t<traction>" << tractionVector_[0] << "," << tractionVector_[1]
        << "," << tractionVector_[2] << "</traction>\n"
        << "\t\t</surface_load>\n"
        << "\t</Loads>\n";
    ...
    fileContents << "\t<LoadData>\n"
                 ...
                 << "\t\t\t<loadpoint>1," << activationFactor_
                 << "</loadpoint>\n"
                 << "\t\t</loadcurve>\n"
                 << "\t\t<loadcurve id=\"2\"> <!-- for external load (surface "
                    "traction) that stretches the muscle -->\n"
                 << "\t\t\t<loadpoint>0,0</loadpoint>\n"
                 ...
                 // available variables:
                 // https://help.febio.org/FEBio/FEBio_um_2_9/index.html
                 // Sec. 3.17.1.2 and 3.17.1.3
                 << "\t\t\t<node_data file=\"febio3_geometry_output.txt\" "
                    "format=\"%i,%g,%g,%g,%g,%g,%g,%g,%g,%g\" "
                    "data=\"x;y;z;ux;uy;uz;Rx;Ry;Rz\"/>\n"
                 << "\t\t\t<element_data file=\"febio3_stress_output.txt\" "
                    "format=\"%i,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%"
                    "g,%g,%g,%g,%g,%g,%g\" "
                    "data=\"sx;sy;sz;sxy;syz;sxz;Ex;Ey;Ez;Exy;Eyz;Exz;J;Fxx;"
                    "Fxy;Fxz;Fyx;Fyy;Fyz;Fzx;Fzy;Fzz\"/>\n"
                 << "\t\t</logfile>\n"
                 << "\t</Output>\n"
                 << "</febio_spec>\n";
...
void NonlinearElasticitySolverFebio::loadFebioOutputFile() {
  LOG(TRACE) << "loadFebioOutputFile";

  std::ifstream fileGeometry;
  fileGeometry.open("febio3_geometry_output.txt",
                    std::ios::in | std::ios::binary);

  if (!fileGeometry.is_open()) {
    LOG(WARNING) << "Could not read output file \"febio3_geometry_output.txt\" "
                    "that should have been created by FEBio.";
    return;
  }
...
*/
  ...
  while (!fileGeometry.eof()) {
    ...
    if (line.find("*Step") != std::string::npos) {
      nStepsContainedInFile =
          (int)(atoi(line.substr(line.find("=") + 1).c_str()));
    }
  }
    ...
    if (currentStepNo == nStepsContainedInFile) {
      if (line.find("*") == std::string::npos &&
        ...
        node_no_t nodeNoLocal =
            this->data_.functionSpace()->meshPartition()->getNodeNoLocal(
                nodeNoGlobalPetsc, isLocal);

        LOG(DEBUG) << "read point " << id << ": " << Vec3({x, y, z})
                   << ", global: " << nodeNoGlobalPetsc
                   << ", local: " << nodeNoLocal;
  ...
  if (!fileStress.is_open()) {
    LOG(WARNING) << "Could not read output file \"febio3_stress_output.txt\" "
                    "that should have been created by FEBio.";
    return;
  }
  ...
  std::vector<int> nSummands(
      this->data_.functionSpace()->nNodesLocalWithoutGhosts(), 0);
  ...
  for (;;) {
    ...
    element_no_t elementNoLocal =
        data_.functionSpace()->meshPartition()->getElementNoLocal(
            elementNoGlobalPetsc, isOnLocalDomain);

    LOG(DEBUG) << "read element global " << elementNoGlobalPetsc
               << ", local: " << elementNoLocal
               << ", isOnLocalDomain: " << isOnLocalDomain;

    if (isOnLocalDomain) {
      std::array<dof_no_t, FunctionSpace::nNodesPerElement()> elementNodeNos =
          data_.functionSpace()->getElementNodeNos(elementNoLocal);
      ...
      // S = J F^-1 σ F^-T
      Tensor2<3> cauchyStress{Vec3{sx, sxy, sxz}, Vec3{sxy, sy, syz},
                              Vec3{sxz, syz, sz}};
      Tensor2<3> deformationGradient{Vec3{Fxx, Fyx, Fzx}, Vec3{Fxy, Fyy, Fzy},
                                     Vec3{Fxz, Fyz, Fzz}};
      double determinant = 0;
      double approximateMeshWidth = 0;
      Tensor2<3> inverseDeformationGradient = MathUtility::computeInverse(
          deformationGradient, approximateMeshWidth, determinant);

      Tensor2<3> deformationGradientCofactor =
          MathUtility::computeCofactorMatrix<double>(
              deformationGradient); // cof(M) = det(M) * M^{-T}
      Tensor2<3> pk2Stress = inverseDeformationGradient * cauchyStress *
                             deformationGradientCofactor;

      LOG(DEBUG) << "local element " << elementNoLocal << " of "
                 << this->data_.functionSpace()->nElementsLocal() << ", "
                 << FunctionSpace::nNodesPerElement()
                 << " elementNodeNos: " << elementNodeNos;

      LOG(DEBUG) << "F: " << deformationGradient << ", J: " << J << "="
                 << determinant;
      LOG(DEBUG) << "pk2Stress = " << pk2Stress << " = "
                 << inverseDeformationGradient << "*" << cauchyStress << "*"
                 << deformationGradientCofactor;

      for (node_no_t elementalNodeNo = 0;
           elementalNodeNo < FunctionSpace::nNodesPerElement();
           elementalNodeNo++) {
        node_no_t nodeNoLocal = elementNodeNos[elementalNodeNo];

        if (nodeNoLocal >=
            this->data_.functionSpace()->nNodesLocalWithoutGhosts())
          continue;

        this->data_.pk2Stress()->setValue(
            nodeNoLocal,
            std::array<double, 6>{pk2Stress[0][0], pk2Stress[1][1],
                                  pk2Stress[2][2], pk2Stress[1][0],
                                  pk2Stress[2][1], pk2Stress[2][0]},
            ADD_VALUES);
        this->data_.cauchyStress()->setValue(
            nodeNoLocal, std::array<double, 6>{sx, sy, sz, sxy, syz, sxz},
            ADD_VALUES);
        this->data_.greenLagrangeStrain()->setValue(
            nodeNoLocal, std::array<double, 6>{Ex, Ey, Ez, Exy, Eyz, Exz},
            ADD_VALUES);
    ...
    if (line.find("*") != std::string::npos || fileStress.eof()) {
      if (nElementsLoaded != data_.functionSpace()->nElementsLocal()) {
        LOG(ERROR) << "Loaded " << nElementsLoaded
                   << " local elements from \"febio3_stress_output.txt\", but "
                   << this->data_.functionSpace()->nElementsLocal()
                   << " were expected.";
      }
      break;
  ...
  for (node_no_t nodeNoLocal = 0;
       nodeNoLocal < data_.functionSpace()->nNodesLocalWithoutGhosts();
       nodeNoLocal++) {
    // PK2 stress
    std::array<double, 6> pk2StressValues =
        this->data_.pk2Stress()->getValue(nodeNoLocal);
    pk2StressValues /= nSummands[nodeNoLocal];
    this->data_.pk2Stress()->setValue(nodeNoLocal, pk2StressValues,
                                      INSERT_VALUES);

    // cauchy stress
    std::array<double, 6> cauchyStressValues =
        this->data_.cauchyStress()->getValue(nodeNoLocal);
    cauchyStressValues /= nSummands[nodeNoLocal];
    this->data_.cauchyStress()->setValue(nodeNoLocal, cauchyStressValues,
                                         INSERT_VALUES);

    // green lagrange strain
    std::array<double, 6> greenLagrangeStrainValues =
        this->data_.greenLagrangeStrain()->getValue(nodeNoLocal);
    greenLagrangeStrainValues /= nSummands[nodeNoLocal];
    this->data_.greenLagrangeStrain()->setValue(
        nodeNoLocal, greenLagrangeStrainValues, INSERT_VALUES);

    // relative volume
    double relativeVolumeValue =
        this->data_.relativeVolume()->getValue(nodeNoLocal);
    relativeVolumeValue /= nSummands[nodeNoLocal];
    this->data_.relativeVolume()->setValue(nodeNoLocal, relativeVolumeValue,
                                           INSERT_VALUES);
  }
  ...
  this->data_.functionSpace()->geometryField().setValuesWithoutGhosts(
      geometryValues);
  ...
  LOG(DEBUG)
      << "geometryField pointer: "
      << this->data_.functionSpace()->geometryField().partitionedPetscVec();
  LOG(DEBUG) << "referenceGeometry pointer: "
             << this->data_.referenceGeometry()->partitionedPetscVec();
}

void NonlinearElasticitySolverFebio::communicateElementValues(
    std::vector<double> &activationValuesGlobal,
    std::vector<int> &nodeNosGlobal) {
  int ownRankNo = DihuContext::ownRankNoCommWorld();
  ...
  for (element_no_t elementNoLocal = 0; elementNoLocal < ownSize;
       elementNoLocal++) {
    std::array<double, FunctionSpace::nDofsPerElement()>
        activationElementalValues;
    data_.activation()->getElementValues(elementNoLocal,
                                         activationElementalValues);

    LOG(DEBUG) << "element " << elementNoLocal
               << ", activationElementalValues: " << activationElementalValues;

    Vec3 xi({0.5, 0.5, 0.5});
    double activationValue =
        this->data().functionSpace()->interpolateValueInElement(
            activationElementalValues, xi);
    ...
    std::vector<dof_no_t> dofNosLocal(elementNodeNos.begin(),
                                      elementNodeNos.end());
    std::vector<PetscInt> dofNosGlobalPetsc;
    this->data().functionSpace()->meshPartition()->getDofNoGlobalPetsc(
        dofNosLocal, dofNosGlobalPetsc);
  ...
  for (std::vector<int>::const_iterator iter = sizesOnRanks.cbegin();
       iter != sizesOnRanks.cend(); iter++) {
  ...
  MPIUtility::handleReturnValue(MPI_Gatherv(activationValuesLocal.data(),
                                            sizesOnRanks[ownRankNo], MPI_DOUBLE,
                                            activationValuesGlobal.data(),
                                            sizesOnRanks.data(), offsets.data(),
                                            MPI_DOUBLE, 0, MPI_COMM_WORLD),
                                "MPI_Gatherv");
  MPIUtility::handleReturnValue(
      MPI_Gatherv(nodeNosLocal.data(), sizesOnRanksNodeNos[ownRankNo], MPI_INT,
                  nodeNosGlobal.data(), sizesOnRanksNodeNos.data(),
                  offsetsNodeNos.data(), MPI_INT, 0, MPI_COMM_WORLD),
      "MPI_Gatherv");

  if (ownRankNo == 0) {
    LOG(DEBUG) << "activationValuesLocal: " << activationValuesLocal
               << ", activationValuesGlobal: " << activationValuesGlobal;
  }
}

void NonlinearElasticitySolverFebio::communicateNodeValues(
    std::vector<double> &nodePositionValuesGlobal) {
  int ownRankNo = DihuContext::ownRankNoCommWorld();
  int nRanks = DihuContext::nRanksCommWorld();
  ...
  MPIUtility::handleReturnValue(MPI_Gatherv(nodePositionValuesLocal.data(),
                                            sizesOnRanks[ownRankNo], MPI_DOUBLE,
                                            nodePositionValuesGlobal.data(),
                                            sizesOnRanks.data(), offsets.data(),
                                            MPI_DOUBLE, 0, MPI_COMM_WORLD),
                                "MPI_Gatherv");
}

void NonlinearElasticitySolverFebio::run() {
...
void NonlinearElasticitySolverFebio::setTimeSpan(double startTime,
                                                 double endTime) {
  endTime_ = endTime;
}
...
void NonlinearElasticitySolverFebio::callOutputWriter(int timeStepNo,
                                                      double currentTime,
                                                      int callCountIncrement) {
  this->outputWriterManager_.writeOutput(this->data_, 1, endTime_);
}

void NonlinearElasticitySolverFebio::initialize() {
  ...
  std::shared_ptr<FunctionSpace> functionSpace =
      context_.meshManager()->functionSpace<FunctionSpace>(specificSettings_);
...
NonlinearElasticitySolverFebio::getSlotConnectorData() {
  return this->data_.getSlotConnectorData();
}

//! output the given data for debugging

std::string NonlinearElasticitySolverFebio::getString(
    std::shared_ptr<
        typename NonlinearElasticitySolverFebio::SlotConnectorDataType>
        data) {
  std::stringstream s;
