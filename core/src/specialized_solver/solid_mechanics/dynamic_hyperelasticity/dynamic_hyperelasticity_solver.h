 ...
 * changes to account for inertia effects. Furthermore, an integration scheme
 * for 2nd order ODEs is needed. This is given by leap frog integration.
 *
 */
template <typename Term =
              Equation::SolidMechanics::MooneyRivlinIncompressible3D,
          bool withLargeOutput = true,
          typename MeshType = Mesh::StructuredDeformableOfDimension<3>>
class DynamicHyperelasticitySolver : public TimeSteppingScheme {
public:
  typedef SpatialDiscretization::HyperelasticitySolver<Term, withLargeOutput,
                                                       MeshType, 6>
      HyperelasticitySolverType; // the hyperelasticity solver that solves the
  ...
  //! prescribed beforehand this calls addBoundaryConditions() of the
...
private:
  ...
  //! recreate the rhs of the neumann boundary conditions, needed if traction
  //! was specified in current configuration
  void updateNeumannBoundaryConditions();

  //! compute the total bearing forces and moments at the bottom (z-) and top
  //! (z+) of the domain
