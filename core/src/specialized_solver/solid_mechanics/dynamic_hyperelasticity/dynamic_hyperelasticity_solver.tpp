...
DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
    ...
    : TimeSteppingScheme(context["DynamicHyperelasticitySolver"]),
      hyperelasticitySolver_(context, "DynamicHyperelasticitySolver"),
      data_(context_) {
  ...
  inputMeshIsGlobal_ =
      specificSettings_.getOptionBool("inputMeshIsGlobal", true);
  // viscosity_ = specificSettings_.getOptionDouble("viscosity", 0.0,
  // PythonUtility::NonNegative);

  // initialize output writers
  this->outputWriterManager_.initialize(
      this->context_["dynamic"], this->context_["dynamic"].getPythonConfig());
  isReferenceGeometryInitialized_ = false;
}

template <typename Term, bool withLargeOutput, typename MeshType>
void DynamicHyperelasticitySolver<Term, withLargeOutput,
                                  MeshType>::initialize() {
  LOG_SCOPE_FUNCTION;
  ...
  // determine if there is any traction defined in current configuration, if so
  // it has to be converted to reference configuration in every timestep
  isTractionInCurrentConfiguration_ =
      hyperelasticitySolver_.neumannBoundaryConditions()
  ...
  PetscErrorCode ierr;
  ierr = VecDuplicate(uvp_->valuesGlobal(), &internalVirtualWork_);
  CHKERRV(ierr);
  ierr = VecDuplicate(uvp_->valuesGlobal(), &accelerationTerm_);
  CHKERRV(ierr);
  ierr = VecDuplicate(uvp_->valuesGlobal(), &externalVirtualWorkDead_);
  CHKERRV(ierr);
  ...
  if (this->specificSettings_.hasKey(
          "updateDirichletBoundaryConditionsFunction")) {
    PyObject *object = this->specificSettings_.getOptionPyObject(
        "updateDirichletBoundaryConditionsFunction");
    if (object != Py_None) {
      pythonUpdateDirichletBoundaryConditionsFunction_ =
          this->specificSettings_.getOptionFunction(
              "updateDirichletBoundaryConditionsFunction");
      updateDirichletBoundaryConditionsFunctionCallInterval_ =
          this->specificSettings_.getOptionInt(
  ...
  if (this->specificSettings_.hasKey(
    ...
    PyObject *object = this->specificSettings_.getOptionPyObject(
        "updateNeumannBoundaryConditionsFunction");
    if (object != Py_None) {
      pythonUpdateNeumannBoundaryConditionsFunction_ =
          this->specificSettings_.getOptionFunction(
              "updateNeumannBoundaryConditionsFunction");
      updateNeumannBoundaryConditionsFunctionCallInterval_ =
          this->specificSettings_.getOptionInt(
  ...
  if (this->specificSettings_.hasKey("totalForceFunction")) {
    ...
    PyObject *object =
        this->specificSettings_.getOptionPyObject("totalForceFunction");
    if (object != Py_None) {
      pythonTotalForceFunction_ =
          this->specificSettings_.getOptionFunction("totalForceFunction");
      pythonTotalForceFunctionCallInterval_ =
          this->specificSettings_.getOptionInt("totalForceFunctionCallInterval",
                                               1, PythonUtility::Positive);
    }
  }
  ...
  if (this->specificSettings_.hasKey("totalForceTopElementNosGlobal")) {
    ...
    this->specificSettings_.getOptionVector<global_no_t>(
        "totalForceTopElementNosGlobal", totalForceTopElementsGlobal);
    std::set<global_no_t> totalForceTopElementsGlobalSet(
        totalForceTopElementsGlobal.begin(), totalForceTopElementsGlobal.end());

    LOG(DEBUG) << "given elements top: " << totalForceTopElementsGlobalSet;

    // iterate over local elements
    for (element_no_t elementNoLocal = 0;
         elementNoLocal < this->data_.functionSpace()->nElementsLocal();
         elementNoLocal++) {
      ...
      if (totalForceTopElementsGlobalSet.find(elementNoGlobal) !=
          totalForceTopElementsGlobalSet.end()) {
        LOG(DEBUG) << " (" << elementNoLocal << "->" << elementNoGlobal
                   << ") yes";
        bottomTopElements_.push_back(
            std::tuple<element_no_t, bool>(elementNoLocal, true));
      } else
        LOG(DEBUG) << " (" << elementNoLocal << "->" << elementNoGlobal
                   << ") no";
    }
  ...
  if (this->specificSettings_.hasKey("totalForceBottomElementNosGlobal")) {
    ...
    this->specificSettings_.getOptionVector<global_no_t>(
        "totalForceBottomElementNosGlobal", totalForceBottomElementsGlobal);
    std::set<global_no_t> totalForceBottomElementsGlobalSet(
        totalForceBottomElementsGlobal.begin(),
        totalForceBottomElementsGlobal.end());

    LOG(DEBUG) << "given elements bottom: "
               << totalForceBottomElementsGlobalSet;
    // iterate over local elements
    for (element_no_t elementNoLocal = 0;
         elementNoLocal < this->data_.functionSpace()->nElementsLocal();
         elementNoLocal++) {
      ...
      if (totalForceBottomElementsGlobalSet.find(elementNoGlobal) !=
          totalForceBottomElementsGlobalSet.end()) {
        LOG(DEBUG) << " (" << elementNoLocal << "->" << elementNoGlobal
                   << ") yes";
        bottomTopElements_.push_back(
            std::tuple<element_no_t, bool>(elementNoLocal, false));
      } else
        LOG(DEBUG) << " (" << elementNoLocal << "->" << elementNoGlobal
                   << ") no";
    }
  ...
  totalForceLogFilename_ =
      this->specificSettings_.getOptionString("totalForceLogFilename", "");
...
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
  ...
  if (updateDirichletBoundaryConditionsFunctionCallCount_ %
          updateDirichletBoundaryConditionsFunctionCallInterval_ !=
      0) {
  ...
  PyObject *returnValue = PyObject_CallObject(
      pythonUpdateDirichletBoundaryConditionsFunction_, arglist);
  ...
  std::vector<std::pair<global_no_t, std::array<double, 6>>>
      newDirichletBCValues = PythonUtility::convertFromPython<std::vector<
          std::pair<global_no_t, std::array<double, 6>>>>::get(returnValue);

  LOG(DEBUG) << "newDirichletBCValues: " << newDirichletBCValues
             << "vecs: " << hyperelasticitySolver_.combinedVecSolution() << ","
             << uvp_;
...
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
  ...
  hyperelasticitySolver_.combinedVecSolution()
      ->updateDirichletBoundaryConditions(newDirichletBCValues,
                                          inputMeshIsGlobal_);
  uvp_->updateDirichletBoundaryConditions(newDirichletBCValues,
                                          inputMeshIsGlobal_);
}
...
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
    addDirichletBoundaryConditions(
        std::vector<typename SpatialDiscretization::DirichletBoundaryConditions<
            DisplacementsFunctionSpace, 6>::ElementWithNodes>
            &boundaryConditionElements,
        bool overwriteBcOnSameDof) {
  hyperelasticitySolver_.addDirichletBoundaryConditions(
      boundaryConditionElements, overwriteBcOnSameDof);
...
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
  ...
  // only call this function at defined intervals
  if (updateNeumannBoundaryConditionsFunctionCallCount_ %
          updateNeumannBoundaryConditionsFunctionCallInterval_ !=
      0) {
  ...
  PyObject *arglist = Py_BuildValue("(d)", t);
  PyObject *returnValue = PyObject_CallObject(
      pythonUpdateNeumannBoundaryConditionsFunction_, arglist);
  ...
  using NeumannBoundaryConditionsType =
      typename SpatialDiscretization::NeumannBoundaryConditions<
          typename HyperelasticitySolverType::DisplacementsFunctionSpace,
          Quadrature::Gauss<3>, 3>;
  std::shared_ptr<NeumannBoundaryConditionsType> newNeumannBoundaryConditions =
      std::make_shared<NeumannBoundaryConditionsType>(this->context_);
  newNeumannBoundaryConditions->initialize(
      returnValue, this->data_.functionSpace(), "neumannBoundaryConditions");
  newNeumannBoundaryConditions->setDeformationGradientField(
      this->hyperelasticitySolver_.data().deformationGradient());

  isTractionInCurrentConfiguration_ =
      newNeumannBoundaryConditions->isTractionInCurrentConfiguration();

  hyperelasticitySolver_.updateNeumannBoundaryConditions(
...
template <typename Term, bool withLargeOutput, typename MeshType>
void DynamicHyperelasticitySolver<Term, withLargeOutput,
                                  MeshType>::updateNeumannBoundaryConditions() {
  LOG(INFO) << "Updating right hand side because traction was specified in "
               "current configuration.";

  // update rhs with new traction values in reference configuration if there are
  // moving loads (traction in current configuration)
...
void DynamicHyperelasticitySolver<Term, withLargeOutput,
  ...
  if (inputMeshIsGlobal) {
    ...
    this->specificSettings_.template getOptionVector<Vec3>(
        "initialValuesVelocities", nDofsGlobal, localValuesVelocities);
    displacementsFunctionSpace->meshPartition()->extractLocalDofsWithoutGhosts(
        localValuesVelocities);
  } else {
    ...
    this->specificSettings_.template getOptionVector<Vec3>(
        "initialValuesVelocities", nDofsLocal, localValuesVelocities);
  }
  ...
  for (int componentNo = 0; componentNo < 3; componentNo++) {
    ...
    uvp_->setValues(
        3 + componentNo, nDofsLocalWithoutGhosts,
        displacementsFunctionSpace->meshPartition()->dofNosLocal().data(),
        localValues.data());
  }
  uvp_->zeroGhostBuffer();
...
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
  ...
  // update the reference geometry to use the current value, in the first time
  // step
  ...
  // in case there have been new displacement values set by the data connector
  // slots, update the uvp_ variable
  if (this->startTime_ < 1e-12) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      LOG(DEBUG) << "component " << i
                 << ", initialize displacement values: " << values;
      uvp_->setValues(
          i, data_.functionSpace()->meshPartition()->nDofsLocalWithoutGhosts(),
          data_.functionSpace()->meshPartition()->dofNosLocal().data(),
          values.data(), INSERT_VALUES);
    }
    // representation is combined-global
    // assemble vector
    PetscErrorCode ierr;
    ierr = VecAssemblyBegin(uvp_->valuesGlobal());
    CHKERRV(ierr);
    ierr = VecAssemblyEnd(uvp_->valuesGlobal());
    CHKERRV(ierr);
  }
  ...
  LOG(DEBUG) << "DynamicHyperelasticitySolver::advanceTimeSpan, timeSpan="
  ...
  for (; timeStepNo < this->numberTimeSteps_;) {
    ...
    {
      LOG(INFO) << "DynamicHyperelasticitySolver, timestep " << timeStepNo
                << "/" << this->numberTimeSteps_ << ", t=" << currentTime;
    }

    LOG(DEBUG) << "solveDynamicProblem";

    // advance simulation time, staticSolver needs the new time in order to
    // output results after the solve is done
    timeStepNo++;
    currentTime = this->startTime_ +
                  double(timeStepNo) / this->numberTimeSteps_ * timeSpan;

    // set the current Time to the hyperelasticity solver and then solve the
    // dynamic problem
    hyperelasticitySolver_.setTimeSpan(-1, currentTime);
    hyperelasticitySolver_.solveDynamicProblem(
        uvp_, timeStepNo == 0, internalVirtualWork_, externalVirtualWorkDead_,
        accelerationTerm_, withOutputWritersEnabled);

    // stop duration measurement
    if (this->durationLogKey_ != "")
      Control::PerformanceMeasurement::stop(this->durationLogKey_);

    // copy resulting values to data object such that they can be written by
    // output writer
    hyperelasticitySolver_.setDisplacementsVelocitiesAndPressureFromCombinedVec(
        uvp_->valuesGlobal(), this->data_.displacements(),
        this->data_.velocities());

    hyperelasticitySolver_.setDisplacementsVelocitiesAndPressureFromCombinedVec(
        internalVirtualWork_, this->data_.internalVirtualWork());
    hyperelasticitySolver_.setDisplacementsVelocitiesAndPressureFromCombinedVec(
        externalVirtualWorkDead_, this->data_.externalVirtualWorkDead());
    hyperelasticitySolver_.setDisplacementsVelocitiesAndPressureFromCombinedVec(
        accelerationTerm_, this->data_.accelerationTerm());
    ...
    // potentially update DirichletBC by calling
    // "updateDirichletBoundaryConditionsFunction"
    callUpdateDirichletBoundaryConditionsFunction(currentTime);

    // potentially update NeumannBC by calling
    // "updateNeumannBoundaryConditionsFunction"
    ...
    // compute the total force and torque at the z+ and z- surfaces of the
    // volume
...
template <typename Term, bool withLargeOutput, typename MeshType>
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::run() {
...
template <typename Term, bool withLargeOutput, typename MeshType>
void DynamicHyperelasticitySolver<
    Term, withLargeOutput, MeshType>::callOutputWriter(int timeStepNo,
                                                       double currentTime,
                                                       int callCountIncrement) {
  // call the output writer of the nested solver
  this->hyperelasticitySolver_.callOutputWriter(timeStepNo, currentTime,
                                                callCountIncrement);
...
void DynamicHyperelasticitySolver<Term, withLargeOutput, MeshType>::
  ...
  hyperelasticitySolver_.computeBearingForceAndMoment(
      bottomTopElements_, bearingForceBottom, bearingMomentBottom,
      bearingForceTop, bearingMomentTop);
  ...
  if (DihuContext::ownRankNoCommWorld() == 0) {
    ...
    if (!headerWritten) {
      file
          << "currentTime;forceBottomX;forceBottomY;forceBottomZ;momentBottomX;"
             "momentBottomY;momentBottomZ;"
  ...
  if (pythonTotalForceFunction_) {
    // only call this function at defined intervals
    if (pythonTotalForceFunctionCallCount_ %
            pythonTotalForceFunctionCallInterval_ !=
        0) {
    ...
    PyObject *bearingForceBottomList =
        PythonUtility::convertToPython<Vec3>::get(bearingForceBottom);
    PyObject *bearingMomentBottomList =
        PythonUtility::convertToPython<Vec3>::get(bearingMomentBottom);
    PyObject *bearingForceTopList =
        PythonUtility::convertToPython<Vec3>::get(bearingForceTop);
    PyObject *bearingMomentTopList =
        PythonUtility::convertToPython<Vec3>::get(bearingMomentTop);

    // compose callback function
    PyObject *arglist = Py_BuildValue(
        "(d,O,O,O,O)", currentTime, bearingForceBottomList,
        bearingMomentBottomList, bearingForceTopList, bearingMomentTopList);
    PyObject *returnValue =
        PyObject_CallObject(pythonTotalForceFunction_, arglist);
...
template <typename Term, bool withLargeOutput, typename MeshType>
typename DynamicHyperelasticitySolver<Term, withLargeOutput,
                                      MeshType>::HyperelasticitySolverType &
