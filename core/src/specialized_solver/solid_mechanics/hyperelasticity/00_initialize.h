...
public:
  ...
  //! constructor
  HyperelasticityInitialize(DihuContext context,
  ...
  //! nullptr, only copy to u
  void setDisplacementsAndPressureFromCombinedVec(
      Vec x, std::shared_ptr<DisplacementsFieldVariableType> u = nullptr,
      std::shared_ptr<PressureFieldVariableType> p = nullptr);

  //! copy entries of combined vector x to u, v and p, if all u,v and p are
  //! nullptr, use this->data_.displacements(), this->data_.velocities() and
  //! this->data_.pressure(), if only p is nullptr, only copy to u
  void setDisplacementsVelocitiesAndPressureFromCombinedVec(
      Vec x, std::shared_ptr<DisplacementsFieldVariableType> u = nullptr,
      std::shared_ptr<DisplacementsFieldVariableType> v = nullptr,
      std::shared_ptr<PressureFieldVariableType> p = nullptr);

  //! copy the values of the vector x which contains (u and p) or (u,v and p)
  //! values to this->data_.displacements(), this->data_.velocities() and
  //! this->data_.pressure(); This simply calls
  //! setDisplacementsAndPressureFromCombinedVec or
  //! setDisplacementsVelocitiesAndPressureFromCombinedVec depending on static
  //! or dynamic problem.
  void setUVP(Vec x);

  //! copy the entries in the combined solution vector to
  ...
  combinedVecResidual(); //< the vector that holds the computed residual
  ...
  //! add new dirichlet bc's, it is also possible to set new dofs that were not
  //! prescribed beforehand this calles addBoundaryConditions() of the
  //! dirichletBoundaryConditions_ object
  ...
  //! get the Petsc Vec of the current state (uvp vector), this is needed to
  //! save and restore checkpoints from the PreciceAdapter
  Vec currentState();
...
protected:
  ...
  //! compute the elemental coordinate frame (elementalX,elementalY,elementalZ)
  //! at the node (i,j,k) in the element with node positions given by geometry
  void getElementalBasis(int i, int j, int k,
                         const std::array<Vec3, 27> &geometry, Vec3 &elementalX,
                         Vec3 &elementalY, Vec3 &elementalZ);
