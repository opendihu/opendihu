...
HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  VLOG(3) << "compute deformation gradient Fij, displacements: "
          << displacements;
  ...
  for (int dimensionColumn = 0; dimensionColumn < 3; dimensionColumn++) {
    ...
    VecD<3, double_v_t> du_dX({0}); // vector contains entries for all i, du_dXj
                                    // with j = dimensionColumn
...
HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
                                    nDisplacementComponents>::
    computeDeformationGradientTimeDerivative(
        const std::array<VecD<3, double_v_t>,
                         DisplacementsFunctionSpace::nDofsPerElement()>
            &velocities,
        const Tensor2<3, double_v_t> &inverseJacobianMaterial,
        const std::array<double, 3> xi) {
  // compute the time derivative of the deformation gradient d(x_i,j)/dt =
  // d/dt(δ_ij + u_i,j) = v_i,j where j is dimensionColumn and i is component of
  // the used Vec3's

  VLOG(3)
      << "compute deformation gradient time derivative Fdot_ij, velocities: "
      << velocities;
  ...
  VLOG(3) << "deformationGradientTimeDerivative: "
...
HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  // compute C = F^T*F where F is the deformationGradient and C is the right
  // Cauchy-Green Tensor the quantities are 3x3 tensors for the 3D case
  Tensor2<3, double_v_t> rightCauchyGreenTensor(
      {std::array<double_v_t, 3>({0})});
    ...
    for (int dimensionRow = 0; dimensionRow < 3; dimensionRow++) {
      for (int k = 0; k < 3; k++) {
        ...
        rightCauchyGreenTensor[dimensionColumn][dimensionRow] +=
            deformationGradient[dimensionColumn][k] *
            deformationGradient[dimensionRow][k];
      }
    }
  }
...
HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  if (Term::usesFiberDirection) {
    ...
    invariants[4] = a0C2a0;
    // LOG(DEBUG) << "computed I4: " << invariants[3] << ", I5: " <<
    // invariants[4] << ", a0: " << fiberDirection;
  }
...
HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  // 2D: Cbar = J^{-2/2}*C = J^{-1}*C = J^{-1}*F^T*F =
  // J^{-1}*J^{1/2}*J^{1/2}*Fbar^T*Fbar = Fbar^T*Fbar such that det(Cbar) = 1
  ...
  reducedInvariants[0] =
      ...
      invariants[0];
  reducedInvariants[1] =
      MathUtility::pow(deformationGradientDeterminant, factor43) *
      invariants[1];
...
#ifndef HAVE_STDSIMD
    LOG(ERROR) << "J=det F is negative: " << deformationGradientDeterminant
               << ". Result will be unphysical.\n"
               << "For dynamic problems, reduce time step width, for static "
                  "problems, add smaller \"loadFactors\" or reduce load.";
#else
...
HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  Tensor2<3, double_v_t> inverseRightCauchyGreen =
      MathUtility::computeInverse<3>(rightCauchyGreen, approximateMeshWidth,
                                     determinant);
    ...
    for (int b = 0; b < 3; b++) {
      ...
      for (int c = 0; c < 3; c++) {
        ...
        for (int d = 0; d < 3; d++) {
          ...
          // LOG(DEBUG) << "  .PP_" << a << b << c << d << ": " << p_abcd << "="
          // << ii_abcd << "-1/3*" << cInv_ab * c_cd << " (" << cInv_ab << ","
          // << c_cd << "), Ii: " << ii_abcd;
