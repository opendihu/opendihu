...
class HyperelasticityMaterialComputations
    : public HyperelasticityInitialize<Term, withLargeOutput, MeshType,
...
public:
  ...
  //! constructor
  HyperelasticityMaterialComputations(
      DihuContext context, std::string settingsKey = "HyperelasticitySolver");

  //! not relevant, as it does nothing. Contains a lot of commented out
  //! debugging code that is helpful to debug the analytic jacobian matrix
  template <typename double_v_t>
  void materialTesting(
  ...
  //! output internalVirtualWork, externalVirtualWorkDead and accelerationTerm
  //! which is the acceleration contribution to δW
  void solveDynamicProblem(
      std::shared_ptr<VecHyperelasticity> displacementsVelocitiesPressure,
      bool isFirstTimeStep, Vec internalVirtualWork,
      Vec &externalVirtualWorkDead, Vec accelerationTerm,
      bool withOutputWritersEnabled = true);

  //! compute u from the equation ∂W_int - externalVirtualWork = 0 and J = 1,
  //! the result will be in displacements, the previous value in displacements
  //! will be used as initial guess (call zeroEntries() of displacements
  //! beforehand, if no initial guess should be provided)
  void
  solveForDisplacements(std::shared_ptr<VecHyperelasticity> externalVirtualWork,
                        std::shared_ptr<VecHyperelasticity> displacements);

  //! compute the resulting forces and moments at the 2- (bottom) and 2+ (top)
  //! surfaces of the volume
  //! @param elements <element_no, isTop> a list of elements that are at bottom
  //! and top of the volume and on which the integration over forces and moments
  //! is performed, !isTop means bottom element
  void computeBearingForceAndMoment(
...
protected:
  typedef HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                                    nDisplacementComponents>
      Parent;

  //! compute δW_int, input is in this->data_.displacements() and
  //! this->data_.pressure(), output is in solverVariableResidual_
  //! @param communicateGhosts if startGhostManipulation() and
  //! finishGhostManipulation() will be called on combinedVecResidual_ inside
  //! this method, if set to false, you have to do it manually before and after
  //! this method
  ...
  //! compute the nonlinear function F(x), x=solverVariableSolution_,
  //! F=solverVariableResidual_ solverVariableResidual_[0-2] contains δW_int -
  //! δW_ext, solverVariableResidual_[3] contains int_Ω (J-1)*Ψ dV
  //! @param loadFactor: a factor with which the rhs is scaled. This is
  //! equivalent to set the body force and traction (neumann bc) to this
  //! fraction
  //! @return true if computation was successful (i.e. no negative jacobian)
  ...
  //! add the acceleration term that is needed for the dynamic problem to δW_int
  //! - δW_ext,dead, and secondly, add the velocity/displacement equation 1/dt
  ...
  //! this method
  void
  materialAddAccelerationTermAndVelocityEquation(bool communicateGhosts = true);

  //! compute the jacobian of the Newton scheme
  //! @return true if computation was successful (i.e. no negative jacobian)
  bool materialComputeJacobian();

  //! compute the deformation gradient, F inside the current element at position
  //! xi, the value of F is still with respect to the reference configuration,
  //! the formula is F_ij = x_i,j = δ_ij + u_i,j
  template <typename double_v_t>
  Tensor2<3, double_v_t> computeDeformationGradient(
      const std::array<VecD<3, double_v_t>,
                       DisplacementsFunctionSpace::nDofsPerElement()>
          &displacements,
      const Tensor2<3, double_v_t> &inverseJacobianMaterial,
      const std::array<double, 3> xi);

  //! compute the time velocity of the deformation gradient, Fdot inside the
  //! current element at position xi, the value of F is still with respect to
  //! the reference configuration, the formula is Fdot_ij = d/dt x_i,j = v_i,j
  template <typename double_v_t>
  Tensor2<3, double_v_t> computeDeformationGradientTimeDerivative(
      const std::array<VecD<3, double_v_t>,
                       DisplacementsFunctionSpace::nDofsPerElement()>
          &velocities,
      const Tensor2<3, double_v_t> &inverseJacobianMaterial,
  ...
  //! compute the right Cauchy Green tensor, C = F^T*F. This is simply a matrix
  //! matrix multiplication
  template <typename double_v_t>
  Tensor2<3, double_v_t> computeRightCauchyGreenTensor(
      const Tensor2<3, double_v_t> &deformationGradient);

  //! compute the invariants, [I1,I2,I3,I4,I5] where I1 = tr(C), I2 = 1/2 *
  //! (tr(C)^2 - tr(C^2)), I3 = det(C), I4 = a0•C a0, I5 = a0•C^2 a0
  template <typename double_v_t>
  std::array<double_v_t, 5>
  computeInvariants(const Tensor2<3, double_v_t> &rightCauchyGreen,
                    const double_v_t rightCauchyGreenDeterminant,
                    const VecD<3, double_v_t> fiberDirection);

  //! compute the reduced invariants, [Ibar1, Ibar2]
  template <typename double_v_t>
  std::array<double_v_t, 5>
  computeReducedInvariants(const std::array<double_v_t, 5> invariants,
                           const double_v_t deformationGradientDeterminant);

  //! compute the 2nd Piola-Kirchhoff pressure, S
  template <typename double_v_t>
  Tensor2<3, double_v_t> computePK2Stress(
      double_v_t &pressure, //< [in/out] pressure value p
      const Tensor2<3, double_v_t> &rightCauchyGreen,        //< [in] C
      const Tensor2<3, double_v_t> &inverseRightCauchyGreen, //< [in] C^{-1}
      const std::array<double_v_t, 5>
          invariants, //< [in] the strain invariants I_1, ..., I_5
      const std::array<double_v_t, 5>
          reducedInvariants, //< [in] the reduced invariants Ibar_1, ..., Ibar_5
      const double_v_t deformationGradientDeterminant, //< [in] J = det(F)
      VecD<3, double_v_t> fiberDirection, //< [in] a0, direction of fibers
  ...
  //! value in data, for output
  void computePK2StressField();

  //! compute the material elasticity tensor
  template <typename double_v_t>
  void
  computeElasticityTensor(const Tensor2<3, double_v_t> &rightCauchyGreen,
                          const Tensor2<3, double_v_t> &inverseRightCauchyGreen,
                          double_v_t deformationGradientDeterminant,
                          double_v_t pressure,
                          std::array<double_v_t, 5> invariants,
                          std::array<double_v_t, 5> reducedInvariants,
                          const Tensor2<3, double_v_t> &fictitiousPK2Stress,
                          const Tensor2<3, double_v_t> &pk2StressIsochoric,
                          VecD<3, double_v_t> fiberDirection,
                          Tensor4<3, double_v_t> &fictitiousElasticityTensor,
                          Tensor4<3, double_v_t> &elasticityTensorIso,
                          Tensor4<3, double_v_t> &elasticityTensor);

  //! compute P : Sbar
  template <typename double_v_t>
  Tensor2<3, double_v_t>
  computePSbar(const Tensor2<3, double_v_t> &fictitiousPK2Stress,
               const Tensor2<3, double_v_t> &rightCauchyGreen);

  //! compute the value of Sbar : C
  template <typename double_v_t>
  double computeSbarC(const Tensor2<3, double_v_t> &Sbar,
