...
HyperelasticityInitialize<
    ...
    nDisplacementComponents>::HyperelasticityInitialize(DihuContext context,
                                                        std::string settingsKey)
    : context_(context[settingsKey]), data_(context_),
      pressureDataCopy_(context_), initialized_(false), endTime_(0),
      lastNorm_(0), secondLastNorm_(0), currentLoadFactor_(1.0),
      lastSolveSucceeded_(false), nNonZerosJacobian_(0) {
  // get python config
  ...
  useAnalyticJacobian_ =
      this->specificSettings_.getOptionBool("useAnalyticJacobian", true);
  useNumericJacobian_ =
      this->specificSettings_.getOptionBool("useNumericJacobian", true);
  nNonlinearSolveCalls_ = this->specificSettings_.getOptionInt(
      "nNonlinearSolveCalls", 1, PythonUtility::Positive);
  loadFactorGiveUpThreshold_ = this->specificSettings_.getOptionDouble(
      "loadFactorGiveUpThreshold", 1e-5, PythonUtility::Positive);

  scaleInitialGuess_ = false;
  if (this->specificSettings_.hasKey("scaleInitialGuess"))
    scaleInitialGuess_ =
        this->specificSettings_.getOptionBool("scaleInitialGuess", false);

  // parse constant body force, a value of "None" yields the default value,
  // (0,0,0)
  constantBodyForce_ =
      this->specificSettings_.template getOptionArray<double, 3>(
          "constantBodyForce", Vec3{0.0, 0.0, 0.0});

  if (!useAnalyticJacobian_ && !useNumericJacobian_) {
    LOG(WARNING)
        << "Cannot set both \"useAnalyticJacobian\" and \"useNumericJacobian\" "
           "to False, now using numeric jacobian.";
  ...
  LOG(DEBUG) << "HyperelasticitySolver: parsed parameters "
             << materialParameters_;
  ...
  displacementsScalingFactor_ =
      specificSettings_.getOptionDouble("displacementsScalingFactor", 1.0);
  dumpDenseMatlabVariables_ =
      specificSettings_.getOptionBool("dumpDenseMatlabVariables", false);
  dampingFactor_ = 0;

  // for the dynamic equation
  if (nDisplacementComponents == 6) {
    density_ = specificSettings_.getOptionDouble("density", 1.0,
                                                 PythonUtility::Positive);
    timeStepWidth_ = specificSettings_.getOptionDouble("timeStepWidth", 1.0,
                                                       PythonUtility::Positive);
    extrapolateInitialGuess_ =
        specificSettings_.getOptionBool("extrapolateInitialGuess", true);

    if (specificSettings_.hasKey("dampingFactor"))
      dampingFactor_ = specificSettings_.getOptionDouble(
          "dampingFactor", 0.0, PythonUtility::NonNegative);
  }

  // initialize output writers
  this->outputWriterManager_.initialize(this->context_,
                                        this->specificSettings_);
  this->outputWriterManagerPressure_.initialize(
      this->context_["pressure"], this->context_["pressure"].getPythonConfig());
  this->outputWriterManagerLoadIncrements_.initialize(
      this->context_["LoadIncrements"],
      this->context_["LoadIncrements"].getPythonConfig());
}

template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                               nDisplacementComponents>::initialize() {
  if (this->initialized_)
    return;

  LOG(DEBUG) << "initialize HyperelasticitySolver with "
             << nDisplacementComponents
             << " displacement components, mesh type "
             << StringUtility::demangle(typeid(MeshType).name()) << ", term "
             << StringUtility::demangle(typeid(Term).name());
  assert(this->specificSettings_.pyObject());

  // create function space / mesh, the geometry is from the settings
  displacementsFunctionSpace_ =
      context_.meshManager()->functionSpace<DisplacementsFunctionSpace>(
          specificSettings_);

  pressureFunctionSpace_ =
      PressureFunctionSpaceCreator<typename PressureFunctionSpace::Mesh>::
          createPressureFunctionSpace(context_.meshManager(),
                                      displacementsFunctionSpace_);
  ...
  if (dirichletBoundaryConditions_ == nullptr) {
    dirichletBoundaryConditions_ = std::make_shared<DirichletBoundaryConditions<
        DisplacementsFunctionSpace, nDisplacementComponents>>(this->context_);
  ...
  if (neumannBoundaryConditions_ == nullptr) {
    ...
    neumannBoundaryConditions_ = std::make_shared<NeumannBoundaryConditions<
        DisplacementsFunctionSpace, QuadratureType, 3>>(this->context_);
    neumannBoundaryConditions_->initialize(this->specificSettings_,
                                           this->data_.functionSpace(),
                                           "neumannBoundaryConditions");
    neumannBoundaryConditions_->setDeformationGradientField(
        this->data_.deformationGradient());
  }
  ...
  if (!loadFactors_.empty()) {
    if (fabs(loadFactors_.back() - 1.0) > 1e-12) {
      LOG(WARNING) << this->specificSettings_
                   << "[\"loadFactors\"]: Last load factor "
                   << loadFactors_.back() << " is not 1.0.";
    }
  }
...
void HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::initializeFiberDirections() {
  std::vector<std::string> fiberMeshNames;
  this->specificSettings_.template getOptionVector<std::string>(
      "fiberMeshNames", fiberMeshNames);
  ...
  if (fiberMeshNames.empty()) {
    Vec3 fiberDirection =
        this->specificSettings_.template getOptionArray<double, 3>(
            "fiberDirection", Vec3{0, 0, 0});

    if (fiberDirection[0] == 0 && fiberDirection[1] == 0 &&
        fiberDirection[2] == 0) {
      // fiberDirection was not specified, check localFiberDirection
      Vec3 fiberDirectionInElement =
          this->specificSettings_.template getOptionArray<double, 3>(
              "fiberDirectionInElement", Vec3{0, 0, 1});

      const int nNodes1D = ::FunctionSpace::FunctionSpaceBaseDim<
          1, BasisFunction::LagrangeOfOrder<2>>::nNodesPerElement();
      assert(nNodes1D == 3);
      const int nDofsPerElement = DisplacementsFunctionSpace::nDofsPerElement();

      // loop over elements
      for (element_no_t elementNoLocal = 0;
           elementNoLocal < this->displacementsFunctionSpace_->nElementsLocal();
           elementNoLocal++) {
        // get all node positions of nodes of this element
        std::array<Vec3, nDofsPerElement> geometry;
        this->displacementsFunctionSpace_->getElementGeometry(elementNoLocal,
                                                              geometry);

        // get indices of element-local dofs
        std::array<dof_no_t, nDofsPerElement> dofNosLocal =
            this->displacementsFunctionSpace_->getElementDofNosLocal(
                elementNoLocal);
        ...
        for (int k = 0; k < nNodes1D; k++) {
          for (int j = 0; j < nNodes1D; j++) {
            for (int i = 0; i < nNodes1D; i++) {
              ...
              getElementalBasis(i, j, k, geometry, elementalX, elementalY,
                                elementalZ);

              Vec3 fiberDirection = fiberDirectionInElement[0] * elementalX +
                                    fiberDirectionInElement[1] * elementalY +
                                    fiberDirectionInElement[2] * elementalZ;
              this->data_.fiberDirection()->setValue(dofNoLocal, fiberDirection,
                                                     INSERT_VALUES);
            }
          }
        }
      ...
      LOG(DEBUG)
          << "displacements field variable type data: "
          << StringUtility::demangle(
                 typeid(typename Data::DisplacementsFieldVariableType).name());
      ...
      std::vector<Vec3> fiberDirections(
          ...
          fiberDirection);
      this->data_.fiberDirection()->setValues(
          this->displacementsFunctionSpace_->meshPartition()->dofNosLocal(),
          fiberDirections);
    }
  } else {
    ...
    for (int fiberNo = 0; fiberNo < fiberMeshNames.size(); fiberNo++) {
      ...
      LOG(DEBUG) << "fiber " << fiberNo << "/" << fiberMeshNames.size()
                 << ", mesh \"" << fiberMeshName << "\".";

      std::shared_ptr<FiberFunctionSpace> fiberFunctionSpace =
          context_.meshManager()->functionSpace<FiberFunctionSpace>(
              fiberMeshName);

      LOG(DEBUG) << "create mapping  1D fiber -> 3D: \""
                 << fiberFunctionSpace->meshName() << "\" -> \""
                 << this->displacementsFunctionSpace_->meshName() << "\".";

      // initialize mapping 1D fiber -> 3D
      DihuContext::mappingBetweenMeshesManager()
          ->template mappingBetweenMeshes<FiberFunctionSpace,
                                          DisplacementsFunctionSpace>(
              fiberFunctionSpace, this->displacementsFunctionSpace_);
    }

    using SourceFunctionSpaceType =
        FieldVariable::FieldVariable<FiberFunctionSpace, 3>;
    ...
    std::shared_ptr<FieldVariable::FieldVariable<FiberFunctionSpace, 3>>
        direction;
    DihuContext::mappingBetweenMeshesManager()
        ->template prepareMapping<SourceFunctionSpaceType,
                                  TargetFunctionSpaceType>(
            direction, this->data_.fiberDirection(), -1);
    ...
    for (int fiberNo = 0; fiberNo < fiberMeshNames.size(); fiberNo++) {
      ...
      // define direction field variable on the fiber that will store the
      ...
      fiberFunctionSpace->geometryField().getValuesWithoutGhosts(
          geometryFieldValues);
      ...
      for (dof_no_t dofNoLocal = 0; dofNoLocal < nDofsLocalWithoutGhosts;
        ...
        dof_no_t index1 = std::min((dof_no_t)(nDofsLocalWithoutGhosts - 1),
                                   (dof_no_t)(dofNoLocal + 1));

        // get direction of 1D fiber
        Vec3 fiberDirection =
            -geometryFieldValues[index0] + geometryFieldValues[index1];
        directionValues[dofNoLocal] = fiberDirection;
      }

      direction->setValuesWithoutGhosts(directionValues);

      // transfer direction values from 1D fibers to 3D field, -1 means all
      // components
      DihuContext::mappingBetweenMeshesManager()
          ->template map<SourceFunctionSpaceType, TargetFunctionSpaceType>(
              direction, this->data_.fiberDirection(), -1, -1, false);
    }

    // finalize the mapping to the target mesh, compute final values by dividing
    // by the factors
    DihuContext::mappingBetweenMeshesManager()
        ->template finalizeMapping<SourceFunctionSpaceType,
                                   TargetFunctionSpaceType>(
            direction, this->data_.fiberDirection(), -1, -1, false);
  }
  ...
  this->data_.fiberDirection()->getValuesWithoutGhosts(
      valuesLocalWithoutGhosts);

  for (dof_no_t dofNoLocal = 0;
       dofNoLocal < this->data_.fiberDirection()->nDofsLocalWithoutGhosts();
       dofNoLocal++) {
    if (MathUtility::normSquared<3>(valuesLocalWithoutGhosts[dofNoLocal]) <
        1e-10) {
      valuesLocalWithoutGhosts[dofNoLocal] = Vec3({0, 0, 1});
      LOG(DEBUG) << "dof " << dofNoLocal
                 << ", fiberDirection was (0,0,0), set to (0,0,1)";
    } else {
      MathUtility::normalize<3>(valuesLocalWithoutGhosts[dofNoLocal]);
#ifndef NDEBUG
      LOG(DEBUG) << "dof " << dofNoLocal << ", fiberDirection normalized: "
                 << valuesLocalWithoutGhosts[dofNoLocal] << " (norm: "
                 << MathUtility::norm<3>(valuesLocalWithoutGhosts[dofNoLocal])
                 << ")";
#endif
    }
  }

  this->data_.fiberDirection()->setValuesWithoutGhosts(
      valuesLocalWithoutGhosts);
...
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                               nDisplacementComponents>::
    getElementalBasis(int i, int j, int k, const std::array<Vec3, 27> &geometry,
                      Vec3 &elementalX, Vec3 &elementalY, Vec3 &elementalZ) {
  const int nNodes1D = ::FunctionSpace::FunctionSpaceBaseDim<
      1, BasisFunction::LagrangeOfOrder<2>>::nNodesPerElement(); // 3
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
std::shared_ptr<typename HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::VecHyperelasticity>
HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::createPartitionedPetscVec(std::string name) {
  LOG(DEBUG) << "createPartitionedPetscVec(" << name << ")";
  return std::make_shared<VecHyperelasticity>(
      displacementsFunctionSpace_->meshPartition(),
      pressureFunctionSpace_->meshPartition(), dirichletBoundaryConditions_,
      name);
}
...
std::shared_ptr<typename HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::MatHyperelasticity>
HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::createPartitionedPetscMat(std::string name) {
  ...
  LOG(INFO) << "Preallocation for matrix \"" << name
            << "\": diagonal nz: " << nNonZerosDiagonal
            << ", offdiagonal nz: " << nNonZerosOffdiagonal;
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
unsigned int HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::materialDetermineNumberNonzerosInJacobian() {
  ...
  nNonZeros =
      nElementsLocal * MathUtility::sqr(nDisplacementsDofsPerElement * D);

  if (nDisplacementComponents == 6) {
    nNonZeros +=
        nElementsLocal * MathUtility::sqr(nDisplacementsDofsPerElement * D) * 3;
  }

  if (Term::isIncompressible) {
    nNonZeros += nElementsLocal * nPressureDofsPerElement *
                 nDisplacementsDofsPerElement * D * 2;
    nNonZeros += nElementsLocal * nPressureDofsPerElement;
  }
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
Vec HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                              nDisplacementComponents>::externalVirtualWork() {
...
void HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::initializePetscVariables() {
  /*
   ...
   *  (P P P 0)
   */
  // prepare for data structures without ghost dofs, these are normal Petsc
  // Mat's and Vec's for which all solvers are available
  ...
  // vector for the external virtual work contribution that does not depend on
  // u, δW_ext,dead (this is the same as δW_ext for static case)
  combinedVecExternalVirtualWorkDead_ =
      createPartitionedPetscVec("combinedVecExternalVirtualWorkDead");

  // get number of local entries of the vectors, this will be the number of rows
  // and columns of the matrix
  int nMatrixRowsLocal = combinedVecSolution_->nEntriesLocal();

  // output
  LOG(DEBUG) << "n dofs displacements: " << nDisplacementComponents << "*"
             << displacementsFunctionSpace_->nDofsLocalWithoutGhosts() << " = "
             << nDisplacementComponents *
                    displacementsFunctionSpace_->nDofsLocalWithoutGhosts();
  LOG(DEBUG) << "n dofs pressure: "
             << pressureFunctionSpace_->nDofsLocalWithoutGhosts() << ", total: "
             << displacementsFunctionSpace_->nDofsLocalWithoutGhosts() *
                        nDisplacementComponents +
                    pressureFunctionSpace_->nDofsLocalWithoutGhosts();
  LOG(DEBUG) << "n BC dofs: ";

  LOG(DEBUG) << "  ux: "
             << dirichletBoundaryConditions_->boundaryConditionsByComponent()[0]
                    .dofNosLocal.size();
  LOG(DEBUG) << "  uy: "
             << dirichletBoundaryConditions_->boundaryConditionsByComponent()[1]
                    .dofNosLocal.size();
  LOG(DEBUG) << "  uz: "
             << dirichletBoundaryConditions_->boundaryConditionsByComponent()[2]
                    .dofNosLocal.size();

  if (nDisplacementComponents == 6) {
    LOG(DEBUG) << "  vx: "
               << dirichletBoundaryConditions_
                      ->boundaryConditionsByComponent()[3]
                      .dofNosLocal.size();
    LOG(DEBUG) << "  vy: "
               << dirichletBoundaryConditions_
                      ->boundaryConditionsByComponent()[4]
                      .dofNosLocal.size();
    LOG(DEBUG) << "  vz: "
               << dirichletBoundaryConditions_
                      ->boundaryConditionsByComponent()[5]
                      .dofNosLocal.size();
  }
  LOG(DEBUG) << "number of non-BC dofs total: " << nMatrixRowsLocal;

  // create matrix with same dof mapping as vectors
  // std::shared_ptr<::FunctionSpace::Generic> genericFunctionSpace =
  // context_.meshManager()->createGenericFunctionSpace(nMatrixRowsLocal,
  // displacementsFunctionSpace_->meshPartition(), "genericMesh");

  combinedMatrixJacobian_ = createPartitionedPetscMat("combinedJacobian");

  solverMatrixAdditionalNumericJacobian_ = PETSC_NULL;

  // if both numeric and analytic jacobian are used, create additional matrix
  // that will hold the numeric jacobian
  if (useNumericJacobian_ && useAnalyticJacobian_) {
    combinedMatrixAdditionalNumericJacobian_ =
        createPartitionedPetscMat("combinedJacobianNumeric");

    solverMatrixAdditionalNumericJacobian_ =
        combinedMatrixAdditionalNumericJacobian_->valuesGlobal();
  }

  // extract the Petsc Vec's of the PartitionedPetscVecForHyperelasticity
  // objects
  ...
  externalVirtualWorkDead_ =
      combinedVecExternalVirtualWorkDead_->valuesGlobal();

  // create vector with all zeros in it, this is needed for zeroing the diagonal
  // of the stiffness matrix for initialization in evaluateAnalyticJacobian
  PetscErrorCode ierr;
  ierr = VecDuplicate(solverVariableResidual_, &zeros_);
  CHKERRV(ierr);
  ierr = VecZeroEntries(zeros_);
  CHKERRV(ierr);
  ierr = VecDuplicate(solverVariableResidual_, &lastSolution_);
  CHKERRV(ierr);
  ierr = VecDuplicate(solverVariableResidual_, &bestSolution_);
  CHKERRV(ierr);

  LOG(DEBUG) << "for debugging: " << combinedVecSolution_->getString();

  // compute the external virtual work, because it is constant throughout the
  // solution process
  materialComputeExternalVirtualWorkDead();

  // output pointer values for debugging
  LOG(DEBUG) << "-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.";
  LOG(DEBUG) << "pointer value solverVariableResidual_: "
             << solverVariableResidual_;
  LOG(DEBUG) << "pointer value solverVariableSolution_: "
             << solverVariableSolution_;
  LOG(DEBUG) << "pointer value solverMatrixJacobian_:   "
             << solverMatrixJacobian_;
  LOG(DEBUG) << "pointer value solverMatrixAdditionalNumericJacobian_:  "
             << solverMatrixAdditionalNumericJacobian_;
  LOG(DEBUG) << "-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.";

  // if a numeric jacobian is in use, assemble the matrix
  // depending on if also the analytic jacobian will be computed, the storage
  // for the numeric jacobian is either combinedMatrixJacobian_ or
  // combinedMatrixAdditionalNumericJacobian_
  ...
  if (useAnalyticJacobian_) {
    // jacobian matrix is already preallocated, but there might be even more
    // entries required
    ierr = MatSetOption(solverMatrixJacobian_, MAT_NEW_NONZERO_ALLOCATION_ERR,
                        PETSC_FALSE);
    CHKERRV(ierr);
    ...
    // print info about preallocation, this can also be done by -mat_view
    // ::ascii_info_detail
    // ierr = MatView(solverMatrixJacobian_, PETSC_VIEWER_ASCII_INFO_DETAIL);
    // CHKERRV(ierr);
...
void HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::initializeSolutionVariable() {
  // set variable to all zero and dirichlet boundary condition value
  LOG(DEBUG) << "zeroEntries, representation: "
             << combinedVecSolution_->currentRepresentation();
  ...
  if (inputMeshIsGlobal) {
    ...
    this->specificSettings_.template getOptionVector<Vec3>(
        "initialValuesDisplacements", nDofsGlobal, localValuesDisplacements);
    displacementsFunctionSpace->meshPartition()->extractLocalDofsWithoutGhosts(
        localValuesDisplacements);
  } else {
    // input is already only the local dofs, use all
    const int nDofsLocal =
        displacementsFunctionSpace->nDofsLocalWithoutGhosts();
    this->specificSettings_.template getOptionVector<Vec3>(
        "initialValuesDisplacements", nDofsLocal, localValuesDisplacements);
  }
  VLOG(1) << "set initial values for displacements to "
          << localValuesDisplacements;
  ...
  for (int componentNo = 0; componentNo < 3; componentNo++) {
    ...
    combinedVecSolution_->setValues(
        componentNo, nDofsLocalWithoutGhosts,
        displacementsFunctionSpace->meshPartition()->dofNosLocal().data(),
        localValues.data());
  }
  ...
  LOG(DEBUG) << "values: "
             << PetscUtility::getStringVector(solverVariableSolution_);
...
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                               nDisplacementComponents>::
    setDisplacementsAndPressureFromCombinedVec(
        Vec x, std::shared_ptr<DisplacementsFieldVariableType> u,
        std::shared_ptr<PressureFieldVariableType> p) {
  assert(nDisplacementComponents == 3);

  // copy entries of combined vector x to this->data_.displacements() and
  // this->data_.pressure()
  std::vector<double> values;

  if (VLOG_IS_ON(1)) {
    PetscUtility::getVectorEntries(x, values);
    VLOG(1) << "setDisplacementsAndPressureFromCombinedVec, x="
            << PetscUtility::getStringVector(x);
  }
  ...
  if (!u && !p) {
    ...
    if (Term::isIncompressible)
      p = this->data_
              .pressure(); // p is only needed for incompressible formulation
  }
  ...
  for (int componentNo = 0; componentNo < 3; componentNo++) {
    ...
    combinedVecSolution_->getValues(
  ...
  if (p) {
    ...
    combinedVecSolution_->getValues(
        pressureComponent, nEntries,
        pressureFunctionSpace_->meshPartition()->dofNosLocal().data(),
        values.data());
    p->setValues(0, nEntries,
                 pressureFunctionSpace_->meshPartition()->dofNosLocal().data(),
                 values.data());
...
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                               nDisplacementComponents>::
    setDisplacementsVelocitiesAndPressureFromCombinedVec(
        Vec x, std::shared_ptr<DisplacementsFieldVariableType> u,
  ...
  if (VLOG_IS_ON(1)) {
    PetscUtility::getVectorEntries(x, values);
    VLOG(1) << "setDisplacementsVelocitiesAndPressureFromCombinedVec, x="
            << PetscUtility::getStringVector(x);
  }
  ...
  for (int componentNo = 0; componentNo < 3; componentNo++) {
    ...
    if (VLOG_IS_ON(1))
      LOG(DEBUG) << "setDisplacementsVelocitiesAndPressureFromCombinedVec, "
                 << nEntries << " u values: " << values;
  ...
  if (v) {
    ...
    for (int componentNo = 0; componentNo < 3; componentNo++) {
      ...
      values.resize(nEntries);
      combinedVecSolution_->getValues(
          3 + componentNo, nEntries,
          displacementsFunctionSpace_->meshPartition()->dofNosLocal().data(),
          values.data());

      if (VLOG_IS_ON(1))
        VLOG(1) << "setDisplacementsVelocitiesAndPressureFromCombinedVec, "
                << nEntries << " v values: " << values;
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                               nDisplacementComponents>::setUVP(Vec x) {
...
void HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::dumpJacobianMatrix(Mat jac) {
  ...
  // if there are both numeric and analytic jacobian in use, this is the numeric
  // jacobian
  if (jac == solverMatrixAdditionalNumericJacobian_) {
    filename << "_numeric";
    combinedMatrixAdditionalNumericJacobian_->dumpMatrixGlobalNatural(
        filename.str());
  } else if (jac == solverMatrixJacobian_) {
    // this is the normal jacobian, either numeric or analytic, if only one of
    // both is in use
    ...
    if (solverMatrixAdditionalNumericJacobian_ != PETSC_NULL) {
      ...
      MatNorm(solverMatrixAdditionalNumericJacobian_, NORM_1,
              &numericJacobianNorm);
      ...
      if (numericJacobianNorm > 1e-5) {
        ...
        Mat difference;
        PetscErrorCode ierr;
        ierr = MatDuplicate(solverMatrixAdditionalNumericJacobian_,
                            MAT_COPY_VALUES, &difference);
        CHKERRV(ierr);
        ierr = MatCopy(solverMatrixAdditionalNumericJacobian_, difference,
                       SAME_NONZERO_PATTERN);
        CHKERRV(ierr);
        MatAXPY(difference, -1, solverMatrixJacobian_,
        ...
        LOG(INFO)
            << "difference between analytic and numeric jacobian matrices: "
            << "1-norm: " << norm1 << ", frobenius norm: " << normF
        ...
        if (nDisplacementComponents == 6)
          nRows = 3;
        if (!Term::isIncompressible) // compressible formulation does not have
                                     // pressure component
          nRows--;

        for (int i = 0; i < nRows; i++) {
          for (int j = 0; j < nRows; j++) {
            Mat analyticJacobianSubmatrix =
                combinedMatrixJacobian_->getSubmatrix(i, j);
            Mat numericJacobianSubmatrix =
                combinedMatrixAdditionalNumericJacobian_->getSubmatrix(i, j);

            Mat differenceSubmatrix;
            ierr = MatDuplicate(analyticJacobianSubmatrix, MAT_COPY_VALUES,
                                &differenceSubmatrix);
            CHKERRV(ierr);
            ierr = MatCopy(analyticJacobianSubmatrix, differenceSubmatrix,
                           SAME_NONZERO_PATTERN);
            CHKERRV(ierr);
            MatAXPY(differenceSubmatrix, -1, numericJacobianSubmatrix,
                    DIFFERENT_NONZERO_PATTERN);
    ...
    LOG(ERROR) << "Could not output jacobian matrix " << jac
               << " (solverMatrixJacobian_: " << solverMatrixJacobian_
               << ", solverMatrixAdditionalNumericJacobian_: "
               << solverMatrixAdditionalNumericJacobian_ << ")";
  }
}

//! get the PartitionedPetsVec for the residual and result of the nonlinear
//! function
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
std::shared_ptr<typename HyperelasticityInitialize<
...
HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                          nDisplacementComponents>::combinedVecResidual() {
...
std::shared_ptr<typename HyperelasticityInitialize<
    ...
    nDisplacementComponents>::VecHyperelasticity>
HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                          nDisplacementComponents>::combinedVecSolution() {
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
std::shared_ptr<DirichletBoundaryConditions<
    typename HyperelasticityInitialize<
        Term, withLargeOutput, MeshType,
        nDisplacementComponents>::DisplacementsFunctionSpace,
    nDisplacementComponents>>
HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::dirichletBoundaryConditions() {
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
std::shared_ptr<NeumannBoundaryConditions<
    typename HyperelasticityInitialize<
        Term, withLargeOutput, MeshType,
        nDisplacementComponents>::DisplacementsFunctionSpace,
...
HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::neumannBoundaryConditions() {
    ...
    updateNeumannBoundaryConditions(
        std::shared_ptr<NeumannBoundaryConditions<
            typename HyperelasticityInitialize<
                ...
                nDisplacementComponents>::DisplacementsFunctionSpace,
            Quadrature::Gauss<3>, 3>>
            newNeumannBoundaryConditions) {
  neumannBoundaryConditions_ = newNeumannBoundaryConditions;

  // compute new value for the rhs, δW_ext,dead = int_Ω B^L * phi^L * phi^M *
  // δu^M dx + int_∂Ω T^L * phi^L * phi^M * δu^M dS
  materialComputeExternalVirtualWorkDead();
}
...
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
  ...
  bool inputMeshIsGlobal =
      this->specificSettings_.getOptionBool("inputMeshIsGlobal", true);
  combinedVecSolution_->updateDirichletBoundaryConditions(newDirichletBCValues,
                                                          inputMeshIsGlobal);
}
...
void HyperelasticityInitialize<Term, withLargeOutput, MeshType,
    ...
    addDirichletBoundaryConditions(
        std::vector<typename DirichletBoundaryConditions<
            DisplacementsFunctionSpace,
            nDisplacementComponents>::ElementWithNodes>
            &boundaryConditionElements,
        bool overwriteBcOnSameDof) {
  LOG(DEBUG) << "addDirichletBoundaryConditions, Term: "
             << StringUtility::demangle(typeid(Term).name());
  if (!Term::isIncompressible) {
    LOG(DEBUG) << "addDirichletBoundaryConditions on compressible material";
  }

  dirichletBoundaryConditions_->addBoundaryConditions(boundaryConditionElements,
                                                      overwriteBcOnSameDof);

  // an incompressible material has 3+1 (static problem) or 6+1 (dynamic
  // problem) components (displacements+velocities+pressure) a compressible
  // material has only 3 (static problem) or 6 (dynamic problem) components (no
  // pressure)
  ...
  std::array<std::vector<double>, nComponents>
      combinedVecExternalVirtualWorkDeadValues;

  int nDisplacementsVelocityValues =
      this->displacementsFunctionSpace_->meshPartition()
          ->nDofsLocalWithoutGhosts();
  int nPressureValues =
      this->pressureFunctionSpace_->meshPartition()->nDofsLocalWithoutGhosts();
  std::vector<int> indices(nDisplacementsVelocityValues);
  std::iota(indices.begin(), indices.end(), 0);

  // loop over displacement components (and velocity components, if dynamic) and
  ...
  for (int componentNo = 0; componentNo < nComponents; componentNo++) {
    ...
    combinedVecSolution_->getValues(componentNo, nValues, indices.data(),
                                    combinedSolutionValues[componentNo].data());

    // residual vector
    // combinedResidualValues[componentNo].resize(nValues);
    // combinedVecResidual_->getValues(componentNo, nValues, indices.data(),
    // combinedResidualValues[componentNo].data());

    // vector for the external virtual work contribution that does not depend on
    // u, δW_ext,dead (this is the same as δW_ext for static case)
    combinedVecExternalVirtualWorkDeadValues[componentNo].resize(nValues);
    combinedVecExternalVirtualWorkDead_->getValues(
        componentNo, nValues, indices.data(),
        combinedVecExternalVirtualWorkDeadValues[componentNo].data());
  }

  // remove generic meshes "genericMesh" and
  // "genericMeshForMatrixcombinedJacobian"
  DihuContext::meshManager()->deleteFunctionSpace("genericMesh");
  DihuContext::meshManager()->deleteFunctionSpace(
      "genericMeshForMatrixcombinedJacobian");
  ...
  for (int componentNo = 0; componentNo < nComponents; componentNo++) {
    ...
    // solution vector
    combinedVecSolution_->setValues(componentNo, nValues, indices.data(),
                                    combinedSolutionValues[componentNo].data());

    // residual vector
    // combinedVecResidual_->setValues(componentNo, nValues, indices.data(),
    // combinedResidualValues[componentNo].data());
  ...
  PetscErrorCode ierr;
  ierr = VecAssemblyBegin(solverVariableSolution_);
  CHKERRV(ierr);
  ierr = VecAssemblyEnd(solverVariableSolution_);
  CHKERRV(ierr);
}
...
template <typename Term, bool withLargeOutput, typename MeshType,
          int nDisplacementComponents>
Vec HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                              nDisplacementComponents>::currentState() {
...
std::string
HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                          nDisplacementComponents>::getString(Vec x) {
  if (x == solverVariableSolution_) {
...
std::shared_ptr<typename HyperelasticityInitialize<
    Term, withLargeOutput, MeshType,
    nDisplacementComponents>::SlotConnectorDataType>
HyperelasticityInitialize<Term, withLargeOutput, MeshType,
                          nDisplacementComponents>::getSlotConnectorData() {
