...
/** Helper class that creates a first-order pressure function space from the 2nd
 * order displacements function space by only using the nodes on the corners of
 * the quadratic elements
 */
template <typename MeshType> class PressureFunctionSpaceCreator {};
...
public:
  ...
  //! create the first order discretized pressure function space from the second
  //! order displacements function space
  static std::shared_ptr<PressureFunctionSpace> createPressureFunctionSpace(
      std::shared_ptr<Mesh::Manager> meshManager,
      std::shared_ptr<DisplacementsFunctionSpace> displacementsFunctionSpace);

  //! from a vector of values that correspond to a field variable on the
  //! displacements function space, extract the values that correspond to the
  //! pressure function space
  template <typename T>
  static void extractPressureFunctionSpaceValues(
      std::shared_ptr<DisplacementsFunctionSpace> displacementsFunctionSpace,
 ...
 */
template <> class PressureFunctionSpaceCreator<Mesh::CompositeOfDimension<3>> {
public:
