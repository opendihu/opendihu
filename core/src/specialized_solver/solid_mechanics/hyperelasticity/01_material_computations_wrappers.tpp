...
bool HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  // copy the values from the combined vector to displacements and pressure,
  // this is needed for materialComputeResidual
...
void HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  PetscErrorCode ierr;
  ierr = VecSwap(rhs, externalVirtualWorkDead_);
  CHKERRV(ierr);

  Vec result = displacements->valuesGlobal();
  ierr = VecSwap(result, solverVariableSolution_);
  CHKERRV(ierr);
...
void HyperelasticityMaterialComputations<Term, withLargeOutput, MeshType,
  ...
  // solve δW_int - δW_ext,dead + ∫_Ω ρ0 1/dt (v^(n+1),L - v^(n),L)/dt ϕ^L ϕ^M
  // δu dV = 0, 1/dt (u^(n+1) - u^(n)) - v^(n+1) = 0 ∫ (J-1) δp = 0   ∀ δp,δu,δv
  ...
  if (this->extrapolateInitialGuess_) {
    ...
    // this->data_.velocities() and this->data.pressure()
    ...
    LOG(DEBUG) << "extrapolateInitialGuess: initial u: "
               << *this->data_.displacements();
    LOG(DEBUG) << "extrapolateInitialGuess: initial v: "
               << *this->data_.velocities();

    // displacements = u + dt*v
    ierr = VecAXPY(u, this->timeStepWidth_, v);
    CHKERRV(ierr);

    // compute a = (v^(i) - v^(i-1)) / dt in the buffer
    // velocitiesPreviousTimestep() velocitiesPreviousTimestep = v^(i-1) - v^(i)
    ierr = VecAXPY(vprev, -1, v);
    CHKERRV(ierr);

    // velocitiesPreviousTimestep = (v^(i) - v^(i-1)) / dt = acceleration
    ierr = VecScale(vprev, -1.0 / this->timeStepWidth_);
    CHKERRV(ierr);
    Vec a = vprev;

    // velocities = v + dt*a
    ierr = VecAXPY(v, this->timeStepWidth_, a);
    CHKERRV(ierr);

    LOG(DEBUG) << "extrapolateInitialGuess: new u: "
               << *this->data_.displacements();
    LOG(DEBUG) << "extrapolateInitialGuess: new v: "
               << *this->data_.velocities();
    LOG(DEBUG) << "extrapolateInitialGuess: a: "
  ...
  this->setDisplacementsVelocitiesAndPressureFromCombinedVec(
      solverVariableSolution_, this->data_.displacementsPreviousTimestep(),
      this->data_.velocitiesPreviousTimestep(),
  ...
  if (this->extrapolateInitialGuess_) {
    ...
    // store new displacements in combinedVecSolution_ which is
    ...
    LOG(DEBUG) << "extrapolateInitialGuess: initial values: "
               << getString(solverVariableSolution_);
  }

  // find the solution for u,v,p of the nonlinear equation, potentially multiple
  // load steps and repeated solve calls
  this->nonlinearSolve();

  LOG(DEBUG) << "result: " << getString(solverVariableSolution_);

  // update geometry fields from displacements, compute PK2Stress, write output
  // with output writers
  ...
  materialComputeInternalVirtualWork();
  ierr = VecCopy(solverVariableResidual_, internalVirtualWork);
  CHKERRV(ierr);

  // compute acceleration term
  ierr = VecZeroEntries(solverVariableResidual_);
  CHKERRV(ierr);
  materialAddAccelerationTermAndVelocityEquation();
  ierr = VecCopy(solverVariableResidual_, accelerationTerm);
  CHKERRV(ierr);
