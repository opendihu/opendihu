...
void MuscleContractionSolver<MeshType, Term,
                             withLargeOutputFiles>::computeLambda() {
  typedef typename DynamicHyperelasticitySolverType::HyperelasticitySolverType::
      DisplacementsFieldVariableType DisplacementsFieldVariableType;
  typedef typename DynamicHyperelasticitySolverType::HyperelasticitySolverType::
      Data::DeformationGradientFieldVariableType
          DeformationGradientFieldVariableType;
  ...
  if (isDynamic_) {
    ...
    deformationGradientVariable =
        dynamicHyperelasticitySolver_->hyperelasticitySolver()
            .data()
            .deformationGradient();
    fDotVariable = dynamicHyperelasticitySolver_->hyperelasticitySolver()
                       .data()
    ...
    deformationGradientVariable =
        staticHyperelasticitySolver_->data().deformationGradient();
    fDotVariable = staticHyperelasticitySolver_->data()
                       .deformationGradientTimeDerivative();
    velocitiesVariable = staticHyperelasticitySolver_->data().velocities();
  }

  // compute lambda and \dot{lambda} (contraction velocity)
  // std::shared_ptr<DisplacementsFieldVariableType> displacementsVariable =
  ...
  for (dof_no_t dofNoLocal = 0;
       dofNoLocal < data_.functionSpace()->nDofsLocalWithoutGhosts();
       dofNoLocal++) {
    ...
    const VecD<9> deformationGradientValues =
        deformationGradientVariable->getValue(dofNoLocal);
    ...
    // const double lambda = displacement[0] * fiberDirection[0] +
    // displacement[1] * fiberDirection[1] + displacement[2] *
    // fiberDirection[2];

    // convert fiber direction from reference configuration into current
    // configuration
    Vec3 fiberDirectionCurrentConfiguration =
        deformationGradient * fiberDirection; // F a0
    // λ = ||F a0||
    const double lambda = MathUtility::norm<3>(
        fiberDirectionCurrentConfiguration); // stretch in current configuration
    ...
    //                 * (2*(F11*a1 + F12*a2)*(F11'*a1 + F12'*a2) + 2*(F21*a1 +
    //                 F22*a2)*(F21'*a1 + F22'*a2))
    //               = (F a0) • (F' a0) / ||F a0||

    // compute lambda dot
    // d/dt λ = d/dt ||F a0|| = (F a0) • (Fdot a0) / ||F a0||   (where Fdot =
    // d/dt F) d/dt dx = d/dt F d/dt lambda = d/dt ||dx•a0|| = 1 / ||Fa|| (F a0
    // • Fdot a0) = 1/lambda (Fa • Fdot a0)
    Vec3 FdotA0 = fDot * fiberDirection;
    const double lambdaDot =
        1 / lambda *
        MathUtility::dot(fiberDirectionCurrentConfiguration, FdotA0) *
...
template <typename MeshType, typename Term, bool withLargeOutputFiles>
void MuscleContractionSolver<MeshType, Term,
                             withLargeOutputFiles>::computeActiveStress() {
  LOG(DEBUG) << "computeActiveStress";

  typedef typename DynamicHyperelasticitySolverType::HyperelasticitySolverType::
      StressFieldVariableType StressFieldVariableType;
  typedef typename DynamicHyperelasticitySolverType::HyperelasticitySolverType::
      DisplacementsFieldVariableType DisplacementsFieldVariableType;
  typedef typename Data::ScalarFieldVariableType FieldVariableType;
  ...
  // Heidlauf 2013: "Modeling the Chemoelectromechanical Behavior of Skeletal
  // Muscle Using the Parallel Open-Source Software Library OpenCMISS", p.4, Eq.
  // (11)
  ...
  for (dof_no_t dofNoLocal = 0;
    ...
    if (enableForceLengthRelation_) {
      if (0.6 <= lambdaRelative && lambdaRelative <= 1.4) {
        f = -25. / 4 * lambdaRelative * lambdaRelative +
            25. / 2 * lambdaRelative - 5.25;
      }
    }
    ...
    LOG(DEBUG) << "dof " << dofNoLocal << ", lambda: " << lambda
               << ", lambdaRelative: " << lambdaRelative << ", pmax_: " << pmax_
               << ", f: " << f << ", gamma: " << gamma
               << ", => factor: " << factor
               << ", fiberDirection: " << fiberDirection;

    // if lambda is not yet computed (before first computation), set active
    // stress to zero
