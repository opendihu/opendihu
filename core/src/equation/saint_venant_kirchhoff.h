...
struct SaintVenantKirchhoff : HyperelasticityBase {
  ...
  static const auto constexpr strainEnergyDensityFunctionCoupled =
      INT(1) / INT(2) * lambd * pow(trE, INT(2));
  ...
  // compute E^2 = E:E
  static constexpr auto EE = pow(E11, INT(2)) + INT(2) * pow(E12, INT(2)) +
                             INT(2) * pow(E13, INT(2)) + pow(E22, INT(2)) +
                             INT(2) * pow(E23, INT(2)) + pow(E33, INT(2));
