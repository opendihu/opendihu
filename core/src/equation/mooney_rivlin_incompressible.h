...
/** Base class that defines the parameters of the strain energy density
 * function. All hyperelasticity formulations should inherit from this class.
 */
struct HyperelasticityBase {
  // define helper variables for strain energy density function
  // note, the order of VAR(0) - VAR(16) is reflected in the code and cannot be
  // changed anymore
  ...
  static constexpr auto Ibar2 =
      VAR(1); //< 2nd reduced or modified strain invariant Ibar2 = 1/2
              ...
              // Note, there is no 3rd reduced or modified strain invariant
              // needed, Ibar3 = det(Cbar) = 1 (incompressibility)
  static constexpr auto Ibar4 =
      VAR(2); //< 4th reduced or modified strain invariant Ibar4 = a0•Cbar a0
  static constexpr auto Ibar5 =
      VAR(3); //< 5th reduced or modified strain invariant Ibar5 = a0•Cbar^2 a0
  static constexpr auto lambda =
      sqrt(Ibar4); //< fiber stretch, helper variable that can also be used in
                   //`strainEnergyDensityFunctionIsochoric`
  ...
  static constexpr auto I2 =
      VAR(6); //< 2nd strain invariant I2 = 1/2 (tr(C)^2 - tr(C^2))
  static constexpr auto I3 = VAR(7); //< 3rd strain invariant I3 = det(C) = J^2

  // components of the right Cauchy Green tensor, arguments of
  // `strainEnergyDensityFunctionCoupledDependentOnC
  static constexpr auto C11 =
      VAR(8); //< entry C11 of the right Cauchy Green tensor, C
  static constexpr auto C12 =
      VAR(9); //< entry C12 = C21 of the right Cauchy Green tensor, C
  static constexpr auto C13 =
      VAR(10); //< entry C13 = C31 of the right Cauchy Green tensor, C
  static constexpr auto C22 =
      VAR(11); //< entry C22 of the right Cauchy Green tensor, C
  static constexpr auto C23 =
      VAR(12); //< entry C23 = C32 of the right Cauchy Green tensor, C
  static constexpr auto C33 =
      VAR(13); //< entry C33 of the right Cauchy Green tensor, C
  ...
  static constexpr auto I4 =
      C11 * a1 * a1 + INT(2) * C12 * a1 * a2 + INT(2) * C13 * a1 * a3 +
      C22 * a2 * a2 + INT(2) * C23 * a2 * a3 +
      C33 * a3 * a3; //< non-reduced 4th strain invariant, I4 = a0•C a0
};

/** Isotropic hyperelastic solid mechanics formulation with arbitrary strain
...
struct MooneyRivlinIncompressible3D : HyperelasticityBase {
  ...
  //// complicated artifical equation for debugging
...
/** Transversely-isotropic hyperelastic solid mechanics formulation, here
 * Mooney-Rivlin
 */
struct TransverselyIsotropicMooneyRivlinIncompressible3D : HyperelasticityBase {
  ...
  static constexpr int nMaterialParameters =
      4; //< number of material parameters

  //! the isochoric part of the decoupled strain energy function,
  //! Ψ_iso(Ibar1,Ibar2,Ibar4,Ibar5), in terms of the reduced invariants
  //! function from chemo-electro-mechanical muscle model (Heidlauf 2013
  //! "Modeling the chemo-electro-mechanical behaviour of... p.4)
  static const auto constexpr strainEnergyDensityFunctionIsochoric =
      c1 * (Ibar1 - INT(3)) + c2 * (Ibar2 - INT(3)) +
      b / d * (pow(lambda, d) - INT(1)) - b * ln(lambda);
...
/** Tendon model from "Carniel, T. A., & Fancello, E. A. (2017). A transversely
 * isotropic coupled hyperelastic model for the mechanical behavior of tendons.
 * Journal of biomechanics, 54, 49-57."
 */
struct HyperelasticTendon : HyperelasticityBase {
  ...
  static constexpr int nMaterialParameters =
      8; //< number of material parameters

  static constexpr auto psi_NH =
      mu / INT(2) * (Ibar1 - INT(3)); //< Neo-Hookean expression
  ...
  static constexpr auto psi_f =
      k1 * pow(I4 - INT(1), INT(2)) +
      k2 * pow(I4 - INT(1), INT(3)); //< fiber strain energy
